...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT b.dwmc fgs,a.dwmc bm,DECODE(sq_yhxx.sfzszg,'M','<font color=\"#009999\">实习生</font>','Y','转正','N','<font color=\"#0000CC\">试用期</font>','D','<font color=\"#CCCC00\">待岗</font>','S','<font color=\"#CC0000\">辞职</font>','T','<font color=\"#CC00CC\">辞退</font>'),sq_yhxx.bianhao,sq_yhxx.yhmc,sq_yhxx.xzzwbm,sq_yhxx.ygbh,dm_zwbm.zwmc,DECODE(sq_yhxx.kfgssq,'0','不授权','1','授权单个分公司','2','授权多个分公司','3','授权单个店面','4','授权某一分公司多个店面','5','授权多个分公司多个店面') kfgssq, DECODE(sq_yhxx.yhjs,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','分公司店面','F3','设计室','G0','供应商','J0','加盟商'),sq_yhxx.sjsbh,kqhm, DECODE(sq_yhxx.kdlxtbz,'Y','是','N','否'), DECODE(sq_yhxx.xb,'M','男','W','女'),sq_yhxx.csrq, DECODE(sq_yhxx.hf,'Y','是','N','否'),sq_yhxx.mz,'SFZ:'||sq_yhxx.sfzh,sq_yhxx.hjszd,sq_yhxx.dacfd,sq_yhxx.byyx,sq_yhxx.zy,dm_xlbm.xueli,DECODE(sq_yhxx.dhsfgb,'1','本公司内','2','不受限制','9','不公布'),sq_yhxx.dh,sq_yhxx.bgdh,sq_yhxx.jtdh,sq_yhxx.email,sq_yhxx.zz,sq_yhxx.cjgzsj,ROUND((SYSDATE-sq_yhxx.cjgzsj)/365,1)-sq_yhxx.wgzsj,sq_yhxx.rzsj,sq_yhxx.syjzrq,sq_yhxx.lzrq, DECODE(sq_yhxx.sfqs,'Y','是','N','否'),sq_yhxx.jjlxr,sq_yhxx.lxrdh,sq_yhxx.zkl, DECODE(sq_yhxx.ckjgbz,'Y','可看','N','不可'), DECODE(sq_yhxx.zdyhbz,'Y','是','N','否'), sq_yhxx.bjjb,rs_sjsjb.sjsjbmc, DECODE(sq_yhxx.sfydbs,'Y','是','N','否'), DECODE(sq_yhxx.sfytj,'Y','是','N','否'),sq_yhxx.tjrq,sq_yhxx.qgzdw,sq_yhxx.lrsj,sq_yhxx.bz ";
	ls_sql+=" FROM dm_xlbm,dm_zwbm,rs_sjsjb,sq_dwxx a,sq_dwxx b,sq_yhxx,dm_xzzwbm,dm_zwflbm  ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_yhxxTqList.jsp","SelectTqSq_yhxx.jsp","","EditTqSq_yhxx.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Sq_yhxxTqList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	{
		String[] sql=new String[8];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from sq_yhssz where yhdlm in(select yhdlm from sq_yhxx where "+chooseitem+")";
		sql[1]="delete from sq_yhewqx where yhdlm in(select yhdlm from sq_yhxx where "+chooseitem+")";
		sql[2]="delete from sq_sjsbjjb where "+chooseitem;
		sql[3]="delete from sq_sqfgs where "+chooseitem;
		sql[4]="delete from sq_sqbm where "+chooseitem;
		sql[5]="delete from rs_ygjbgzb where "+chooseitem;
		sql[6]="delete from rs_ygsbxx where "+chooseitem;
		sql[7]="delete from sq_yhxx where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="1%">工龄</td>
	<td  width="2%">入职日期</td>
	<td  width="2%">试用截至日期</td>
	<td  width="2%">离职日期</td>
	<td  width="1%">与公司有亲属关系</td>
	<td  width="2%">紧急联系人</td>
	<td  width="3%">急联系人手机</td>
	<td  width="1%">允许折扣率</td>
