...
try {
	...
	ls_sql="select count(*)";
	ls_sql+=" from  sq_yhxx";
	ls_sql+=" where bianhao='"+bianhao+"' and ssfgs='"+ssfgs+"'";
	...
	ls_sql+=" ,sfcjbx,ylbx,jylbx,sybx,gsby,syx   ,sfjgjj,ckjgbz,zdyhbz,kdlxtbz,sjxzbz   ,ipxzbz,wkxzbz,khdhbz,kkglfx,kfgssq,gzlb ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?";
	ls_sql+="  ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?";
	ls_sql+="  ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?";
	ls_sql+="  ,?,?";
	ls_sql+="  ,'N','N','N','N','N','N'  ,'N','N','N','N','N'   ,'N','N','N','N','0','1') ";
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,ygbh);
