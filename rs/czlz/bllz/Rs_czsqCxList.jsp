...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (ssfgs!=null)
	{
		...
		if (!(ssfgs.equals("")))	wheresql+=" and  (rs_czsq.fgsbh='"+ssfgs+"')";
	}
	dwbh=request.getParameter("dwbh");
	if (dwbh!=null)
	{
		dwbh=cf.GB2Uni(dwbh);
		if (!(dwbh.equals("")))	wheresql+=" and  (rs_czsq.dwbh='"+dwbh+"')";
	}
    ...
    ls_sql+=" and rs_czsq.clzt in('00','03')";
    ls_sql+=wheresql;
    ls_sql+=" order by rs_czsq.lrsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Rs_czsqCxList.jsp","SelectCxRs_czsq.jsp","","InsertRs_yhlzjl.jsp");
