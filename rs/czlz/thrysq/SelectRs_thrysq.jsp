...
<body bgcolor="#FFFFFF">
<form method="post" action="Rs_thrysqList.jsp" name="selectform">
  ...
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	if (yhjs.equals("A0"))
	{
        ...
        out.println("        <select name=\"rs_thrysq_fgsbh\" style=\"FONT-SIZE:12PX;WIDTH:152PX\" onChange=\"selectItem(rs_thrysq_fgsbh,rs_thrysq_dwbh,"+dwstr+");\">");
		out.println("<option value=\"\"></option>");
		cf.selectItem(out,"select dwbh,dwmc from sq_dwxx where dwlx in('A0','F0') order by dwbh","");
        out.println("        </select>");
	}
	else{
        out.println("        <select name=\"rs_thrysq_fgsbh\" style=\"FONT-SIZE:12PX;WIDTH:152PX\" >");
...
%> </td>
      ...
      <td width="32%"> 
        <select name="rs_thrysq_dwbh" style="FONT-SIZE:12PX;WIDTH:152PX">
          <%
	if (yhjs.equals("A0"))
	{
		out.println("<option value=\"\"></option>");
...
%> </td>
      ...
      <td width="32%"> 
        <select name="rs_thrysq_xzzwbm" style="FONT-SIZE:12PX;WIDTH:152PX">
          <option value=""></option>
          <%
...
%> 
    ...
    <tr bgcolor="#FFFFFF"> 
      <td align="right" width="17%">退回人员申请序号</td>
      <td width="33%"> 
        <input type="text" name="rs_thrysq_sqxh" size="20" maxlength="10" >
      </td>
	...
	{
		%> 
        <input type="text" name="rs_thrysq_lrr" size="20" maxlength="20" >
        <%
	}
	else if (yhjs.equals("A1") && dwflbm.equals("04"))//04：人事部
	{
	...
	else if ( (yhjs.equals("F0") || yhjs.equals("F1")) && dwflbm.equals("04"))//04：人事部
	{
		...
		%> 
        <input type="text" name="rs_thrysq_lrr" size="20" maxlength="20" value="<%=yhmc%>" readonly>
        <%
	}
%> </td>
      <td align="right" width="18%">录入部门</td>
      <td width="32%"> 
        <select name="rs_thrysq_lrbm" style="FONT-SIZE:12PX;WIDTH:152PX">
          <option value=""></option>
          <%
...
%> 
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="33%"> 
        <select name="rs_thrysq_clzt" style="FONT-SIZE:12PX;WIDTH:152PX">
          <option value="01">等待审批</option>
        </select>
      </td>
      <td align="right" width="18%">审批状态</td>
      <td width="32%"> 
        <select name="rs_thrysq_ztbm" style="FONT-SIZE:12PX;WIDTH:152PX">
          <%
	cf.selectItem(out,"select ztbm,ztmc from rs_ztbm where ztbm='1401'","");
%> 
        </select>
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="33%"> 
        <input type="text" name="rs_thrysq_spxh" size="20" maxlength="16" >
      </td>
      <td align="right" width="18%">审核结论</td>
      <td width="32%"> 
        <select name="rs_thrysq_shjl" style="FONT-SIZE:12PX;WIDTH:152PX">
          <option value=""></option>
          <%
...
%> 
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="33%"> 
        <input type="text" name="rs_thrysq_shsj" size="20" maxlength="10" onDblClick="JSCalendar(this)">
      </td>
      <td align="right" width="18%">到</td>
      <td width="32%"> 
        <input type="text" name="rs_thrysq_shsj2" size="20" maxlength="10" onDblClick="JSCalendar(this)">
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td align="right" width="17%">审核人</td>
      <td width="33%"> 
        <input type="text" name="rs_thrysq_shr" size="20" maxlength="20" >
      </td>
      <td align="right" width="18%">&nbsp;</td>
      <td width="32%">&nbsp; </td>
    </tr>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isDatetime(FormName.rs_thrysq_lrsj, "录入时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_thrysq_lrsj2, "录入时间"))) {
		return false;
	}
	...
	if(!(isNumber(FormName.rs_thrysq_spxh, "审批序号"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_thrysq_shsj, "审核时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_thrysq_shsj2, "审核时间"))) {
		return false;
	}
	FormName.submit();
	return true;
