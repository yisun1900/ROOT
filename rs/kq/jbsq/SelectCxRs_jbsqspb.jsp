...
<div align="center">加班申请--审批查询</div>
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr bgcolor="#FFFFFF"> 
      ...
      <td align="right" width="18%">加班申请序号</td>
      <td width="32%"> 
...
{
	if(!(isNumber(FormName.rs_jbsqspb_spxh, "审批序号"))) {
		return false;
	}
	if(!(isNumber(FormName.rs_jbsqd_ygbh, "员工序号"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_jbsqd_lrsj, "申请录入时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_jbsqd_lrsj2, "申请录入时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_jbsqspb_shsj, "审核时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.rs_jbsqspb_shsj2, "审核时间"))) {
		return false;
	}
	FormName.submit();
	return true;
