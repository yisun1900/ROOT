...
try {
	...
	ls_sql="update rs_ccsqd set fgsbh=?,dwbh=?,ygbh=?,yhmc=?,xb=?,xzzwbm=?,kssj=?,jzsj=?,ljts=?   ,yysm=?,sqrq=?,lrr=?,lrsj=?,lrbm=?,bz=?,txzsm=?,jtgj=?,btbz=?,zsbz=?,zwdlr=?,xcsm=?";
	ls_sql+=" where sqxh='"+sqxh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setString(15,bz);
	ps.setString(16,txzsm);
	ps.setString(17,jtgj);
	ps.setDouble(18,btbz);
	ps.setDouble(19,zsbz);
	ps.setString(20,zwdlr);
	ps.setString(21,xcsm);
