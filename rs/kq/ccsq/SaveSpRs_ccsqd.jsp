...
try {
	...
	//取审核表序号
	long spxh=0;
	ls_sql="select max(spxh)  from rs_ccsqspb ";
	ps= conn.prepareStatement(ls_sql);
	rs=ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql="insert into rs_ccsqspb (spxh,sqxh,shjl,shyj,shr,shsj,clzt,ztbm   ,yspxh,yclzt,yztbm,yshjl,yshyj,yshr,yshsj)";
	ls_sql+=" values("+spxh+",?,?,?,?,?,?,?   ,?,?,?,?,?,?,?)";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,sqxh);
	ps.setString(2,shjl);
	...
	ls_sql="update rs_ccsqd set spxh=?,shr=?,shsj=?,shjl=?,shyj=?,clzt=?,ztbm=? ";
	ls_sql+=" where sqxh='"+sqxh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,spxh);
	ps.setString(2,shr);
