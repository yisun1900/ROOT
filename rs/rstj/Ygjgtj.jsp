...
%>
  ...
  <%
	...
	while (rs.next())
	{
		...
		ls_sql=" SELECT count(distinct sq_yhxx.ygbh)";
		ls_sql+=" FROM sq_yhxx";
		...
		ls_sql+=" SELECT dm_dwflbm.dwtjflbm,dwtjflmc,count(distinct sq_yhxx.ygbh) bmsl";
		...
		%>
			<td ><a href="Sq_yhxxList.jsp?ssfgs=<%=fgs%>&sflz=1" title="点击查看明细" target="_back"><%=fgszs%></a></td>
		  </tr> 
		<%
