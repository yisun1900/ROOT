...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		lrr=cf.GB2Uni(lrr);
		if (!(lrr.equals("")))	wheresql+=" and  (rs_fysq.lrr='"+lrr+"')";
	}
	lrsj=request.getParameter("lrsj");
	if (lrsj!=null)
	{
		lrsj=lrsj.trim();
		if (!(lrsj.equals("")))	wheresql+="  and (rs_fysq.lrsj>=TO_DATE('"+lrsj+"','YYYY/MM/DD'))";
	}
    ...
    ls_sql+=" where sfkg='Y' and rs_fysq.fgs=sq_dwxx.dwbh";
    ls_sql+=wheresql;
	ls_sql+=" order by sqsj";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Rs_fysqList.jsp","SelectRs_fysq.jsp","","EditRs_fysq.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Rs_fysqList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] fysqbh = request.getParameterValues("fysqbh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(fysqbh,"fysqbh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from rs_fysq where "+chooseitem;
