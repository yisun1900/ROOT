...
try {
	...
	ls_sql="insert into rs_yggzjl ( xh,fxrq,ksrq,zzrq,lrr,lrsj,lrbm,ssfgs,bz,lx ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,? ) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,xh);
	ps.setDate(2,fxrq);
	...
	ls_sql="insert into rs_yggzb (xh,ygbh,bianhao,yhmc,dwbh,glgz,gwgz,jbgz,jtbt,mrcb,dhbt,yjsxe,jj,qtbt,qtkk,tc,sqgz,ksk,shgz) ";
	ls_sql+=" select '"+xh+"',ygbh,bianhao,yhmc,dwbh,NVL(glgz,0),NVL(gwgz,0),NVL(jbgz,0),NVL(jtbt,0),NVL(cb,0),NVL(dhbt,0),NVL(yjsxe,0),0,0,0,0,0,0,0 ";
	ls_sql+=" from sq_yhxx  ";
