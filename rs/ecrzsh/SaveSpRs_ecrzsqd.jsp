...
try {
	...
	{
		ls_sql="select ztbm";
		ls_sql+=" from  rs_ecrzsqsplc";
		ls_sql+=" where sqxh='"+sqxh+"' and ztbm>'"+yztbm+"'";
		ls_sql+=" order by xh";
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		...
		if (xyztbm==null)
		{
			xyztbm="0698";//审批通过且审批结束
		}
	}
	else{
		xyztbm="0699";//审批取消且审批结束
	}
	...
	//取审核表序号
	long spxh=0;
	ls_sql="select max(spxh)  from rs_ecrzspb ";
	ps= conn.prepareStatement(ls_sql);
	rs=ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql="insert into rs_ecrzspb (spxh,sqxh,shjl,shyj,shr,shsj,clzt,ztbm   ,yspxh,yclzt,yztbm,yshjl,yshyj,yshr,yshsj)";
	ls_sql+=" values("+spxh+",?,?,?,?,?,?,?   ,?,?,?,?,?,?,?)";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,sqxh);
	ps.setString(2,shjl);
	...
	ls_sql="update rs_ecrzsqd set spxh=?,shr=?,shsj=?,shjl=?,shyj=?,clzt=?,ztbm=? ";
	ls_sql+=" where sqxh='"+sqxh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,spxh);
	ps.setString(2,shr);
