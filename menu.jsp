...
if (kkbbz.equals("Y"))//Y：可拷贝；N：不可拷贝
{
	...
	<noscript><iframe scr="*.htm,*.html"></iframe></noscript>
	<%
}
else{
	%>
...
<%
if ((txfs.equals("1") || txfs.equals("9")) && (txjg!=0))
{
	%>
	<SCRIPT LANGUAGE="JavaScript">
	...
	function startCall()
	{
		...
		timer = setInterval("getTxxx()",1000*<%=txjg%>); //setInterval() 方法可按照指定的周期（以毫秒计）来调用函数或计算表达式
		if (timer==0)
		{
			alert("错误！提醒未启动");
		}
	...
	//	alert(ldsj);
		var actionStr="/yhtxxx/getTxxx.jsp?yhdlm=<%=yhdlm%>&ldsj="+ldsj;
	...
	*/
	...
	function doCall()
	{
		if (innum.value=="")
		{
			if (callMark==null)
			{
//				alert("错误！呼叫中心未正常连接，请关闭窗口，连接好设备，重新登陆系统");
				location="callerror.html";
			}
...
//			parent.document.title="来电:"+innum.value+"－呼叫中心状态:"+callMark;
		...
		if (PhoneOcx.CheckCallStatus()==1)//=0：没有有效来电号码；=1：有有效来电号码
		{
			...
			if (zt==1)//判断外接普通电话摘/挂机状态： 0：已经挂机；1：已经摘机结束
			{
				...
				{
					StartReadSoundCStrF(filename.value);
				}
			}
	...
	*/
			...
			else if (callMark==13)//13:拨号
			{
				...
				zt=PhoneOcx.SetContinueReturn450Hz(true);
				zt=PhoneOcx.ReadBusyBit();
				if (zt==1)//1: 检测到忙音
				{
					gjHM();//挂机
					callMark=0;//0:设备挂机
				...
				{
					//登记接通状态
					var actionStr="/CallCenter/SaveQdJtzt.jsp?xh="+xh.value;
					czlx="6";
				//	window.open(actionStr);
					openAjax(actionStr);
	...
	*/
		...
		else if (czlx=="2")
		{
			...
			xh.value=khxxArr[0];
			filename.value="<%=cf.replace(lywjml,"\\","\\\\")%>\\"+"out-"+khxxArr[0]+"-"+khxxArr[1]+"-"+khxxArr[2]+".wav";
	/*
			khbh.value=khxxArr[1];
			...
			fwdz.value=khxxArr[3];
	*/
			parent.document.title="【"+khxxArr[2]+"】去电:"+innum.value+"、【房屋地址】"+khxxArr[3]+"－呼叫中心状态:"+callMark;
	...
	//开始录音
	function StartReadSoundCStrF(filename)
	{
		if (callMark==9 || callMark==11 || callMark==12)//9:开始放音
		{
			alert("错误！开始放音，不能录音");
	...
	function dialXX(outnum)
	{
		...
		var ldsj=dateObj.toLocaleTimeString();
	//	alert(ldsj);
		var actionStr="/CallCenter/outRec.jsp?outTelNum="+outnum+"&bjhm=<%=bjhm%>&ldsj="+ldsj+"&lywjml=<%=java.net.URLEncoder.encode(lywjml,"UTF-8")%>";
