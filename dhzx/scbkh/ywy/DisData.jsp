...
String ywybm=null;
String ywybmStr=null;
String ywyssxz=null;
String ywyssxzStr=null;
...
<div align="center">导入客户信息</div>
  <table width="300%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="0" cellspacing="1">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		cqbmstr=outStr[i][4].trim();
		xqbm=outStr[i][5].trim();
		louhao=outStr[i][6].trim();
		fwdz=outStr[i][7].trim();
		fwmjstr=outStr[i][8].trim();
		sfxhfstr=outStr[i][9].trim();
		xchfsjstr=outStr[i][10].trim();
		ywy=outStr[i][11].trim();
		ywyssxzStr=outStr[i][12].trim();
		ywybmStr=outStr[i][13].trim();
		fgsmc=outStr[i][14].trim();
		yjzxsjstr=outStr[i][15].trim();
		khzyxz=outStr[i][16].trim();
		bz=outStr[i][17].trim();
...
//			return;
		...
		sql="select dwbh";
		sql+=" from  sq_dwxx";
		sql+=" where dwmc='"+ywybmStr.trim()+"'";
		ps= conn.prepareStatement(sql);
		rs =ps.executeQuery();
		if (rs.next())
		{
			ywybm=rs.getString("dwbh");
		}
		else{
			out.print("<BR>序号『"+row+"』错误！业务员部门『"+ywybmStr+"』不存在！");
//			return;
		}
		rs.close();
		ps.close();
		...
		//检查『业务员所属小组』是否正确
		if (!ywyssxzStr.trim().equals(""))
		{
			sql="select dwbh";
			sql+=" from  sq_dwxx";
			sql+=" where dwmc='"+ywyssxzStr.trim()+"'";
			ps= conn.prepareStatement(sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
				ywyssxz=rs.getString("dwbh");
			}
			else{
				out.print("<BR>序号『"+row+"』错误！业务员所属小组『"+ywyssxzStr+"』不存在！");
//				return;
			}
			rs.close();
			ps.close();
...
//				return;
    ...
    <tr bgcolor="<%=bgcolor%>" align="center"> 
	  ...
	  <td><%=ywyssxzStr%>
        <input name="ywyssxz" type="hidden" value="<%=ywyssxz%>" >
      </td> 
	  <td><%=ywybmStr%>
        <input name="ywybm" type="hidden" value="<%=ywybm%>" >
      </td> 
	  <td><%=fgsmc%>
