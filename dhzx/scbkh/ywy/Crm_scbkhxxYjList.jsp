...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_scbkhxx.scbkhbh, DECODE(crm_scbkhxx.zt,'1','跟踪','6','准备失败','2','失败','5','准备移交','3','移交','4','已接收','7','移交过期'),sckhyxmc,crm_scbkhxx.yyddsj,crm_scbkhxx.hfjg,crm_scbkhxx.zjgzsj, DECODE(crm_scbkhxx.sfxhf,'Y','需要','N','否'),crm_scbkhxx.xchfsj, crm_scbkhxx.khxm,crm_scbkhxx.fwdz,crm_scbkhxx.ywy,crm_scbkhxx.xqlx,crm_scbkhxx.khzyxz,c.dwmc ywyssxz,e.dwmc ywybm,crm_scbkhxx.yjzxsj,crm_scbkhxx.jzdsmc,crm_scbkhxx.lrr,crm_scbkhxx.lrsj,a.dwmc fgs,crm_scbkhxx.bz  ";
	ls_sql+=" FROM crm_scbkhxx,sq_dwxx a,sq_dwxx c,sq_dwxx e,dm_sckhyxbm  ";
    ls_sql+=" where crm_scbkhxx.ssfgs=a.dwbh(+)";
    ls_sql+=" and crm_scbkhxx.ywyssxz=c.dwbh(+) and crm_scbkhxx.ywybm=e.dwbh(+)";
    ls_sql+=" and crm_scbkhxx.sckhyxbm=dm_sckhyxbm.sckhyxbm(+)";
    ls_sql+=" and crm_scbkhxx.zt in('1','5','6','7')";//1：跟踪；2：失败；3：移交；4：已接收；5：准备移交；6：准备失败；7：移交过期
    ls_sql+=wheresql;
    ls_sql+=" order by crm_scbkhxx.scbkhbh desc";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Crm_scbkhxxYjList.jsp","","","YjCrm_scbkhxx.jsp");
...
//设置主键
	...
	pageObj.setEditStr("移交");

//设置按钮参数
	String[] buttonName={"批量移交"};//按钮的显示名称
	String[] buttonLink={"PlYjCrm_scbkhxx.jsp"};//按钮单击调用的网页，可以增加参数
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="3%">&nbsp;</td>
	<td  width="3%">市场客户编号</td>
	<td  width="2%">状态</td>
	<td  width="3%">客户意向</td>
	<td  width="4%">预约到店时间</td>
	<td  width="2%">回访间隔 </td>
	<td  width="4%">最近跟踪时间</td>
	<td  width="2%">需回访</td>
	...
	<td  width="3%">业务员</td>
	<td  width="3%">小区类型</td>
	<td  width="3%">客户资源性质</td>
	<td  width="5%">业务员所属小组</td>
	<td  width="5%">业务员部门</td>

	<td  width="4%">预计装修时间</td>
	<td  width="12%">竞争对手名称</td>
