...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by crm_scbkhxx.sbdjsj desc";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Crm_scbkhxxXzSbdjXgList.jsp","","","XgDjCrm_scbkhxx.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_scbkhxxXzSbdjXgList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_scbkhxx set zt='1',sbyybm=null,sbyysm=null,sbdjr=null,sbdjsj=null where "+chooseitem;
