...
String[] lrbm =request.getParameterValues("lrbm");
...
try {
	...
	for (i=0;i<khxm.length ;i++ )
	{
		...
		ls_sql="insert into crm_scbkhxx ( scbkhbh,khxm,lxfs,cqbm,xqbm,louhao,fwdz,fwmj ";
		ls_sql+=" ,sfxhf,xchfsj,ywy,ssfgs,yjzxsj,lrr,lrsj,lrbm,bz,zt,khzyxz,sfxxq) ";
		ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?  ,?,?,?,?,?,?,?,?,?,?,?   )";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,scbkhbh);
		ps.setString(2,cf.GB2Uni(khxm[i]));
		ps.setString(3,cf.GB2Uni(lxfs[i]));
		ps.setString(4,cf.GB2Uni(qtdh[i]));
		ps.setString(5,cqbm[i]);
		ps.setString(6,cf.GB2Uni(xqbm[i]));
		ps.setString(7,cf.GB2Uni(louhao[i]));
		ps.setString(8,cf.GB2Uni(fwdz[i]));
		ps.setInt(9,fwmj);

		ps.setString(10,sfxhf[i]);
		ps.setDate(11,xchfsj);
		ps.setString(12,cf.GB2Uni(ywy[i]));
		ps.setString(13,fgsbh[i]);
		ps.setDate(14,yjzxsj);

		ps.setString(15,cf.GB2Uni(lrr[i]));
		ps.setDate(16,lrsj);
		ps.setString(17,lrbm[i]);
