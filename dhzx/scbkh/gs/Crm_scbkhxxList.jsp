...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_scbkhxx.scbkhbh, DECODE(crm_scbkhxx.zt,'1','跟踪','6','准备失败','2','失败','5','准备移交','3','移交','4','已接收','7','移交过期'),crm_scbkhxx.xqlx,crm_scbkhxx.khzyxz, DECODE(crm_scbkhxx.sfxhf,'Y','需要','N','否'),crm_scbkhxx.xchfsj,crm_scbkhxx.khxm,crm_scbkhxx.fwdz,crm_scbkhxx.lxfs,crm_scbkhxx.ywy,c.dwmc ywyssxz,e.dwmc ywybm,crm_scbkhxx.lrr,crm_scbkhxx.lrsj,b.dwmc lrbm,a.dwmc fgs,crm_scbkhxx.bz,sbyymc,crm_scbkhxx.sbyysm,crm_scbkhxx.sbdjr,crm_scbkhxx.sbdjsj,crm_scbkhxx.yjr,crm_scbkhxx.yjsj,d.dwmc yjbm,crm_scbkhxx.yjsm,crm_scbkhxx.jsr,crm_scbkhxx.jssj,crm_scbkhxx.khbh,cqmc,crm_scbkhxx.jiedao,crm_scbkhxx.xqbm,crm_scbkhxx.louhao,crm_scbkhxx.fwmj,hxmc,crm_scbkhxx.fj,crm_scbkhxx.yjzxsj,crm_scbkhxx.jzdsmc,crm_scbkhxx.zjgzsj, DECODE(crm_scbkhxx.sfxxq,'Y','是','N','否')  ";
	ls_sql+=" FROM crm_scbkhxx,sq_dwxx a,sq_dwxx b,sq_dwxx c,sq_dwxx d,sq_dwxx e,dm_cqbm,dm_hxbm,dm_sbyybm  ";
    ...
    ls_sql+=" and crm_scbkhxx.zt in('1','5','6','3','7')";//1：跟踪；2：失败；3：移交；4：已接收；5：准备移交；6：准备失败；7：移交过期
...
//设置按钮参数
	String[] buttonName={"删除"};//按钮的显示名称
	String[] buttonLink={"Crm_scbkhxxList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(scbkhbh,"scbkhbh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[3];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from crm_scbkhxx where "+chooseitem;
		sql[1]="delete from crm_scbkhyjjl where "+chooseitem;
		sql[2]="delete from crm_scbkhgzjl where "+chooseitem;
