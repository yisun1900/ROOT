...
try {
	...
	ls_sql="insert into crm_sjhtxx ( khbh,sjs,sjhth,qsjhtsj,pms,zqdj,zkl,dj,zqsjhtje,sjhtje   ,lrr,lrsj,lrbm,bz,clgw ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,? ) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
	ps.setString(2,sjs);
	ps.setString(3,hth);
	ps.setDate(4,qsjhtsj);
	ps.setDouble(5,pms);
	ps.setDouble(6,zqdj);
	ps.setDouble(7,zkl);
	ps.setDouble(8,dj);
	ps.setDouble(9,zqsjhtje);
	ps.setDouble(10,sjhtje);

	ps.setString(11,lrr);
	ps.setDate(12,lrsj);
	ps.setString(13,lrbm);
	ps.setString(14,bz);
	ps.setString(15,clgw);
	ps.executeUpdate();
	ps.close();
