...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_dhzxjl.zxjlh as zxjlh,crm_dhzxjl.khxm as crm_dhzxjl_khxm, DECODE(crm_dhzxjl.xb,'1','男','2','女') as crm_dhzxjl_xb,crm_dhzxjl.lxdh as crm_dhzxjl_lxdh,crm_dhzxjl.fwdz as crm_dhzxjl_fwdz,crm_dhzxjl.hxmj as crm_dhzxjl_hxmj,crm_dhzxjl.jfrq as crm_dhzxjl_jfrq, DECODE(crm_dhzxjl.sfxhf,'Y','需要','N','不需要') as crm_dhzxjl_sfxhf,crm_dhzxjl.hfrq as crm_dhzxjl_hfrq,dm_zxlxbm.zxlxmc as dm_zxlxbm_zxlxmc  ";
	ls_sql+=" FROM crm_dhzxjl,dm_zxlxbm  ";
    ls_sql+=" where crm_dhzxjl.zxlxbm=dm_zxlxbm.zxlxbm";
    ls_sql+=wheresql;
    ls_sql+=" order by crm_dhzxjl.hfrq";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","/dhzx/dhhf/InsertCrm_zxhfjl.jsp");
...
//设置主键
	...
	pageObj.setEditStr("回访");
//设置列超级连接
	Hashtable coluParmHash=new Hashtable();
	ColuParm coluParm=null;

	coluParm=new ColuParm();//生成一个列参数对象
	String[] coluKey={"zxjlh"};//设置列参数：coluParm.key的主键
	coluParm.key=coluKey;//设置列参数：coluParm.key的主键
	coluParm.link="/dhzx/dhzx/ViewCrm_dhzxjl.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("zxjlh",coluParm);//列参数对象加入Hash表
