...
<%
if (cf.check((String)session.getAttribute("yhdlm"),request.getRemoteHost(),"030121")==0) 
{
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_zxkhxx.khbh,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过'),crm_zxkhxx.khxm,ywy,a.dwmc ywybm,c.dwmc ywyssxz,crm_zxkhxx.fwdz,sjs,khjl,DECODE(ysshbz,'N','未审核','Y','通过','M','审核未通过'),zjzbjsj,qsjhtsj,sjhtje,lfdj,jlfdjsj,cxhdbm,hddj,jhddjsj,sjf,jsjfsj,lfsj,ctsj,cxgtsj,csgtsj,DECODE(crm_zxkhxx.jzbz,'1','家装','2','公装'),b.dwmc as zxdm,sbyymc,lrsj,fwlxmc,xxlysm";
	ls_sql+=" FROM crm_zxkhxx,dm_fwlxbm,sq_dwxx a,sq_dwxx b,sq_dwxx c,dm_sbyybm ";
	ls_sql+=" where crm_zxkhxx.fwlxbm=dm_fwlxbm.fwlxbm(+) and crm_zxkhxx.sbyybm=dm_sbyybm.sbyybm(+)";
	ls_sql+=" and crm_zxkhxx.ywybm=a.dwbh(+) and crm_zxkhxx.zxdm=b.dwbh(+) and crm_zxkhxx.ywyssxz=c.dwbh(+) ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxScbAllCxList.jsp","","","");
...
<CENTER >
  <B><font size="3">查询全部市场部客户</font></B>
</CENTER>
