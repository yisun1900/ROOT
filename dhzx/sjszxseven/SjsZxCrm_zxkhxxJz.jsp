...
String xqbm=null;
String louhao=null;
String sfzdzbj=null;//是否做电子报价,N：未做；Y：已做
...
try {
	...
	ls_sql+=" where dqbm='"+dqbm+"' and sfzybb='1' and TRUNC(SYSDATE)>=qysj  ";//1：未启用；2：当前版；3：可用版；4：已停用
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql+=" and bjbbh not in(select distinct bjbbh from bj_jzbjb1 where dqbm='"+dqbm+"') ";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	ls_sql+=" where dqbm='"+dqbm+"' and bjbbh in(select bjbbh from bj_dzbjbb where dqbm='"+dqbm+"' and sfzybb='4') ";
	ls_sql+=" and bjbbh not in(select distinct bjbbh from bj_glxmb1 where dqbm='"+dqbm+"') ";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	ls_sql+=" where dqbm='"+dqbm+"' and bjbbh in(select bjbbh from bj_dzbjbb where dqbm='"+dqbm+"' and sfzybb='4') ";
	ls_sql+=" and bjbbh not in(select distinct bjbbh from bj_jzglfxb1 where dqbm='"+dqbm+"') ";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
        ...
        <form method="post" action="" name="editform" target="_blank">
          <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
            <tr bgcolor="#FFFFCC" align="center"> 
              <td colspan="4"><b><font color="#000066">设计师咨询情况（咨询记录号：<%=zxjlh%>）</font></b></td>
            </tr>
...
%> </td>
            ...
            <tr bgcolor="#E8E8FF"> 
              <td width="18%" align="right" bgcolor="#E8E8FF"><font color="#CC0000">*</font>年龄区间</td>
              <td width="32%"> 
...
%> 
              ...
              <td width="18%" align="right"><font color="#CC0000">*</font>职业</td>
              <td width="32%"> 
...
%> 
            ...
            <tr bgcolor="#E8E8FF"> 
              <td width="18%" align="right">常住人口</td>
              <td width="32%"> 
...
%> 
            ...
            <tr bgcolor="#CCCCFF"> 
              <td width="18%" align="right" height="2"><font color="#CC0000">*</font>装修房屋楼号/门牌号</td>
            ...
            <tr bgcolor="#CCCCFF"> 
              ...
              <td colspan="3" bgcolor="#CCCCFF"><%=cxhdbm%><input name="button3" type="button" onClick="window.open('/dhzx/tqwh/xgcxhd/EditCrm_zxkhxx.jsp?khbh=<%=khbh%>')"  value="调整活动"></td>
            </tr>
            <tr bgcolor="#CCCCFF"> 
            ...
            <tr bgcolor="#CCCCFF"> 
              <td width="18%" align="right">参加精装促销活动</td>
...
%> 
              ...
              <td width="18%" align="right"><font color="#CC0000">*</font>建筑面积</td>
              <td width="32%"> 
...
%> 
            ...
            <tr bgcolor="#E8E8D4"> 
              <td width="18%" align="right">房屋用途</td>
              <td width="32%"> 
...
%> 
              ...
              <td width="18%" align="right" bgcolor="#CCCCFF">精装名称</td>
              <td width="32%"><%=tcmc%></td>
            </tr>
            <tr bgcolor="#CCCCFF"> 
...
%></td>
		...
		if (!bjbbh.equals(""))
		{
			...
			cf.selectItem(out,"select bjbbh c1,bjbbh||'（'||dqmc||'）（'||DECODE(bj_dzbjbb.sfzybb,'1','未启用','2','当前版','3','可用版','4','停用')||'）' c2 from bj_dzbjbb,dm_dqbm where bj_dzbjbb.dqbm=dm_dqbm.dqbm and bj_dzbjbb.sfzybb='2' and bj_dzbjbb.dqbm='"+dqbm+"' order by bj_dzbjbb.dqbm,bjbbh","");
		}
%> 
                </select>			  </td>
            </tr>
...
%></td>
            ...
            <tr bgcolor="#FFFFCC">
              <td align="right"><font color="#CC0000">*</font>出平面图标志</td>
              <td><%
...
%></td>
            ...
            <tr bgcolor="#FFFFCC">
              <td align="right">是否小精装</td>
              <td><%
		cf.radioToken(out, "sfxjz","N+否&Y+小精装",sfxjz);
%></td>
              <td align="right">&nbsp;</td>
              <td>&nbsp;</td>
            </tr>
            ...
            <tr> 
              <td colspan="4" align="center" > 
                  ...
                  <input type="button"  value="选房间" onClick="f_lrfj(editform)" name="zjfj" disabled>
                  <input type="button"  value="修改房间" onClick="f_xgfj(editform)" name="xgfj" disabled>
                  <input type="button"  value="精装主材" onClick="f_jzzc(editform)" name="jzzc" disabled>
                  <input type="button"  value="精装工程" onClick="f_jzgc(editform)" name="jzgc" disabled>
                  <input type="button"  value="选其它收费" onClick="f_ewlr(editform)" name="ewlr" disabled>
                  <input type="button"  value="修改其它收费" onClick="f_ewck(editform)" name="xgsf" disabled>
                  <p>
                    <input type="button" value="导入报价" onClick="f_drmb(editform)" name="drmb" disabled>
                    <input type="button" value="报价另存为" onClick="f_bcmb(editform)" name="cwmb" disabled>
                    <input type="button" value="调报价级别" onClick="f_tbjjb(editform)" name="tbjjb" disabled>
...
*/
	...
	FormName.jzzc.disabled=false;
	FormName.jzgc.disabled=false;
...
function f_bcmb(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/cwmb.jsp";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/cwmb.jsp";
	}
...
function f_lrfj(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/ChooseBj_fjxx.jsp";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/ChooseBj_fjxx.jsp";
	}
...
function f_xgfj(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/EditBj_fjxx.jsp";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/EditBj_fjxx.jsp";
	}
...
function f_ewlr(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		...
		FormName.action="/dzbjseven/zxbjmx/InsertBj_sfxmmxJz.jsp?dqbm=<%=dqbm%>";
...
function f_ewck(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		...
		FormName.action="/dzbjseven/zxbjmx/Bj_sfxmmxListJz.jsp";
	FormName.submit();
	return true;
}
...
function f_xzxm(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="/dzbjseven/zxbjmx/bj_khbjmxMain.jsp?dqbm=<%=dqbm%>";

	FormName.submit();
	return true;
}

function f_jzbj(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		alert("请输入[客户编号]！");
		FormName.khbh.focus();
	...
	FormName.action="/dzbjseven/zxbjmx/bj_khbjmxMainJz.jsp?dqbm=<%=dqbm%>";
...
function f_dybj(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="ylbj.jsp?dqbm=<%=dqbm%>";
 ...
 function f_excel(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/Bj_khbjmxExcelList.jsp?dqbm=<%=dqbm%>";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/Bj_khbjmxExcelList.jsp?dqbm=<%=dqbm%>";
	}
...
function f_drmb(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/drmb.jsp?dqbm=<%=dqbm%>";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/drmb.jsp?dqbm=<%=dqbm%>";
	}
...
function f_tbjjb(FormName)//参数FormName:Form的名称
{
	...
	if ("<%=jzbz%>"=="1")
	{
		FormName.action="/dzbjseven/zxbjmx/EditAllBjjb.jsp";
	}
	else{
		FormName.action="/dzbjseven/zxbjmx/EditAllBjjb.jsp";
	}
...
function f_xmpx(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		...
		winOpen("/dzbjseven/zxbjmx/TzXhBjxmList.jsp?khbh=<%=khbh%>",900,600,'YES',"NO");
}
function f_fjxmpx(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		...
		winOpen("/dzbjseven/zxbjmx/TzFjXhBjxmList.jsp?khbh=<%=khbh%>",900,600,'YES',"NO");
}
...
function f_xmdj(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		...
		winOpen("/dzbjseven/zxbjmx/TzDjBjxmList.jsp?khbh=<%=khbh%>",900,600,'YES',"NO");
...
function f_dxzk(FormName)//参数FormName:Form的名称
{
	...
	window.open("/dzbjseven/zxbjmx/Bj_sfxmmxZklList.jsp?khbh=<%=khbh%>");
}
...
function f_zjsl(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="/dzbjseven/zxbjmx/zjtccl.jsp";
	FormName.submit();
	return true;
}

function f_zj(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		alert("请输入[客户编号]！");
		FormName.khbh.focus();
	...
	FormName.action="/dzbjseven/zxbjmx/BjDyList.jsp?dqbm=<%=dqbm%>";
...
function f_jzzc(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		alert("请输入[客户编号]！");
		FormName.khbh.focus();
	...
	FormName.action="/dzbjseven/zxbjmx/Bj_khzcxmxjCxList.jsp?dqbm=<%=dqbm%>";

	FormName.submit();
	return true;
}

function f_jzgc(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.khbh)=="") {
		alert("请输入[客户编号]！");
		FormName.khbh.focus();
	...
	FormName.action="/dzbjseven/zxbjmx/Bj_khbjmxList3Jz.jsp?dqbm=<%=dqbm%>";
