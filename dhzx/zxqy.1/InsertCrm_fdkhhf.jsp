...
try {
	...
	ls_sql="select ssfgs,sjs,khbh,khxm,lxfs,xb,fwdz,zxdm,zxzt ";
	...
	ls_sql+=" ,zgshjl,zgshr,zgshsj,zgshyj";
	...
	if (rs.next())
	{
		...
		zgshjl=cf.fillNull(rs.getString("zgshjl"));
		zgshr=cf.fillNull(rs.getString("zgshr"));
		zgshsj=cf.fillNull(rs.getDate("zgshsj"));
		zgshyj=cf.fillNull(rs.getString("zgshyj"));
	}
	rs.close();
	ps.close();
...
<form method="post" action="SaveInsertCrm_fdkhhf.jsp" name="selectform">
...
%></td>
	  ...
	  <tr bgcolor="#FFFFFF">
	    <td align="right" bgcolor="#FFFFCC">联系电话</td>
	    <td colspan="3" bgcolor="#FFFFCC"><%=lxfs%></td>
      </tr>
	  <tr bgcolor="#FFFFFF">
...
%>        </td>
      ...
      <tr bgcolor="#FFFFFF">
        <td align="right" bgcolor="#FFFFFF">接待主管审核结论</td>
        <td><%
	cf.radioToken(out,"Y+通过&N+未通过",zgshjl,true);
%>        </td>
        <td align="right">&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
      <tr bgcolor="#FFFFFF">
        <td align="right">接待主管审核人</td>
        <td><%=zgshr%>        </td><td align="right">接待主管审核时间</td>
        <td><%=zgshsj%>        </td></tr>
      <tr bgcolor="#FFFFFF" >
        <td align="right" >接待主管审核意见</td>
        <td colspan="3" ><%=zgshyj%></td></tr>
      <tr bgcolor="#FFFFFF">
        <td align="right"><span class="STYLE1">*</span>回访人</td>
        <td><input name="hfr" type="text"  value="<%=yhmc%>" size="20" maxlength="10" >        </td>
        <td align="right"><span class="STYLE1">*</span>回访时间</td>
        <td><input name="hfsj" type="text" value="<%=cf.today()%>" size="20" maxlength="10" onDblClick="JSCalendar(this)">        </td>
      </tr>
      <tr bgcolor="#FFFFFF">
        <td align="right"><span class="STYLE1">*</span>回访挽回客户</td>
        <td><%
	cf.radioToken(out,"hfwhkh", "Y+挽回&N+否","");
%></td>
        <td align="right">&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
...
%>
      ...
      <tr bgcolor="#FFFFFF">
        <td align="right">飞单时间</td>
        <td><input type="text" name="hfsbsj" value="<%=cf.today()%>" size="20" maxlength="10" onDblClick="JSCalendar(this)"></td>
        <td align="right">&nbsp;</td>
        <td>&nbsp;</td>
      </tr>
      <tr bgcolor="#FFFFFF">
        <td align="right"><span class="STYLE1">*</span>飞单原因详述</td>
        <td colspan="3"><textarea name="hfsbyyxs" cols="71" rows="6"></textarea>        </td>
      </tr>
      <tr bgcolor="#FFFFFF">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if (FormName.hfwhkh[1].checked)
	{
		...
		if(	javaTrim(FormName.hfsbsj)=="") {
			alert("请输入[回访飞单时间]！");
			FormName.hfsbsj.focus();
			return false;
		}
