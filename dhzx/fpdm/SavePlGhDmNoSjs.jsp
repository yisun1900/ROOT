...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		String zxzt="5";
			...
			if (!yzxzt.equals("2") && fpsj<2 && ysjs.equals(""))//2天内只做修改，0：未分配店面；1: 分配店面；2: 设计师提交飞单；3：已签约；4：飞单；5：飞单审核未通过
			{
				...
				ls_sql="update crm_zxkhxx set zxdm=?,zxdmfpsj=?,sjs=null,sjsfpsj=null";//zxzt,0：未分配店面；1: 分配店面；2: 设计师提交飞单；3：已签约；4：飞单；5：飞单审核未通过
				ls_sql+=" where  (khbh='"+khbh[i]+"') ";
//				out.println(ls_sql);
				ps= conn.prepareStatement(ls_sql);
				ps.setString(1,zxdm);
				...
				ls_sql="update crm_sjsjdjl set jssj=TRUNC(SYSDATE),jslrr=?,sbyyxs='"+ghdmyy+"'";
				...
				ls_sql="update crm_zxkhxx set zxdm=?,zxdmfpsj=?,sjs=null,sjsfpsj=null,zxzt=?,khfwjlh=?,dmjdjlh=?";//zxzt,0：未分配店面；1: 分配店面；2: 设计师提交飞单；3：已签约；4：飞单；5：飞单审核未通过
				ls_sql+=" where  (khbh='"+khbh[i]+"') ";
			//	out.println(ls_sql);
				ps= conn.prepareStatement(ls_sql);
				ps.setString(1,zxdm);
