...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_zxhfjl.hfjlh,crm_zxhfjl.hfcs,DECODE(crm_zxhfjl.clfs,'0','不提醒','1','提醒店面','2','提醒设计师'),crm_zxkhxx.khxm,crm_zxkhxx.fwdz,crm_zxkhxx.sjs,crm_zxhfjl.hfsj,crm_zxhfjl.hfr,crm_zxhfjl.khhfqk,sq_dwxx.dwmc, DECODE(crm_zxhfjl.lfbz,'N','未量房','Y','已量房'),crm_zxhfjl.lfsj, DECODE(crm_zxhfjl.ctbz,'N','未出图','Y','已出图'),crm_zxhfjl.ctsj, DECODE(crm_zxhfjl.yqghdm,'N','不要求','Y','要求更换'), DECODE(crm_zxhfjl.yqghsjs,'N','不要求','Y','要求更换'),c.hfjgmc sjszysp,d.hfjgmc sjsfw,e.hfjgmc qtryfw,f.hfjgmc gcbj,g.hfjgmc gsztyx,h.hfjgmc xwgjfm, DECODE(crm_zxhfjl.sfjxhf,'Y','继续回访','N','回访结束'),crm_zxhfjl.xchfrq,crm_zxhfjl.khbh  ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxhfjlList.jsp","","","EditCrm_zxhfjl.jsp");
...
//设置列超级连接
	...
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("crm_zxkhxx_khxm",coluParm);//列参数对象加入Hash表
	pageObj.setColuLink(coluParmHash);//列参数对象加入Hash表
}
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="3%">回访记录号</td>
	<td  width="2%">回访次数</td>
	<td  width="4%">是否提醒责任人</td>
	<td  width="2%">客户姓名</td>
	<td  width="8%">房屋地址</td>
	<td  width="3%">设计师</td>
	<td  width="3%">回访时间</td>
	<td  width="3%">回访人</td>
	<td  width="18%">客户回访情况</td>
	<td  width="5%">回访部门</td>
	<td  width="3%">量房标志</td>
	<td  width="3%">量房时间</td>
