...
try {
	...
	ls_sql =" update crm_zxkhxx set lfbz=?,lfsj=?,ctbz=?,ctsj=?,sjszysp=?,sjsfw=?,qtryfw=?,gcbj=?,gsztyx=?,xwgjfm=?   ,yqghdm=?,yqghsjs=?  ,hfjlh=?,zxhfsj=TRUNC(SYSDATE),khfwjlh=?,yzxxbz=? ,cxgtbz=?,cxgtsj=?,csgtbz=?,csgtsj=?,zxkhyxbm=?,yylfsj=?,xclfsj=?,sjsgtnl=?,hfcs=?";
	ls_sql+=" where khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,lfbz);
	ps.setDate(2,lfsj);
	...
	ps.setString(11,yqghdm);
	ps.setString(12,yqghsjs);
	ps.setString(13,hfjlh);
	ps.setString(14,khfwjlh);
	ps.setString(15,yzxxbz);
	...
	ps.setString(23,sjsgtnl);
	ps.setString(24,hfcs);
	ps.executeUpdate();
	ps.close();

	ls_sql =" update crm_khsmjl set sfhf='Y',hfr=?,hfsj=TRUNC(SYSDATE),hfsm=? ";
	ls_sql+=" where jlh='"+jlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,hfr);
	ps.setString(2,khhfqk);
	ps.executeUpdate();
	ps.close();
