...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_tdjdjList.jsp","SelectCrm_tdjdj.jsp","ViewCrm_tdjdj.jsp","EditCrm_tdjdj.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_tdjdjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] tdjxh = request.getParameterValues("tdjxh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(tdjxh,"tdjxh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from crm_tdjdj where "+chooseitem;
