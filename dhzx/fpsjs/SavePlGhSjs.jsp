...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		int fpsj=0;
		ls_sql="select ssfgs,zxzt,sfxhf,hfrq,zxdm,sjs,sbyybm,khfwjlh,sjsjdjlh,sjsfpsj,SYSDATE-sjsfpsj fpsj";
		...
		sjsjdjlh=khbh[i]+cf.addZero(count+1,3);
		...
		if (ysjs.equals(sjs))//未改变
		{
			...
			//	out.println(ls_sql);
				...
				//结束：设计师接单记录（crm_sjsjdjl）
				ls_sql="update crm_sjsjdjl set jssj=TRUNC(SYSDATE),jslrr=?,sbyyxs='更换设计师'";
				ls_sql+=" where  sjsjdjlh='"+ysjsjdjlh+"' and jssj is null";
				ps= conn.prepareStatement(ls_sql);
				ps.setString(1,yhmc);
				ps.executeUpdate();
				ps.close();
				...
				ls_sql="insert into crm_zxkhfwjl (khfwjlh,khbh,ssfgs,zxdm,sjs,czlx,xzxzt,yzxzt,ysfxhf,yhfrq,yzxdm,ysjs,ysbyybm,ykhfwjlh,lrr,lrsj,lrbm)";
				ls_sql+=" values ( ?,?,?,?,?,'8',?,?,?,?,?,?,?,?,?,TRUNC(SYSDATE),?)";//8：换设计师
				ps= conn.prepareStatement(ls_sql);
				...
				ps.setString(7,yzxzt);
				ps.setString(8,ysfxhf);
				ps.setDate(9,yhfrq);
				ps.setString(10,yzxdm);
				ps.setString(11,ysjs);
				ps.setString(12,ysbyybm);
				ps.setString(13,ykhfwjlh);
				ps.setString(14,yhmc);
