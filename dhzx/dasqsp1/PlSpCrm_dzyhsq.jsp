...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		if (spjg.equals("1"))//1：同意；2：不同意
		{
			...
			while (rs.next())
			{
				sfxmbm=rs.getString("sfxmbm");
				spdxzkl=rs.getDouble("spdxzkl");

				ls_sql="update bj_sfxmmx set dxzkl=? ";
				ls_sql+=" where  khbh='"+khbh[i]+"' and  sfxmbm='"+sfxmbm+"'";
				ps1= conn.prepareStatement(ls_sql);
				ps1.setDouble(1,spdxzkl);
				ps1.executeUpdate();
				ps1.close();
			}
			rs.close();
			...
			ls_sql="update crm_dxdzyh set spdxzkl=10";
			ls_sql+=" where khbh='"+khbh[i]+"'";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
