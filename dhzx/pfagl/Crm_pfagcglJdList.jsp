...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT pfajlh, crm_zxkhxx.khxm,DECODE(crm_zxkhxx.lfbz,'N','未量房','Y','已量房'),crm_zxkhxx.fwdz,DECODE(sfzdzbj,'N','未做','Y','已做'),dzbjze,bjjbmc,DECODE(crm_zxkhxx.ysshbz,'N','未审核','Y','审核通过','M','审核未通过'),crm_zxkhxx.ysshsj,crm_zxkhxx.ysshr,crm_zxkhxx.ysshyj,lrsj,dwmc,sjs,ywy,crm_zxkhxx.hfrq,crm_zxkhxx.khbh,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') as crm_zxkhxx_zxzt";
	ls_sql+=" FROM crm_zxkhxx,sq_dwxx,bdm_bjjbbm,crm_pfagcgl ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_pfagcglJdList.jsp","","EditCrm_pfagcgl.jsp","");
...
//设置主键
	String[] keyName={"pfajlh"};
	pageObj.setKey(keyName);
	pageObj.setViewStr("修改");
	pageObj.setViewBolt("target='_blank'");
	//设置按钮参数
	String[] buttonName={"删除选中的信息"};//按钮的显示名称
	String[] buttonLink={"Crm_pfagcglJdList.jsp"};//按钮单击调用的网页，可以增加参数
...
{
	String[] pfajlh = request.getParameterValues("pfajlh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(pfajlh,"pfajlh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="update crm_pfagcgl set sjsj=null,sjry=null,gc=null,jd=null  where "+chooseitem;
