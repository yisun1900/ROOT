...
try {
	...
	for (int i=0;i<ysshjl.length ;i++ )
	{
		...
		ls_sql="update crm_zxkhxx set ysshbz='"+yysshbz+"'";//N：未申请；B：申请审核；C：受理申请；Y：审核通过；M：审核未通过
		ls_sql+=" where  khbh='"+khbh+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();

		ls_sql="delete from bj_ysshjl ";
		ls_sql+=" where ysshjl='"+ysshjl[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql="update bj_ysshjl set zxbz='Y'";
		ls_sql+=" where ysshjl='"+yysshjl+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
	}
