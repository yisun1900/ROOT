...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String ysshsj=null;
	ysshsj=request.getParameter("ysshsj");
	if (ysshsj!=null)
	{
		ysshsj=ysshsj.trim();
		if (!(ysshsj.equals("")))	wheresql+="  and (bj_ysshjl.ysshsj>=TO_DATE('"+ysshsj+"','YYYY/MM/DD'))";
	}
	ysshsj=request.getParameter("ysshsj2");
	if (ysshsj!=null)
	{
		ysshsj=ysshsj.trim();
		if (!(ysshsj.equals("")))	wheresql+="  and (bj_ysshjl.ysshsj<=TO_DATE('"+ysshsj+"','YYYY/MM/DD'))";
	...
	String ysshr=null;
	ysshr=request.getParameter("ysshr");
	if (ysshr!=null)
	{
		ysshr=cf.GB2Uni(ysshr);
		if (!(ysshr.equals("")))	wheresql+=" and  (bj_ysshjl.ysshr='"+ysshr+"')";
	}
	


	ls_sql="SELECT bj_ysshjl.ysshjl,DECODE(crm_zxkhxx.ysshbz,'N','未申请','B','申请审核','C','受理申请','Y','审核通过','M','未通过','S','修改方案授权'),crm_zxkhxx.khbh,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,crm_zxkhxx.sjs,bj_ysshjl.jsy,bj_ysshjl.xgfasqr,bj_ysshjl.xgfasqsj,bj_ysshjl.xgfasqsm,bj_ysshjl.ysshr,bj_ysshjl.ysshsj,bj_ysshjl.ysshyj,khjl,ywy,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') as crm_zxkhxx_zxzt,sq_dwxx.dwmc";
	...
	ls_sql+=" and bj_ysshjl.ysshbz in('S')";
	...
	ls_sql+=wheresql;
    ls_sql+=" order by bj_ysshjl.xgfasqsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxXgFasqList.jsp","","","");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteSqBj_ysshjl.jsp"};//按钮单击调用的网页，可以增加参数
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="3%">设计师</td>
	<td  width="3%">价审员</td>
	<td  width="3%">修改方案授权人</td>
	<td  width="4%">修改方案授权时间</td>
	<td  width="18%">修改方案授权说明</td>
	<td  width="3%">审核人</td>
	<td  width="4%">审核时间</td>
	<td  width="17%">审核意见</td>
	<td  width="3%">客户经理</td>
