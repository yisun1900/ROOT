...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		ysshsj=ysshsj.trim();
		if (!(ysshsj.equals("")))	wheresql+="  and (crm_zxkhxx.ysshsj>=TO_DATE('"+ysshsj+"','YYYY/MM/DD'))";
	}
	...
	if (ysshr!=null)
	{
		...
		if (!(ysshr.equals("")))	wheresql+=" and  (crm_zxkhxx.ysshr='"+ysshr+"')";
	}
	String ysshbz=null;
	...
	ls_sql="SELECT crm_zxkhxx.khbh,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,DECODE(crm_zxkhxx.ysshbz,'N','未申请','B','申请审核','C','受理申请','Y','审核通过','M','审核未通过'),ysshsj,ysshr,ysshyj,sjs,khjl,ywy,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') as crm_zxkhxx_zxzt,lrsj,sq_dwxx.dwmc";
	ls_sql+=" FROM crm_zxkhxx,sq_dwxx ";
    ls_sql+=" where crm_zxkhxx.zxdm=sq_dwxx.dwbh(+) ";
	ls_sql+=" and crm_zxkhxx.ysshbz!='N'";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxCxYsShList.jsp","","","");
