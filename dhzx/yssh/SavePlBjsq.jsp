...
String sqbz=cf.GB2Uni(request.getParameter("sqbz"));
...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		String bjjb=null;
		String ybjbbh=null;
		ls_sql="SELECT zxzt,ssfgs,hxbm,bjjb,bjbbh";
		...
		if (rs.next())
		{
			...
			ybjbbh=cf.fillNull(rs.getString("bjbbh"));
		}
		rs.close();
		ps.close();
		...
		if (getzxzt.equals("3"))
		{
			...
			ls_sql+=" where khbh='"+khbh[i]+"' and wcbz='1'";//1：未完成；2：完成；3：业绩结转
			ps= conn.prepareStatement(ls_sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
		...
		{
			conn.rollback();
			out.println("错误！客户未做报价，不需授权:"+khbh[i]);
			return;
		}
		...
		ls_sql="insert into crm_bjbbsqjl (khbh,bjbbh,ybjbbh,sqr,sqsj,sqbz)";
		ls_sql+=" values(?,?,?,?,?,?) ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,khbh[i]);
		ps.setString(2,bjbbh);
		ps.setString(3,ybjbbh);
		ps.setString(4,sqr);
		ps.setDate(5,sqsj);
		ps.setString(6,sqbz);
		ps.executeUpdate();
