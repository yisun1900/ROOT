...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	sffp=request.getParameter("sffp");
	if (sffp!=null)
	{
		if (sffp.equals("Y"))	
		{
			wheresql+=" and  (crm_zxkhxx.ssfgs is not null)";
		}
		else{
			wheresql+=" and  (crm_zxkhxx.ssfgs is null)";
		}
	}
	...
	ls_sql="SELECT crm_zxkhxx.khbh,crm_zxkhxx.khxm,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') as crm_zxkhxx_zxzt,crm_zxkhxx.fwdz,b.dwmc as ssfgs,a.dwmc zxdm,sjs,fwlxmc,lrsj,crm_zxkhxx.hfrq,DECODE(crm_zxkhxx.hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他') as crm_zxkhxx_hdbz  ";
	ls_sql+=" FROM crm_zxkhxx,dm_fwlxbm,sq_dwxx a,sq_dwxx b ";
    ls_sql+=" where (crm_zxkhxx.fwlxbm=dm_fwlxbm.fwlxbm(+) )";
    ls_sql+=" and crm_zxkhxx.zxdm=a.dwbh(+) and crm_zxkhxx.ssfgs=b.dwbh(+) and zxzt not in('3','4')";
	ls_sql+=wheresql;
    ls_sql+=" order by crm_zxkhxx.khxm";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxCxList2.jsp","","","FpDmCrm_zxkhxx.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"plGhfgs.jsp"};//按钮单击调用的网页，可以增加参数
