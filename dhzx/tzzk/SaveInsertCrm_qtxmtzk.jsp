...
try {
	...
	for (int i=0;i<sfxmbm.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>收费项目编码："+sfxmbm[i]+"错误！[折扣]不能为空");
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>收费项目编码："+sfxmbm[i]+"错误！[折扣]不是数字类型："+zkStr[i].trim());
			return;
		}
		...
		{
			conn.rollback();
			out.println("<BR>收费项目编码："+sfxmbm[i]+"错误！[折扣]应该在0和10之间！");
			return;
		}

		ls_sql="insert into crm_qtxmtzk (tzjlh,khbh,sfxmbm,zk) ";
		ls_sql+=" values(?,?,?,?)";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,tzjlh);
		ps.setString(2,khbh);
		ps.setString(3,cf.GB2Uni(sfxmbm[i]));
		ps.setDouble(4,zk);
		ps.executeUpdate();
		ps.close();
