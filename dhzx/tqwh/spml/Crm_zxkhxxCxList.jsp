...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_zxkhxx.khbh,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,sjs,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过'),NVL(DECODE(mlspbz,'M','未审批','N','不同意','Y','同意'),'未审批'),khmll||'%',mlspr,mlspsj,mlspyj,lrsj,DECODE(sfzdzbj,'N','未做','Y','已做') sfzdzbj,dzbjze,bjjbmc,ywy,a.dwmc zxdjbm,b.dwmc as zxdm,fwlxmc as crm_zxkhxx_fwlxbm,crm_zxkhxx.hfrq as crm_zxkhxx_hfrq";
	...
	ls_sql+=" and (crm_zxkhxx.mlspbz='M' OR crm_zxkhxx.mlspbz is null)";//M：未审批；N：不同意；Y：同意
    ls_sql+=" order by crm_zxkhxx.lrsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxCxList.jsp","","","InsertCrm_tzzkjl.jsp");
