...
try {
	...
	if (!clzt.equals("1"))//1：申请；2：发布；3：已预交底
	{
	...
	ls_sql="update crm_yjdjl set zzqrjdrq=?,zzqrjdsj=?,fbr=?,fbsj=?,sgd=?,sgbz=?,zjxm=?,clzt='2' ";
	ls_sql+=" where  jdjlh='"+jdjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	ls_sql="update crm_zxkhxx set xcjdbz='3',sgd=?,sgbz=?,zjxm=?";
