...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql+=" ,crm_tsjl.slsj,crm_tsbm.dwbh,sq_dwxx.dwmc zrbm,yhmc,crm_tsbm.tzsj,crm_tsbm.tsyybm,crm_tsbm.tspp";
	...
	ls_sql+=" FROM crm_tsjl,crm_khxx,dm_slfsbm,dm_tsxlbm,crm_tsbm,sq_dwxx,sq_sgd,sq_yhxx ";
    ls_sql+=" where crm_tsjl.slfsbm=dm_slfsbm.slfsbm and crm_tsjl.tsjlh=crm_tsbm.tsjlh";
    ls_sql+=" and crm_tsbm.ygbh=sq_yhxx.ygbh(+) ";
	...
	ls_sql+=" and crm_tsbm.clzt='1'";//1：未接收；2：接收后直接出方案不需审核；4：接收后直接出方案需审核；6：已接收等待出方案；7：等待出现场后出方案；8：已出现场方案不需审核；9：已出现场方案需审核；3：已解决；5：结案
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_tsjlJsList.jsp","","","JsCrm_tsbm.jsp");
//	pageObj.setPageRow(300);//设置每页显示记录数
//设置显示列
	String[] disColName={"tsjlh","clzt","zrbm","yhmc","tsxlmc","khxm","fwdz","sjs","sgdmc","slfsmc","slsj","jazt","tspp"};
