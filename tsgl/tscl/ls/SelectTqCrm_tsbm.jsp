...
<body bgcolor="#FFFFFF">
<form method="post" action="Crm_tsbmTqList.jsp" name="selectform">
          
        <div align="center">投诉处理－特权</div>
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr bgcolor="#FFFFCC"> 
      <td width="18%" align="right" >责任部门处理状态</td>
      <td colspan="3" >
        <input type="radio" name="clzt" value="1" checked>
        未解决 
      ...
      <td width="32%" > 
        <select name="dwbh" style="FONT-SIZE:12PX;WIDTH:152PX">
		  <%
			...
			{
				out.println("<option value=\"\"></option>");
				cf.selectItem(out,"select sq_tsclbm.dwbh,sq_dwxx.dwmc from sq_tsclbm,sq_dwxx where sq_tsclbm.dwbh=sq_dwxx.dwbh and  sq_dwxx.cxbz='N' and sq_tsclbm.ssfgs='"+ssfgs+"' order by sq_tsclbm.dwbh","");
			}
			else if (kfgssq.equals("3"))
			{
				cf.selectItem(out,"select dwbh,dwmc from sq_dwxx where dwlx='F2' and dwbh  dwbh in(select dwbh from sq_sqbm where ygbh='"+ygbh+"') ","");
			}
			else if (kfgssq.equals("4"))
			{
				out.println("<option value=\"\"></option>");
				cf.selectItem(out,"select dwbh,dwmc from sq_dwxx where dwlx='F2' and dwbh  dwbh in(select dwbh from sq_sqbm where ygbh='"+ygbh+"') order by dwbh ","");
			}
			else{
				out.println("错误！未进行客户资料授权，请联系系统管理员");
				return;
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="32%" > 
        <select name="crm_tsbm_tslxbm" style="FONT-SIZE:12PX;WIDTH:152PX" onclick="getThisOne('tslxbm')" onChange="tsxlbmMark=0;crm_tsbm_tsxlbm.length=1">
          <option value=""></option>
        </select>
      </td>
      <td width="18%"  align="right">投诉小类</td>
      <td width="32%" > 
        <select name="crm_tsbm_tsxlbm" style="FONT-SIZE:12PX;WIDTH:152PX" onclick="getThisOne('tsxlbm')">
          <option value=""></option>
        </select>
      </td>
    </tr>
    <tr bgcolor="#999999"> 
      <td width="18%" bgcolor="#E8E8FF" align="right"><b><font color="#0000CC">投诉报修品牌</font></b></td>
...
function changeFgs(FormName) 
{
	...
	{
		sql="select sq_tsclbm.dwbh,sq_dwxx.dwmc from sq_tsclbm,sq_dwxx where sq_tsclbm.dwbh=sq_dwxx.dwbh and  sq_dwxx.cxbz='N' and sq_tsclbm.ssfgs='"+FormName.fgs.value+"' order by sq_tsclbm.dwbh";
	}
	else{
		sql="select dwbh,dwmc from sq_dwxx where dwlx='F2' and  cxbz='N' and ssfgs='"+FormName.fgs.value+"' and dwbh in(select dwbh from sq_sqbm where ygbh='<%=ygbh%>') order by dwbh";
	}
