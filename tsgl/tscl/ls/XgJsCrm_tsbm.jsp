...
try {
	...
	ls_sql="select crm_khxx.fgsbh,crm_tsjl.slsj,crm_tsjl.slr,crm_tsjl.lrr,crm_tsjl.lrsj,crm_tsjl.tsbxsj,crm_tsjl.sfxtz,tsnr,slbm,crm_tsjl.bz,yzcdbm,slfsbm,hth,crm_khxx.khbh,khxm,fwdz,lxfs,qtdh,sjs,sgd,sgbz,crm_khxx.dwbh,zjxm,kgrq,jgrq,sjkgrq,sjjgrq,crm_tsjl.sfxhf,crm_tsjl.hfsj,clzt,crm_tsjl.yqjjsj";
	...
	ls_sql="select crm_tsbm.dwbh,dwmc,tslxbm,tsxlbm,ldyqjjsj,ldclyj,ldclr,ldclsj,clzt,jssm";
	ls_sql+=" ,crm_tsbm.jhcfasj,crm_tsbm.sjcfasj,crm_tsbm.jhjjsj,crm_tsbm.jjfa,crm_tsbm.fashbm,crm_tsbm.fashr";
...
//	out.println(ls_sql);
	...
	if (rs.next())
	{
		...
		zrbmclzt=cf.fillNull(rs.getString("clzt"));
		...
		fashbm=cf.fillNull(rs.getString("fashbm"));
		fashr=cf.fillNull(rs.getString("fashr"));
		jssm=cf.fillNull(rs.getString("jssm"));
	}
	rs.close();


	if (zrbmclzt.equals("2"))
	{
		jhfaStr="display:block";
		sjfaStr="display:none";
		jhjjStr="display:block";
		fashStr="display:none";
		fanrStr="display:none";
	}
	else if (zrbmclzt.equals("4"))
	{
		jhfaStr="display:none";
		sjfaStr="display:block";
		jhjjStr="display:block";
		fashStr="display:none";
		fanrStr="display:block";
	}
	else if (zrbmclzt.equals("6"))
	{
		jhfaStr="display:none";
		sjfaStr="display:block";
		jhjjStr="display:block";
		fashStr="display:block";
		fanrStr="display:block";
	}
%>
...
<div align="center">接收投诉－修改（投诉记录号：<%=tsjlh%>） </div>
...
<%
	...
	while (rs.next())
	{
		...
		getjjfa=cf.fillHtml(rs.getString("jjfa"));
...
<form method="post" action="SaveXgJsCrm_tsbm.jsp" name="editform">
        <div align="center"><b>（<%=cldwmc%>）接收投诉－修改</b></div>
...
%> 
    ...
    <tr> 
      <td width="18%" align="right"><font color="#FF0000">*</font><font color="#0000CC">投诉报修原因</font></td>
...
%>
    ...
    <tr>
      ...
      <td colspan="3"><textarea name="jssm" cols="87" rows="5" ><%=jssm%></textarea></td>
    </tr>
    <tr> 
      <td width="18%" align="right"><font color="#FF0000">*</font>处理计划</td>
      <td colspan="3">
		<INPUT type="radio" name="clzt" value="2" <%if (zrbmclzt.equals("2")) out.print("checked");%> onClick="jhfa.style.display='block';sjfa.style.display='none';jhjj.style.display='block';fanr.style.display='none';fash.style.display='none'">已接收，等待出方案<BR>
		<INPUT type="radio" name="clzt" value="4" <%if (zrbmclzt.equals("4")) out.print("checked");%> onClick="jhfa.style.display='none';sjfa.style.display='block';jhjj.style.display='block';fanr.style.display='block';fash.style.display='none'">已出方案，方案不需审核<BR>
	<INPUT type="radio" name="clzt" value="6" <%if (zrbmclzt.equals("6")) out.print("checked");%> onClick="jhfa.style.display='none';sjfa.style.display='block';jhjj.style.display='block';fanr.style.display='block';fash.style.display='block'">已出方案，方案需审核<BR>    </tr>
    
    <tr bgcolor="#FFFFFF" id="jhfa" style="<%=jhfaStr%>">
    ...
    <tr bgcolor="#FFFFFF" id="sjfa" style="<%=sjfaStr%>">
      <td align="right"><font color="#FF0000">*</font>实际出方案时间</td>
      <td><input type="text" name="sjcfasj" size="20" maxlength="10" value="<%=sjcfasj%>" onDblClick="JSCalendar(this)"></td>
      <td align="right">&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <tr bgcolor="#FFFFFF" id="jhjj" style="<%=jhjjStr%>">
    ...
    <tr bgcolor="#FFFFFF" id="fash" style="<%=fashStr%>">
      ...
      <td><select name="fashbm" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="getZrbm(editform)">
        <option value=""></option>
        <%
	cf.selectItem(out,"select dwbh,dwmc from sq_dwxx where ssfgs='"+ssfgs+"' and cxbz='N' order by dwbh",fashbm);
%>
      </select></td>
      ...
      <td><select name="fashr" style="FONT-SIZE:12PX;WIDTH:152PX">
        <option value=""></option>
         <%
	cf.selectItem(out,"select ygbh,yhmc||'('||xzzwbm||')' from sq_yhxx where sq_yhxx.sfzszg in('Y','N') and dwbh='"+fashbm+"' order by yhmc",fashr);
%>
     </select></td>
    </tr>
    
    <tr bgcolor="#FFFFFF" id="fanr" style="<%=fanrStr%>">
      <td width="18%" align="right"><font color="#FF0000">*</font>解决方案</td>
      <td colspan="3"> 
        <textarea name="jjfa" cols="87" rows="10"><%=jjfa%></textarea>      </td>
    </tr>
    <tr> 
