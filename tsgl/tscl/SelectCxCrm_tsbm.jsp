...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isDatetime(FormName.jhcxcsj2, "计划出现场时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.sjcxcsj, "实际出现场时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.sjcxcsj2, "实际出现场时间"))) {
		return false;
	}
	...
	if(!(isDatetime(FormName.jhcfasj2, "计划出方案时间"))) {
		return false;
	}
	...
	if(!(isDatetime(FormName.sjcfasj2, "实际出方案时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.shsj, "实际出方案时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.shsj2, "方案审核时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.jhjjsj, "方案审核时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.jhjjsj2, "计划解决时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.clsj, "最新处理时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.clsj2, "最新处理时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.sjjjsj, "责任部门解决时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.sjjjsj2, "责任部门解决时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.qrjjsj, "回访确认解决时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.qrjjsj2, "回访确认解决时间"))) {
		return false;
	}
	FormName.submit();
	return true;
