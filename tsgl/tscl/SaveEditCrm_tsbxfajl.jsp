...
try {
	...
	ls_sql="update crm_tsbxfajl set jjfa=?,sjcfar=?,sjcfasj=?,jhjjsj=?,sfpfkh=?,khpfze=?,sfnbcf=?,nbcfze=?,lrr=?,lrsj=?,lrbm=? ";
	...
	ls_sql="update crm_tsbm set jjfa=?,sjcfar=?,sjcfasj=?,jhjjsj=?,sfpfkh=?,khpfze=?,sfnbcf=?,nbcfze=?,clzt=? ";
	ls_sql+=" where tsjlh='"+tsjlh+"' and tsyybm='"+tsyybm+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,jjfa);
	ps.setString(2,sjcfar);
