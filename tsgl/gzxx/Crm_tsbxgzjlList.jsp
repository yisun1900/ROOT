...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_tsbxgzjlList.jsp","","","EditCrm_tsbxgzjl.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_tsbxgzjlList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] gzjlh = request.getParameterValues("gzjlh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(gzjlh,"gzjlh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from crm_tsbxgzjl where "+chooseitem;
