...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_tsbxxckcjlList.jsp","SelectCrm_tsbxxckcjl.jsp","","EditCrm_tsbxxckcjl.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_tsbxxckcjlList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] kcjlh = request.getParameterValues("kcjlh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(kcjlh,"kcjlh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[2];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from crm_xckczp where "+chooseitem;
		sql[1]="delete from crm_tsbxxckcjl where "+chooseitem;
