...
try {
	...
	rs.close();
	ps.close();
	if (!getclzt.equals("0"))//0：未受理；1：已受理；2：在处理；3：结案；9:不立案
	{
	...
	ls_sql="update crm_tsjl set slfsbm=?,yqjjsj=?,sfxhf=?,hfsj=?, bz=?,tsnr=?,clzt=?,wxsgd=?   ,tsbxsj=?,slr=?,slsj=?,zrbmclzt=?,sfxtz=?,sfxnbgz=?,nbgzsj=?";
	ls_sql+=" where tsjlh='"+tsjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setString(6,tsnr);
	ps.setString(7,clzt);
	ps.setString(8,wxsgd);

	ps.setDate(9,tsbxsj);
	ps.setString(10,slr);
	ps.setDate(11,slsj);
	ps.setString(12,"1");//1：未处理；2：在处理；3：已解决
	ps.setString(13,sfxtz);
	...
	ls_sql="update crm_tsbm set clzt=?,sfytz=?";
	ls_sql+=" where tsjlh='"+tsjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,"1");//1：未处理；2：在处理；3：已解决
	ps.setString(2,sfytz);
	ps.executeUpdate();
	ps.close();
