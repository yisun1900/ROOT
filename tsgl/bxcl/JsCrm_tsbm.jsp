...
try {
	...
	ls_sql+=" ,crm_tsjl.hsgdyy,crm_tsjl.sfhsgd,crm_tsjl.pdsgd,crm_tsjl.ysgd,crm_tsjl.ysgdsflz,crm_tsjl.wxfl,crm_khxx.fgsbh";
...
<div align="center">报修处理－接收（报修记录号：<%=tsjlh%>） </div>
...
<form method="post" action="SaveJsCrm_tsbm.jsp" name="editform">
        <div align="center"><b>报修处理－接收</b></div>
...
%></td>
      ...
      <td><input type="text" name="jhcxcsj" size="20" maxlength="10" value="" onDblClick="JSCalendar(this)"></td>
    </tr>
    <tr>
    ...
    <tr bgcolor="#FFFFFF">
      <td height="27" align="right"><font color="#0000CC">原施工队</font></td>
              <td>
...
%> 
      ...
      <td align="right"><font color="#0000CC">原施工队是否离职</font></td>
      <td><%
					cf.radioToken(out,"Y+离职&N+否",ysgdsflz,true);
				%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td height="27" align="right"><font color="#0000CC">维修分类</font></td>
      <td bgcolor="#FFFFFF"><%
    ...
    <tr>
      <td align="right"><font color="#FF0033">*</font>是否需施工队</td>
      <td><%
					cf.radioToken(out, "sfhsgd","Y+换施工队&N+原施工队&T+不需施工队","");
				%></td>
      <td align="right">维修施工队</td>
      <td><select name="pdsgd" style="FONT-SIZE:12PX;WIDTH:152PX">
        <option value=""></option>
		<%
...
%>
    ...
    <tr>
      <td align="right">换施工队原因</td>
      <td colspan="3"><textarea name="hsgdyy" cols="87" rows="3" ><%=hsgdyy%></textarea></td>
    </tr>
    <tr> 
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if (FormName.sfhsgd[0].checked)//换施工队
	{
	...
	else if (FormName.sfhsgd[1].checked)//原施工队
	{
		if (FormName.pdsgd.value!="<%=ysgd%>")
		{
			alert("提醒！【维修施工队】未选择【原施工队】，系统自动做了选择！");
			FormName.pdsgd.value="<%=ysgd%>";
			FormName.pdsgd.focus();
			return false;
		}
		if(	javaTrim(FormName.pdsgd)=="") {
