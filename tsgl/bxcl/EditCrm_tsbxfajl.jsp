...
try {
	...
	ls_sql+=" ,crm_tsbm.jssm,crm_tsbm.jhcxcsj,crm_tsbm.jssj,crm_tsbm.jsr,crm_tsbm.jhcfasj,crm_tsbm.sjcxcsj,crm_tsbm.cxcsm,cfajlh";
	ls_sql+=" ,sffswxf,sfnbcf,wxfze,nbcfze,jhjjsj,jjfa,sjcfar,sjcfasj";
	...
	if (rs.next())
	{
		...
		sffswxf=cf.fillNull(rs.getString("sffswxf"));
		sfnbcf=cf.fillNull(rs.getString("sfnbcf"));
		wxfze=rs.getDouble("wxfze");
...
<html>
<head>
<title>报修处理－出方案－修改</title>
<meta http-equiv="Content-Type" content="text/html; charset=GBK">
</head>

<body bgcolor="#FFFFFF" style="FONT-SIZE:14">
<div align="center">报修处理－出方案－修改（报修记录号：<%=tsjlh%>） </div>
...
<table width="90%" border="1" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="1" cellspacing="0">
  <tr bgcolor="#FFFFFF" align="center"> 
	...
	<td width="6%"><strong>维修费总额</strong></td>
	<td width="6%"><strong>内部处罚总额</strong></td>
  </tr>
<%
	String fa_dwmc="";
	String fa_tsyymc="";
	...
	ls_sql+=" ,DECODE(crm_tsbxfajl.shjl,'Y','通过','N','未通过','未审核') shjl,crm_tsbxfajl.cfajlh,crm_tsbxfajl.sjcfasj,crm_tsbxfajl.sjcfar,crm_tsbxfajl.jhjjsj,crm_tsbxfajl.jjfa,DECODE(crm_tsbxfajl.sffswxf,'1','不需赔付','2','需赔付') sffswxf,crm_tsbxfajl.wxfze,DECODE(crm_tsbxfajl.sfnbcf,'1','不需处罚','2','需处罚') sfnbcf,crm_tsbxfajl.nbcfze";
...
<form method="post" action="SaveEditCrm_tsbxfajl.jsp" name="editform"  >
        <div align="center"><b>报修处理－出方案－修改</b>（出方案记录号：<%=cfajlh%>）</div>
...
%> 
    ...
    <tr>
      ...
      <td><%=jhcfasj%></td>
      <td align="right"><font color="#0000CC">出现场时间</font></td>
      <td><%=sjcxcsj%></td>
    </tr>
    <tr>
    ...
    <tr>
      <td align="right"><strong>出现场说明</strong></td>
    ...
    <tr>
      ...
      <td colspan="3"><%
	cf.radioToken(out, "clzt","7+已出方案等待审核&8+已出方案不需审核",clzt);
%></td>
    </tr>
    
    <tr>
      <td align="right"><font color="#FF0000">*</font>是否发生维修费</td>
      <td><%
	cf.radioToken(out, "sffswxf","1+未发生&2+发生",sffswxf);
%></td>
      <td align="right"><font color="#FF0000">      *</font>是否内部处罚</td>
      <td><%
	cf.radioToken(out, "sfnbcf","1+不需处罚&2+需处罚",sfnbcf);
%></td>
    </tr>
    <tr>
      <td align="right">维修费总额</td>
      <td><input type="text" name="wxfze" value="<%=wxfze%>" size="20" maxlength="20" ></td>
      <td align="right">内部处罚总额</td>
      <td><input type="text" name="nbcfze" value="<%=nbcfze%>" size="20" maxlength="20" ></td>
    </tr>
    ...
    <tr bgcolor="#FFFFFF">
      <td align="right" bgcolor="#FFFFFF"><font color="#FF0000">*</font>解决方案</td>
      <td colspan="3"><textarea name="jjfa" cols="71" rows="9"><%=jjfa%></textarea>      </td>
    </tr>
    <tr> 
      <td width="18%" align="right"><font color="#FF0000">*</font>实际出方案人</td>
      <td width="32%"> 
        <input type="text" name="sjcfar" value="<%=sjcfar%>" size="20" maxlength="20" >      </td>
      <td width="17%" align="right"><font color="#FF0000">*</font>实际出方案时间</td>
      <td width="33%"> 
        <input type="text" name="sjcfasj" value="<%=sjcfasj%>" size="20" maxlength="10" onDblClick="JSCalendar(this)">      </td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if (FormName.sffswxf[1].checked)
	{
		...
		if(!(isFloat(FormName.wxfze, "维修费总额"))) {
			return false;
		}
	}
	else{
	...
	if (FormName.sfnbcf[1].checked)
	{
		...
		if(!(isFloat(FormName.nbcfze, "内部处罚总额"))) {
			return false;
		}
	}
	else{
	...
	FormName.action="SaveEditCrm_tsbxfajl.jsp";
	FormName.submit();
	return true;
}
