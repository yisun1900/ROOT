...
try {
	...
	ls_sql="update crm_tsbxfajl set shsm=?,shr=?,shsj=?,shjl=?,sffswxf=?,wxfze=?,sfnbcf=?,nbcfze=?,clzt='2' ";
	...
	ls_sql="update crm_tsbm set shsm=?,shr=?,shsj=?,clzt=?,sffswxf=?,wxfze=?,sfnbcf=?,nbcfze=? ";
	ls_sql+=" where tsjlh='"+tsjlh+"' and tsyybm='"+tsyybm+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,shsm);
	ps.setString(2,shr);
	...
	ls_sql="update crm_tsjl set zrbmclzt=?,sffswxf=?,sfnbcf=?";//0：未受理；1：已受理；4：已接收；5：已出现场；6：已出方案；7：方案审核通过；2：在处理；3：结案；9:不立案
	ls_sql+=" where tsjlh='"+tsjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,clzt);
	ps.setString(2,ddsffswxf);
