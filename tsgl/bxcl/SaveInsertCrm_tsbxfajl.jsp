...
try {
	...
	ls_sql="insert into crm_tsbxfajl ( cfajlh,tsjlh,tsyybm,dwbh,jjfa,sjcfar,sjcfasj,jhjjsj,sffswxf,wxfze,sfnbcf,nbcfze,lrr,lrsj,lrbm,yddclzt,yzrbmclzt,ycfajlh,clzt ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'1') ";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setDate(8,jhjjsj);
	ps.setString(9,sffswxf);
	ps.setDouble(10,wxfze);
	...
	ls_sql="update crm_tsbm set jjfa=?,sjcfar=?,sjcfasj=?,jhjjsj=?,sffswxf=?,wxfze=?,sfnbcf=?,nbcfze=?,clzt=?,cfajlh=?";
	ls_sql+=" where tsjlh='"+tsjlh+"' and tsyybm='"+tsyybm+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,jjfa);
