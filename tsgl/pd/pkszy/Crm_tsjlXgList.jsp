...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	crm_tsjl_tsbxsj=request.getParameter("crm_tsjl_tsbxsj");
	if (crm_tsjl_tsbxsj!=null)
	{
		crm_tsjl_tsbxsj=crm_tsjl_tsbxsj.trim();
		if (!(crm_tsjl_tsbxsj.equals("")))	wheresql+="  and (crm_tsjl.tsbxsj>=TO_DATE('"+crm_tsjl_tsbxsj+"','YYYY/MM/DD'))";
	}
	crm_tsjl_tsbxsj=request.getParameter("crm_tsjl_tsbxsj2");
	if (crm_tsjl_tsbxsj!=null)
	{
		crm_tsjl_tsbxsj=crm_tsjl_tsbxsj.trim();
		if (!(crm_tsjl_tsbxsj.equals("")))	wheresql+="  and (crm_tsjl.tsbxsj<=TO_DATE('"+crm_tsjl_tsbxsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT crm_tsjl.tsjlh,DECODE(crm_tsjl.lx,'1','投诉','2','报修') lx,crm_tsjl.kszy,crm_khxx.hth,crm_khxx.khbh,slfsmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_tsjl.tsbxsj,crm_tsjl.tsnr,DECODE(crm_tsjl.clzt,0,'未受理',1,'已受理',2,'在处理',3,'结案','9','不立案') as clzt,DECODE(crm_tsjl.sfxtz,'Y','需通知','N','不需通知') as sfxtz";
	...
	ls_sql+=" and  crm_tsjl.kszy is not null ";
...
//进行对象初始化
	pageObj.initPage("Crm_tsjlXgList.jsp","","","PdCrm_tsjl.jsp");
//	pageObj.setPageRow(10);//设置每页显示记录数
//设置显示列
	String[] disColName={"tsjlh","clzt","lx","tsbxsj","kszy","hth","khxm","fwdz","sjs","sgd","sfxtz","slfsmc","tsnr"};
...
//设置按钮参数
	String[] buttonName={"删除客诉专员"};//按钮的显示名称
	String[] buttonLink={"Crm_tsjlXgList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_tsjl set  kszy=null,kszypdr=null,kszypdsj=null where "+chooseitem;
