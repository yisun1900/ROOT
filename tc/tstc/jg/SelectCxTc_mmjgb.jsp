...
<body bgcolor="#FFFFFF">
<form method="post" action="Tc_mmjgbCxList.jsp" name="selectform">
...
<tr bgcolor="#FFFFFF">
  ...
  <td width="35%"> 
<%
	cf.radioToken(out, "tc_mmjgb_sfyqxtpp","Y+是&N+否","");
%>
  </td>
...
<tr bgcolor="#FFFFFF">
  ...
  <td width="35%"> 
<%
	cf.radioToken(out, "tc_mmjgb_sfkjm","1+不可减免&2+可减免","");
%>
  </td>
...
{
	if(!(isNumber(FormName.tc_mmjgb_tccplbbm, "产品小类"))) {
		return false;
	}
	if(!(isNumber(FormName.tc_mmjgb_tccpdlbm, "产品大类"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_bzsl, "标准数量"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_bzmj, "标准面积"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_mtmzjmj, "每个增加面积"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_ccbfjj, "超出部分加价"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_jmdj, "减免单价"))) {
		return false;
	}
	if(!(isFloat(FormName.tc_mmjgb_sjzj, "升级折价"))) {
		return false;
	}
	if(!(isDatetime(FormName.tc_mmjgb_lrsj, "录入时间"))) {
		return false;
	}
	FormName.submit();
	return true;
