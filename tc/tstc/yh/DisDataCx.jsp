...
try 
{
	...
	%> 
<form method="post" action="SaveLoadDataCx.jsp" name="editform" target="_blank">
<div align="center">
    <p>导入套餐促销项目</p>
  ...
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		cxxmbm=outStr[i][0].trim();
		xmmc=outStr[i][1].trim();
		ppmc=outStr[i][2].trim();
		xh=outStr[i][3].trim();
		jldw=outStr[i][4].trim();
		djStr=outStr[i][5].trim();
		bz=outStr[i][6].trim();


		if (cxxmbm.equals(""))
		{
			out.println("行号["+row+"]错误！促销项目名称为空");
			return;
		}
