...
wheretccpdlbm=cf.GB2Uni(request.getParameter("wheretccpdlbm"));
...
try {
	...
	ls_sql="update tc_tczcxm set dqbm=?,bjjbbm=?,tccpdlbm=?,tccplbbm=?,bzsl=?,sfxzsl=?,lrr=?,lrsj=?,sfbxx=?  ,sfyxsj=?,tcsjflbm=?,sfyqxtpp=?,jldw=?,sfkjm=?,yxjmzdsl=?,jmdj=?    ,sfkgh=?,khzdsl=?,ghzj=?  ";
	...
	ps.setString(14,sfkjm);
	ps.setDouble(15,yxjmzdsl);
	ps.setDouble(16,jmdj);

	ps.setString(17,sfkgh);
	ps.setDouble(18,khzdsl);
	ps.setDouble(19,ghzj);
