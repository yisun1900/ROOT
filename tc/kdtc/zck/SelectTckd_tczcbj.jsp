...
<body bgcolor="#FFFFFF">
<form method="post" action="Tckd_tczcbjList.jsp" name="selectform">
...
<tr bgcolor="#FFFFFF">
  ...
  <td align="right" width="18%">单品内部编码</td> 
...
%>
  ...
  <td width="32%">
	<INPUT type="radio" name="tckd_tczcbj_sfcscp" value="Y">是
    <INPUT type="radio" name="tckd_tczcbj_sfcscp" value="N">否  </td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr align="center"> 
  <td colspan="4"> 
    ...
    <input type="button" onClick="f_sh(selectform)"  value="批量修改损耗">
    <input type="button" onClick="f_xdb(selectform)"  value="批量修改下单比">
    <input type="button" onClick="f_gys(selectform)"  value="批量修改供应商">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="Tckd_tczcbjList.jsp";
	FormName.submit();
	return true;
}

function f_xdb(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.tckd_tczcbj_dqbm)=="") {
		alert("请选择[地区]！");
		FormName.tckd_tczcbj_dqbm.focus();
		return false;
	}
	if(	javaTrim(FormName.tckd_tczcbj_bjjbbm)=="") {
		alert("请输入[报价级别]！");
		FormName.tckd_tczcbj_bjjbbm.focus();
		return false;
	}
	...
	FormName.action="PlUpdateXdb.jsp";
	FormName.submit();
	return true;
}

function f_sh(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.tckd_tczcbj_dqbm)=="") {
		alert("请选择[地区]！");
		FormName.tckd_tczcbj_dqbm.focus();
	...
	FormName.action="PlUpdateSh.jsp";
	FormName.submit();
	return true;
}

function f_gys(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.tckd_tczcbj_dqbm)=="") {
		alert("请选择[地区]！");
		FormName.tckd_tczcbj_dqbm.focus();
	...
	FormName.action="PlUpdateGys.jsp";
	FormName.submit();
	return true;
}
//-->
