...
<div align="center">
  ...
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		if (fwmjStr.equals(""))
		{
			out.println("<BR>行号["+row+"]错误！[套内基准面积]不能为空");
			return;
		}
		else{
			try{
				...
				out.println("<BR>行号["+row+"]错误！[套内基准面积:"+fwmjStr+"]不是整数:"+e);
				return;
			}
		}
		...
		if (tcjgStr.equals(""))
		{
			out.println("<BR>行号["+row+"]错误！[套餐价格]不能为空");
			return;
		}
		else{
			try{
				...
				out.println("<BR>行号["+row+"]错误！[套餐价格:"+tcjgStr+"]不是数字:"+e);
				return;
			}
		}
		...
		{
			out.println("<BR>行号["+row+"]错误！[涂料改为壁纸加价]不能为空");
			return;
		}
		else{
			try{
				...
				out.println("<BR>行号["+row+"]错误！[涂料改为壁纸加价:"+tlgbzjjStr+"]不是数字:"+e);
				return;
			}
		}
		if (dbhddzjjStr.equals(""))
		{
			out.println("<BR>行号["+row+"]错误！[地板换大磁砖收费]不能为空");
			return;
		}
		else{
			try{
				dbhddzjj=Double.parseDouble(dbhddzjjStr.trim());
			}
			catch (Exception e){
				out.println("<BR>行号["+row+"]错误！[地板换大磁砖收费:"+dbhddzjjStr+"]不是数字:"+e);
				return;
			}
		}
    ...
    <tr bgcolor="<%=bgcolor%>" align="center"> 
      ...
      <td><%=dbhddzjj%>
        <input name="dbhddzjj" type="hidden" value="<%=dbhddzjj%>" >
      </td>
    </tr>
    <%
	}
