...
try {
	...
	{
		ls_sql="update tc_csrjzck set lrr='"+yhmc+"',lrsj=SYSDATE";
		ls_sql+=" where  (1=1) ";
		ls_sql+=wheresql;
		...
		ls_sql="update tc_csrjzcsjcjb set sjsfj=ROUND(ysjsfj*"+xs+",0)";
		ls_sql+=" where cpbm in(select cpbm from tc_csrjzck where (1=1) "+wheresql+")";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
	}
	else if (jgqz.equals("N"))
	{
		...
		ls_sql="update tc_csrjzcsjcjb set sjsfj=ROUND(ysjsfj*"+xs+",2)";
		ls_sql+=" where cpbm in(select cpbm from tc_csrjzck where (1=1) "+wheresql+")";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
