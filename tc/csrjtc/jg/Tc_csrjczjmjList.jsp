...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT tc_csrjczjmj.dqbm,dm_dqbm.dqmc,tc_csrjczjmj.bjjbbm,bdm_bjjbbm.bjjbmc,tc_csrjczjmj.tccplbbm,jxc_clxlbm.clxlmc,jxc_cldlbm.cldlmc, DECODE(tc_csrjczjmj.sfyqxtpp,'Y','是','N','否') sfyqxtpp,tc_csrjczjmj.jldw, DECODE(tc_csrjczjmj.sfkjm,'1','不可减免','2','可减免') sfkjm,tc_csrjczjmj.jmdj,DECODE(tcnbz,'Y','套餐内','N','否') tcnbz,DECODE(sfbxx,'Y','必选项','N','否') sfbxx,tc_csrjczjmj.lrr,tc_csrjczjmj.lrsj  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Tc_csrjczjmjList.jsp","","","EditTc_csrjczjmj.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Tc_csrjczjmjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from tc_csrjczjmj where "+chooseitem;
...
<CENTER >
  <B><font size="3">套餐磁砖减免价－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
