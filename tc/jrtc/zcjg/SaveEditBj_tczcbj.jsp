...
try {
	...
	ls_sql="update bj_tczcbj set sku=?,tccpdlbm=?,tccplbbm=?,cpmc=?,ppmc=?,xh=?,gg=?,jldw=?,xdjldw=?,xdb=?    ,dj=?,tccldj=?,jsj=?,sh=?,sfcscp=?,gysmc=?,xuhao=?,lrr=?,lrsj=?,bz=?   ,sfxclxs=?,tcsjflbm=?,sfyx=?,yxkssj=?,yxjzsj=?,sfkgdj=?,cllx=? ";
