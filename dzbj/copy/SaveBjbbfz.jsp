...
try {
	...
	if (!gcbj.equals("null"))//工程报价
	{
		...
		sql+=" select               '"+newbjbbh+"','"+newdqbm+"',bjjbbm,xmbh,xmmc,xmdlbm,xmxlbm,xmflbm,smbm,jldwbm,sfbxx,glxmbh,sfxycl,sfxyyjd,clcb,rgcb,jxcb,cbenj,clbj,rgbj,jxbj,zcf,shf,ysf,qtf,bj,sgdbj,sgcbj,sfyxdz,zdzk,gycl,flmcgg,xh,'"+yhmc+"',SYSDATE,bz,gcjcbjxs,sgcbbjxs,khbjxs";
		sql+=" from bj_jzbjb ";
		sql+=" where dqbm='"+olddqbm+"' and bjbbh='"+oldbjbbh+"'";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
		ps.close();

		sql ="insert into bj_glxmb (bjbbh,dqbm,bjjbbm,xmbh,glxmbh)";
		sql+=" select     '"+newbjbbh+"','"+newdqbm+"',bjjbbm,xmbh,glxmbh ";
		sql+=" from bj_glxmb ";
		sql+=" where dqbm='"+olddqbm+"' and bjbbh='"+oldbjbbh+"'";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
		...
		sql+=" select                 "+newglbxh+"+rownum,'"+newbjbbh+"','"+newdqbm+"',bjjbbm,xmbh,glfxclbm,glfxclmc,glflbm,dj,jldwbm,xhl,jqsfl,bsfl,tvocsfl,lx,wlllbz,'"+yhmc+"',SYSDATE,bz ";
		sql+=" from bj_jzglfxb ";
		sql+=" where dqbm='"+olddqbm+"' and bjbbh='"+oldbjbbh+"'";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
	...
	if (!tcjg.equals("null"))//套餐－价格表
	{
		...
		sql+=" select               '"+newbjbbh+"','"+newdqbm+"',bjjbbm,hxbm,tcmc,fwmj,tcjg,pmzjdj,tlgbzjj";
		sql+=" from bj_tcjgb ";
		sql+=" where dqbm='"+olddqbm+"' and bjbbh='"+oldbjbbh+"'";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
	...
	if (!tcsj.equals("null"))//套餐－升级差价表
	{
		...
		sql+=" select                 '"+newbjbbh+"','"+newdqbm+"',tcsjflbm,bjjbbm,bjjbbm1,jldw,sjcj,sjsfycx,cxkssj,cxjssj,cxsjcj";
		sql+=" from bj_tcsjcjb ";
		sql+=" where dqbm='"+olddqbm+"' and bjbbh='"+oldbjbbh+"'";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
	...
	if (!tcbzgc.equals("null"))//套餐－标准工程项目
	{
		...
		sql+=" select                  '"+newbjbbh+"','"+newdqbm+"',bjjbbm,hxbm,xmbh,bzsl,sfxzsl,ccbfjj,'"+yhmc+"',SYSDATE";
	...
	if (!tcbzzc.equals("null"))//套餐－标准主材项目
	{
		...
		sql+=" select                  '"+newbjbbh+"','"+newdqbm+"',bjjbbm,hxbm,jgwzbm,tccplbbm,tcsjflbm,sfbxx,sfyxsj,jldw,sfxzsl,bzsl,sfkjm,jmfl,jmjldw,yxjmzdsl,jmdj,sfkgh,khzdsl,ghzj,sfyqxtpp,'"+yhmc+"',SYSDATE";
	...
	if (!tczcbj.equals("null"))//套餐－主材报价
	{
		...
		while (rs.next())
		{
			...
			sql+=" select                '"+cpbm+"','"+newdqbm+"',bjjbbm,tccpdlbm,tccplbbm,tcsjflbm,cpmc,xh,gg,ppmc,gysmc,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,dj,tccldj,sfkgdj,jsj,sh,sfcscp,sku,xuhao,'"+yhmc+"',SYSDATE,bz,cllx";
			sql+=" from bj_tczcbj ";
