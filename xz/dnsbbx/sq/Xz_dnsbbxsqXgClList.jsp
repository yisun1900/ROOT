...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT bxxh,DECODE(clzt,'1','未接收','2','接收','3','维修中','4','采购中','5','送修中','6','完成'),a.dwmc bxbm,sqr,bxrq,jhlqrq,dnsbflmc,dnsbgzmc,bgr, DECODE(sfyzysj,'N','无','Y','有'),gzxxsm,jsr,jssj,jdjg,clqk,wxr,wxlrsj,wcsj,wxfy,lrr,gsbh,gdzcmc,bmbh,gdzcflmc,xh,pp,pz1,pz2,pz3,pz4,pz5,pz6,b.dwmc fgsmc  ";
	ls_sql+=" FROM xz_dnsbbxsq,sq_dwxx a,sq_dwxx b ";
    ls_sql+=" where xz_dnsbbxsq.dwbh=a.dwbh(+) and xz_dnsbbxsq.fgsbh=b.dwbh(+)";
	ls_sql+=" and xz_dnsbbxsq.clzt in('3','4','5','6')";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Xz_dnsbbxsqXgClList.jsp","","","XgClXz_dnsbbxsq.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Xz_dnsbbxsqXgClList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update xz_dnsbbxsq set clzt='2',jdjg=null,clqk=null,wxr=null,wxlrsj=null,wcsj=null,wxfy=0,wxbm=null where "+chooseitem+" and clzt in('3','4','5','6')";
...
<CENTER >
  <B><font size="3">报修申请－修改处理</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="2%">报修申请人</td>
	<td  width="2%">报修日期</td>
	<td  width="2%">希望完成日期</td>
	<td  width="3%">电脑及设备分类</td>
	<td  width="3%">电脑及设备故障分类</td>
	<td  width="2%">设备保管人</td>
	<td  width="2%">设备中是否有重要数据</td>
	<td  width="9%">故障详细说明</td>
	<td  width="2%">接收人</td>
	<td  width="2%">接收时间</td>
	<td  width="8%">诊断结果</td>
	<td  width="8%">处理情况</td>
	<td  width="2%">维修人</td>
	<td  width="2%">维修录入时间</td>
	<td  width="2%">维修完成时间</td>
	<td  width="2%">维修费用</td>
	<td  width="2%">录入人</td>
	<td  width="2%">固定资产编号</td>
	<td  width="3%">固定资产名称</td>
	<td  width="2%">固定资产部门内编号</td>
	<td  width="3%">固定资产分类</td>
	<td  width="2%">型号</td>
	<td  width="2%">品牌</td>
	<td  width="3%">配置(CPU)</td>
	<td  width="3%">配置(内存)</td>
	<td  width="3%">配置(硬盘)</td>
	<td  width="3%">配置(主板)</td>
	<td  width="3%">配置(显卡)</td>
	<td  width="3%">配置(显示器)</td>
	<td  width="3%">报修分公司</td>
</tr>
<%
	pageObj.displayDate();
