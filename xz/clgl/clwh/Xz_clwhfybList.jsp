...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by xz_clwhfyb.ssfgs,xz_clwhfyb.qcph,xz_clwhfyb.lrsj";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Xz_clwhfybList.jsp","SelectXz_clwhfyb.jsp","","EditXz_clwhfyb.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Xz_clwhfybList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from xz_clwhfyb where "+chooseitem;
