...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and xz_pcsq.clzt in('2','3')";
    ls_sql+=wheresql;
    ls_sql+=" order by xz_pcsq.dwbh,xz_pcsq.sqxh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Xz_pcsqXgShList.jsp","","","ShXz_pcsq.jsp");
...
//设置按钮参数
	String[] buttonName={"删除派车"};//按钮的显示名称
	String[] buttonLink={"Xz_pcsqXgShList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update xz_pcsq set clzt='1',qcph=null,pzr=null,pzsj=null,sjxm=null where "+chooseitem;
