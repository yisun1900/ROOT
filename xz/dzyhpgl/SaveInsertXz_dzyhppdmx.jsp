...
int xh=0;
String xhs=null;
ls_sql2="select NVL(max(TO_NUMBER(pdxh))+1,1)";
ls_sql2+=" from  xz_dzyhppdmx";
xhs=cf.executeQuery(ls_sql2);
xh=Integer.parseInt(xhs);
pdxh=cf.addZero(xh,7);
...
try {
	...
	ls_sql="insert into xz_dzyhppdmx ( pdxh,bh,mc,dw,sl,xhjgg,grsj,dwbh,gryz,zrr,pdr,pdsj,bz ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,? ) ";
