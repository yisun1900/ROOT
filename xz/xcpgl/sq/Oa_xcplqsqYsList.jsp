...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT oa_xcplqsq.sqxh, DECODE(oa_xcplqsq.sfxsp,'N','不需要','Y','需要'), DECODE(oa_xcplqsq.lqzt,'1','未完成','2','完成申请','3','一级审批','4','二级审批','5','三级审批','6','已领取'),a.dwmc dw,b.dwmc fgsbh ,oa_xcplqsq.slr,oa_xcplqsq.jhlqrq,oa_xcplqsq.lrr,oa_xcplqsq.lrrq,oa_xcplqsq.jhlqsl,oa_xcplqsq.jhlqje,oa_xcplqsq.bz  ";
	ls_sql+=" FROM oa_xcplqsq,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where oa_xcplqsq.dwbh=a.dwbh(+) and oa_xcplqsq.fgsbh=b.dwbh(+) ";
    ls_sql+=" and ((oa_xcplqsq.lqzt='2' )  OR  (oa_xcplqsq.lqzt='4' and oa_xcplqsq.ejspjl='2') )";
    ls_sql+=wheresql;
    ls_sql+=" order by oa_xcplqsq.sqxh desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_xcplqsqYsList.jsp","SelectYsOa_xcplqsq.jsp","","YsOa_xcplqsq.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Oa_xcplqsqYsList.jsp?yjspjl=1","Oa_xcplqsqYsList.jsp?yjspjl=3"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String yjspjl=request.getParameter("yjspjl");
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update oa_xcplqsq set lqzt='3',yjspr='"+yhmc+"',yjspsj=TRUNC(SYSDATE),yjspjl='"+yjspjl+"',yjspsm='批量审批' where lqzt='2' and ("+chooseitem+")";
...
<CENTER >
  <B><font size="3">印刷品领取申请---一级审批</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="7%">领取状态</td>
	<td  width="11%">申领单位</td>
	<td  width="10%">申领分公司</td>
	<td  width="5%">申领人</td>
	<td  width="7%">计划领取日期</td>
	<td  width="5%">录入人</td>
	<td  width="7%">录入日期</td>
	<td  width="6%">计划领取数量</td>
	<td  width="6%">计划领取金额</td>
	<td  width="19%">备注</td>
</tr>
<%
	pageObj.displayDate();
