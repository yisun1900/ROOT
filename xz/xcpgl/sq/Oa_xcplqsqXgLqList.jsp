...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by oa_xcplqsq.lqsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_xcplqsqXgLqList.jsp","SelectXgLqOa_xcplqsq.jsp","","XgLqOa_xcplqsq.jsp");
...
//设置按钮参数
	String[] buttonName={"删除领取"};//按钮的显示名称
	String[] buttonLink={"Oa_xcplqsqXgLqList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update oa_xcplqmx set slqsl=0 where "+chooseitem;
		sql[1]="update oa_xcplqsq set lqzt='5',blr=null,lqsj=null,slqsl=0,slqje=0 where sfxsp='Y' and lqzt='6' and ("+chooseitem+")";
		sql[2]="update oa_xcplqsq set lqzt='2',blr=null,lqsj=null,slqsl=0,slqje=0 where sfxsp='N' and lqzt='6' and ("+chooseitem+")";
...
<CENTER >
  <B><font size="3">印刷品领取申请---修改领取</font></B>
</CENTER>
<%
	pageObj.out=out;
