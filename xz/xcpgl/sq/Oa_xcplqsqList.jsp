...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql+=" and ( oa_xcplqsq.lqzt in('1','2') OR (oa_xcplqsq.lqzt='3' and oa_xcplqsq.yjspjl='2') )";
    ls_sql+=wheresql;
    ls_sql+=" order by oa_xcplqsq.sqxh desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_xcplqsqList.jsp","SelectOa_xcplqsq.jsp","","EditOa_xcplqsq.jsp");
...
//设置主键
	...
	pageObj.setEditBolt("");
//设置按钮参数
	String[] buttonName={"删除完成状态","删除----领取申请"};//按钮的显示名称
	String[] buttonLink={"DeleteEnd.jsp","Oa_xcplqsqList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from oa_xcplqmx where "+chooseitem;
		sql[1]="delete from oa_xcplqsq where "+chooseitem;
