...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT oa_xcplqsq.sqxh, DECODE(oa_xcplqsq.sfxsp,'N','不需要','Y','需要'), DECODE(oa_xcplqsq.lqzt,'1','未完成','2','完成申请','3','一级审批','4','二级审批','5','三级审批','6','已领取'),a.dwmc dw,b.dwmc fgsbh ,oa_xcplqsq.slr,oa_xcplqsq.jhlqrq,oa_xcplqsq.lrr,oa_xcplqsq.lrrq,oa_xcplqsq.yjspr,oa_xcplqsq.yjspsj, DECODE(oa_xcplqsq.yjspjl,'1','同意','2','返回上级','3','撤销'),oa_xcplqsq.yjspsm,oa_xcplqsq.ejspr,oa_xcplqsq.ejspsj, DECODE(oa_xcplqsq.ejspjl,'1','同意','2','返回上级','3','撤销'),oa_xcplqsq.ejspsm,oa_xcplqsq.jhlqsl,oa_xcplqsq.jhlqje,oa_xcplqsq.bz  ";
	ls_sql+=" FROM oa_xcplqsq,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where oa_xcplqsq.dwbh=a.dwbh(+) and oa_xcplqsq.fgsbh=b.dwbh(+) ";
    ls_sql+=wheresql;
    ls_sql+=" order by oa_xcplqsq.ejspsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_xcplqsqSsList.jsp","SelectSsOa_xcplqsq.jsp","","SsOa_xcplqsq.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Oa_xcplqsqSsList.jsp?sjspjl=1","Oa_xcplqsqSsList.jsp?sjspjl=3"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String sjspjl=request.getParameter("sjspjl");
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update oa_xcplqsq set lqzt='5',sjspr='"+yhmc+"',sjspsj=TRUNC(SYSDATE),sjspjl='"+sjspjl+"',sjspsm='批量审批' where lqzt='4' and ("+chooseitem+")";
...
<CENTER >
  <B><font size="3">印刷品领取申请---三级审批</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="5%">申领分公司</td>
	<td  width="3%">申领人</td>
	<td  width="4%">计划领取日期</td>
	<td  width="3%">录入人</td>
	<td  width="4%">录入日期</td>
	<td  width="3%">一级审批人</td>
	<td  width="4%">一级审批时间</td>
	<td  width="3%">一级审批结论</td>
	<td  width="11%">一级审批说明</td>
	<td  width="3%">二级审批人</td>
	<td  width="4%">二级审批时间</td>
	<td  width="3%">二级审批结论</td>
	<td  width="15%">二级审批说明</td>
