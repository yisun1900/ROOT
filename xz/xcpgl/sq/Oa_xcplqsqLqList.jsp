...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT oa_xcplqsq.sqxh,oa_xcplqsq.slr,oa_xcplqsq.jhlqrq,oa_xcplqsq.lrr,oa_xcplqsq.lrrq,oa_xcplqsq.jhlqsl,oa_xcplqsq.jhlqje, DECODE(oa_xcplqsq.sfxsp,'N','不需要','Y','需要'), DECODE(oa_xcplqsq.lqzt,'1','未完成','2','完成申请','3','一级审批','4','二级审批','5','三级审批','6','已领取'),oa_xcplqsq.yjspr,oa_xcplqsq.yjspsj, DECODE(oa_xcplqsq.yjspjl,'1','同意','2','返回上级','3','撤销'),oa_xcplqsq.yjspsm,oa_xcplqsq.ejspr,oa_xcplqsq.ejspsj, DECODE(oa_xcplqsq.ejspjl,'1','同意','2','返回上级','3','撤销'),oa_xcplqsq.ejspsm,oa_xcplqsq.sjspr,oa_xcplqsq.sjspsj, DECODE(oa_xcplqsq.sjspjl,'1','同意','2','返回上级','3','撤销'),oa_xcplqsq.sjspsm,oa_xcplqsq.bz,a.dwmc dw,b.dwmc fgsbh  ";
	ls_sql+=" FROM oa_xcplqsq,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where oa_xcplqsq.dwbh=a.dwbh(+) and oa_xcplqsq.fgsbh=b.dwbh(+)";
    ls_sql+=" and ((oa_xcplqsq.sfxsp='Y' and oa_xcplqsq.lqzt='5' and oa_xcplqsq.sjspjl='1') or (oa_xcplqsq.sfxsp='N' and oa_xcplqsq.lqzt='2') )";
    ls_sql+=wheresql;
    ls_sql+=" order by oa_xcplqsq.sqxh desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_xcplqsqLqList.jsp","SelectLqOa_xcplqsq.jsp","","LqOa_xcplqsq.jsp");
...
//设置主键
	...
	pageObj.setEditStr("领取");

//设置按钮参数
	String[] buttonName={"批量领取"};//按钮的显示名称
	String[] buttonLink={"Oa_xcplqsqLqList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update oa_xcplqmx set slqsl=jhlqsl where "+chooseitem;
		sql[1]="update oa_xcplqsq set lqzt='6',blr='"+yhmc+"',lqsj=TRUNC(SYSDATE),bz=bz||'，批量领取' where sfxsp='Y' and lqzt='5' and ("+chooseitem+")";
		sql[2]="update oa_xcplqsq set lqzt='6',blr='"+yhmc+"',lqsj=TRUNC(SYSDATE),bz=bz||'，批量领取' where sfxsp='N' and lqzt='2' and ("+chooseitem+")";
		sql[3]="update oa_xcplqsq set slqsl=(select sum(slqsl) from oa_xcplqmx where oa_xcplqmx.sqxh=oa_xcplqsq.sqxh),slqje=(select sum(slqsl*dj) from oa_xcplqmx where oa_xcplqmx.sqxh=oa_xcplqsq.sqxh) where lqzt='6' and ("+chooseitem+")";
...
<CENTER >
  <B><font size="3">印刷品领取申请---领取</font></B>
</CENTER>
<%
	pageObj.out=out;
