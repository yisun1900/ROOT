...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql+=" and  oa_bgypbm.fgsbh='"+fgsbh+"'";
    ls_sql+=wheresql;
    ls_sql+=" order by oa_bgypbm.fgsbh,oa_bgypbm.bgypbm";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Oa_bgypbmList.jsp?fgsbh="+fgsbh,"SelectOa_bgypbm.jsp","","EditOa_bgypbm.jsp?fgsbh="+fgsbh);
...
//设置按钮参数
	...
	String[] buttonLink={"Oa_bgypbmList.jsp?fgsbh="+fgsbh};//按钮单击调用的网页，可以增加参数
...
{
	String[] bgypbm = request.getParameterValues("bgypbm");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(bgypbm,"bgypbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from oa_bgypbm where fgsbh='"+fgsbh+"' and "+chooseitem;
