...
finally 
{
	...
	{
		rkph=cf.GB2Uni(rkph);
		if (!(rkph.equals("")))	wheresql+=" and  (jxc_kcb.rkph='"+rkph+"')";
	}
	...
	ls_sql="SELECT jxc_kcb.lsh,DECODE(jxc_kcb.ztbz,'Y','在途','N','否'),jxc_kcb.clbm,jxc_kcb.nbbm,jxc_kcb.clmc,DECODE(jxc_kcb.zljb,'1','正常品','2','残次品','3','报废品'),jxc_kcb.kcsl,TO_CHAR(jxc_cljgb.dgzjg),jxc_kcb.kcsl*jxc_cljgb.dgzjg,dqmc,jxc_kcb.rkph,jxc_kcb.scph,jxc_kcb.scrq,TO_CHAR(jxc_kcb.bzq),TO_CHAR(jxc_kcb.bztxq),ckmc,hjmc,hwmc,DECODE(jxc_kcb.cllb,'0','主材','1','辅材'),jxc_kcb.cldlbm,jxc_kcb.clxlbm,jxc_kcb.scsmc,jxc_kcb.ppmc,jxc_kcb.gysmc,jxc_kcb.xh,jxc_kcb.gg,jxc_kcb.jldwbm,jxc_kcb.cgzq,jxc_kcb.bz  ";
	ls_sql+=" FROM jxc_kcb,jxc_cljgb,dm_dqbm,jxc_ckmc,jxc_hjmc,jxc_hwmc  ";
    ls_sql+=" where jxc_kcb.clbm=jxc_cljgb.clbm(+) and jxc_kcb.dqbm=jxc_cljgb.dqbm(+)";
    ls_sql+=" and jxc_kcb.dqbm=dm_dqbm.dqbm and jxc_kcb.ckbh=jxc_ckmc.ckbh(+) ";
    ls_sql+=" and jxc_kcb.hjbh=jxc_hjmc.hjbh(+) and jxc_kcb.hwbh=jxc_hwmc.hwbh(+) ";
    ls_sql+=wheresql;
	ls_sql+=ordersql;
//	out.print(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_kcbCkdbList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"仓库调拨"};//按钮的显示名称
	String[] buttonLink={"Ckdb.jsp?dqbm="+jxc_kcb_dqbm};//按钮单击调用的网页，可以增加参数
