...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		dqbm=cf.GB2Uni(dqbm);
		if (!(dqbm.equals("")))	wheresql+=" and  (jxc_hjtzjl.dqbm='"+dqbm+"')";
	}
	...
	{
		tzr=cf.GB2Uni(tzr);
		if (!(tzr.equals("")))	wheresql+=" and  (jxc_hjtzjl.tzr='"+tzr+"')";
	}
	tzrq=request.getParameter("tzrq");
	if (tzrq!=null)
	{
		tzrq=tzrq.trim();
		if (!(tzrq.equals("")))	wheresql+="  and (jxc_hjtzjl.tzrq>=TO_DATE('"+tzrq+"','YYYY/MM/DD'))";
	}
	...
	{
		ssbm=cf.GB2Uni(ssbm);
		if (!(ssbm.equals("")))	wheresql+=" and  (jxc_hjtzjl.ssbm='"+ssbm+"')";
	}
	bz=request.getParameter("bz");
	if (bz!=null)
	{
		bz=cf.GB2Uni(bz);
		if (!(bz.equals("")))	wheresql+=" and  (jxc_hjtzjl.bz like '%"+bz+"%')";
	}
	ls_sql="SELECT tzph,dqmc,ckmc,tzr,tzrq,a.dwmc ssfgs,b.dwmc ssbm,jxc_hjtzjl.bz  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_hjtzjlCxList.jsp","SelectCxJxc_hjtzjl.jsp","ViewJxc_hjtzjl.jsp","");
