...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		jzr=cf.GB2Uni(jzr);
		if (!(jzr.equals("")))	wheresql+=" and  (jxc_ckd.jzr='"+jzr+"')";
	}
	String jzsj=null;
	...
	{
		jzsj=jzsj.trim();
		if (!(jzsj.equals("")))	wheresql+="  and (jxc_ckd.jzsj>=TO_DATE('"+jzsj+"','YYYY/MM/DD'))";
	}
	jzsj=request.getParameter("jzsj2");
	if (jzsj!=null)
	{
		jzsj=jzsj.trim();
		if (!(jzsj.equals("")))	wheresql+="  and (jxc_ckd.jzsj<=TO_DATE('"+jzsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	String myxssl=null;
	myxssl=request.getParameter("myxssl");

	ls_sql="SELECT jxc_ckd.ckph,dwmc,ckmc,jxc_ckd.jzsj, jxc_ckd.ckzsl,jxc_ckd.ckzje,jxc_ckd.khbh,crm_khxx.khxm,crm_khxx.fwdz,jxc_ckd.jzr,jxc_ckd.lrsj,jxc_ckd.bz ,DECODE(jxc_ckd.ckdzt,'0','未提交','1','等待报废审核','2','等待退货审核','3','等待调拨入库','4','已结算','5','退货审核通过','6','报废审核未通过','7','退货审核未通过','8','接受方已入库','9','提交'), DECODE(jxc_ckd.cklx,'D','订单出库','B','报废出库','T','退货出库','S','申购出库','P','材料单出库','J','辅材代金券出库','X','销售出库','R','调拨出库') ";
...
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jxc_ckdScJzList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"删除结帐"};//按钮的显示名称
	String[] buttonLink={"DeleteJzJxc_ckd.jsp"};//按钮单击调用的网页，可以增加参数
