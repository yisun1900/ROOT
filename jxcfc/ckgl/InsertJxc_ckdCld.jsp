...
String ckbh=null;
String ckjxz="1";//出库价选择,1：取价格表；2：取入库时销售价；3：取材料单价格
try {
	conn=cf.getConnection();

	ls_sql="select jxc_clpsd.khbh ";
...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveInsertJxc_ckdCld.jsp" name="insertform" >
<div align="center">材料单<strong>[<%=psph%>]</strong>出库记录</div>
...
<%
    ...
    ls_sql+=" and jxc_ckd.lydh='"+psph+"' and jxc_ckd.cklx='"+cklx+"'";
...
%>
  ...
  <td><input type="text" name="lydh" value="<%=psph%>" size="20" maxlength="13" readonly></td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3">
<%
	cf.radioToken(out, "ckjxz","1+取价格表&3+取开单价格","");
%>
  </td>
  </tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><textarea name="bz" cols="71" rows="3"></textarea></td>
	...
	<input type="button" onClick="window.open('/jxcfc/psdgl/ViewKcJxc_clpsd.jsp?psph=<%=psph%>')"  value="查看来源单信息"></td>
    </tr>
</table>
</form>
</body>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(	!radioChecked(FormName.ckjxz)) {
		alert("请选择[出库价选择]！");
		FormName.ckjxz[0].focus();
		return false;
	}
