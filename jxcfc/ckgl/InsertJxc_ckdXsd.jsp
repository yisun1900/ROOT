...
String ckbh=null;
String ckjxz="1";//出库价选择,1：取价格表；2：取入库时销售价；3：取销售单价格
try {
	conn=cf.getConnection();


	ls_sql="select dqbm,ssfgs";
...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveInsertJxc_ckdXsd.jsp" name="insertform" >
<div align="center">销售单<strong>[<%=xsph%>]</strong>出库记录</div>
...
<%
    ...
    ls_sql+=" and jxc_ckd.lydh='"+xsph+"' and jxc_ckd.cklx='X'";
...
<%
	cf.selectToken(out,"D+订单出库&B+报废出库&T+退货出库&S+申购出库&P+销售单出库&J+代金券出库&X+销售出库&R+调拨出库",cklx,false);
%>
  </select></td>
  <td align="right" class="STYLE2"><span class="STYLE1">*</span>来源单号</td>
  <td><input type="text" name="lydh" value="<%=xsph%>" size="20" maxlength="13" readonly></td>
</tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><textarea name="bz" cols="71" rows="3"></textarea></td>
	...
	<input type="button" onClick="window.open('/jxcfc/psdgl/ViewKcJxc_clpsd.jsp?xsph=<%=xsph%>')"  value="查看来源单信息"></td>
    </tr>
</table>
</form>
</body>
