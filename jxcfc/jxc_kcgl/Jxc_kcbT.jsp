...
finally 
{
	...
	String ztbz=null;
	ztbz=request.getParameter("ztbz");
	if (ztbz!=null)
	{
		ztbz=cf.GB2Uni(ztbz);
		if (!(ztbz.equals("")))	wheresql+=" and  (jxc_kcb.ztbz='"+ztbz+"')";
	}
	...
	{
	ls_sql="SELECT dqmc,ckmc,hjmc,jxc_kcb.clbm clbm,jxc_kcb.clmc clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh xh,jxc_kcb.gg gg,jxc_kcb.nbbm nbbm,'<img src=\"/jxc/jxc_clbm/clzp/'||jxc_kcb.zp||'\" width=\"100px\" height=\"100px\">' zp,SUM(jxc_kcb.kcsl),jldwbm ";
	}
	else
	{
		ls_sql="SELECT dqmc,ckmc,hjmc,jxc_kcb.clbm clbm,jxc_kcb.clmc clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh xh,jxc_kcb.gg gg,jxc_kcb.nbbm nbbm,'' zp,SUM(jxc_kcb.kcsl),jldwbm ";
	}
	ls_sql+=" FROM jxc_kcb,dm_dqbm,jxc_cljgb,jxc_ckmc,jxc_hjmc ";
    ls_sql+=" where jxc_kcb.dqbm=dm_dqbm.dqbm and jxc_kcb.dqbm=jxc_cljgb.dqbm and jxc_kcb.clbm=jxc_cljgb.clbm ";
	ls_sql+=" and jxc_kcb.ckbh=jxc_ckmc.ckbh and jxc_kcb.hjbh=jxc_hjmc.hjbh ";
    ls_sql+=wheresql;
	if (sfxstp!=null && sfxstp!="")
	{
	ls_sql+=" group by dqmc,ckmc,hjmc,jxc_kcb.clbm,jxc_kcb.clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh,jxc_kcb.gg,jxc_kcb.nbbm,'<img src=\"/jxc/jxc_clbm/clzp/'||jxc_kcb.zp||'\" width=\"100px\" height=\"100px\">',jldwbm";
	}
	else
	{ls_sql+=" group by dqmc,ckmc,hjmc,jxc_kcb.gysmc,jxc_kcb.clbm,jxc_kcb.clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh,jxc_kcb.gg,jxc_kcb.nbbm,'',jldwbm ";}
//	ls_sql+=ordersql;
	ls_sql+=" order by dqmc ";
//	out.print(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
