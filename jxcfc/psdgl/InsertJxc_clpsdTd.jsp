...
try { 
	...
	ls_sql="select khbh,khxm,fwdz,lxfs,dwbh,sjs,sgd,zjxm,hth,kgrq,jgrq,sgbz,NVL(guanlif,0)+NVL(suijin,0)+qye qye,fgsbh";
...
<body bgcolor="#FFFFFF">
<div align="center">退料单－录入（材料单号：<%=psph%>）
  <BR>
  <strong><font color="#FF0000">注意：在提交前点击【分类统计】按钮，查看是否符合标准，否则提交后经过集团审批才能出库</font></strong>
</div>
...
<%
	...
	ls_sql="SELECT jxc_clpsd.psph,DECODE(fclb,'1','开工领料','2','补料','3','中期领料','4','退单'),DECODE(jxc_clpsd.psdzt,'0','等待提交','1','等待审核','2','等待出库','3','已出库','5','部分出库','6','审核未通过','7','作废') psdzt,DECODE(jxc_clpsd.fclb,'1','开工','2','补料','3','中期') ,DECODE(jxc_clpsd.psfs,'1','配送','2','自提','9','其它') psfs,jxc_clpsd.jhshsj,jxc_clpsd.pszsl,jxc_clpsd.pszje,jxc_clpsd.yf,jxc_clpsd.qtsf,jxc_clpsd.fkje,jxc_clpsd.lrrq,jxc_clpsd.bz  ";
	ls_sql+=" FROM jxc_clpsd";
...
%> </td>
    ...
    <tr bgcolor="#FFFFFF">
      <td align="right"><strong><font color="#0000CC">签约额</font></strong></td>
      <td><%=qye%>（工程＋管理费＋税金）</td>
      <td align="right"><strong><font color="#0000CC">标准领用额</font></strong></td>
      <td><%=allbzlyje%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td align="right"><strong><font color="#0000CC">领用下限</font></strong></td>
      <td><%=alllyxxje%></td>
      <td align="right"><strong><font color="#0000CC">领用上限</font></strong></td>
      <td><%=alllysxje%></td>
    </tr>
	<tr bgcolor="#FFFFFF"> 
      <td align="right" width="18%"><font color="#CC0000">*</font>材料类别</td>
      <td width="32%"> 
        <input type="radio" name="fclb" value="4" checked>
       退单</td>
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="32%"> 
        ...
        自提 
        <input name="psfs" type="radio" value="9" checked>
      其它 </td>
    ...
    <tr align="center"> 
      <td colspan="4"> 
        <p>
          ...
          <input type="hidden" name="ssfgs" value="<%=fgs%>">
          <input type="button"  value="存盘" onClick="f_do(insertform)">
          <input type="button"  value="配送单录入" onClick="f_pllr(insertform)" name="pllr" disabled>
          ...
          <input type="button"  value="整单打印" onClick="f_print(insertform)" name="print" disabled>
          <input type="button"  value="供应商打印" onClick="f_gysprint(insertform)" name="gysprint" disabled>
          <input type="reset"  value="重输" name="reset"></p>
      </td>
    </tr>
  </table>
</form>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveInsertJxc_clpsdTd.jsp";
	FormName.submit();
	...
	FormName.gysprint.disabled=false;
	return true;
}
...
function f_pllr(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="PlJxc_clpsmxTd.jsp";
	FormName.submit();
...
function f_cklr(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SelectCxJxc_cljgbTd.jsp";
	FormName.submit();
	FormName.tj.disabled=false;
	return true;
}
...
function f_ck(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="EditAllJxc_clpsmxTd.jsp";
	FormName.submit();
	FormName.tj.disabled=false;
	return true;
}
...
function f_gysprint(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.psph)=="") {
		alert("请输入[材料单号]！");
		FormName.psph.focus();
