...
try {
	...
	double khllyblsx=0;
	double khlysxje=0;
	ls_sql=" select bzlybl,ROUND(bzlybl/100*"+zxsgcbj+",2),lyblxx,ROUND(lyblxx/100*"+zxsgcbj+",2),lyblsx,ROUND(lyblsx/100*"+zxsgcbj+",2),dqbm";
	ls_sql+=" FROM sq_dwxx ";
...
<CENTER >
  <B><font size="3">材料单－小类领用统计 </font></B>
<BR>录入时间：（<%=tjsj%>－－<%=tjsj2%>）
</CENTER>
...
%></td>
    ...
    <tr bgcolor="#FFFFFF">
      <td align="right"><font color="#0000CC"><strong>标准领用比例</strong></font></td>
      <td><%=khbzlybl%>%</td>
      <td align="right"><font color="#0000CC"><strong>标准领用金额</strong></font></td>
      <td><%=khbzlyje%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td align="right"><font color="#990000"><strong>累计领用比例</strong></font></td>
      <td><%=cf.formatDouble(pszjebl)%>%</td>
      <td align="right"><font color="#990000"><strong>累计领用金额</strong></font></td>
      <td><%=pszje%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
<%
	...
	while (rs.next())
	{
		...
		double lyblsx=0;
		double lysxje=0;
		ls_sql=" select bzlybl,ROUND(bzlybl/100*"+qye+",2),lyblxx,ROUND(lyblxx/100*"+qye+",2),lyblsx,ROUND(lyblsx/100*"+qye+",2)";
		...
		if (rs1.next())
		{
			...
			bzlyje=rs1.getDouble(2);
			lyblxx=rs1.getDouble(3);
			lyxxje=rs1.getDouble(4);
			lyblsx=rs1.getDouble(5);
			lysxje=rs1.getDouble(6);
		}
		rs1.close();
		ps1.close();
