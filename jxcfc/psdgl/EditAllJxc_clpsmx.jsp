...
<CENTER >
  <B><font size="3">修改材料单明细(材料单号:<%=psph%>)</font></B>
</CENTER>
...
<%
	...
	while (rs2.next())
	{
		...
		if (rs.next())
		{
			...
			ls_sql="select zxqdl,dhsl,ycgsl ";
			ls_sql+=" from  jxc_cljgb";
			ls_sql+=" where jxc_cljgb.clbm='"+clbm+"' and  jxc_cljgb.ssfgs='"+ssfgs+"'";
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
			{
			...
			ls_sql+= " from jxc_kcb ";
			ls_sql +=" where clbm='"+clbm+"' and ckbh in(select ckbh from jxc_ckssfgs where ssfgs='"+ssfgs+"')  ";
			ps1= conn.prepareStatement(ls_sql);
			rs1=ps1.executeQuery();
			if (rs1.next())
			{
				zcpzsl=rs1.getDouble(1);
			}
			rs1.close();
			ps1.close();
