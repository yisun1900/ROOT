...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String mddqbm=null;
	mddqbm=request.getParameter("mddqbm");
	if (mddqbm!=null)
	{
		mddqbm=cf.GB2Uni(mddqbm);
		if (!(mddqbm.equals("")))	wheresql+=" and  (jxc_ckd.mddqbm='"+mddqbm+"')";
	}
	...
	ls_sql="SELECT jxc_ckd.ckph,c.dqmc mddqbm,DECODE(jxc_ckd.ckdzt,'0','未提交','1','等待报废审核','2','等待退货审核','3','等待调拨入库','4','已结算','5','退货审核通过','6','报废审核未通过','7','退货审核未通过','8','审核通过','9','提交'), DECODE(jxc_ckd.cklx,'D','订单出库','B','报废出库','T','退货出库','S','申购出库','P','材料单出库','J','辅材代金券出库','X','销售出库','R','调拨出库'), jxc_ckd.ckzsl,jxc_ckd.ckzje,jxc_ckd.ckzcb, DECODE(jxc_ckd.ckfs,'1','先生产先出库','2','先入库先出库','3','手工出库'), DECODE(jxc_ckd.psfs,'0','厂家配送','1','公司送货','2','自取'),jxc_ckd.jhshsj,jxc_ckd.lydh,jxc_ckd.jsr,jxc_ckd.ckr,jxc_ckd.cksj, DECODE(jxc_ckd.sfth,'Y','提货','N','未提货'), DECODE(jxc_ckd.sfjz,'Y','结账','N','未结账'), DECODE(jxc_ckd.sfyfkc,'1','无负库存','2','负库存未入库','3','负库存入库'),jxc_ckd.lrr,jxc_ckd.lrsj,b.dwmc lrbm,d.dqmc lydq,a.dwmc fgsbh,ckmc, DECODE(jxc_ckd.ckcllx,'0','主材','1','辅材'),jxc_ckd.bz  ";
	ls_sql+=" FROM jxc_ckd,dm_dqbm c,dm_dqbm d,sq_dwxx a,sq_dwxx b,jxc_ckmc  ";
    ls_sql+=" where  jxc_ckd.mddqbm=c.dqbm(+) and jxc_ckd.dqbm=d.dqbm(+)";
    ls_sql+=" and jxc_ckd.fgsbh=a.dwbh(+)";
    ls_sql+=" and jxc_ckd.lrbm=b.dwbh(+)";
    ls_sql+=" and jxc_ckd.ckbh=jxc_ckmc.ckbh(+)";
    ls_sql+=" and jxc_ckd.cklx not in('D','P','X')";
    ls_sql+=" and jxc_ckd.ckcllx='1'";//0：主材；1：辅材

	if (kfgssq.equals("1") || kfgssq.equals("2"))//0：不授权；1：授权单个分公司；2：授权多个分公司；3：授权单个店面；4：授权某一分公司多个店面；5：授权多个分公司多个店面
	{
		ls_sql+=" and jxc_ckd.mddqbm in(select dm_dqbm.dqbm from sq_sqfgs,sq_dwxx,dm_dqbm where sq_sqfgs.ssfgs=sq_dwxx.dwbh and sq_dwxx.dqbm=dm_dqbm.dqbm and sq_sqfgs.ygbh='"+ygbh+"')";
	}
	else
	{
		ls_sql+=" and jxc_ckd.mddqbm in(select dm_dqbm.dqbm from sq_sqbm,sq_dwxx,dm_dqbm where sq_sqbm.dwbh=sq_dwxx.dwbh and sq_dwxx.dqbm=dm_dqbm.dqbm and sq_sqbm.ygbh='"+ygbh+"' )";
	}
    ls_sql+=" and jxc_ckd.sfxnckd='N'";
    ls_sql+=wheresql;
    ls_sql+=" order by jxc_ckd.lrsj desc";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jxc_ckdCxList.jsp","","","InsertJxc_rkdCkd.jsp?rklx=F");
