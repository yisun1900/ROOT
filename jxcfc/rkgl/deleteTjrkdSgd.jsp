...
try {
	...
	for (int i=0;i<rkph.length ;i++ )
	{
		...
		ls_sql+=" where rkph in(select glrkph from jxc_rkdglrkdh where rkph='"+rkph[i]+"')";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql="delete from jxc_rkdglrkdh ";
		ls_sql+=" where rkph='"+rkph[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql+=" where ckph in(select glckph from jxc_rkdglckdh where rkph='"+rkph[i]+"')";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql="delete from jxc_rkdglckdh ";
		ls_sql+=" where rkph='"+rkph[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
