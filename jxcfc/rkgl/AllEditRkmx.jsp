...
%>
		...
		<td><A HREF="DeleteEditRkmx.jsp?rkxh=<%=rkxh%>&rkph=<%=rkph%>" target="_blank">删除</A></td>
		<td><%=(row+1)%></td>
		<td><%=nbbm%>
		...
		<td><%=jldwbm%></td>
		<td>
			<input type="text" name="rkj" value="<%=rkjstr%>" size="8" maxlength="8" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onKeyUp="keyGo(rksl[<%=row%>])" >
		</td>
		<td>
			<input type="text" name="rksl" value="<%=rkslstr%>" size="7" maxlength="8" readonly style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onKeyUp="keyGo(scrq[<%=row%>])" >
		</td>
		<td>
...
{
	FormName.action="SaveAllEditRkmx.jsp";
	FormName.submit();
	return true;
} 
