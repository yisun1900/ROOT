...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT clxlbm,nbbm,clmc,gg,jldwbm,jxc_cljgb.zcpzsl,jxc_cljgb.dhsl,jxc_cljgb.ycgsl,jxc_cljgb.cbj,DECODE(jxc_cljgb.xsfs,'1','代销品','2','现货',3,'期货销售',4,'可负库存') xsfs,DECODE(jxc_cljgb.sfcy,'Y','常用','N','否') sfcy,bzgg,ppmc,jxc_cljgb.gysmc,DECODE(cglb,'J','集采','D','地方采'),jxc_cljgb.clbm";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_cljgbCxList.jsp?rkph="+rkph,"","","InsertJxc_rkmx.jsp?rkph="+rkph+"&dqbm="+dqbm);
...
<CENTER >
  <B><font size="3">入库选择材料（入库批号：<%=rkph%>）</font></B>
</CENTER>
<%
	pageObj.out=out;
