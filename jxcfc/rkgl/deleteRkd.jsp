	...
	for (int i=0;i<rkph.length ;i++ )
	{
		...
		else if (rklx.equals("T"))//T：退货入库
		{
			...
			if (rs.next())
			{
				...
				conn.rollback();
				out.print("错误！退料单批号["+lydh+"]不存在！");
				return;
			}
			rs.close();
			ps.close();

			if (!psdzt.equals("3"))//0：等待提交；1：等待审核；2：等待出库；3：已出库；5：部分出库；6：驳回重新修改；7：作废
			{
				conn.rollback();
				out.print("错误！退料单未入库！");
				return;
			...
			//出库单
			ls_sql="update jxc_clpsd set psdzt='2'";//9：提交
			ls_sql+=" where psph='"+lydh+"' ";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();

			ls_sql="update jxc_clpsmx set cksl=0";
			ls_sql+=" where psph='"+lydh+"' ";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
