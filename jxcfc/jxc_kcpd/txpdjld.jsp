...
try {
	...
	ls_sql+=" where jxc_kcpdjl.dqbm=dm_dqbm.dqbm and jxc_kcpdjl.pdrssfgs=sq_dwxx.dwbh and jxc_kcpdjl.ckbh=jxc_ckmc.ckbh and pdr='"+yhmc+"' and pdzt='1' and pdlb='1' ";
...
%> 
  ...
  <td><A HREF="llwj.jsp?pdph=<%=pdph%>">导入明细</A></td>
   <td><A HREF="Jxc_kcpdmxList.jsp?pdph=<%=pdph%>">数据维护</A></td>
  <td><A HREF="dckcexcel.jsp?pdph=<%=pdph%>">下载库存数据</A></td>
    <td><A HREF="tjkcpd.jsp?pdph=<%=pdph%>">提交</A></td>
    <td><A HREF="ViewJxc_kcpdjl2.jsp?pdph=<%=pdph%>" target="_blank"><%=pdph%></A></td>
	<td><%=dqbm%></td>
    <td><%=ckbh%></td>
	<td><%=pdzt%></td>
	<td><%=pdr%></td>
