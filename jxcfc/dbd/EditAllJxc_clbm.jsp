...
<%
	...
	while (rs2.next())
	{
		...
		ls_sql="SELECT jxc_dbmx.dbdj,DECODE(jxc_dbmx.xsfs,'1','代销品','2','现货','3','期货调拨','4','允许负库存') xsfsmc,jxc_dbmx.xsfs,jxc_dbmx.dbph,jxc_dbmx.clbm,jxc_dbmx.nbbm,jxc_dbmx.clmc,jxc_dbmx.xh,jxc_dbmx.gg,jxc_dbmx.ppmc,jxc_dbmx.jldwbm,jxc_dbmx.dbsl";
	...
	%>
			...
			<td> 
				<input type="text" name="dbdj" size="6" maxlength="10" value=<%=dbdj%> style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" >
			</td>
			<td> 
...
function f_do(FormName)
{
	if (<%=row%><=1)
	{
		if(	javaTrim(FormName.dbdj)=="") {
			alert("请输入[调拨单价]！");
			FormName.dbdj.select();
			return false;
		}
		if(!(isFloat(FormName.dbdj, "调拨单价"))) {
			return false;
		}
	...
	else//如果只有一条记录不能通过数组的形式访问
	{
		for (i=0;i<<%=row%> ;i++ )
		{
			if(	javaTrim(FormName.dbdj[i])=="") {
				alert("请输入[调拨单价]！");
				FormName.dbdj[i].select();
				return false;
			}
			if(!(isFloat(FormName.dbdj[i], "调拨单价"))) {
				return false;
			}
