...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_clcgjhShList.jsp","","","ShJxc_clcgjh.jsp");
...
//设置按钮参数
	String[] buttonName={"同意","不同意"};//按钮的显示名称
	String[] buttonLink={"Jxc_clcgjhShList.jsp?shjg=Y","Jxc_clcgjhShList.jsp?shjg=N"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update jxc_clcgjh set jhdzt='2',shr='"+yhmc+"',shsj=SYSDATE,shjg='"+shjg+"',shnr='批量审核' where "+chooseitem;
