...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT cgjhph,DECODE(jhdzt,'0','未提交','1','等待审核','2','审核完成','3','已分解订单','5','办理入库','9','计划完结'),cgjhmc,dwmc,cgsqr,jhje,jhkssj,jhjssj,jhnr,lrr,lrsj,jxc_clcgjh.bz  ";
	ls_sql+=" FROM jxc_clcgjh,sq_dwxx  ";
    ls_sql+=" where jxc_clcgjh.ssfgs=sq_dwxx.dwbh ";
    ls_sql+=" and jxc_clcgjh.jhdzt in('0','1') ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_clcgjhList.jsp","","","EditJxc_clcgjh.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Jxc_clcgjhList.jsp?"};//按钮单击调用的网页，可以增加参数
...
//设置列超级连接
	...
	coluParm.link="ViewJxc_clcgjh.jsp";//为列参数：coluParm.link设置超级链接
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(cgjhph,"cgjhph"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[4];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from jxc_clcgjh where "+chooseitem;
		sql[1]="delete from jxc_cgjhmx where "+chooseitem;
		sql[2]="update jxc_clsgd set sfzcgd='N' where sgph in(select sgph from jxc_sgzcgdzb where "+chooseitem+")";
		sql[3]="delete from jxc_sgzcgdzb where "+chooseitem;
