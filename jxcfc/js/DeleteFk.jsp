...
try {
	...
	for (int i=0;i<jsjlh.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>错误，【"+jsjlh[i]+"】未付款");
			return;
		}

		ls_sql="update jc_fcgysjs set shbz='Y',fksj=null,fkr=null";
		ls_sql+=" where jsjlh='"+jsjlh[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		while (rs1.next())
		{
			...
			if (ddbh.length()==9)
			{
				...
				{
					conn.rollback();
					out.println("<BR>错误，订单【"+ddbh+"】未付款");
					return;
				}

				ls_sql="update jc_zcdd set sfjs='M' ";//N：未结算；C：已选择结算；Y：已结算；M：结算已审核；F：结算付款
				ls_sql+=" where ddbh='"+ddbh+"'";
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
				...
				{
					conn.rollback();
					out.println("<BR>错误，增减项【"+ddbh+"】未付款");
					return;
				}

				ls_sql="update jc_zczjx set sfjs='M' ";
				ls_sql+=" where zjxbh='"+ddbh+"'";
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
