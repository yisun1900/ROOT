...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		cwzq=cf.GB2Uni(cwzq);
		if (!(cwzq.equals("")))	wheresql+=" and  (jc_fcgysjs.cwzq like '%"+cwzq+"%')";
	}
	...
	{
		shbz=cf.GB2Uni(shbz);
		if (!(shbz.equals("")))	wheresql+=" and  (jc_fcgysjs.shbz='"+shbz+"')";
	}
	...
	{
		jsjlh=jsjlh.trim();
		if (!(jsjlh.equals("")))	wheresql+=" and (jc_fcgysjs.jsjlh="+jsjlh+") ";
	}
	...
	{
		jssj=jssj.trim();
		if (!(jssj.equals("")))	wheresql+="  and (jc_fcgysjs.jssj>=TO_DATE('"+jssj+"','YYYY/MM/DD'))";
	}
	...
	{
		jsr=cf.GB2Uni(jsr);
		if (!(jsr.equals("")))	wheresql+=" and  (jc_fcgysjs.jsr='"+jsr+"')";
	}
	...
	{
		shsj=shsj.trim();
		if (!(shsj.equals("")))	wheresql+="  and (jc_fcgysjs.shsj>=TO_DATE('"+shsj+"','YYYY/MM/DD'))";
	}
	shsj=request.getParameter("shsj2");
	if (shsj!=null)
	{
		shsj=shsj.trim();
		if (!(shsj.equals("")))	wheresql+="  and (jc_fcgysjs.shsj<=TO_DATE('"+shsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}
	String shr=null;
	...
	{
		shr=cf.GB2Uni(shr);
		if (!(shr.equals("")))	wheresql+=" and  (jc_fcgysjs.shr='"+shr+"')";
	}
	...
	ls_sql="SELECT jc_fcgysjs.jsjlh,jc_fcgysjs.jssj,jc_fcgysjs.ppbm,crm_khxx.khbh,crm_khxx.khxm,jc_fcgysjsmx.ddbh,jc_zcdd.cbze,ROUND(NVL(jc_zcdd.ycf,2)+NVL(jc_zcdd.qtf,2),2),jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,crm_khxx.hth,crm_khxx.fwdz,DECODE(jc_fcgysjs.shbz,'N','未审核','Y','审核') shbz,jc_fcgysjs.gys,jc_fcgysjs.cwzq,jc_fcgysjs.jsr,TO_CHAR(jc_fcgysjs.cbzj) cbzj,TO_CHAR(jc_fcgysjs.jsje) jsje,TO_CHAR(jc_fcgysjs.kcfy) kcfy";
	ls_sql+=" FROM jc_fcgysjs,jc_fcgysjsmx,crm_khxx,jc_zcdd  ";
    ls_sql+=" where jc_fcgysjs.jsjlh=jc_fcgysjsmx.jsjlh";
    ls_sql+=" and jc_fcgysjsmx.ddbh=jc_zcdd.ddbh";
    ls_sql+=" and jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_fcgysjs.lx='1'";//1：订单；2：代购主材
	...
	ls_sql+=" SELECT jc_fcgysjs.jsjlh,jc_fcgysjs.jssj,jc_fcgysjs.ppbm,crm_khxx.khbh,crm_khxx.khxm,jc_fcgysjsmx.ddbh,jc_zczjx.zjxcbze,ROUND(NVL(jc_zczjx.zjxycf,2)+NVL(jc_zczjx.zjxqtf,2),2),jc_zczjx.clgw,jc_zczjx.ztjjgw,jc_zczjx.xmzy,crm_khxx.hth,crm_khxx.fwdz,DECODE(jc_fcgysjs.shbz,'N','未审核','Y','审核') shbz,jc_fcgysjs.gys,jc_fcgysjs.cwzq,jc_fcgysjs.jsr,TO_CHAR(jc_fcgysjs.cbzj) cbzj,TO_CHAR(jc_fcgysjs.jsje) jsje,TO_CHAR(jc_fcgysjs.kcfy) kcfy";
	ls_sql+=" FROM jc_fcgysjs,jc_fcgysjsmx,crm_khxx,jc_zczjx  ";
    ls_sql+=" where jc_fcgysjs.jsjlh=jc_fcgysjsmx.jsjlh";
...
//    ls_sql+=" order by jc_fcgysjs.jsjlh,crm_khxx.khxm,jc_fcgysjsmx.ddbh desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("jc_fcgysjsmxCxList.jsp","","","");
...
//设置显示合并列
	...
	spanColHash.put("cwzq","1");//列参数对象加入Hash表
	spanColHash.put("jsr","1");//列参数对象加入Hash表
	spanColHash.put("kcfy","1");//列参数对象加入Hash表
	spanColHash.put("jsje","1");//列参数对象加入Hash表
	spanColHash.put("cbzj","1");//列参数对象加入Hash表
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="6%">财务帐期</td>
	<td  width="3%">结算人</td>
	<td  width="6%">结算单材料成本</td>
	<td  width="6%">结算单结算金额</td>
	<td  width="5%">结算单扣除费用</td>
</tr>
<%
