...
try {
	...
	ls_sql+=" where crm_khxx.sgd=sq_sgd.sgd(+) and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD') and sq_sgd.sgdlx='S1'";
	ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	if (fgs!=null)
	{
		if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
	}
	ls_sql+=" group by crm_khxx.sgd";
//	out.println(ls_sql);
	...
	ls_sql="select sgd,sgdmc,DECODE(sgdlx,'S0','非直属','S1','直属') sgdlx,bzs,sum(sl),sum(je)";
	ls_sql+=" from";
	ls_sql+=" (";

	ls_sql+=" select crm_khxx.sgd,sgdmc,sq_sgd.sgdlx,bzs,count(*) sl,sum(qye) je";
	ls_sql+=" from crm_khxx,sq_sgd";
	ls_sql+=" where crm_khxx.sgd=sq_sgd.sgd and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')  and sq_sgd.sgdlx='S1'";
	ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	if (fgs!=null)
	...
	ls_sql+=" group by crm_khxx.sgd,sgdmc,sq_sgd.sgdlx,bzs";

	ls_sql+=" union";

	ls_sql+=" select sgd,sgdmc,sq_sgd.sgdlx,bzs,0 sl,0 je";
	ls_sql+=" from sq_sgd";
	ls_sql+=" where cxbz='N' and sq_sgd.sgdlx='S1'";
	if (fgs!=null)
	{
	...
	ls_sql+=" )";
	ls_sql+=" group by sgd,sgdmc,sgdlx,bzs";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		ls_sql+=" where crm_khxx.hdr='"+dwmc+"' and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		if (fgs!=null)
		{
			if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
		...
		ls_sql=" select count(*),sum(qye)";
		ls_sql+=" from crm_khxx";
		ls_sql+=" where crm_khxx.sgd='"+sgd+"' and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		if (fgs!=null)
		{
			if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
...
%>
          ...
          <td align="left"><A HREF="ViewSgdcz.jsp?sgd=<%=sgd%>&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>" target="_blank"><img src="/images/ColuGif.gif" width="<%=je*80/maxje%>" height="14"></A></td>
        </tr>
<%
