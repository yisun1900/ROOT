...
<TR bgcolor="#CCCCCC"  align="center" style="FONT-WEIGHT: bold; Z-INDEX: 5; POSITION: relative;TOP: expression(this.offsetParent.scrollTop)"> 
  ...
  <td  width="5%" style="BORDER-LEFT: #000000 1px solid;BORDER-BOTTOM: #000000 1px solid;POSITION: relative; BACKGROUND-COLOR: #CC6600">管理费(折后)</td>
...
<%
	...
	if (px.equals("qye"))
	{
		...
		ls_sql+=" from ";
		ls_sql+=" ( ";
		ls_sql+=" SELECT sjs,dwmc,crm_khxx.dwbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(qye+NVL(zhzjxje,0)) zjhze,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
		...
		while (rs.next())
		{
			...
			ls_sql+=" and crm_khxx.dwbh='"+dmbh+"' and crm_khxx.sjs='"+getsjs+"' and jzkscsj is not null";
			ls_sql+=wheresql;
			...
			ls_sql+=" FROM crm_khxx";
			ls_sql+=" where crm_khxx.zt in('2','5')";
			ls_sql+=" and crm_khxx.dwbh='"+dmbh+"' and crm_khxx.sjs='"+getsjs+"' and cwjzkscsj is not null";
			ls_sql+=wheresql;
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
			...
			ls_sql+=" and crm_khxx.dwbh='"+dmbh+"' and crm_khxx.sjs='"+getsjs+"' and cw_khfkjl.fklxbm in('11','54','96') and cw_khfkjl.scbz='N' ";
			ls_sql+=wheresql;
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
	...
	{
		ls_sql=" SELECT sjs,dwmc,dwbh,sum(qys) qys,sum(qye) qye,sum(suijin) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(zjhze) zjhze,sum(suijinze) suijinze,sum(sjf) sjf,sum(glf) glf,sum(guanlif) guanlif,sum(fwmj) fwmj,sum(suijin) suijin";
		ls_sql+=" from ";
		ls_sql+=" ( ";
		ls_sql+=" SELECT sjs,dwmc,crm_khxx.dwbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(qye+NVL(zhzjxje,0)) zjhze,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
		ls_sql+=" FROM crm_khxx,sq_dwxx";
...
//		ls_sql+=" having sum(wdzgce)!=0 and sum(fwmj)!=0";
		...
		if (sjs.equals(""))
		{
			...
			ls_sql+=" SELECT yhmc sjs,dwmc,sq_yhxx.dwbh,0 qys,0 qye,0 suijin,0 wdzgce,0 zhzjxje,0 zjxsuijin,0 zjxguanlif,0 zjhze,0 suijinze,0 sjf,0 glf,0 guanlif,0 fwmj";
			ls_sql+=" FROM sq_yhxx,sq_dwxx";
			ls_sql+=" where sq_yhxx.dwbh=sq_dwxx.dwbh and zwbm in('04','23') and sflz='N'";
			ls_sql+=wheresql1;
		}
	...
	{
		ls_sql=" SELECT sjs,dwmc,dwbh,sum(qys) qys,sum(qye) qye,sum(suijin) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(zjhze) zjhze,sum(suijinze) suijinze,sum(sjf) sjf,sum(glf) glf,sum(guanlif) guanlif,sum(fwmj) fwmj,DECODE(sum(qys),0,0,sum(qye)/sum(qys)) pjde";
		ls_sql+=" from ";
		ls_sql+=" ( ";
		ls_sql+=" SELECT sjs,dwmc,crm_khxx.dwbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(qye+NVL(zhzjxje,0)) zjhze,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
		ls_sql+=" FROM crm_khxx,sq_dwxx";
...
//		ls_sql+=" having sum(wdzgce)!=0 and sum(fwmj)!=0";
	...
	{
		ls_sql=" SELECT sjs,dwmc,dwbh,sum(qys) qys,sum(qye) qye,sum(suijin) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(zjhze) zjhze,sum(suijinze) suijinze,sum(sjf) sjf,sum(glf) glf,sum(guanlif) guanlif,sum(fwmj) fwmj,DECODE(sum(fwmj),0,0,sum(qye)/sum(fwmj)) pmjj";
		ls_sql+=" from ";
		ls_sql+=" ( ";
		ls_sql+=" SELECT sjs,dwmc,crm_khxx.dwbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(qye+NVL(zhzjxje,0)) zjhze,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
		ls_sql+=" FROM crm_khxx,sq_dwxx";
...
//		out.println(ls_sql);
	...
	{
		ls_sql=" SELECT sjs,dwmc,dwbh,sum(qys) qys,sum(qye) qye,sum(suijin) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(zjhze) zjhze,sum(suijinze) suijinze,sum(sjf) sjf,sum(glf) glf,sum(guanlif) guanlif,sum(fwmj) fwmj,DECODE(sum(wdzgce),0,0,sum(qye)*10/sum(wdzgce)) zkl";
		ls_sql+=" from ";
		ls_sql+=" ( ";
		ls_sql+=" SELECT sjs,dwmc,crm_khxx.dwbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(zjxguanlif) zjxguanlif,sum(qye+NVL(zhzjxje,0)) zjhze,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
		ls_sql+=" FROM crm_khxx,sq_dwxx";
		...
		ls_sql+=" group by sjs,dwmc,dwbh";
		ls_sql+=" order by zkl desc,qye desc";
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery(ls_sql);
		while (rs.next())
		{
...
<tr align="center">
	...
	<input type=button value='查看家装、集成签单明细' onclick="f_jzjc(listform)">
	</td>
