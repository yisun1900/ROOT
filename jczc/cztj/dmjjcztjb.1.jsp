...
<table border="1" width="3480" cellspacing="0" cellpadding="1"  style='FONT-SIZE: 14px'>
  ...
  <%
	...
	{	
		...
		ls_sql+=" where crm_khxx.zt='2' and crm_khxx.fgsbh='"+fgsbh+"'";//2：家装客户；3：退单客户；4：非家装客户；5：设计客户
		ls_sql+=wheresql;
		...
		//计算分公司：木门金额计算＝本期合同金额＋本期增减项－本期退单金额
		//签约
		int mmqds=0;
		double mmqde=0;
		...
		int mmtds=0;
		double mmtde=0;
		ls_sql="SELECT sum(jc_mmydd.zjhze) zjhze,count(distinct jc_mmydd.khbh) mmqys";
		ls_sql+=" FROM jc_mmydd";
		ls_sql+=" where  jc_mmydd.fgsbh='"+fgsbh+"'";
		ls_sql+=wheresql2;
		...
		double cgtde=0;
		int cgtds=0;
		ls_sql="SELECT sum(jc_cgdd.zjhze) zjhze,count(distinct jc_cgdd.khbh) cgqys";
		ls_sql+=" FROM jc_cgdd";
		ls_sql+=" where jc_cgdd.fgsbh='"+fgsbh+"'";
		ls_sql+=wheresql2;
		...
		double jjtde=0;
		int jjtds=0;
		ls_sql="SELECT sum(jc_jjdd.zjhze) zjhze,count(distinct jc_jjdd.khbh) jjqys";
		ls_sql+=" FROM jc_jjdd";
		ls_sql+=" where jc_jjdd.fgsbh='"+fgsbh+"'";
		ls_sql+=wheresql2;
		...
		//计算分公司：家居签约数，不能把4项加在一起，有重复的，注意需要减退单数
		...
		ls_sql+=" and jc_mmydd.htrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and jc_mmydd.htrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ls_sql+=" and jc_mmydd.clzt>='04' and jc_mmydd.clzt!='98'";
		...
		ls_sql+=" and jc_cgdd.qhtsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and jc_cgdd.qhtsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ls_sql+=" and jc_cgdd.clzt>='13' and jc_cgdd.clzt!='98'";
		...
		ls_sql+=" and jc_jjdd.qhtsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and jc_jjdd.qhtsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ls_sql+=" and jc_jjdd.clzt>='13' and jc_jjdd.clzt!='98'";
		...
		if(rs1.next())
		{
			xjjctds=rs1.getInt(1);//家居数量
		}
		rs1.close();
		ps1.close();

		xjjcqys=xjjcqys-xjjctds;
		...
		ls_sql+=" UNION ALL";
		ls_sql+=" SELECT dwbh ,dwmc dmmc,0 jzqys,0 jzqye ";
		ls_sql+=" FROM sq_dwxx ";
		ls_sql+=" where  dwlx='F2' and cxbz='N' and ssfgs='"+fgsbh+"'";
		ls_sql+=wheresql1;
...
*/
			...
			mmqys=mmqds-mmtds;
			mmqye=mmqde-mmtde+mmzjx;//签单－退单＋增减项
			...
			cgqys=cgqds-cgtds;
			cgqye=cgqde-cgtde+cgzjx;//签单－退单＋增减项
			...
			jjqys=jjqds-jjtds;
			jjqye=jjqde-jjtde+jjzjx;//签单－退单＋增减项
...
//			out.println(ls_sql);
			...
			zcqys=zcqds-zctds;
			zcqye=zcqde-zctde+zczjx;//签单－退单＋增减项
