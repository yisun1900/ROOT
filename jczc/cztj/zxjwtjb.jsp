...
<table border="1" width="700%" cellspacing="0" cellpadding="1"  style='FONT-SIZE: 14px'>
<tr bgcolor="#CCCCCC"  align="center">
  <td  width="2%" rowspan="2">分公司</td>
  <td  width="1%" rowspan="2">签约数</td>
  <td  width="2%" rowspan="2">签约额</td>
...
<%
	...
	while (rs.next())
	{
		...
		zxjwmc=rs.getString("zxjwmc");

		%>
		  <td  width="7%" colspan="6"><%=zxjwmc%></td>
		<%
	}
	rs.close();
	ps.close();
...
<%
	...
	while (rs.next())
	{
		...
		while (rs1.next())
		{
			zxjwbm=rs1.getString("zxjwbm");
			zxjwmc=rs1.getString("zxjwmc");
			...
			ls_sql+=" where crm_khxx.zt in('2','5')";
			ls_sql+=" and crm_khxx.fgsbh='"+getfgsbh+"'";
			ls_sql+=" and crm_khxx.zxjwbm='"+zxjwbm+"'";
			ls_sql+=wheresql;
			ps2= conn.prepareStatement(ls_sql);
			rs2 =ps2.executeQuery();
			if (rs2.next())
