...
//定义变量,获取参数
	...
	String hdbz=request.getParameter("hdbz");
...
try {
	...
	ls_sql+=" and qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS') and zt='2'";//1：非回单； 2：设计师回单；3：施工队回单
	if (fgs!=null)
	{
		if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
	}
	ls_sql+=" and hdbz='"+hdbz+"'";
	ls_sql+=" group by hdr";
	...
	ls_sql+=" from crm_khxx";
	ls_sql+=" where qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
	ls_sql+=" and qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS') and zt='2'";
	if (fgs!=null)
	{
		if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
	}
...
%>
          ...
          <td align="left"><A HREF="ViewHdtj.jsp?hdbz=<%=hdbz%>&hdr=<%=dwmc%>&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>" target="_blank"><img src="/images/ColuGif.gif" width="<%=je*200/maxje%>" height="14"></A></td>
        </tr>
<%
