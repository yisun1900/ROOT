...
<%
	...
	String sgbz=cf.GB2Uni(cf.getParameter(request,"sgbz"));
	...
	ls_sql="SELECT khbh,khxm,fwdz,qye,a.dwmc qydm,qyrq,DECODE(hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他'),hdr,DECODE(jlbz,'0','正常派','1','回单奖励','2','公司奖励','3','设计师指定派单','4','客户指定派单','5','大单抵小单','6','小单抵大单','7','特殊情况'),pdsj,pdr,pdsm,sjkgrq,sjjgrq,sgdmc,sgbz,sjs,zjxm";
	ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_sgd";
    ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and zt='2'";
    ls_sql+=" and crm_khxx.sgd='"+sgd+"' and crm_khxx.sgbz='"+sgbz+"'";
...
<CENTER >
  <B><font size="3"> 班长接单统计 （<%=dwmc%>：<%=sgbz%>）
