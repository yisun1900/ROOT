...
<%
	...
	String ysy=cf.GB2Uni(cf.getParameter(request,"ysy"));
	...
	ls_sql="SELECT '',ysy,hth,khxm,fwdz,DECODE(crm_khxx.jzbz,'1','家装','2','公装'),cxhdbm,wdzgce,TO_CHAR(zkl),qye,suijinbfb||'%',suijin,guanlif,qye+NVL(suijin,0)+NVL(guanlif,0),TO_CHAR(jzkscsj,'YYYY-MM-DD'),TO_CHAR(cwjzkscsj,'YYYY-MM-DD'),sjf,glf,zjxje,zhzjxje,zjxsuijin,zjxguanlif,zhzjxje+NVL(zjxsuijin,0)+NVL(zjxguanlif,0),qye+NVL(suijin,0)+NVL(guanlif,0)+zhzjxje+NVL(zjxsuijin,0)+NVL(zjxguanlif,0),NVL(suijin,0)+NVL(zjxsuijin,0),DECODE(crm_khxx.sffj,'Y','返卷','N','否'),crm_khxx.fjje,crm_khxx.gsfje,DECODE(crm_khxx.sfyrz,'Y','认证','N','否'),b.dwmc rzsc,a.dwmc qydm,TO_CHAR(lrsj,'YYYY-MM-DD HH24:MI'),TO_CHAR(qyrq,'YYYY-MM-DD'),DECODE(TO_CHAR(lrsj,'MM')-TO_CHAR(qyrq,'MM'),0,'是','<font color=\"#FF0000\">跨月</font>'),sgdmc sgd,zjxm";
    ...
    ls_sql+=" and crm_khxx.ysy='"+ysy+"' and crm_khxx.dwbh='"+dwbh+"'";
