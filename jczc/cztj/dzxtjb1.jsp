...
<%
	...
	ls_sql+=" where crm_khxx.cqbm=dm_cqbm.cqbm";
	ls_sql+=" and qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
	ls_sql+=" and qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	ls_sql+=wheresql;
	if (fgs!=null)
	...
	while (rs.next())
	{
		...
		//--------------------------
			...
			ls_sql+=" ,sum(crm_khxx.sjf) sz10,sum(sfke) sz11,sum(fjje) sz12,sum(gsfje) sz13";//其它
			ls_sql+=" ,sum(crm_khxx.qyrq-crm_zxkhxx.lrsj) sz19,sum(crm_khxx.fwmj) sz20,crm_khxx.ywy ";//其它
			ls_sql+=" FROM crm_khxx,crm_zxkhxx,dm_cqbm";
			ls_sql+=" where crm_khxx.khbh=crm_zxkhxx.khbh and crm_khxx.zt in('2','5') ";
			ls_sql+=" and crm_khxx.cqbm=dm_cqbm.cqbm";
			ls_sql+=" and dm_cqbm.cqmc='"+cqmc+"'";
			ls_sql+=" and qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
			ls_sql+=" and qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
			ls_sql+=wheresql;
			if (fgs!=null)
...
<%
	...
	while (rs3.next())
	{
		...
		ls_sql+=" group by dm_cqbm.cqbm,cqmc";
		ls_sql+=" order by count(*) desc";
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery(ls_sql);
		while (rs.next())
...
<%
	//城区大小单统计
	ls_sql="SELECT cqbm,cqmc";
	ls_sql+=" FROM dm_cqbm";
	ls_sql+=" order by cqbm ";
	ps3= conn.prepareStatement(ls_sql);
	rs3 =ps3.executeQuery();
	while (rs3.next())
	{
		cqbm=rs3.getString("cqbm");
		cqmc=rs3.getString("cqmc");

		//分类小计
		ls_sql="SELECT count(*),sum(qye),sum(fwmj)";
		ls_sql+=" FROM crm_khxx";
		ls_sql+=" where crm_khxx.cqbm='"+cqbm+"'";
		ls_sql+=" and qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ls_sql+=wheresql;
		if (fgs!=null)
