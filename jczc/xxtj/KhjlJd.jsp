...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="15%">交款总额</td>
	<td  width="10%">量房订金客户数</td>
	<td  width="10%">活动订金客户数</td>
...
try {
	...
	while (rs.next())
	{
		...
		if (zxdm!=null)
		{
			if (!(zxdm.equals("")))	ls_sql+=" and  (dmbh='"+zxdm+"')";
		}
		ls_sql+=" order by dwbh";
		...
		while (rs3.next())
		{
			...
			ls_sql="SELECT crm_zxkhxx.khjl,count(distinct crm_zxkhxx.khbh) jds,sum(cw_khfkjl.fkje) fkze";
			...
			while(rs1.next())
			{
				...
				if (getkhjl.equals("&nbsp;"))
				{
					...
					ls_sql+=" and  cw_khfkjl.fklxbm='52'";
					ls_sql+=" and  cw_khfkjl.fkje!=0";
					ls_sql+=" and  cw_khfkjl.scbz='N'";
					...
					ls_sql+=" and  crm_zxkhxx.zxdm='"+dmbh+"'";
					ls_sql+=" and  crm_zxkhxx.khjl is null";
					ls_sql+=" and  cw_khfkjl.fklxbm='51'";
					ls_sql+=" and  cw_khfkjl.fkje!=0";
					ls_sql+=" and  cw_khfkjl.scbz='N'";
					ps2= conn.prepareStatement(ls_sql);
					rs2 =ps2.executeQuery();
				...
				<tr align="center">
					<td><%=dmmc%></td>
					<td><A HREF="ViewKhjlMx.jsp?dwbh=<%=dmbh%>&khjl=<%=getkhjl%>&skfw=<%=skfw%>&wheresql=<%=wheresql%>" target="_blank"><%=getkhjl%></A></td>
					<td><%=jds%></td>
					<td><%=fkze%></td>
					<td><%=lfdjkhs%></td>
					<td><%=hddjkhs%></td>
