...
try {
	...
	while (rs.next())
	{
		...
		while (rs3.next())
		{
			...
			ls_sql="SELECT crm_zxkhxx.sjs,count(distinct crm_zxkhxx.khbh) jds,sum(cw_khfkjl.fkje) fkze";
			...
			while(rs1.next())
			{
				...
				<tr align="center">
					<td><%=dmmc%></td>
					<td><A HREF="ViewSjsMx.jsp?dwbh=<%=dmbh%>&sjs=<%=getsjs%>&skfw=<%=skfw%>&wheresql=<%=wheresql%>" target="_blank"><%=getsjs%></A></td>
					<td><%=jds%></td>
					<td><%=fkze%></td>
					<td><%=lfdjkhs%></td>
					<td><%=hddjkhs%></td>
