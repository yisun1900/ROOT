...
try {
	...
	ls_sql="select khxm,fwdz,lxfs,crm_khxx.dwbh,dwmc,sjs,crm_khxx.sgd,sgdmc,zjxm,hth,sgbz,fgsbh,qyrq,tdsj,wdzgce,qye  ,zt";
	...
	ls_sql+=" where khbh='"+khbh+"' and zt!='3'";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
		yjjzsj=cf.fillNull(rs.getDate("jzrq"));
	}
	rs.close();
	ps.close();
	...
	if (rs.next())
	{
		tdyjjzsj=cf.fillNull(rs.getDate("jzrq"));
	}
	rs.close();
	ps.close();
