...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="2%" bgcolor="#99FFFF">折前</td>
	<td  width="2%" bgcolor="#99FFFF">折后</td>
	<td  width="2%" bgcolor="#99FFFF">减免额</td>
...
<%
	ls_sql="SELECT '',crm_zxkhxx.sjs,crm_zxkhxx.hth,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,DECODE(crm_khxx.jzbz,'1','家装','2','工装'),DECODE(crm_khxx.zt,'2','家装','4','集成','5','设计','3','<font color=\"#FF00FF\">已退单</font>'),crm_tddj.ytje,crm_tddj.stje,crm_khxx.tdsj,crm_khxx.qyrq ";
	...
	ls_sql+=" ,crm_khxx.sjf,cwsfke,DECODE((qye+NVL(guanlif,0)+NVL(suijin,0)),0,'',ROUND(cwsfke*100/(qye+NVL(guanlif,0)+NVL(suijin,0)),1)||'%' ),sgdmc sgd,crm_khxx.zjxm,crm_khxx.ywy ";

	ls_sql+=" FROM crm_khxx,sq_sgd,crm_zxkhxx,crm_tddj";
...
<%
	ls_sql="SELECT '',crm_zxkhxx.sjs,crm_zxkhxx.hth,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,DECODE(crm_khxx.jzbz,'1','家装','2','工装'),DECODE(crm_khxx.zt,'2','家装','4','集成','5','设计','3','<font color=\"#FF00FF\">已退单</font>'),crm_khxx.tdsj,crm_khxx.qyrq ";
	ls_sql+=" ,crm_zxkhxx.dzbjze,TO_CHAR(crm_khxx.zkl),TO_CHAR(crm_khxx.zjxzkl) ";
	ls_sql+=" ,wdzgce+NVL(zqguanlif,0)+NVL(suijin,0),qye+NVL(guanlif,0)+NVL(suijin,0)";
	ls_sql+=" ,crm_khxx.wdzgce,crm_khxx.qye,NVL(cdzwjmje,0),NVL(zqguanlif,0),NVL(guanlif,0),NVL(glfjmje,0),NVL(suijin,0),NVL(sjjmje,0) ";
	ls_sql+=" ,TO_CHAR(round(DECODE(crm_khxx.fwmj,0,0,qye/crm_khxx.fwmj))) pmjj,NVL(zhzjxje,0),NVL(zjxguanlif,0),NVL(zjxsuijin,0) ";
	ls_sql+=" ,NVL(zhzjxje,0)+NVL(zjxguanlif,0)+NVL(zjxsuijin,0)+qye+NVL(guanlif,0)+NVL(suijin,0) ";
	ls_sql+=" ,crm_khxx.sjf,cwsfke,DECODE((qye+NVL(guanlif,0)+NVL(suijin,0)),0,'',ROUND(cwsfke*100/(qye+NVL(guanlif,0)+NVL(suijin,0)),1)||'%' ),sgdmc sgd,zjxm,crm_khxx.ywy,crm_khxx.cxhdbm ";
