...
//	out.println(ls_sql);
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT sq_dwxx.dwmc fgs,cw_qmjzmx.fgsbh,count(*) qys,sum(qye) qye,sum(NVL(suijin,0)) suijin,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(zjxsuijin) zjxsuijin,sum(NVL(zjxguanlif,0)) zjxguanlif,sum(NVL(suijin,0)+NVL(zjxsuijin,0)) suijinze,sum(NVL(sjf,0)) sjf,sum(NVL(glf,0)) glf,sum(NVL(guanlif,0)) guanlif,sum(fwmj) fwmj";
	ls_sql+=" FROM cw_qmjzmx,sq_dwxx";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		ls_sql+=" where cw_qmjzmx.fgsbh='"+getdwbh+"'";
		ls_sql+=" and zt='3' ";
		ls_sql+=wheresql;
		...
		ls_sql+=" and cw_qmjzmx.fgsbh='"+getdwbh+"' and jzkscsj is not null";
		ls_sql+=wheresql;
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		...
		ls_sql+=" FROM cw_qmjzmx";
		ls_sql+=" where cw_qmjzmx.zt in('2','5')";
		ls_sql+=" and cw_qmjzmx.fgsbh='"+getdwbh+"' and cwjzkscsj is not null";
		ls_sql+=wheresql;
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		...
		ls_sql+=" and cw_qmjzmx.fgsbh='"+getdwbh+"' and cw_khfkjl.fklxbm in('11','54','96') and cw_khfkjl.scbz='N' ";
		ls_sql+=wheresql;
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
