...
<tr bgcolor="#CCCCCC"  align="center" class="STYLE_TR">
  ...
  <td  width="4%" class="STYLE2">财务收款签约额</td>
  <td  width="3%" class="STYLE2">退单额</td>
  <td  width="4%" class="STYLE2">业绩</td>
  ...
  <td  width="3%" class="STYLE2">增减项</td>
...
//	out.print(ls_sql);
	...
	ls_sql+=",sum(wdzgce+NVL(zqguanlif,0)+NVL(suijin,0)) dmzqyj";//折前业绩
	ls_sql+=",sum(qye+NVL(guanlif,0)+NVL(suijin,0)) dmzhyj";//折后业绩
	ls_sql+=",sum(crm_khxx.wdzgce) dmgcybj";//工程原报价
	ls_sql+=",sum(crm_khxx.qye) dmgcqye";//工程签约额
	ls_sql+=",sum(NVL(cdzwjmje,0)) dmgcjme";//减免额
	ls_sql+=",sum(NVL(zqguanlif,0)) dmzqglf";//折前管理费
	ls_sql+=",sum(NVL(guanlif,0)) dmzhglf";//折后管理费
	ls_sql+=",sum(NVL(glfjmje,0)) dmjmeglf";//减免额管理费
	ls_sql+=",sum(NVL(suijin,0)) dmsj";//税金
	ls_sql+=",sum(NVL(sjjmje,0)) dmsjjme";//减免额
...
//	out.print(ls_sql);
	...
	while (rs.next())
	{
		...
		//退单
		ls_sql="SELECT sum(crm_khxx.qye+NVL(crm_khxx.guanlif,0)+NVL(crm_khxx.suijin,0) + (crm_tddj.stje-crm_tddj.ytje) ) cwsgqye ";
		...
		if (cxbz.equals("Y") && tdje<=0)//Y：已撤销；N：未撤销
		{
//			getdwmc="<font color=\"#FF0033\">"+getdwmc+"("+kdsj+"至"+gdsj+")</font>";
		}
		else{
...
<tr bgcolor="#CCCCCC"  align="center" class="STYLE_TR">
  ...
  <td  width="3%" class="STYLE2">签约额比例</td>
  <td  width="3%" class="STYLE2">增减项</td>
  <td  width="4%" class="STYLE2">增减后总额 </td>
  <td  width="3%" class="STYLE2">平均单额</td>
  <td  width="3%" class="STYLE2">平米均价</td>
  <!-- <td  width="5%" class="STYLE2">设计费</td> -->
...
<%
	//获取最大数
	ls_sql="SELECT max(TRUNC(sum(qye+NVL(guanlif,0)+NVL(suijin,0))))";
	ls_sql+=" FROM crm_khxx,sq_dwxx";
	ls_sql+=" where crm_khxx.zt in ('2') and crm_khxx.fgsbh=sq_dwxx.dwbh";
	ls_sql+=" and crm_khxx.yjjzsj is null";//未结转的单，都计算在内
	ls_sql+=" and crm_khxx.cwsfke>=(qye+NVL(guanlif,0)+NVL(suijin,0))*((sq_dwxx.jryjbfb-0.5)/100)";
	...
	ls_sql+=",sum(wdzgce+NVL(zqguanlif,0)+NVL(suijin,0)) fgszqyj";//折前业绩
	ls_sql+=",sum(qye+NVL(guanlif,0)+NVL(suijin,0)) fgszhyj";//折后业绩
	ls_sql+=",sum(crm_khxx.wdzgce) fgsgcybj";//工程原报价
	ls_sql+=",sum(crm_khxx.qye) fgsgcqye";//工程签约额
	ls_sql+=",sum(NVL(cdzwjmje,0)) fgsgcjme";//减免额
	ls_sql+=",sum(NVL(zqguanlif,0)) fgszqglf";//折前管理费
	ls_sql+=",sum(NVL(guanlif,0)) fgszhglf";//折后管理费
	ls_sql+=",sum(NVL(glfjmje,0)) fgsjmeglf";//减免额管理费
	ls_sql+=",sum(NVL(suijin,0)) fgssj";//税金
	ls_sql+=",sum(NVL(sjjmje,0)) fgssjjme";//减免额
	ls_sql+=" FROM crm_khxx,sq_dwxx";
	ls_sql+=" where crm_khxx.zt in ('2') and crm_khxx.fgsbh=sq_dwxx.dwbh";
	ls_sql+=" and crm_khxx.yjjzsj is null";//未结转的单，都计算在内
	ls_sql+=" and crm_khxx.cwsfke>=(qye+NVL(guanlif,0)+NVL(suijin,0))*((sq_dwxx.jryjbfb-0.5)/100)";
	...
	while (rs.next())
	{
		...
		else
		{
			...
			<!-- <A HREF="ViewDyFgscz.jsp?dwbh=<%=getfgsbh%>&jzbz=<%=jzbz%>" target="_blank"><%=row%></A> -->
			</td>
			<td  class="STYLE3"><%=getfgsmc%></td>
		...
		else
		{
			...
			<!-- <A HREF="ViewDyFgscz.jsp?dwbh=<%=getfgsbh%>&jzbz=<%=jzbz%>" target="_blank"><%=cf.formatDouble(qye)%></A> -->
			</td>
			<td  align="right" class="STYLE4"><%=cf.formatDouble(tdje)%></td>
			<td  align="right" class="STYLE4"><%=cf.formatDouble(yjje)%></td>
			<td  align="right"  class="STYLE4"><%=qys%></td>
...
<tr bgcolor="#CCCCCC"  align="center" class="STYLE_TR">
  <td  width="2%" class="STYLE1">排名</td>
  <td  width="3%" class="STYLE1">大区</td>
  <td  width="4%" class="STYLE2">财务收款签约额</td>
  <td  width="3%" class="STYLE2">退单额</td>
  <td  width="4%" class="STYLE2">业绩</td>
  <td  width="3%" class="STYLE2">签约数</td>
  <td  width="3%" class="STYLE2">签约数比例</td>
  <td  width="3%" class="STYLE2">合同金额</td>
  <td  width="3%" class="STYLE2">签约额比例</td>
  <td  width="3%" class="STYLE2">增减项</td>
  <td  width="4%" class="STYLE2">增减后总额 </td>
  <td  width="3%" class="STYLE2">平均单额</td>
...
<%
	...
	ls_sql+=",sum(wdzgce+NVL(zqguanlif,0)+NVL(suijin,0)) dqzqyj";//折前业绩
	ls_sql+=",sum(qye+NVL(guanlif,0)+NVL(suijin,0)) dqzhyj";//折后业绩
	ls_sql+=",sum(crm_khxx.wdzgce) dqgcybj";//工程原报价
	ls_sql+=",sum(crm_khxx.qye) dqgcqye";//工程签约额
	ls_sql+=",sum(NVL(cdzwjmje,0)) dqgcjme";//减免额
	ls_sql+=",sum(NVL(zqguanlif,0)) dqzqglf";//折前管理费
	ls_sql+=",sum(NVL(guanlif,0)) dqzhglf";//折后管理费
	ls_sql+=",sum(NVL(glfjmje,0)) dqjmeglf";//减免额管理费
	ls_sql+=",sum(NVL(suijin,0)) dqsj";//税金
	ls_sql+=",sum(NVL(sjjmje,0)) dqsjjme";//减免额
	ls_sql+=" FROM crm_khxx,sq_dwxx";
	ls_sql+=" where crm_khxx.zt in ('2') and crm_khxx.fgsbh=sq_dwxx.dwbh";
	ls_sql+=" and crm_khxx.yjjzsj is null";//未结转的单，都计算在内
	ls_sql+=" and crm_khxx.cwsfke>=(qye+NVL(guanlif,0)+NVL(suijin,0))*((sq_dwxx.jryjbfb-0.5)/100)";
