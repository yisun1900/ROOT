...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (zcdlbm!=null)
	{
		...
		if (!(zcdlbm.equals("")))	wheresql+=" and  (jc_zcddmx.zcdlbm='"+zcdlbm+"')";
	}
	String zcxlbm=null;
	...
	if (zcxlbm!=null)
	{
		...
		if (!(zcxlbm.equals("")))	wheresql+=" and  (jc_zcddmx.zcxlbm='"+zcxlbm+"')";
	}
	String ppbm=null;
	...
	{
		ppbm=cf.GB2Uni(ppbm);
		if (!(ppbm.equals("")))	wheresql+=" and  (jc_zcddmx.dwbh='"+ppbm+"')";
	}
	String ppbm2=null;
	...
	if (ppbm2!=null)
	{
		...
		if (!(ppbm2.equals("")))	wheresql+=" and  (jc_zcddmx.dwbh like '%"+ppbm2+"%')";
	}
	String ppmc=null;
	...
	{
		ppmc=cf.GB2Uni(ppmc);
		if (!(ppmc.equals("")))	wheresql+=" and  (jc_zcddmx.ppmc='"+ppmc+"')";
	}
	String ppmc2=null;
	...
	if (ppmc2!=null)
	{
		...
		if (!(ppmc2.equals("")))	wheresql+=" and  (jc_zcddmx.ppmc like '%"+ppmc2+"%')";
	}
	String gys=null;
	...
	{
		gys=cf.GB2Uni(gys);
		if (!(gys.equals("")))	wheresql+=" and  (jc_zcddmx.gys='"+gys+"')";
	}
	String gys2=null;
	...
	{
		gys2=cf.GB2Uni(gys2);
		if (!(gys2.equals("")))	wheresql+=" and  (jc_zcddmx.gys like '%"+gys2+"%')";
	}
	...
	sxhtyy=request.getParameter("sxhtyy2");
	if (sxhtyy!=null)
	{
		sxhtyy=cf.GB2Uni(sxhtyy);
		if (!(sxhtyy.equals("")))	wheresql+=" and  (jc_zcddmx.sxhtyy='"+sxhtyy+"')";
	...
	ls_sql="SELECT crm_khxx.khxm,jc_zcddmx.ddbh,jc_zcddmx.zcmc,jc_zcddmx.zcdlbm,jc_zcddmx.zcxlbm,jc_zcddmx.dwbh,jc_zcddmx.ppmc,jc_zcddmx.gys,jc_zcddmx.zclbbm,jc_zcddmx.xinghao,jc_zcddmx.gg,jc_zcddmx.zcysbm,jc_zcddmx.jldwbm,TO_CHAR(jc_zcddmx.lsj),TO_CHAR(jc_zcddmx.yhj), DECODE(jc_zcddmx.sfycx,'N','没促销','Y','有促销'),TO_CHAR(jc_zcddmx.cxj),jc_zcddmx.cxkssj,jc_zcddmx.cxjzsj, DECODE(jc_zcddmx.xzjg,'1','销售限价','2','促销价'),TO_CHAR(jc_zcddmx.dj),jc_zcddmx.sl,jc_zcddmx.je,jc_zcddmx.ycf, DECODE(jc_zcddmx.qtfy,'1','安装费','2','加工费','3','其它收费'),jc_zcddmx.qtfyje,jc_zcddmx.zjhsl,jc_zcddmx.zjhje,jc_zcddmx.zjhycf,jc_zcddmx.zjhqtfy,jc_zcddmx.zcpzwzbm, DECODE(jc_zcddmx.cxhdbz,'Y','参加','N','不参加'),TO_CHAR(jc_zcddmx.cxhdbl),jc_zcddmx.cxhdje, DECODE(jc_zcddmx.lx,'1','合同项','2','增项'),jc_zcddmx.lrr,jc_zcddmx.lrsj, DECODE(jc_zcddmx.sxbz,'1','非手写','2','手写'),jc_zcddmx.sxhtyy,jc_zcddmx.bz  ";
