...
try {
	...
	for (int i=0;i<fgsbh.length ;i++ )
	{
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[客户管理费比例应收比例(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[客户管理费比例实际收取(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[税金及其他比例应收比例(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[税金及其他比例实际收取(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[工程毛利率去年平均(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[工程毛利率本周指数(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[主材毛利率去年平均(%)]不是数字:"+e);
			return;
		}
		...
		try{
			...
			conn.rollback();
			out.println("<BR>"+cf.GB2Uni(fgsbh[i])+"[主材毛利率本周指数(%)]不是数字:"+e);
			return;
		}
