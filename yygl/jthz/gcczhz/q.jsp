...
<table border="1" width="140%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
 ...
 <%
	...
	while(rs1.next())
	{
		...
		ls_sql=" select count(*) sjsrs,sum(yy_sjbdj_sydjcb),sum(yy_sjbdj_bylj),sum(yy_sjbdj_szjh),sum(yy_sjbdj_szwc),sum(yy_sjbdj_bzyj),sum(yy_sjbdj_bztdj),sum(yy_sjbdj_byljtdj) ";
		ls_sql+=" ,sum(yy_sjbgccz_bylj),sum(yy_sjbgccz_szjh),sum(yy_sjbgccz_szwc),sum(yy_sjbgccz_bzyj) ";
		ls_sql+=" from yy_sjb";
		...
		if(rs.next())
		{
			...
			yy_sjbdj_sydjcb=yy_sjbdj_byjh=rs.getDouble(2);
			...
			yy_sjbgccz_bylj=rs.getDouble(9);
			yy_sjbgccz_szjh=rs.getDouble(10);
			yy_sjbgccz_szwc=rs.getDouble(11);
			yy_sjbgccz_bzyj=rs.getDouble(12);
		}
		rs.close();
		ps.close();

		ls_sql="select yy_sjsjb_tqgccz,yy_sjsjb_gccz,yy_sjzky_sqdj";
		ls_sql+=" from yy_sj";
		...
		if(rs.next())
		{
			...
			yy_sjbgccz_byjh=rs.getDouble("yy_sjsjb_gccz");
			yy_sjbdj_byjh=rs.getDouble("yy_sjzky_sqdj");
		}
		rs.close();
		ps.close();
...
<tr bgcolor="#FFFFFF" align="center">
	<td >小计</td>
	<td><%=xjsjsrs%></td>
	<td><%=cf.round(xjyy_sjbdj_sydjcb,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_byjh,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_bylj,2)%></td>
	<td><%=cf.round(djdcl,2)%>%</td>
	<td><%=cf.round(rjdj,2)%></td>

	<td><%=cf.round(xjyy_sjbdj_szjh,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_szwc,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_bzyj,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_bztdj,2)%></td>
	<td><%=cf.round(xjyy_sjbdj_byljtdj,2)%></td>

	<td><%=cf.round(xjyy_sjsjb_tqgccz,2)%></td>

	<td><%=cf.round(xjyy_sjbgccz_byjh,2)%></td>
	<td><%=cf.round(xjyy_sjbgccz_bylj,2)%></td>
	<td><%=cf.round(gcdcl,2)%>%</td>
	<td><%=cf.round(gcrj,2)%></td>
	<td><%=cf.round(xjyy_sjbgccz_szjh,2)%></td>
	<td><%=cf.round(xjyy_sjbgccz_szwc,2)%></td>
	<td><%=cf.round(xjyy_sjbgccz_bzyj,2)%></td>
</tr>
</table>
<%
}
