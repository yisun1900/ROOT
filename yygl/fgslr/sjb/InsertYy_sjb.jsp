...
<body bgcolor="#FFFFFF">
<form method="post" action="InsertYy_sjb1.jsp" name="selectform">
<div align="center">设计部录入</div>
...
<tr bgcolor="#FFFFFF">
  ...
  <td width="56%"> 
	<select name="yy_sjb_zqs" style="FONT-SIZE:12PX;WIDTH:252PX" >
	<%
...
{
	if(	javaTrim(FormName.yy_sjb_zqs)=="") {
		alert("请输入[周期数]！");
		FormName.yy_sjb_zqs.focus();
		return false;
	}
	if(!(isNumber(FormName.yy_sjb_zqs, "周期数"))) {
		return false;
	}
	if(	javaTrim(FormName.yy_sjb_ssfgs)=="") {
		alert("请输入[所属分公司]！");
		FormName.yy_sjb_ssfgs.focus();
		return false;
	}
	if(	javaTrim(FormName.yy_sjb_ssbm)=="") {
		alert("请输入[所属部门]！");
		FormName.yy_sjb_ssbm.focus();
		return false;
	}
