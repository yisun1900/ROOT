...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and crm_gjzb.sybz='Y'";
    ls_sql+=wheresql;
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_gjzbXgSpList.jsp","","","EditSpCrm_gjzb.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_gjzbXgSpList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_gjzb set sybz='N',syr=null,sysj=null,sysm=null where sybz='Y' and "+chooseitem;
...
<CENTER >
  <B><font size="3">管家周报－－修改审批</font></B>
</CENTER>
<%
	pageObj.out=out;
