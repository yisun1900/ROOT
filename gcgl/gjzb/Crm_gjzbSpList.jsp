...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_gjzbSpList.jsp","","","EditSpCrm_gjzb.jsp");
...
//设置按钮参数
	String[] buttonName={"批量审批"};//按钮的显示名称
	String[] buttonLink={"Crm_gjzbSpList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_gjzb set sybz='Y',syr='"+yhmc+"',sysj=TRUNC(SYSDATE),sysm='批量审批' where sybz='N' and "+chooseitem;
