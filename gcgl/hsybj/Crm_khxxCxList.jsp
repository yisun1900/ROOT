...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ybjss=request.getParameter("ybjss");
	if (ybjss!=null)
	{
		ybjss=cf.GB2Uni(ybjss);
		if (!(ybjss.equals("")))	wheresql+=" and  (ybjss='"+ybjss+"')";
	}
	...
	ls_sql="SELECT khbh,khxm, DECODE(xb,'M','男','W','女') as xb,fwdz,lxfs,ybjbh,DECODE(ybjss,'0','未核实','Y','属实','N','不属实') as ybjss,qyrq,kgrq,dwmc,lrsj  ";
	ls_sql+=" FROM crm_khxx,sq_dwxx  ";
    ls_sql+=" where (crm_khxx.dwbh=sq_dwxx.dwbh(+) and zt='2' and ybjss in('0','Y','N'))";
    ls_sql+=wheresql;
	ls_sql+=" order by khbh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxCxList.jsp","SelectCxCrm_khxx.jsp","EditCrm_khxx.jsp","");
//	pageObj.setPageRow(10);//设置每页显示记录数
	pageObj.setViewStr("核实");
/*
//设置显示列
...
//设置列超级连接
	...
	coluParm=new ColuParm();//生成一个列参数对象
	String[] coluKey1={"ybjbh"};//设置列参数：coluParm.key的主键
	coluParm.key=coluKey1;//设置列参数：coluParm.key的主键
	coluParm.link="/khxx/ViewYbjxx.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("ybjbh",coluParm);//列参数对象加入Hash表
	pageObj.setColuLink(coluParmHash);//列参数对象加入Hash表
/*
//设置主键的显示方式
