...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (hdsgd!=null)
	{
		...
		if (!(hdsgd.equals("")))	wheresql+=" and  (hdsgd='"+hdsgd+"')";
	}
	String hdsgbz=null;
	...
	if (hdsgbz!=null)
	{
		...
		if (!(hdsgbz.equals("")))	wheresql+=" and  (hdsgbz='"+hdsgbz+"')";
	}
	String hdsjs=null;
	...
	if (hdsjs!=null)
	{
		...
		if (!(hdsjs.equals("")))	wheresql+=" and  (hdsjs='"+hdsjs+"')";
	}
	String cgdz=null;
	...
	{
		sbrq=sbrq.trim();
		if (!(sbrq.equals("")))	wheresql+="  and (sbrq>=TO_DATE('"+sbrq+"','YYYY/MM/DD'))";
	}
	sbrq=request.getParameter("sbrq2");
	if (sbrq!=null)
	{
		sbrq=sbrq.trim();
		if (!(sbrq.equals("")))	wheresql+="  and (sbrq<=TO_DATE('"+sbrq+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT khbh,khxm, DECODE(xb,'M','男','W','女') as xb,fwdz,lxfs,qyrq,kgrq,dwmc,sbrq, DECODE(hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他') as hdbz  ";
	ls_sql+=" FROM crm_khxx,sq_dwxx  ";
    ls_sql+=" where (crm_khxx.dwbh=sq_dwxx.dwbh(+) and hdbz!='1')";
    ls_sql+=wheresql;
	ls_sql+=" order by khbh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxList.jsp","SelectCrm_khxx.jsp","","EditCrm_khxx.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_khxxList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from crm_khxx where "+chooseitem;
