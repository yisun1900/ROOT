...
try {
	...
	ls_sql="select khbh,sqrq,sqr,sqrlxdh,sfxtp,jhjdrq,jhjdsj,sqsm,zcwcsj,lrr,lrsj,lrbm,jdlxbm,sfycp,DECODE(tpsftg,'Y','通过','N','未通过') tpsftg,tpshr,tpshsj,tpsm ";
	...
	if (rs.next())
	{
		...
		tpshr=cf.fillNull(rs.getString("tpshr"));
		tpshsj=cf.fillNull(rs.getDate("tpshsj"));
		tpsm=cf.fillNull(rs.getString("tpsm"));
	}
	rs.close();
	ps.close();
...
<%
	...
	ls_sql="SELECT jdxmmc,DECODE(sfty,'Y','同意','N','不同意','M','不需确认'),qrsj,qrr,qrsm ";
	ls_sql+=" FROM crm_jdqrjl,dm_jdxmbm  ";
    ls_sql+=" where crm_jdqrjl.jdxmbm=dm_jdxmbm.jdxmbm";
    ls_sql+=" and crm_jdqrjl.jdjlh='"+jdjlh+"'";
    ls_sql+=" order by crm_jdqrjl.jdxmbm,crm_jdqrjl.qrsj";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","");
...
<table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
  <tr bgcolor="#FFFFFF">
    <td align="right"><span class="STYLE2">特批是否通过</span></td>
    <td><%=tpsftg%></td>
    <td align="right">&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
  <tr bgcolor="#FFFFFF">
    <td align="right"><span class="STYLE2">特批说明</span></td>
    <td colspan="3"><%=tpsm%></td>
  </tr>
  <tr bgcolor="#FFFFFF">
    <td align="right"><span class="STYLE2">特批审核人</span></td>
    <td><%=tpshr%></td>
    <td align="right"><span class="STYLE2">特批审核时间</span></td>
    <td><%=tpshsj%></td>
  </tr>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveEditQrSbCrm_xcjdjl.jsp";
	FormName.submit();
	return true;
}
