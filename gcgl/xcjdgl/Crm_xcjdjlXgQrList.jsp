...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT distinct crm_xcjdjl.jdjlh, DECODE(crm_xcjdjl.clzt,'1','申请','2','确认','3','部分确认','4','特批完成','5','发布','6','已交底'),DECODE(crm_xcjdjl.jdqrjg,'Y','通过','N','未通过'), DECODE(crm_xcjdjl.sfxtp,'Y','是','N','否'),crm_xcjdjl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs,dwmc,crm_xcjdjl.sqrq,crm_xcjdjl.sqr,crm_xcjdjl.sqrlxdh,crm_xcjdjl.jhjdrq,crm_xcjdjl.jhjdsj||'时',crm_xcjdjl.sqsm,crm_xcjdjl.zcwcsj,crm_xcjdjl.lrr,crm_xcjdjl.lrsj";
	...
	ls_sql+=" and crm_xcjdjl.clzt in('2','3')";//1：申请；2：确认；3：部分确认；4：特批完成；5：发布；6：已交底
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_xcjdjlXgQrList.jsp","","","QrCrm_xcjdjl.jsp");
