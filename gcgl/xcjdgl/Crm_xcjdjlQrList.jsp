...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT distinct crm_xcjdjl.jdjlh,DECODE(crm_xcjdjl.clzt,'1','申请','2','确认','3','部分确认','4','特批完成','5','发布','6','已交底'), DECODE(crm_xcjdjl.sfxtp,'Y','是','N','否'),DECODE(crm_xcjdjl.sfycp,'1','有产品','2','无'),crm_xcjdjl.cplx,crm_xcjdjl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs,dwmc,crm_xcjdjl.sqrq,crm_xcjdjl.sqr,crm_xcjdjl.sqrlxdh,crm_xcjdjl.jhjdrq,crm_xcjdjl.jhjdsj||'时',crm_xcjdjl.sqsm,crm_xcjdjl.zcwcsj,crm_xcjdjl.lrr,crm_xcjdjl.lrsj";
	ls_sql+=" FROM crm_khxx,crm_xcjdjl,sq_dwxx,crm_jdxmmx,dm_jdxmbm  ";
	...
	ls_sql+=" and crm_xcjdjl.sfycp='1'";//1：有产品、2：无产品
	ls_sql+=" and ((crm_xcjdjl.clzt in('1','3') and crm_xcjdjl.sfxtp='N')";//1：申请；2：确认；3：部分确认；4：特批完成；5：发布；6：已交底
	ls_sql+=" OR (crm_xcjdjl.clzt in('4','3') and crm_xcjdjl.sfxtp='Y' and crm_xcjdjl.tpsftg='Y'))";
	ls_sql+=" and crm_jdxmmx.clzt='1'";//1：未确认；2：确认同意；3：确认不同意
	ls_sql+=" and crm_xcjdjl.jdjlh=crm_jdxmmx.jdjlh and crm_jdxmmx.jdxmbm=dm_jdxmbm.jdxmbm";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by crm_xcjdjl.sqrq desc";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Crm_xcjdjlQrList.jsp","","","QrCrm_xcjdjl.jsp");
