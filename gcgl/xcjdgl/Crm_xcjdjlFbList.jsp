...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_xcjdjl.jdjlh,TO_CHAR(crm_xcjdjl.jhjdrq,'YYYY-MM-DD'),crm_xcjdjl.jhjdsj||'时', DECODE(crm_xcjdjl.clzt,'1','申请','2','确认','3','部分确认','4','特批完成','5','发布','6','已交底'),DECODE(crm_xcjdjl.jdqrjg,'Y','通过','N','未通过'),DECODE(crm_xcjdjl.sfxtp,'Y','是','N','否'), DECODE(crm_xcjdjl.tpsftg,'Y','通过','N','未通过'),DECODE(crm_xcjdjl.sfycp,'1','有产品','2','无'),crm_xcjdjl.cplx,crm_xcjdjl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs,dwmc,crm_xcjdjl.lrsj,TO_CHAR(crm_xcjdjl.sqrq,'YYYY-MM-DD'),crm_xcjdjl.sqr,crm_xcjdjl.sqrlxdh,crm_xcjdjl.sqsm,crm_xcjdjl.zcwcsj,crm_xcjdjl.lrr";
	ls_sql+=" FROM crm_khxx,crm_xcjdjl,sq_dwxx  ";
    ls_sql+=" where crm_xcjdjl.khbh=crm_khxx.khbh and crm_khxx.dwbh=sq_dwxx.dwbh";

	ls_sql+=" and (  "; 
    ls_sql+="      ( crm_xcjdjl.sfycp='2' and crm_xcjdjl.sfxtp='N' and crm_xcjdjl.clzt='1' )";//1：有产品、2：无产品
    ls_sql+="   OR ( crm_xcjdjl.sfycp='2' and crm_xcjdjl.sfxtp='Y' and crm_xcjdjl.clzt='4' )";//1：有产品、2：无产品
    ls_sql+="   OR ( crm_xcjdjl.sfycp='1' and crm_xcjdjl.clzt='2' and crm_xcjdjl.jdqrjg='Y' )";//;1：申请；2：确认；3：部分确认；4：特批完成；5：发布；6：已交底
    ls_sql+="   OR crm_xcjdjl.clzt='1'"; 
    ls_sql+=" ) "; 

//是否有产品  1：有产品、2：无产品
//是否需特批  Y：是；N：否
//1：申请；2：确认；3：部分确认；4：特批完成；5：发布；6：已交底；7：返回修改
...
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Crm_xcjdjlFbList.jsp","","","FbCrm_xcjdjl.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"plfb.jsp"};//按钮单击调用的网页，可以增加参数
