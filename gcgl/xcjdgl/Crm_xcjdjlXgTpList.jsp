...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_xcjdjl.jdjlh, DECODE(crm_xcjdjl.clzt,'1','申请','2','确认','3','部分确认','4','特批完成','5','发布','6','已交底'),crm_xcjdjl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs,dwmc,crm_xcjdjl.sqrq,crm_xcjdjl.sqr,crm_xcjdjl.sqrlxdh, DECODE(crm_xcjdjl.sfxtp,'Y','是','N','否'),crm_xcjdjl.jhjdrq,crm_xcjdjl.jhjdsj||'时',crm_xcjdjl.sqsm,crm_xcjdjl.zcwcsj,crm_xcjdjl.lrr,crm_xcjdjl.lrsj";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_xcjdjlXgTpList.jsp","","","XgTpCrm_xcjdjl.jsp");
...
//	pageObj.setEditStr("特批");

//设置按钮参数
	String[] buttonName={"删除特批"};//按钮的显示名称
	String[] buttonLink={"DeleteTpCrm_xcjdjl.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">交底申请－－修改特批审核</font></B>
</CENTER>
<%
	pageObj.out=out;
