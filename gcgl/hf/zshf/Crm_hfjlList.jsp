...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT distinct crm_hfjl.hfjlh as hfjlh,dm_hflxbm.hflxmc as dm_hflxbm_hflxmc,crm_hfjl.khbh as khbh,khxm,DECODE(sjshfnr,null,'',sjshfnr,'设：'||sjshfnr)||DECODE(zjyhfnr,null,'',zjyhfnr,'<BR>质：'||zjyhfnr)||DECODE(sgdhfnr,null,'',sgdhfnr,'<BR>施：'||sgdhfnr)||DECODE(jchfnr,null,'',jchfnr,'<BR>集成：'||jchfnr) as hfnr,crm_hfjl.hfsj,crm_hfjl.hfr,DECODE(clfs,'0','不需处理','1','反馈相关部门')  ";
	ls_sql+=" FROM crm_hfjl,dm_hflxbm,crm_khxx,crm_gchffkbm   ";
    ls_sql+=" where crm_hfjl.hflxbm=dm_hflxbm.hflxbm";
