...
try {
	...
	if (hfszjlh!=null && !hfszjlh.equals(""))//原来设置了回访信息
	{
		if (gethfszjlh.equals(hfszjlh))//由本次设置的客户回访信息，可以重新设置
		{
			if (!xchflxbm.equals(""))
			{
				...
				ps.setString(4,hfszjlh);
				ps.executeUpdate();
				ps.close();

				ls_sql=" update hf_hfszjl set hflxbm=?,hfsj=?";	
	...
	ls_sql="update hf_zsghfjl set hfjgbm=?,hfwtbm=?,wtclzt=?,hfr=?,hfsj=?,hfsm=?,lrr=?,lrsj=SYSDATE,lrbm=?,xchfrq=?,xchflxbm=?";
	ls_sql+=" where hfjlh='"+hfjlh+"'";
	ps= conn.prepareStatement(ls_sql);
