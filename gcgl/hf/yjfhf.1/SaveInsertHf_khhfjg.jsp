...
String hfsfcg=cf.GB2Uni(request.getParameter("hfsfcg"));
...
try {
	...
	rs.close();
	ps.close();
	if (clzt.equals("Y") || clzt.equals("X"))//S：预交付；Y：回访成功；X：回访失败；W：项目交付
	{
		out.println("错误！已回访");
		return;
	}
	...
	ls_sql="update crm_xmjfjl set hfsfcg=?,hfr=?,hfsj=?,hfsm=?,clzt=?";
	ls_sql+=" where jfjlh='"+jfjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,hfsfcg);
	ps.setString(2,hfr);
	ps.setDate(3,hfsj);
	ps.setString(4,hfsm);
	ps.setString(5,clzt);
