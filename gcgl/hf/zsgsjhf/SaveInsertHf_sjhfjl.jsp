...
try {
	...
	ls_sql="insert into hf_sjhfjl ( hfjlh,khbh,hflxbm,hfjgbm,hfwtbm,wtclzt,hfr,hfsj,hfsm,lrr,lrsj  ,lrbm,hfszjlh,xchfrq,xchflxbm,ysfxhf,yhfrq,yhflxbm) ";
	...
	if (zrbm!=null)
	{
		for (int i=0;i<zrbm.length ;i++ )
		{
			...
			ls_sql="insert into hf_sjhfwtbm (hfjlh,zrbm,khbh,clzt) ";
			ls_sql+=" values ( ?,?,?,? ) ";
			ps= conn.prepareStatement(ls_sql);
			ps.setString(1,hfjlh);
	...
	if (dxpjzbbm!=null)
	{
		for (int i=0;i<dxpjzbbm.length;i++)
		{
			...
			if (pfbm!=null && !pfbm.equals(""))
			{
				...
				ls_sql="insert into hf_sjhfjg (hfjlh,khbh,hflxbm,hfdxbm,hfdxmc,dxpjzbbm,dxpjzb,pfbm,pfmc,df,lrr,lrsj,xh) ";
	...
	if (hfdxbm!=null)
	{
		for (int i=0;i<hfdxbm.length;i++)
		{
			...
			{
				ls_sql="insert into hf_sjhfbmyyy (hfjlh,khbh,hflxbm,hfdxbm,bmyyy,lrr,lrsj) ";
				ls_sql+=" values( ?,?,?,?,?,?,SYSDATE)";
				ps= conn.prepareStatement(ls_sql);
				ps.setString(1,hfjlh);
				ps.setString(2,khbh);
	...
	while (rs.next())
	{
		...
		if (sfkdx.equals("1") || sfkdx.equals("2"))//1：多选；2：单选；3：写说明
		{
			...
			if (khdabh!=null)
			{
				...
				{
					ls_sql="insert into hf_sjwtda (hfjlh,khbh,hflxbm,wtbh,wtmc,sfkdx,yxbz,fbr,fbsj,dabh,damc,sfyxsm,dasm,lrr,lrsj)";
			...
			if (sftxsm.equals("Y"))//是否允许填写说明
			{
				...
				{
					ls_sql="insert into hf_sjwtbcsm (hfjlh,khbh,hflxbm,wtbh,wtmc,txsmts,bcsm,lrr,lrsj)";
			...
			if (sfyxsm.equals("Y"))//是否允许填写说明
			{
				...
				if (count>0)//被选择，要录入答案说明
				{
					if (khdasm==null || khdasm.equals(""))
					{
						...
						ls_sql="update hf_sjwtda set dasm=?";
						ls_sql+=" where  hfjlh='"+hfjlh+"' and wtbh="+wtbh+" and dabh="+dabh;
						ps2= conn.prepareStatement(ls_sql);
