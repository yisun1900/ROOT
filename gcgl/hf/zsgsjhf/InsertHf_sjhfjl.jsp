...
try {
	...
	//回访类型
	ls_sql=" select hflxbm from hf_zsghfjl";
	ls_sql+=" where khbh='"+khbh+"' order by hfjlh desc";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
...
<form method="post" action="SaveInsertHf_sjhfjl.jsp" name="insertform">
...
<%
//	ybl.common.PageObject pageObj=new ybl.common.PageObject();	
	ls_sql="SELECT hf_sjhfjl.hfjlh,hflxmc,hfjgmc,hfwtmc,DECODE(hf_sjhfjl.wtclzt,'1','未处理','2','在处理','3','已解决'),hf_sjhfjl.hfr,hf_sjhfjl.hfsj,hf_sjhfjl.hfsm ";
	ls_sql+=" FROM hf_sjhfjl,dm_hfjgbm,dm_hfwtbm,dm_hflxbm  ";
    ls_sql+=" where hf_sjhfjl.hfjgbm=dm_hfjgbm.hfjgbm(+)";
    ls_sql+=" and hf_sjhfjl.hfwtbm=dm_hfwtbm.hfwtbm(+)";
    ls_sql+=" and hf_sjhfjl.hflxbm=dm_hflxbm.hflxbm(+)";
    ls_sql+=" and hf_sjhfjl.khbh='"+khbh+"'";
    ls_sql+=" order by lrsj ";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","");
...
<table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td bgcolor="#E8E8FF"><A HREF="/khxx/Crm_gcfxysjlCxList.jsp?khbh=<%=khbh%>" target="_blank">
		...
		<input type="hidden" name="xchflxbm" value="" size="20" maxlength="10"  >
        <input type="hidden" name="xchfrq" value="" size="20" maxlength="10"  >
      </td>
...
%>
      ...
      <td width="17%" rowspan="5" align="right">问题部门</td>
      <td width="33%" rowspan="5">
        <select name="zrbm" style="FONT-SIZE:12PX;WIDTH:252PX" size="11" multiple>
          <option value=""></option>
          <%
