...
try {
	...
	if (rs.next())
	{
		yxmjfzt=cf.fillNull(rs.getString("xmjfzt"));
	}
	rs.close();
	ps.close();
	...
	ls_sql="insert into crm_wghfjl ( hfjlh,khbh,hfjgbm,hfwtbm,wtclzt,hfr,hfsj,hfsm,lrr,lrsj,lrbm,yxmjfzt) ";
	...
	for (int i=0;i<dxpjzbbm.length;i++)
	{
		...
		if (xmjfzt.equals("Y"))//N：未交付；S：预交付；Y：回访成功；X：回访失败；C：失败已处理；W：项目交付
		{
			if (pfbm==null || pfbm.equals(""))
			{
				conn.rollback();
				out.println("错误！回访成功，【"+hfdxmc+"】在【"+dxpjzb+"】未选择【评分情况】");
				return;
			}
		}
		if (pfbm!=null && !pfbm.equals(""))
