...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_xmjfjl.jfjlh,DECODE(crm_xmjfjl.clzt,'S','预交付','Y','回访成功','X','回访失败','W','项目交付'), DECODE(crm_xmjfjl.hfsfcg,'Y','成功','N','失败'),crm_xmjfjl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs,sgdmc,crm_khxx.gj,crm_khxx.dd,crm_xmjfjl.yjfr,crm_xmjfjl.yjfsj,crm_xmjfjl.yjfsm,crm_xmjfjl.hfr,crm_xmjfjl.hfsj,crm_xmjfjl.hfsm, crm_xmjfjl.lrr,crm_xmjfjl.lrsj,dwmc ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_xmjfjlJfList.jsp","","","JfCrm_xmjfjl.jsp");
...
//设置主键
	...
	pageObj.setEditStr("交付");

//设置按钮参数
	String[] buttonName={"批量交付"};//按钮的显示名称
	String[] buttonLink={"Crm_xmjfjlJfList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_xmjfjl set clzt='W',xmjfr='"+yhmc+"',xmjfsj=TRUNC(SYSDATE),xmjfsm='批量交付' where clzt='Y' and "+chooseitem;
