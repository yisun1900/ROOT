...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_sgdfkjlKkList.jsp","","","KkCw_sgdfkjl.jsp");
...
//设置主键
	...
	pageObj.setEditStr("扣款");
//设置按钮参数
	String[] buttonName={"批量扣款"};//按钮的显示名称
	String[] buttonLink={"Cw_sgdfkjlKkList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update cw_sgdfkjl set zt='3',skkr='"+yhmc+"',skksj=TRUNC(SYSDATE),skksm='批量扣款'  where zt='2' and "+chooseitem;
