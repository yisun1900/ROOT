...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveEditCrm_khgtjl.jsp" name="editform">
<div align="center">请修改信息（登记编号：<%=wheredjbh%>）</div>
...
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%"><span class="STYLE1">*</span>沟通人</td> 
  <td width="35%"> 
    <input type="text" name="gtr" size="20" maxlength="20"  value="<%=gtr%>" >  </td>
  <td align="right" width="15%"><span class="STYLE1">*</span>沟通时间</td> 
  <td width="35%"> 
    <input type="text" name="gtsj" size="20" maxlength="10"  value="<%=gtsj%>" onDblClick="JSCalendar(this)">  </td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%"><span class="STYLE1">*</span>情况说明</td> 
  <td colspan="3"> 
    <textarea name="qksm" cols="72" rows="6"><%=qksm%></textarea>  </td>
  </tr>
...
%>
  ...
  <td align="right" width="15%"><span class="STYLE1">*</span>是否需处理</td> 
  <td width="35%"><%
	cf.radioToken(out, "sfxcl","N+不需处理&Y+需处理",sfxcl);
%></td>
</tr>
    <tr> 
      <td colspan="4" >
...
{
	if(	javaTrim(FormName.gtr)=="") {
		alert("请输入[沟通人]！");
		FormName.gtr.focus();
		return false;
	}
	if(	javaTrim(FormName.gtsj)=="") {
		alert("请输入[沟通时间]！");
		FormName.gtsj.focus();
		return false;
	}
	if(!(isDatetime(FormName.gtsj, "沟通时间"))) {
		return false;
	}
