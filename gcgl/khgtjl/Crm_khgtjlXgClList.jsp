...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and crm_khgtjl.sfxcl='M'";
    ls_sql+=wheresql;
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khgtjlXgClList.jsp","","","EditClCrm_khgtjl.jsp");
...
//设置按钮参数
	String[] buttonName={"删除处理"};//按钮的显示名称
	String[] buttonLink={"Crm_khgtjlXgClList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_khgtjl set sfxcl='Y',clr=null,clsj=null,clsm=null where sfxcl='M' and "+chooseitem;
...
<CENTER >
  <B><font size="3">客户沟通记录－－修改处理</font></B>
</CENTER>
<%
	pageObj.out=out;
