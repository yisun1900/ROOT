...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and crm_khgtjl.sfxcl='Y'";
    ls_sql+=wheresql;
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khgtjlClList.jsp","","","EditClCrm_khgtjl.jsp");
...
//设置按钮参数
	String[] buttonName={"批量处理"};//按钮的显示名称
	String[] buttonLink={"Crm_khgtjlClList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_khgtjl set sfxcl='M',clr='"+yhmc+"',clsj=TRUNC(SYSDATE),clsm='批量处理' where sfxcl='Y' and "+chooseitem;
...
<CENTER >
  <B><font size="3">客户沟通记录－－处理</font></B>
</CENTER>
<%
	pageObj.out=out;
