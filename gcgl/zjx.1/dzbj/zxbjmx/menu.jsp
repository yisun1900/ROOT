...
%>
			insDoc(cdbh<%=xmdlbm%>, gLnk(2, "<%=xmxlmc%>", "Bj_jzbjbCxList.jsp?zjxxh=<%=zjxxh%>&jzbz=<%=jzbz%>&dqbm=<%=dqbm%>&khbh=<%=khbh%>&xmdlbm=<%=xmdlbm%>&xmxlbm=<%=xmxlbm%>"));
<%
		}
		rs1.close();
		ps1.close();
	...
	if (zdyCount>0)
	{
		...
		insDoc(zdyxm00, gLnk(2, "选择项目", "Bj_jzbjb1CxList.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&zdyxmshbz=<%=zdyxmshbz%>&bjjb=<%=bjjb%>"));
		<%
	}
...
/*
	...
	if (lx.equals("2"))//1：非套餐；2：套餐
	{
		...
		insDoc(zdyxm00, gLnk(2, "选择主材项目", "Bj_khzcxmxjCxList.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&bjjb=<%=bjjb%>"));
		<%
	}
	...
	{
		%>
		zdyxm01= insFld(foldersTree, gFld("主材套餐", ""));
		insDoc(zdyxm01, gLnk(2, "选择主材套餐", "zctcMain.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&bjjb=<%=bjjb%>&lx=1"));
		<%
	}
	...
	{
		%>
		zdyxm02= insFld(foldersTree, gFld("二次装修", ""));
		insDoc(zdyxm02, gLnk(2, "温暖套餐", "Bj_wntcmcCxList.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&lx=1"));
		insDoc(zdyxm02, gLnk(2, "静音计划", "Bj_wntcmcCxList.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&lx=2"));
		insDoc(zdyxm02, gLnk(2, "色彩计划", "cltcMain.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&lx=1"));
		insDoc(zdyxm02, gLnk(2, "健康计划", "Bj_zctcmcCxList.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&lx=2"));
		<%
	}
*/	
}
