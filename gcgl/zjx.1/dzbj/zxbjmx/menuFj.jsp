...
<script>
		...
		foldersTree = gFld("<B>选[<%=jgwzbm%>]报价项目</B>", '');
...
%>
			insDoc(cdbh<%=xmdlbm%>, gLnk(2, "<%=xmxlmc%>", "Bj_jzbjbFjCxList.jsp?jzbz=<%=jzbz%>&jgwzbm=<%=jgwzbm%>&dqbm=<%=dqbm%>&khbh=<%=khbh%>&xmdlbm=<%=xmdlbm%>&xmxlbm=<%=xmxlbm%>"));
<%
		}
		rs1.close();
		ps1.close();
	...
	if (zdyCount>0)
	{
		...
		insDoc(zdyxm00, gLnk(2, "选择项目", "Bj_jzbjb1FjCxList.jsp?khbh=<%=khbh%>&jgwzbm=<%=jgwzbm%>&zdyxmshbz=<%=zdyxmshbz%>&bjjb=<%=bjjb%>"));
		<%
	}
