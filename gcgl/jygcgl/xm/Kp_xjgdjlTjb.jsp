...
String kfgssq=(String)session.getAttribute("kfgssq");//1：按分公司授权；2：按店面授权；3：不授权
	...
	String wheresql="";
	String gdm_gdjcjl_lrsj=request.getParameter("gdm_gdjcjl_lrsj");
	String gdm_gdjcjl_lrsj2=request.getParameter("gdm_gdjcjl_lrsj2");
	...
	if (gdm_gdjcjl_lrsj2!=null)
	{
		if (!(gdm_gdjcjl_lrsj2.equals("")))	wheresql+=" and (gdm_gdjcjl.lrsj<=TO_DATE('"+gdm_gdjcjl_lrsj2+"','YYYY/MM/DD'))";
	}
...
<div align="center">施工队处罚统计表（录入时间：<%=gdm_gdjcjl_lrsj%>---<%=gdm_gdjcjl_lrsj2%>） </div>
...
try {
	...
	ls_sql+=" SELECT gdm_gdjcjl.sgd,sgdmc,kfzs,fkze";
	ls_sql+=" FROM gdm_gdjcjl,crm_khxx,sq_sgd  ";
    ls_sql+=" where gdm_gdjcjl.khbh=crm_khxx.khbh and gdm_gdjcjl.sgd=sq_sgd.sgd ";
    ls_sql+=wheresql;
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		if (sgdfk>0 || kfs>0)
		{
			...
			ls_sql+=wheresql;
			ls_sql+=" and gdm_gdjcjl.sgd='"+sgd+"' and gdm_gdjcjl.lrdw='FBJ12'";//FBJ12:工程部
			...
			ls_sql+=" and gdm_gdjcjl.sgd=sq_sgd.sgd ";
			ls_sql+=wheresql;
			ls_sql+=" and gdm_gdjcjl.sgd='"+sgd+"' and gdm_gdjcjl.lrdw='FBJ11'";//FBJ11:客户服务中心
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
			{
			...
			ls_sql+=" and gdm_gdjcjl.sgd=sq_sgd.sgd ";
			ls_sql+=wheresql;
			ls_sql+=" and gdm_gdjcjl.sgd='"+sgd+"' and gdm_gdjcjl.lrdw='FBJ16'";//FBJ16:财务
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
			{
			...
			ls_sql+=" and gdm_gdjcjl.sgd=sq_sgd.sgd ";
			ls_sql+=wheresql;
			ls_sql+=" and gdm_gdjcjl.sgd='"+sgd+"' and gdm_gdjcjl.lrdw not in('FBJ12','FBJ11','FBJ16')";//其它
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			if (rs1.next())
			{
		...
		%>  
		  <tr align="center"> 
			<td><%=row%></td>
			<td><A HREF="ViewCfxx.jsp?sgd=<%=sgd%>&lrsj=<%=gdm_gdjcjl_lrsj%>&lrsj2=<%=gdm_gdjcjl_lrsj2%>" target="_blank"><%=sgdmc%></A></td>
			<td><%=kfs%></td>
			<td><%=sgdfk%></td>
			<td><%=gcbkfs%></td>
			<td><%=gcbsgdfk%></td>
