...
java.sql.Date jhybrq=null;
java.sql.Date jhzqrq=null;
String lrdw=null;
String bz=null;
...
ls=request.getParameter("jhybrq");
try{
	if (!(ls.equals("")))  jhybrq=java.sql.Date.valueOf(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量jhybrq不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[计划隐蔽日期]类型转换发生意外:"+e);
	return;
}
ls=request.getParameter("jhzqrq");
try{
	if (!(ls.equals("")))  jhzqrq=java.sql.Date.valueOf(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量jhzqrq不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[计划中期日期]类型转换发生意外:"+e);
	return;
}
...
try {
	...
	if (!ygcjdbm.equals("1"))
	{
		out.println("<BR>！！！错误，工程进度不正确，不能进行【开工登记】");
		return;
	}
	...
	//工程进度登记记录
	ls_sql="insert into crm_gcjddjjl (djjlh,khbh,gcjdbm,djsj,zpsl,ygcjdbm,ysfxhf,yhfrq,yhflxbm,sfszhf  ,lrr,lrsj,lrdw,bz,jhybrq,jhzqrq) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?  ,?,?,?,?,?,?) ";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setString(14,bz);
	ps.setDate(15,jhybrq);
	ps.setDate(16,jhzqrq);
	ps.executeUpdate();
	ps.close();


	ls_sql="update crm_khxx set sjkgrq=?,gcjdbm=?,sgbz=?,jhybgcrq=?,jhzqsj=?";
	ls_sql+=" where  khbh='"+khbh+"' ";
	ps= conn.prepareStatement(ls_sql);
	ps.setDate(1,djsj);
