...
<form method="post" action="Crm_khxxCxList.jsp" name="selectform">
          <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
            ...
            <tr bgcolor="#FFFFFF"> 
              <td width="18%" align="right">监理姓名</td>
              <td width="32%"> 
	...
	{
		out.println("<option value=\"\"></option>");
		cf.selectItem(out,"select yhmc,yhmc||'（'||dh||'）' from sq_yhxx,sq_dwxx where sq_yhxx.dwbh=sq_dwxx.dwbh and zwbm='05' and sfzszg in('Y','N') order by yhmc","");
	}
	else
	{
		...
		{
			out.println("<option value=\"\"></option>");
			cf.selectItem(out,"select yhmc,yhmc||'（'||dh||'）' from sq_yhxx where zwbm='05' and sfzszg in('Y','N') and ssfgs='"+ssfgs+"' order by yhmc","");
		}
		else{
			cf.selectItem(out,"select yhmc,yhmc||'（'||dh||'）' from sq_yhxx where zwbm='05' and sfzszg in('Y','N') and ssfgs='"+ssfgs+"' order by yhmc",yhmc,false);
		}
	}
%> 
                </select>
...
%> 
            ...
            <tr bgcolor="#FFFFFF"> 
              <td width="18%" align="right">出开工证时间 从</td>
              <td width="32%"> 
            ...
            <tr bgcolor="#FFFFFF"> 
              <td width="18%" align="right">合同开工日期 从</td>
              <td width="32%"> 
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isDatetime(FormName.kgzsj2, "出开工证时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kgrq, "合同开工日期"))) {
		return false;
	}
	if(!(isDatetime(FormName.kgrq2, "合同开工日期"))) {
		return false;
	}
