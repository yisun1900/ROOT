...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		sjfw=sjfw.trim();
		if (!(sjfw.equals("")))	wheresql+="  and (crm_zxkhxx.jlfdjsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD'))";
	}
	sjfw=request.getParameter("sjfw2");
	if (sjfw!=null)
	{
		sjfw=sjfw.trim();
		if (!(sjfw.equals("")))	wheresql+="  and (crm_zxkhxx.jlfdjsj<=TO_DATE('"+sjfw+"  23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}
	...
	ls_sql="SELECT crm_zxkhxx.khbh as khbh,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,crm_zxkhxx.lxfs,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','飞单审核未通过','3','已签约','4','飞单','5','飞单审核未通过') as crm_zxkhxx_zxzt,DECODE(shbz,'N','未审核','Y','审核'),DECODE(sfzdzbj,'N','未做','Y','已做') sfzdzbj,dzbjze,crm_zxkhxx.zcbjje,bjjbmc,DECODE(lfdjbz,'N','未交','Y','已交','T','退订金') lfdjbz,lfdj,jlfdjsj,cxhdbm,DECODE(hddjbz,'N','未交','Y','已交','T','退订金') hddjbz,hddj,jhddjsj,DECODE(sjfbz,'N','未交','Y','已交','T','退设计费') sjfbz,sjf,jsjfsj,DECODE(crm_zxkhxx.lfbz,'N','未量房','Y','已量房'),DECODE(crm_zxkhxx.ctbz,'N','未出图','Y','已出图'),DECODE(crm_zxkhxx.jzbz,'1','家装','2','公装'),b.dwmc as zxdm,sjs,ywy,sbyymc,lrsj,a.dwmc zxdjbm,fwlxmc as crm_zxkhxx_fwlxbm";
	...
	ls_sql+=" and crm_zxkhxx.jlfdjsj  is not null";
	ls_sql+=wheresql;
	ls_sql+=" order by crm_zxkhxx.jlfdjsj";
//	out.println(ls_sql);

    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_zxkhxxTypeDjList.jsp","SelectTypeCrm_khxx.jsp","","");
	if (kkbbz.equals("Y"))//Y：可拷贝；N：不可拷贝
	{
...
<CENTER >
  <B><font size="3">交订金客户（未签单）</font></B>
</CENTER>
