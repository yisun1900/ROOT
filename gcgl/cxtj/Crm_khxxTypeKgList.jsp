...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		sjfw=sjfw.trim();
		if (!(sjfw.equals("")))	wheresql+="  and (crm_khxx.sjkgrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD'))";
	}
	sjfw=request.getParameter("sjfw2");
	if (sjfw!=null)
	{
		sjfw=sjfw.trim();
		if (!(sjfw.equals("")))	wheresql+="  and (crm_khxx.sjkgrq<=TO_DATE('"+sjfw+"  23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}
    ...
    ls_sql+=wheresql;
	ls_sql+=" order by crm_khxx.sjkgrq";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxTypeKgList.jsp","SelectTypeCrm_khxx.jsp","","");
	if (kkbbz.equals("Y"))//Y：可拷贝；N：不可拷贝
	{
