...
<%
	...
	try{
		...
		double maxFileSize = 100;//单文件最大大小，单位KB
		//校验文件类型和大小
		for (int i=0; i<su.getFiles().getCount();i++)
			{
				...
				if(maxFileSize<fileSize) System.out.println("单个上传相片的容量不能超过["+maxFileSize+"KB]");
				...
				if (!"".equals(AllowedExtensions)
					...
					System.out.println("您上传的文件[" + suFile.getFileName()
							+ "]的类型为系统禁止上传的文件类型，不能上传！");
				}
				fileCount++;
		  }
	        if (fileCount==0) System.out.println("请选择上传的文件");
	        //准备保存文件
