...
<%
	...
	try 
	{
		...
		String djjlh=cf.GB2Uni(up.getParameter("djjlh"));
		...
		//为照片名称加前缀
		loadname=djjlh.toUpperCase()+"-"+loadname;
		...
		if (rs.next())
		{
			...
			if (!oldysjlh.equals(djjlh))
			{
				out.println("错误！[照片名称]已被--登记记录号--["+oldysjlh+"]使用，不能使用重复名称");
				return;
			}
		}
		rs.close();
		...
		if (!oldysjlh.equals(djjlh))
		{
			...
			ls_sql="update crm_gcjddjjl set zpsl=?";
