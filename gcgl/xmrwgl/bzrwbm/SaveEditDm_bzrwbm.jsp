...
try{
	if (!(ls.equals("")))  sqtxts=Long.parseLong(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量[sqtxts]不存在");
	return;
}
catch (Exception e){
...
try{
	if (!(ls.equals("")))  shtxts=Long.parseLong(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量[shtxts]不存在");
	return;
}
catch (Exception e){
...
try {
	...
	ls_sql="delete from dm_bzrwysxm ";
	ls_sql+=" where rwbm='"+rwbm+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	if (gcysxmbm!=null)
	{
		for (int i=0;i<gcysxmbm.length ;i++ )
		{
			if (gcysxmbm[i].equals(""))
			{
				continue;
			}

			ls_sql="insert into dm_bzrwysxm(rwbm,gcysxmbm) values('"+rwbm+"','"+gcysxmbm[i]+"')";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
		}
	...
	ls_sql="update dm_bzrwbm set rwbm=?,rwmc=?,rwflbm=?,zdjkrw=?,bjlb=?,sqtxnr=?,sqtxts=?,shtxnr=?,shtxts=?,sfyxhxgq=?,sfyxfc=?,sfxys=?,sfwlcb=?,zyrwbm=?,rwxh=?,bz=?,xgytg=?,gytgts=?,sjcpxx=?,sjcpfl=?,ysxm=?,bjjbbm=?,gznr=? ";
	ls_sql+=" where  (rwbm='"+rwbm+"')  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,rwbm);
	ps.setString(2,rwmc);
	...
	ps.setString(21,ysxm);
	ps.setString(22,bjjbbm);
	ps.setString(23,gznr);
	ps.executeUpdate();
	ps.close();
