...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_fbdglXdList.jsp","","","XdCrm_fbdgl.jsp");
...
//设置主键
	...
	pageObj.setEditStr("下单 ");

//设置按钮参数
	String[] buttonName={"批量下单"};//按钮的显示名称
	String[] buttonLink={"Crm_fbdglXdList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_fbdgl set xdr='"+yhmc+"',xdsj=TRUNC(SYSDATE),zt='3' where zt='2' and "+chooseitem;
