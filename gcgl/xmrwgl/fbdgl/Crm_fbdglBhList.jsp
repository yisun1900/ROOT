...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_fbdglBhList.jsp","","","BhCrm_fbdgl.jsp");
...
//设置按钮参数
	String[] buttonName={"批量备货"};//按钮的显示名称
	String[] buttonLink={"Crm_fbdglBhList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_fbdgl set bhlrr='"+yhmc+"',bhwcrq=TRUNC(SYSDATE),zt='4' where zt='3' and "+chooseitem;
...
<CENTER >
  <B><font size="3">返补维修单－－备货完成</font></B>
</CENTER>
<%
	pageObj.out=out;
