...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_xmrwgl.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs ,crm_xmrwgl.rwxh,crm_xmrwgl.rwbm,crm_xmrwgl.rwmc,rwflmc, DECODE(crm_xmrwgl.zdjkrw,'1','重点','2','非重点'), DECODE(crm_xmrwgl.bjlb,'1','事前报警','2','事后提醒','3','事前事后都提醒','9','一般记录'),crm_xmrwgl.sqtxnr,crm_xmrwgl.sqtxts,crm_xmrwgl.shtxnr,crm_xmrwgl.shtxts, DECODE(crm_xmrwgl.sfyxhxgq,'Y','是','N','否'), DECODE(crm_xmrwgl.sfyxfc,'Y','是','N','否'), DECODE(crm_xmrwgl.sfxys,'Y','是','N','否'), DECODE(crm_xmrwgl.sfwlcb,'Y','是','N','否'),crm_xmrwgl.zyrwbm,crm_xmrwgl.jdbz,crm_xmrwgl.zrr,crm_xmrwgl.jddf,crm_xmrwgl.jhkssj,crm_xmrwgl.jhjssj,crm_xmrwgl.jhgzr,crm_xmrwgl.jdsyts,crm_xmrwgl.sjkssj,crm_xmrwgl.sjjssj,crm_xmrwgl.sjgzr,DECODE(crm_xmrwgl.sfxtx,'Y','需提醒','M','已查看','N','否'),bjjbmc,crm_xmrwgl.txrq,crm_xmrwgl.txxs, DECODE(crm_xmrwgl.sfyq,'Y','是','N','否'),crm_xmrwgl.yqts,crm_xmrwgl.yqzrf, DECODE(crm_xmrwgl.sfybgd,'Y','有','N','无'),crm_xmrwgl.zgfa,crm_xmrwgl.yjzgwcsj, DECODE(crm_xmrwgl.zgsfwc,'Y','完成','N','未完成'),crm_xmrwgl.zgwcsj,crm_xmrwgl.sjzgqk, DECODE(crm_xmrwgl.sfyfbd,'Y','有','N','无'), DECODE(crm_xmrwgl.fbsfwc,'Y','完成','N','否'),crm_xmrwgl.lrr,crm_xmrwgl.lrsj,crm_xmrwgl.bz ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_xmrwglCxList.jsp","","","");
