...
<center>
    <span class="STYLE2">项目任务逻辑关系</span>  </center>
  ...
  <table border="0" cellspacing="0" cellpadding="1" width="200%" style='FONT-SIZE: 12px' >
    <tr align="center" bgcolor="#FFFFCC"> 
		...
		<td width="4%">计划工作日</td>
		<td width="5%">实际开始时间</td>
		<td width="5%">实际结束时间</td>
		<td width="4%">实际工作日</td>
...
<%
	...
	while (rs.next())
	{
		...
		ls_sql+=" where  rwbm in(select rwbm from crm_xmrwgl where jishu="+jishu+")";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		{
		...
		ls_sql="select count(*)";
		ls_sql+=" from  crm_xmrwljgx";
		ls_sql+=" where  qzrwbm in(select rwbm from crm_xmrwgl where jishu="+jishu+")";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		{
	...
	ls_sql="update crm_xmrwljgx set rwjs=(select jishu from crm_xmrwgl where crm_xmrwgl.rwbm=crm_xmrwljgx.rwbm)";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();

	ls_sql="update crm_xmrwljgx set qzrwjs=(select jishu from crm_xmrwgl where crm_xmrwgl.rwbm=crm_xmrwljgx.qzrwbm)";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
...
//				leftStr+="号";
			...
			for (int i=2;i<jishu ;i++ )
			{
				...
				ls_sql="select count(*)";
				ls_sql+=" from  crm_xmrwljgx";
				ls_sql+=" where qzrwbm in(select rwbm from crm_xmrwgl where rwxh < "+rwxh+")";
				ls_sql+="   and rwbm   in(select rwbm from crm_xmrwgl where rwxh > "+rwxh+")";
				ls_sql+="   and rwjs="+i;
				ps1= conn.prepareStatement(ls_sql);
				rs1 =ps1.executeQuery();
				if (rs1.next())
