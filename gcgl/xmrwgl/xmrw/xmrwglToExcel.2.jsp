...
<%   
    String filename = new String(("项目进度表").getBytes("GBK"),"ISO-8859-1");    
...
try {
	...
	ls_sql="select fgsbh,dwbh,lxfs,bjjb,jjrksg,DECODE(jjrksg,'1','节、假日不可施工','2','节日不可施工','9','节、假日可施工') jjrksgmc,fwdz,gj,dd,sgdmc,sgbz,khxm,kgrq,jgrq,sjs";
  ...
  <div>
   <table width="100%" cellpadding="0" cellspacing="0" style='FONT-SIZE: 12px'>
	<tr>
	 ...
	 <table  border="1" width="<%=(15+145+15+60+60+25+rqzl*(width+1))%>px" cellpadding="0" cellspacing="0" style='FONT-SIZE: 12px'>
	 <tr>
	   <td width="22%" rowspan="2" valign="middle"  colspan="5">
	   <image src="/images/<%=logo%>" height='30px'> <span class="STYLE1">整体项目进度表</span></td>
	 
	     <td width="13%" colspan="10">姓名:<%=khxm%></td>
		 <td colspan="2"  colspan="16">实际工期:<%=kgrq%>至<%=jgrq%></td>
		 <td width="12%" colspan="11">家装设计师:<%=sjs%><%=sjsdh%></td>
		 <td width="12%" colspan="11">橱/内设计师:</td>
		 <td width="12%" colspan="11">卫浴设计师:</td>
		 <td width="12%" colspan="11">家具设计师:</td>
	    </tr>
	   <tr >
	     <td colspan="2"  colspan="16">地址:<%=fwdz%></td>
		 <td width="11%"  colspan="10"><%=jjrksgmc%></td>
		 <td colspan="11">项目管家:<%=gj%><%=gjdh%></td>
		 <td colspan="11">大工长姓名:<%=sgd%><%=sgddh%></td>
		 <td colspan="11">现场责任人:<%=sgbz%><%=sgbzdh%></td>
		 <td colspan="11">家装履约专员:<%=dd%><%=dddh%></td>
	    </tr>
	 </table>
   </td></tr>
  </div>
<tr><td width="100%">
  <table border="1" width="<%=(15+145+15+60+60+25+rqzl*(width+1))%>px" cellspacing="0" cellpadding="0"  style='FONT-SIZE:8px' >
	...
	while (rs.next())
	{
		...
		<td align="left" valign="middle" colspan="<%=rqzl%>">
			...
			if (jhgzr==0)
			{
				...
				%>
				<image src="/images/bar1.gif" height='5px' width='<%=(start*(width+2))%>px'><image src="/images/bar2.gif" height='5px' width='<%=(jhgzr*(width+2))%>px'>
				<%
			}
			%>
		</td>
