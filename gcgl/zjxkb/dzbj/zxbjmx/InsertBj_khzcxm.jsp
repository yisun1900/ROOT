...
<tr bgcolor="#FFFFFF">
  ...
  <td><select name="ppmc" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changePp(insertform)">
    ...
    <%
	String sql="";
	sql="select distinct  ppmc c1,ppmc c2 from bj_tczcbj where tccplbbm='"+tccplbbm+"' and  bjjbbm='"+bjjbbm+"'  and cpbm not in(select cpbm from bj_khzcxm where khbh='"+khbh+"' and jgwzbm='"+jgwzbm+"') order by ppmc";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where ppmc='"+FormName.ppmc.value+"' and  tccplbbm='"+FormName.tccplbbm.value+"' and  bjjbbm='"+FormName.bjjbbm.value+"'  and cpbm not in(select cpbm from bj_khzcxm where khbh='<%=khbh%>' and jgwzbm='"+FormName.jgwzbm.value+"') ";
