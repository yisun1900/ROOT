...
<%
	...
	ls_sql="SELECT bj_fjxxq.jgwzbm,DECODE(xuhao,'1','一','2','二','3','三','4','四','5','五','6','六','7','七','8','八','9','九','10','十','11','十一','12','十二') xuhao";
	...
	while (rs1.next())
	{
		...
		ls_sql+=" where bj_bjxmmxq.zjxxh='"+zjxxh+"' and bj_bjxmmxq.khbh='"+khbh+"' and bj_gclmxq.jgwzbm='"+jgwzmc+"'";
	...
	//	out.println(ls_sql);
		...
		//输出项目明细
		ls_sql="SELECT bj_bjxmmxq.xh xh,trim(bj_bjxmmxq.xmbh)||DECODE(bjlx,'1','','2','增') xmbh,xmmc,jldwmc,bj_bjxmmxq.dj,bj_gclmxq.sl,bj_gclmxq.sl*bj_bjxmmxq.dj as je,gycl,bj_bjxmmxq.flmcgg,bj_bjxmmxq.bz,bj_bjxmmxq.zcf,bj_bjxmmxq.cldj,bj_bjxmmxq.shf,bj_bjxmmxq.qtf,bj_bjxmmxq.ysf,bj_bjxmmxq.rgdj,bj_bjxmmxq.jxdj";
		ls_sql+=" FROM bj_bjxmmxq,jdm_jldwbm,bj_gclmxq,bdm_xmdlbm  ";
		ls_sql+=" where bj_bjxmmxq.jldwbm=jdm_jldwbm.jldwbm ";
		ls_sql+=" and bj_bjxmmxq.zjxxh='"+zjxxh+"' and bj_bjxmmxq.khbh='"+khbh+"' and bj_gclmxq.jgwzbm='"+jgwzmc+"'";
		ls_sql+=" and bj_bjxmmxq.xmdlbm=bdm_xmdlbm.xmdlbm and bdm_xmdlbm.sftsxm in('1','2')";
		ls_sql+=" and bj_bjxmmxq.zjxxh=bj_gclmxq.zjxxh and bj_bjxmmxq.dqbm=bj_gclmxq.dqbm and bj_bjxmmxq.xmbh=bj_gclmxq.xmbh";
...
%> 
 ...
 <tr bgcolor="#FFFFFF" align="center"  height="25"> 
  ...
  <%  
    ...
    ls_sql+=" where zjxxh='"+zjxxh+"' and bj_sfxmmxq.khbh='"+khbh+"' and  sflx=5";
...
*/
  ...
  <%  
	...
	ls_sql="SELECT sfxmbm,sfxmmc,DECODE(sflx,'1','工程直接费百分比（不可改）','A','工程直接费百分比（可改）','2','固定金额(可改)','3','税金（折后按基数计算）','8','税金（折后按折扣计算）','4','固定金额(不可改)','5','管理费','6','公司购主材百分比','7','自购主材百分比','9','按百分比收设计费') sflx,sfbfb,sfje,bz,DECODE(sfbxx,'1','必选','2','非必选') sfbxx,DECODE(sfjrqye,'1','记入','2','不记入') sfjrqye,DECODE(jrsjbz,'1','记入','2','不记入') jrsjbz,DECODE(jrglfbz,'1','记入','2','不记入') jrglfbz,zqsfje,dxzkl,DECODE(sfkdz,'N','不可打折','Y','可打折') sfkdz,zdzk";
	ls_sql+=" FROM bj_sfxmmxq  ";
    ls_sql+=" where zjxxh='"+zjxxh+"' and bj_sfxmmxq.khbh='"+khbh+"' and  sflx!=5 and  sflx!=3";
    ls_sql+=" order by bj_sfxmmxq.sfxmbm";
//	out.println(ls_sql);
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
    ...
    ls_sql="select count(*)";
	ls_sql+=" from  bj_sfxmmxq";
	ls_sql+=" where zjxxh='"+zjxxh+"' and khbh='"+khbh+"'and sflx=3 ";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	if (rs.next())
	{
	...
	ls_sql="SELECT sfxmbm,sfxmmc,DECODE(sflx,'1','工程直接费百分比（不可改）','A','工程直接费百分比（可改）','2','固定金额(可改)','3','税金（折后按基数计算）','8','税金（折后按折扣计算）','4','固定金额(不可改)','5','管理费','6','公司购主材百分比','7','自购主材百分比','9','按百分比收设计费') sflx,sfbfb,sfje,bz,DECODE(sfbxx,'1','必选','2','非必选') sfbxx,DECODE(sfjrqye,'1','记入','2','不记入') sfjrqye,DECODE(jrsjbz,'1','记入','2','不记入') jrsjbz,DECODE(jrglfbz,'1','记入','2','不记入') jrglfbz,zqsfje,dxzkl,DECODE(sfkdz,'N','不可打折','Y','可打折') sfkdz,zdzk";
	ls_sql+=" FROM bj_sfxmmxq  ";
    ls_sql+=" where zjxxh='"+zjxxh+"' and bj_sfxmmxq.khbh='"+khbh+"' and  sflx=3";
    ls_sql+=" order by bj_sfxmmxq.sfxmbm";
//	out.println(ls_sql);
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
...
<table border="1" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
  <tr bgcolor="#FFFFFF"> 
  <%=dzbjbz%>
  <td colspan="1" height="42">注意事项</td>
  <td colspan="6" height="42">
  <table width="950"  style='FONT-SIZE: 12px'>
  <tr><td width="948">1、实际发生工程量若与报价单不符，一切以实际发生为准。</td>
  </tr>
   <tr><td>2、以上报价不含物业收取装修公司的装修管理费及垃圾费，此项费用客户自理。</td><tr>
   <tr><td>3、物业收取装修公司的装修押金（可退）客户自理，如因我公司施工人员违反物业规定，造成罚款，由我公司承担。</td><tr>
    <tr>
      <td>4、施工期间水费电费由客户承担。</td>
    <tr>
	<tr>
      <td>5、以上报价不含五金件、瓷砖、洁具、木地板、灯具、门、橱具。</td>
    <tr>
	<tr>
      <td>6、我公司鉴于客户安全考虑，不负责：暖气拆改，煤气移位，承担墙拆除。</td>
    <tr>
  </table>
  </td>
  </tr>
