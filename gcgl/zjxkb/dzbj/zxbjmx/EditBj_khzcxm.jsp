...
<tr bgcolor="#FFFFFF">
  ...
  <select name="cpbm" style="FONT-SIZE:12PX;WIDTH:512PX">
    <option value=""></option>
    <%
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where bjbbh='"+bjbbh+"' and  ppmc='"+ppmc+"' and  tccplbbm='"+tccplbbm+"' and  bjjbbm='"+bjjbbm+"'";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where bjbbh='<%=bjbbh%>' and ppmc='"+FormName.ppmc.value+"' and  tccplbbm='"+FormName.tccplbbm.value+"' and  bjjbbm='"+FormName.bjjbbm.value+"'";
