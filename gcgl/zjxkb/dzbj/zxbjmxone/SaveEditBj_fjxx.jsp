...
try {
	...
	//清空序号
	ls_sql="update bj_fjxxh set xuhao=-1 where zjxxh='"+zjxxh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	for (int i=0;i<oldjgwzbm.length ; i++)
	{
		...
		ls_sql+=" where zjxxh='"+zjxxh+"' and jgwzbm!='"+cf.GB2Uni(oldjgwzbm[i])+"' and jgwzbm='"+cf.GB2Uni(jgwzbm[i])+"'";
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		{
		...
		ls_sql="update bj_fjxxh set jgwzbm='"+cf.GB2Uni(jgwzbm[i])+"',fjmj="+fjmj[i]+",fjg="+fjg[i]+",zdyxmmj="+zdyxmmj[i]+",zdybzmj="+zdybzmj[i]+",xuhao="+xuhao[i]+" where zjxxh='"+zjxxh+"' and jgwzbm='"+cf.GB2Uni(oldjgwzbm[i])+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();

		ls_sql="update bj_gclmxh set jgwzbm='"+cf.GB2Uni(jgwzbm[i])+"' where zjxxh='"+zjxxh+"' and jgwzbm='"+cf.GB2Uni(oldjgwzbm[i])+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
	...
	while (rs.next())
	{
		...
		ls_sql="update bj_fjxxh set xuhao="+pxxh+" where zjxxh='"+zjxxh+"' and jgwzbm='"+lsjgwzbm+"'";
