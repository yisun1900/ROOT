...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='050211'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[重做电子报价y]："+xtrzyhmc+"') ";
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_khxx.khbh,khxm,fwdz,DECODE(crm_czbjjl.sfyzczbj,'1','授权重做报价','2','签约额已更新'),pzczbjsj,pzczbjr,ysgcjdmc,wdzgce,zkl,qye,suijin,sjs,zjxm,sgdmc sgd,qyrq,a.dwmc dm, DECODE(hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他') as hdbz  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxCzList.jsp","SelectCzCrm_khxx.jsp","","EditSjsZxCrm_zxkhxx.jsp");
