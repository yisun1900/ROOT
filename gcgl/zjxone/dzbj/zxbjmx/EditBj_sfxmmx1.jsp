...
try {
	...
	ls_sql="select khbh,sfxmbm,DECODE(bj_sfxmmxh.sflx,'1','工程直接费百分比（不可改）','A','工程直接费百分比（可改）','2','固定金额(可改)','3','税金','4','固定金额(不可改)','5','管理费','6','主材费百分比','7','工程加主材费百分比') sflx,dj,jldw,sl,zqsfje,dxzkl,sfje,bz ";
	...
	if (sflx.equals("工程直接费百分比"))
	{
		djmark="readonly";
		slmark="readonly";
	}
	else if (sflx.equals("固定金额(可改)"))
	{
		djmark="";
		slmark="";
	}
	else if (sflx.equals("税金"))
	{
		djmark="readonly";
		slmark="readonly";
	}
	...
	else if (sflx.equals("管理费"))
	{
		djmark="readonly";
		slmark="readonly";
	}
	else if (sflx.equals("设计费"))
	{
		djmark="";
		slmark="";
	}
...
<form method="post" action="SaveEditBj_sfxmmx.jsp" name="insertform">
          <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
            <tr bgcolor="#FFFFFF"> 
              <td width="15%" align="right"><font color="#FF0000">*</font>客户编号</td>
              <td width="35%"> 
            ...
            <tr bgcolor="#FFFFFF"> 
              ...
              <td> 
                <select name="sfxmbm" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="getSfxm(insertform)">
                  <%
	cf.selectItem(out,"select sfxmbm,sfxmmc||DECODE(sfbxx,'1','（必选）','2','') from bdm_sfxmbm where dqbm='"+dqbm+"' order by sfxmbm",sfxmbm);
%> 
                </select>              </td>
              <td align="right"><font color="#FF0000">*</font><font color="#0000CC">收费计算方式</font></td>
              <td> 
                <input name="sflx" type="text" id="sflx" value="<%=sflx%>" size="20" maxlength="17" readonly>              </td>
            </tr>
            <tr bgcolor="#FFFFFF">
            ...
            <tr bgcolor="#FFFFFF"> 
              ...
              <td><input type="text" name="zqsfje" size="20" maxlength="17"  value="<%=zqsfje%>" readonly>
              <strong><font color="#0000FF">不可修改</font></strong></td>
            </tr>
            <tr bgcolor="#FFFFFF"> 
