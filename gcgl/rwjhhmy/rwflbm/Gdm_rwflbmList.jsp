...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	rwdl=request.getParameter("rwdl");
	if (rwdl!=null)
	{
		rwdl=cf.GB2Uni(rwdl);
		if (!(rwdl.equals("")))	wheresql+=" and  (rwdl='"+rwdl+"')";
	}
	ls_sql="SELECT rwflbm,rwflmc, DECODE(rwdl,'1','一般','2','交底','3','进场','4','验收','5','复尺','6','下单','7','入库','8','送货','9','安装') dlmc ";
	ls_sql+=" FROM gdm_rwflbm  ";
    ls_sql+=" where (1=1)";
    ls_sql+=wheresql;
    ls_sql+=" order by rwdl,rwflbm";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Gdm_rwflbmList.jsp","","","EditGdm_rwflbm.jsp");
//	pageObj.setPageRow(10);//设置每页显示记录数
/*
//设置显示列
	String[] disColName={"rwflbm","rwflmc","rwdl"};
...
//设置按钮参数
	...
	String[] buttonLink={"Gdm_rwflbmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from gdm_rwflbm where "+chooseitem;
