...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String sfyfdx=null;
	sfyfdx=request.getParameter("sfyfdx");

	if  (sfyfdx.equals("3"))
	{
	}
	else{
		wheresql+=" and  (crm_khxx.sfyfdx='"+sfyfdx+"')";
	}
	...
	ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,DECODE(sfyfdx,'1','未发','2','已发'),fdxr,fdxsj,crm_khxx.khxm,DECODE(kgzbz,'Y','已出','N','<font color=\"#FF00FF\">未出</font>'),DECODE(zt,'2','家装','3','<font color=\"#FF00FF\">已退单</font>','4','非家装','5','设计客户'),ysgcjdmc,DECODE(crm_khxx.jzbz,'1','家装','2','公装'),crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,crm_khxx.cxhdbm,crm_khxx.lxfs,DECODE(NVL(tgbz,'W'),'W','正常','N','<font color=\"#00FF00\">复工</font>','Y','<font color=\"#FF0000\">已停工</font>'),c.gcyszxmmc ybysxmbm,d.gcyszxmmc zqysxmbm,DECODE(crm_khxx.gcsfyq,'1','未延期','2','延期'),crm_khxx.yqts,zkl,ywy,a.dwmc qydm,crm_khxx.qyrq,crm_khxx.kgrq,jgrq,DECODE(crm_khxx.sffj,'Y','是','N','否'),crm_khxx.fjje,crm_khxx.gsfje,DECODE(crm_khxx.sfyrz,'Y','是','N','否'),b.dwmc rzsc,khlxmc";
...
//设置按钮参数
	String[] buttonName={"导出客户资料"};//按钮的显示名称
	String[] buttonLink={"ExpKhxx.jsp"};//按钮单击调用的网页，可以增加参数
