...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (pdclzt!=null)
	{
		...
		if (!(pdclzt.equals("")))
		{
			wheresql+=" and  (crm_khxx.pdclzt='"+pdclzt+"')";
		}
	}
	...
	ls_sql="SELECT app_ewmscmx.khbh,crm_khxx.hth,crm_khxx.khxm, crm_khxx.fwdz,DECODE(kgsjqd,'Y','已确定','N','<font color=\"#FF00FF\">未确定</font>'),pdsj,qyrq,dwmc,sjs";
	ls_sql+=" FROM app_ewmscmx,crm_khxx,sq_dwxx  ";
//    ls_sql+=" where app_ewmscmx.khbh=crm_khxx.khbh(+) and app_ewmscmx.ewmkkbz='Y' and  crm_khxx.dwbh=sq_dwxx.dwbh(+)   ";

	ls_sql+=" where app_ewmscmx.khbh=crm_khxx.khbh(+)  and  crm_khxx.dwbh=sq_dwxx.dwbh(+)   ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxList.jsp","","","ViewEwm.jsp");
...
//设置主键
	...
	pageObj.setEditStr("下载");
//设置按钮参数
	String[] buttonName={"下载选中二维码"};//按钮的显示名称
	String[] buttonLink={"DownloadEwm.jsp"};//按钮单击调用的网页，可以增加参数  DownloadEwm
