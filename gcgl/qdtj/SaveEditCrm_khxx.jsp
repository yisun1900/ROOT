...
try {
	...
	{
		out.println("错误！该客户已经分解套餐主材，不能再修改");
		return;
	}

	if (!zjxxh.equals(""))
	{
		out.println("错误！已有增减项，不能再修改");
		return;
	}
	...
	if(rs.next())
	{
		zchtje=rs.getDouble("zchtje");//主材销售额
		zchtcb=rs.getInt("zchtcb");//主材成本额
	}
	rs.close();
	ps.close();
	...
	ls_sql=" update crm_khqye set hdzsjz=?,djjje=?,cbj=?,sgcbj=?,gcjcbj=?,htjz=?,mll=?,mle=?,yjjzcs=?,yjjz=?,djjbfb=?,rzfbfb=?,djfxje=?      ,zchtje=?,zchtcb=?,zczxje=?,zczxcb=?,khhtml=?,khzxml=?";
	ls_sql+=" where khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,hdzsjz);
	ps.setDouble(2,djjje);
	ps.setDouble(3,cbj);
	ps.setDouble(4,sgcbj);
	ps.setDouble(5,gcjcbj);
	ps.setDouble(6,htjz);
	ps.setDouble(7,mll);
	ps.setDouble(8,mle);
	ps.setDouble(9,yjjzcs);
	ps.setDouble(10,yjjz);
	ps.setDouble(11,djjbfb);
	ps.setDouble(12,rzfbfb);
	ps.setDouble(13,djfxje);

	ps.setDouble(14,zchtje);
	ps.setDouble(15,zchtcb);
	ps.setDouble(16,zchtje);
	ps.setDouble(17,zchtcb);
	ps.setDouble(18,khhtml);
	ps.setDouble(19,khhtml);
