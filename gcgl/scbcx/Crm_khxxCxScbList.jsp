...
<%
if (cf.check((String)session.getAttribute("yhdlm"),request.getRemoteHost(),"050132")==0) 
{
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_khxx.khbh,crm_khxx.khxm,crm_khxx.hth,ysgcjdmc,crm_khxx.fwdz,ywy,b.dwmc ywybm,c.dwmc ywyssxz,crm_khxx.khjl,crm_khxx.sjs,sgdmc||DECODE(sq_sgd.sgdlx,'S0','','S1','(直属)') sgd,sgbz,crm_khxx.zjxm,DECODE(kgzbz,'Y','已出','N','<font color=\"#FF00FF\">未出</font>'),DECODE(zt,'2','家装','4','集成','5','设计','3','<font color=\"#FF00FF\">已退单</font>'),DECODE(crm_khxx.jzbz,'1','家装','2','公装'),crm_khxx.cxhdbm,DECODE(NVL(tgbz,'W'),'W','正常','N','<font color=\"#00FF00\">复工</font>','Y','<font color=\"#FF0000\">已停工</font>'),DECODE(crm_khxx.gcsfyq,'1','未延期','2','延期'),crm_khxx.yqts,zkl,bjjbmc,a.dwmc qydm,crm_khxx.qyrq,crm_khxx.pdsj,crm_khxx.kgrq,crm_khxx.sjkgrq,crm_khxx.jgrq,sjkgrq+((jgrq-kgrq)+NVL(hzts,0)) as jhwgsj,crm_khxx.sjjgrq,crm_khxx.wdzgce,crm_khxx.qye,NVL(crm_tckhqye.tcsgze,0)+NVL(crm_tckhqye.tcsgzezjx,0),crm_khxx.zqgczjf,crm_khxx.zhgczjf,crm_khxx.glfbfb||'%',crm_khxx.zqguanlif,crm_khxx.guanlif,crm_khxx.suijinbfb||'%' suijinbfb,crm_khxx.zqsuijin,crm_khxx.suijin,crm_khxx.zqsjf,crm_khxx.sjf,crm_khxx.zjxje,crm_khxx.zhzjxje,crm_khxx.zqzjxguanlif,crm_khxx.zjxguanlif,crm_khxx.zqzjxsuijin,crm_khxx.zjxsuijin,crm_khxx.zjxje+crm_khxx.zqzjxguanlif+crm_khxx.zqzjxsuijin as zqzjxzj,crm_khxx.zhzjxje+crm_khxx.zjxguanlif+crm_khxx.zjxsuijin as zhzjxzj,crm_khxx.sgebfb||'%',crm_khxx.sge,crm_khxx.clf,DECODE(crm_khxx.sffj,'Y','是','N','否'),crm_khxx.fjje,crm_khxx.gsfje,DECODE(crm_khxx.sfyrz,'Y','是','N','否'),khlxmc,decode(crm_khxx.xjf,1,'新房',2,'旧房',crm_khxx.xjf),crm_khxx.fwmj,decode(crm_khxx.hxbm,'01','loft','02','1+1','03','2+1','04','2+2','05','3+1','06','3+2','07','4+1','08','4+2','09','4+3','11','复式','12','别墅','13','工装','14','局部改造','15','0+1','16','换装儿童房','17','换装厨房','18','换装卫生间',crm_khxx.hxbm),clgw";

	ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_dwxx b,sq_dwxx c,sq_sgd,dm_gcjdbm,dm_gcysxm,dm_khlxbm,crm_tckhqye,bdm_bjjbbm";
    ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) ";
    ls_sql+=" and crm_khxx.ywybm=b.dwbh(+) ";
    ls_sql+=" and crm_khxx.ywyssxz=c.dwbh(+) ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxCxScbList.jsp","","","");
	if (kkbbz.equals("Y"))//Y：可拷贝；N：不可拷贝
	{
