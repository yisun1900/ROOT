...
try {
	...
	//[合同号]检查
	ret=qddj.checkHth(conn,khbh,hth,qyrq,dwbh,"jrzj");
	if (ret!=null && !ret.equals(""))
	{
		out.println(ret);
		return;
	...
	ls_sql="update crm_zxkhxx set zxzt='3',khxm=?,xb=?,fwdz=?,email=?,cqbm=?,xqbm=?,louhao=?,hth=?,qdsj=?";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khxm);
	ps.setString(2,xb);
	...
	ls_sql+=" ,cbj,zxcbj,sgcbj,zxsgcbj,gcjcbj,zxgcjcbj,htjz,zxhtjz,mll,zxmll   ,mle,zxmle,yjjzcs,yjjz,zxyjjz,djjbfb ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?  ,?,?,?,?,?,?,?,?,?,?    ,?,?,?,?,?,?) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
	...
	ls_sql+=" ,sgbz,fjtcbz,sfyckh,ywybm,pdqybm,clgw,xjf,zqhtzcbj,zhhtzcbj ";
	...
	ls_sql+=" ,zjxwcbz,wtkhbz,sfyfdx,zhzjxje,zjxsuijin   ,khjsbz ,zjjsbz,sjssjfjsbz,jjwjbz,xmjfzt,xcjdbz,sjhtsjf) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?    ,?,?,?,?,?,?,?,?,?,?";
	ls_sql+=" ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,? ";
	ls_sql+=" ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,? ";
	ls_sql+=" ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,? ";
	...
	ls_sql+=" ,'1','N','1',0,0  ,'N','0','0','N','N','0',0) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
	ps.setString(2,khxm);
	ps.setString(3,xb);
