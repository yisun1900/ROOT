...
<%
	...
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[查询-工程担当]："+xtrzyhmc+"') ";
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (ckjgbz.equals("Y"))
	{
		ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,DECODE(kgzbz,'Y','已出','N','<font color=\"#FF00FF\">未出</font>'),DECODE(zt,'2','未退','3','<font color=\"#FF00FF\">已退单</font>'),DECODE(NVL(tgbz,'W'),'W','正常','N','<font color=\"#00FF00\">复工</font>','Y','<font color=\"#FF0000\">已停工</font>'),ysgcjdmc,c.gcyszxmmc ybysxmbm,d.gcyszxmmc zqysxmbm,DECODE(czkbz,'0','','1','<font color=\"#FF0000\">未审核</font>','2','<font color=\"#00FF00\">已审核</font>'),zkl,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,ywy,a.dwmc qydm,crm_khxx.qyrq,jgrq,crm_khxx.qye";
	}
	else{
		ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,DECODE(kgzbz,'Y','已出','N','<font color=\"#FF00FF\">未出</font>'),DECODE(zt,'2','未退','3','<font color=\"#FF00FF\">已退单</font>'),DECODE(NVL(tgbz,'W'),'W','正常','N','<font color=\"#00FF00\">复工</font>','Y','<font color=\"#FF0000\">已停工</font>'),ysgcjdmc,c.gcyszxmmc ybysxmbm,d.gcyszxmmc zqysxmbm,DECODE(czkbz,'0','','1','<font color=\"#FF0000\">未审核</font>','2','<font color=\"#00FF00\">已审核</font>'),zkl,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,ywy,a.dwmc qydm,crm_khxx.qyrq,jgrq,0 ";
	}
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxZjList.jsp","SelectZjCrm_khxx.jsp","","");
...
<CENTER >
  <B><font size="3">签单查询-快速检索</font></B>
</CENTER>
<%
	pageObj.out=out;
