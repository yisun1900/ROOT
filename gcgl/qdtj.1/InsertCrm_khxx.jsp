...
try {
	...
	ls_sql="select sfyckh,ysy,ywyssxz,ywybm,pdqybm,sjsbh,sjf,email,cxhdbmxq,cxhdbmzh,dzbjjc,sfjczjz,cxhdbm,shbz,louhao,ysrbm,zxdm,jzbz,ywy,khxm,xb,cqbm,fwdz,lxfs,sjs,nlqjbm,zybm,fjfwbm,xqbm,khlxbm,hxbm,fwmj,fgyqbm,bjjb,hdbz,cgdz,hdr,ssfgs";
	ls_sql+=" ,khjl,xxlysm,yxtxdz,hyh,sgd,sgbz,zjxm,hth,jiedao,xjf,clgw";
...
//		return;
	...
	if (hth.equals(""))
	{
		hth=qddj.getHth(conn,zxdm,"jrzj");
	}
	...
	ls_sql+=" where khbh='"+khbh+"' and sfjrqye='2'";//是否记入业绩  1：是；2：否
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	if (rs.next())
	{
		bjryjqtf=rs.getDouble(1);
	}
	rs.close();
	ps.close();

	yjjz=cf.round((mle-bjryjqtf)*100/yjjzcs,2);//业绩净值=毛利额÷核算净值参数
...
是
  <input type="radio" name="sfyrz"  value="N" checked onKeyUp="keyGo(sfyyh[0])">
否 </td>
      <td align="right">&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
...
{  
	...
	if (dzbjjc.equals("1"))//1：人工录入[折扣率、工程原报价]，根据[折扣率、工程原报价]自动计算[签约额]
	{
		...
		//业绩净值=毛利额÷业绩净值参数
		var yjjz=(FormName.mle.value*1.0-<%=bjryjqtf%>)*100/FormName.yjjzcs.value;
		yjjz=round(yjjz,2);
...
%>
	...
	if ( <%=sssjf%> < <%=(sjjryjbfb-0.5)%>*FormName.sjf.value/100.0 )
	{
