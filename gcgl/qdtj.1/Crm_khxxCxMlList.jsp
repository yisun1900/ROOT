...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String mllce=null;
	mllce=request.getParameter("mllce");
	if (mllce!=null)
	{
		mllce=cf.GB2Uni(mllce);
		if (!(mllce.equals("")))	wheresql+=" and  (crm_khqye.mll-crm_khqye.zxmll)>="+mllce+"";
	}
	mllce=request.getParameter("mllce2");
	if (mllce!=null)
	{
		mllce=cf.GB2Uni(mllce);
		if (!(mllce.equals("")))	wheresql+=" and  (crm_khqye.mll-crm_khqye.zxmll)<="+mllce+"";
	...
	String mll=null;
	mll=request.getParameter("mll");
	if (mll!=null)
	{
		mll=cf.GB2Uni(mll);
		if (!(mll.equals("")))	wheresql+=" and  (crm_khqye.mll>="+mll+")";
	}
	mll=request.getParameter("mll2");
	if (mll!=null)
	{
		mll=cf.GB2Uni(mll);
		if (!(mll.equals("")))	wheresql+=" and  (crm_khqye.mll<="+mll+")";
	...
	String mle=null;
	mle=request.getParameter("mle");
	if (mle!=null)
	{
		mle=cf.GB2Uni(mle);
		if (!(mle.equals("")))	wheresql+=" and  (crm_khqye.mle>="+mle+")";
	}
	mle=request.getParameter("mle2");
	if (mle!=null)
	{
		mle=cf.GB2Uni(mle);
		if (!(mle.equals("")))	wheresql+=" and  (crm_khqye.mle<="+mle+")";
	...
	ls_sql="SELECT crm_khxx.khbh,crm_khxx.khxm,crm_khqye.mle,crm_khqye.mll||'%',crm_khqye.zxmll||'%',crm_khqye.yjjzcs||'%',crm_khqye.yjjz,crm_khqye.zxmle ,crm_khqye.zxyjjz,crm_khxx.hth,ysgcjdmc,crm_khxx.fwdz,crm_khxx.khjl,crm_khxx.sjs,dm_gcysxm.gcysxmmc,crm_khxx.szqsj,crm_khxx.ysy,sgdmc||DECODE(sq_sgd.sgdlx,'S0','','S1','(直属)') sgd,crm_khxx.zjxm,crm_khxx.smhtbh,DECODE(kgzbz,'Y','已出','N','<font color=\"#FF00FF\">未出</font>'),DECODE(zt,'2','家装','4','集成','5','设计','3','<font color=\"#FF00FF\">已退单</font>'),DECODE(crm_khxx.jzbz,'1','家装','2','公装'),crm_khxx.cxhdbm,DECODE(NVL(tgbz,'W'),'W','正常','N','<font color=\"#00FF00\">复工</font>','Y','<font color=\"#FF0000\">已停工</font>'),DECODE(crm_khxx.gcsfyq,'1','未延期','2','延期'),crm_khxx.yqts,zkl,ywy,a.dwmc qydm,crm_khxx.qyrq,crm_khxx.pdsj,crm_khxx.kgrq,crm_khxx.sjkgrq,crm_khxx.jgrq,crm_khxx.sjjgrq,crm_khxx.wdzgce,crm_khxx.qye,crm_khqye.hdzsjz,crm_khqye.djjje,crm_khqye.htjz,crm_khqye.gcjcbj,crm_khqye.sgcbj,crm_khqye.cbj,crm_khxx.zqgczjf,crm_khxx.zhgczjf,crm_khxx.glfbfb||'%',crm_khxx.zqguanlif,crm_khxx.guanlif,crm_khxx.suijinbfb||'%' suijinbfb,crm_khxx.zqsuijin,crm_khxx.suijin,crm_khxx.zqsjf,crm_khxx.sjf,crm_khxx.zjxje,crm_khxx.zhzjxje,crm_khxx.zqzjxguanlif,crm_khxx.zjxguanlif,crm_khxx.zqzjxsuijin,crm_khxx.zjxsuijin,crm_khxx.zjxje+crm_khxx.zqzjxguanlif+crm_khxx.zqzjxsuijin as zqzjxzj,crm_khxx.zhzjxje+crm_khxx.zjxguanlif+crm_khxx.zjxsuijin as zhzjxzj,crm_khxx.sgebfb||'%',crm_khxx.sge,crm_khxx.clf,DECODE(crm_khxx.sffj,'Y','是','N','否'),crm_khxx.fjje,crm_khxx.gsfje,DECODE(crm_khxx.sfyrz,'Y','是','N','否'),b.dwmc rzsc,khlxmc";
