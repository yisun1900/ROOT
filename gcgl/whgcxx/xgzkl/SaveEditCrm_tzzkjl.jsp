...
try {
	...
	ls_sql="update crm_tzzkjl set zklx=?,zdzk=?,gcfqdzk=?,glfzk=?,sjzk=?,sjfzk=?,qtsfxmzk=?,zjxzklx=?,bfxmdldz=?,tzyy=?   ,lrr=?,lrsj=SYSDATE,lrbm=? ";
	ls_sql+=" where tzjlh='"+tzjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,zklx);
	ps.setDouble(2,zdzk);
	ps.setDouble(3,gcfqdzk);
	ps.setDouble(4,glfzk);
	ps.setDouble(5,sjzk);
	ps.setDouble(6,sjfzk);
	ps.setDouble(7,qtsfxmzk);
	ps.setString(8,zjxzklx);
	ps.setString(9,bfxmdldz);
	ps.setString(10,tzyy);
