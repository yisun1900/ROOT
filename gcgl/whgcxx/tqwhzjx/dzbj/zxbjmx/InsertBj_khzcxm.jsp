...
<%
	...
	ls_sql="SELECT bj_khzcxmh.lx,DECODE(bj_khzcxmh.lx,'1','原项目','2','增项','3','减项','4','新增项') lxmc,bj_khzcxmh.sxh,bjjbmc,bj_khzcxmh.sl,bj_khzcxmh.xdsl,bj_khzcxmh.dj,ROUND(bj_khzcxmh.sl*bj_khzcxmh.dj,2) je,bj_khzcxmh.ptcpsm,bj_khzcxmh.cpbm,bj_khzcxmh.cpmc,bj_khzcxmh.ppmc,bj_khzcxmh.xh,bj_khzcxmh.gg,bj_khzcxmh.jldw";
	...
	ls_sql="SELECT bj_khzcxmh.lx,'减为0' lxmc,bj_khzcxmh.sxh,bjjbmc,bj_khzcxmh.sl,bj_khzcxmh.xdsl,bj_khzcxmh.dj,ROUND(bj_khzcxmh.sl*bj_khzcxmh.dj,2) je,bj_khzcxmh.ptcpsm,bj_khzcxmh.cpbm,bj_khzcxmh.cpmc,bj_khzcxmh.ppmc,bj_khzcxmh.xh,bj_khzcxmh.gg,bj_khzcxmh.jldw";
	ls_sql+=" FROM bdm_bjjbbm,bj_khzcxmh  ";
    ls_sql+=" where bj_khzcxmh.bjjbbm=bdm_bjjbbm.bjjbbm(+)";
    ls_sql+=" and bj_khzcxmh.zjxxh='"+zjxxh+"' and bj_khzcxmh.jgwzbm='"+jgwzbm+"' and bj_khzcxmh.tccplbbm='"+tccplbbm+"'";
    ls_sql+=" and bj_khzcxmh.zclx in('1','2')";//主材类型  1：套餐内；2：套餐外收费；3：大店
	...
	while (rs.next())
	{
		...
		if (lx.equals("1"))//1：原项目；2：增项；3：减项；4：新增项
		{
			if (sl>0)
			{
				...
				<tr bgcolor="#FFFFF"  align="center">
					<td>
						<A HREF="SaveEditHyBjBj_khzcxm.jsp?zjxxh=<%=zjxxh%>&sxh=<%=sxh%>&jgwzbm=<%=jgwzbm%>&tccplbbm=<%=tccplbbm%>" >还原</A>  
						<BR><BR>
						<A HREF="DeleteEditHyBjBj_khzcxm.jsp?zjxxh=<%=zjxxh%>&sxh=<%=sxh%>&jgwzbm=<%=jgwzbm%>&tccplbbm=<%=tccplbbm%>" >删除还原</A>  
					</td>
					<td><%=lxmc%></td>
					<td><%=bjjbmc%></td>
					<td><%=jldw%></td>
...
<tr bgcolor="#FFFFFF">
  ...
  <td><select name="ppmc" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changePp(insertform)">
    ...
    <%
	String sql="";
	sql="select distinct  ppmc c1,ppmc c2 from bj_tczcbj where tccplbbm='"+tccplbbm+"' and  bjjbbm='"+bjjbbm+"'  and cpbm not in(select cpbm from bj_khzcxmh where zjxxh='"+zjxxh+"' and jgwzbm='"+jgwzbm+"' and zclx='1') and sfyx in('1','3') order by ppmc";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where ppmc='"+FormName.ppmc.value+"' and  tccplbbm='"+FormName.tccplbbm.value+"' and  bjjbbm='"+FormName.bjjbbm.value+"'   and  ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE)))  order by cpmc,xh ";
