...
<body bgcolor="#FFFFFF">
  <div align="center">维护-客户调查</div>
  
<form method="post" action="SaveInsertCrm_khdcb.jsp" name="insertform" >
	...
	while (rs.next())
	{
		...
		while (rs1.next())
		{
			...
			if (sfkdx.equals("1"))//1：多选；2：单选；3：写说明
			{
				%>
				<BR>&nbsp;&nbsp;&nbsp;&nbsp;<%=dabh%>：
				<input type="checkbox" name="wtbh<%=wtbh%>" value="<%=dabh%>" <%=checkStr%>>
				<%=damc%>
				<%
			...
			else if (sfkdx.equals("2"))
			{
				%>
				<BR>&nbsp;&nbsp;&nbsp;&nbsp;<%=dabh%>：
				<input type="radio" name="wtbh<%=wtbh%>" value="<%=dabh%>" <%=checkStr%>>
				<%=damc%>
				<%
				if (sfyxsm.equals("Y"))
				{
