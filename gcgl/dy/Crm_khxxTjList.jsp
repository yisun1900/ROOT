...
//以上
	...
	if (groupsl==0)
	{
		...
		selectgroupsql=" select "+selectgroupsql.substring(1)+",count(crm_khxx.khbh),sum(wdzgce),sum(qye),sum(sjf),sum(fjje),sum(gsfje),ROUND(sum(qye)/count(crm_khxx.khbh))";
	}
%>
...
<%
	...
	ls_sql+=" FROM crm_khxx,sq_dwxx a, sq_dwxx b,sq_dwxx c,sq_sgd,dm_gcjdbm,dm_khlxbm,dm_cqbm,dm_xqbm,dm_nlqjbm,dm_ysrbm,dm_zybm,dm_hxbm";
    ...
    ls_sql+=" and crm_khxx.cqbm=dm_cqbm.cqbm(+) and crm_khxx.xqbm=dm_xqbm.xqbm(+)";
