...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		ls_sql="SELECT gcjdbm,sybgcrq,sfxhf,hfrq,hflxbm,hfdjbz,NVL(jhjdsj-TRUNC(SYSDATE),1) jdsj";
		...
		if (sybgcrq.equals(""))
		{
			out.println("<BR>！！！警告，序号为:"+(i+1)+"的客户编号["+khbh[i]+"]隐蔽工程验收日期不存在");
		}


		ls_sql="update crm_khxx set sfxhf='Y',hflxbm=?,hfrq=?,sybhfbz='Y'";
		ls_sql+=" where  khbh='"+khbh[i]+"' ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,"22");
		ps.setDate(2,hfrq);
