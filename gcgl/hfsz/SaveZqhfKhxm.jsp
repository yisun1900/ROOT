...
try {
	...
	for (int i=0;i<khbh.length ;i++ )
	{
		...
		ls_sql="SELECT gcjdbm,szqsj,sfxhf,hfrq,hflxbm,hfdjbz,NVL(jhjdsj-TRUNC(SYSDATE),1) jdsj";
		...
		if (szqsj.equals(""))
		{
			out.println("<BR>！！！警告，序号为:"+(i+1)+"的客户编号["+khbh[i]+"]实际中期验收时间不存在");
		}


		ls_sql="update crm_khxx set sfxhf='Y',hflxbm=?,hfrq=?,szqhfbz='Y'";
		ls_sql+=" where  khbh='"+khbh[i]+"' ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,"23");
		ps.setDate(2,hfrq);
