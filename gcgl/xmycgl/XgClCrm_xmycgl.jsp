...
try {
	...
	ls_sql+=" ,jjfazdr,jjfazdsj,jjfa,jjfashr,jjfashsj,clqklrr,clqklrsj,wtsfjj,sjclqk,clsj,khyyts,khzrbl,fkhyyts,fkhzrbl";
	...
	if (rs.next())
	{
		...
		wtsfjj=cf.fillNull(rs.getString("wtsfjj"));
		sjclqk=cf.fillNull(rs.getString("sjclqk"));

		clsj=cf.fillNull(rs.getDate("clsj"));
		khyyts=cf.fillNull(rs.getString("khyyts"));
		khzrbl=cf.fillNull(rs.getString("khzrbl"));
		fkhyyts=cf.fillNull(rs.getString("fkhyyts"));
		fkhzrbl=cf.fillNull(rs.getString("fkhzrbl"));
	}
	rs.close();
	ps.close();
...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveXgClCrm_xmycgl.jsp" name="editform">
<div align="center">实际处理－维护（登记编号：<%=djbh%>）</div>
...
<tr bgcolor="#FFFFFF">
  <td colspan="4" align="center" bgcolor="#E8E8FF"><strong>实际处理情况</strong></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right"><span class="STYLE1">*</span><span class="STYLE2">处理情况录入人</span></td>
  <td><input type="text" name="clqklrr" size="20" maxlength="20"  value="<%=yhmc%>" readonly></td>
  <td align="right"><span class="STYLE1">*</span><span class="STYLE2">处理情况录入时间</span></td>
  <td><input type="text" name="clqklrsj" size="20" maxlength="10"  value="<%=cf.today()%>" readonly></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right"><span class="STYLE1">*</span>问题是否解决</td>
  <td><%
	cf.radioToken(out, "wtsfjj","Y+解决&N+未解决",wtsfjj);
%></td>
  <td align="right"><span class="STYLE1">*</span>处理时间</td>
  <td><input type="text" name="clsj" size="20" maxlength="10"  value="<%=clsj%>" onDblClick="JSCalendar(this)"></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right">客户原因影响天数</td>
  <td><input type="text" name="khyyts" size="20" maxlength="8"  value="<%=khyyts%>" ></td>
  <td align="right">客户责任比例</td>
  <td><input type="text" name="khzrbl" size="10" maxlength="8"  value="<%=khzrbl%>" >
    %</td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right">非客户原因影响天数</td>
  <td><input type="text" name="fkhyyts" size="20" maxlength="8"  value="<%=fkhyyts%>" ></td>
  <td align="right">非客户责任比例</td>
  <td><input type="text" name="fkhzrbl" size="10" maxlength="8"  value="<%=fkhzrbl%>" >
    %</td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right"><span class="STYLE1">*</span>实际处理情况</td>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isDatetime(FormName.clqklrsj, "处理情况录入时间"))) {
		return false;
	}
	if(	!radioChecked(FormName.wtsfjj)) {
		alert("请选择[问题是否解决]！");
		FormName.wtsfjj[0].focus();
		return false;
	}
	...
	if(!(isNumber(FormName.khyyts, "客户原因影响天数"))) {
		return false;
	}
	if(!(isFloat(FormName.khzrbl, "客户责任比例"))) {
		return false;
	}
	if(!(isNumber(FormName.fkhyyts, "非客户原因影响天数"))) {
		return false;
	}
	if(!(isFloat(FormName.fkhzrbl, "非客户责任比例"))) {
		return false;
	}
