...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT '<A HREF=\"EditCrm_heysjl.jsp?yscsjlh='||crm_heysjl.yscsjlh||'\" target=\"_blank\">修改</A>' xg,crm_heysjl.ysjlh, DECODE(crm_heysjl.sjysjg,'0','合格','1','需整改'), DECODE(crm_heysjl.clbz,'9','录入未完成','0','没问题','1','整改未接收','2','接收','3','已处理','4','已解决','5','验收通过','6','验收未通过'),crm_heysjl.zpsl,crm_heysjl.khkkzpsl,crm_khxx.khxm,crm_khxx.fwdz,crm_heysjl.sjs,sgdmc,crm_heysjl.zjxm,crm_heysjl.gcysxmmc,crm_heysjl.yssj,crm_heysjl.ysr,crm_heysjl.zgcljlh,crm_heysjl.jhjjsj,crm_heysjl.jsr,crm_heysjl.jssj,crm_heysjl.jssm, DECODE(crm_heysjl.cljg,'3','在处理','4','已解决'),crm_heysjl.jjr,crm_heysjl.jjsj,crm_heysjl.jjsm, DECODE(crm_heysjl.zgysjg,'5','合格','6','不合格'),crm_heysjl.zgysr,crm_heysjl.zgyssj,crm_heysjl.zgyssm,crm_heysjl.dwjg,crm_heysjl.dwwz,crm_heysjl.dwjd,crm_heysjl.dwwd,crm_heysjl.dwhb, DECODE(crm_heysjl.padlrbz,'0','电脑','1','客户端')  , DECODE(crm_heysjl.fdxbz,'1','未发送','2','发送'), DECODE(crm_heysjl.xlrmx,'1','不需录入','2','需录入','3','已录入'),crm_heysjl.khbh,crm_heysjl.yscsjlh,dwmc,crm_heysjl.lrr,crm_heysjl.lrsj,crm_heysjl.bz";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by crm_heysjl.lrsj desc,crm_heysjl.ysjlh desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_heysjlTqList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"删除选中的信息","删除定位信息"};//按钮的显示名称
	String[] buttonLink={"DeleteCrm_heysjl.jsp?","DeleteDwxx.jsp?"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">工程验收记录－特权维护</font></B>
</CENTER>
<%
	pageObj.out=out;
