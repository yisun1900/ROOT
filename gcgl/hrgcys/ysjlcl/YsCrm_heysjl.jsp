...
<form method="post" action="SaveYsCrm_heysjl.jsp" name="editform" target="_blank">
<div align="center">整改验收(处理记录号:<%=zgcljlh%>)</div>
<table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
  ...
  <tr bgcolor="#FFFFFF">
    <td height="25" align="right">计划解决时间</td>
    <td><%=jhjjsj%></td>
    <td align="right">整改处理状态</td>
    <td><%=clbz%></td>
  </tr>
  <tr bgcolor="#FFFFFF">
    <td align="right"><font color="#FF0000">*</font>整改验收结果</td>
    <td><%
	cf.radioToken(out, "zgysjg","5+验收通过&6+验收未通过","");
%></td>
    <td align="right">&nbsp;</td>
    <td>&nbsp;</td>
  </tr>
...
<tr bgcolor="#FFFFFF">
  <td align="right"><font color="#CC0000">*</font>整改验收人</td>
  <td><input type="text" name="zgysr" size="20" maxlength="50"  value="<%=yhmc%>" readonly></td>
  <td align="right"><font color="#CC0000">*</font>整改验收时间</td>
  <td><input type="text" name="zgyssj" size="20" maxlength="10"  value="<%=cf.getDate()%>" readonly></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right" width="18%"><font color="#CC0000">*</font>整改验收情况</td> 
  <td colspan="3"> 
    <textarea name="zgyssm" cols="71" rows="6" ></textarea>  </td>
</tr>
    <tr> 
      <td colspan="4" >
	<div align="center">
	...
	<input type="hidden" name="ysjlh"  value="<%=ysjlh%>" >
      ...
      <input name="ckxm" type="button"  onClick="window.open('/gcgl/hrgcys/ysjl/ViewCrm_heysmxb.jsp?ysjlh=<%=ysjlh%>')"  value="查看项目" > 
	<input name="cktp" type="button" id="cktp" onClick="window.open('/gcgl/hrgcys/ysjl/ViewZp1.jsp?ysjlh=<%=ysjlh%>')"  value="全部图片" > 
	  <input type="reset"  value="重输">
	  <input name="sczp" type="button" disabled onClick="f_sczp(editform)"  value="上传验收照片">
      <input name="ckzp" type="button" disabled onClick="f_ckzp(editform)"  value="验收照片">
	</div>      </td>
    </tr>
  </table>
...
function f_sczp(FormName)//参数FormName:Form的名称
{
	...
	{
		var urlStr="loadFilePad.jsp?ysjlh=<%=ysjlh%>&zplx=5&zpxm=<%=zgcljlh%>&zpsm="+encodeURI("验收照片");
		window.open(urlStr);
	}
	else
	{
		var urlStr="loadFile.jsp?ysjlh=<%=ysjlh%>&zplx=5&zpxm=<%=zgcljlh%>&zpsm=验收照片";
		window.open(urlStr);
	}
}
...
{
	var urlStr="viewPhoto.jsp?ysjlh=<%=ysjlh%>&zplx=5&zpxm=<%=zgcljlh%>&zpsm="+encodeURI("验收照片");
	window.open(urlStr);
}
//-->
</SCRIPT>
