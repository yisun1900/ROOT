...
try {
	...
	if (!getclbz.equals("4"))//9：录入未完成；0：没问题；1: 整改未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
	{
	...
	ps.setString(3,"整改验收");
	ps.setString(4,zgysjg);
	ps.setString(5,zgysr);
	ps.setString(6,zgyssm);
	ps.executeUpdate();
	ps.close();

	ls_sql="update crm_heysjl set zgcljlh=?,zgyssj=?,zgysr=?,zgysjg=?,zgyssm=?,clbz=?";
	ls_sql+=" where ysjlh='"+ysjlh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,zgcljlh);
	ps.setDate(2,zgyssj);
	ps.setString(3,zgysr);
	ps.setString(4,zgysjg);
	ps.setString(5,zgyssm);
	ps.setString(6,zgysjg);
	ps.executeUpdate();
	ps.close();
