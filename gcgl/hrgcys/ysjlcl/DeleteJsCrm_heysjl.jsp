...
try {
	...
	for (int i=0;i<zgcljlh.length ;i++ )
	{
		...
		if (!getclbz.equals("2"))//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
		{
			conn.rollback();
		...
		ls_sql="update crm_heysjl set clbz='1',zgcljlh=?,jhjjsj=null,jsr=null,jssj=null,jssm=null ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
		ls_sql+=" where ysjlh='"+ysjlh+"' ";
		ps= conn.prepareStatement(ls_sql);
