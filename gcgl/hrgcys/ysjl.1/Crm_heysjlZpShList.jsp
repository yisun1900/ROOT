...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String zpsl=null;
	zpsl=request.getParameter("zpsl");
	if (zpsl!=null)
	{
		zpsl=zpsl.trim();
		if (!(zpsl.equals("")))	wheresql+=" and  (crm_heysjl.zpsl>="+zpsl+")";
	}
	zpsl=request.getParameter("zpsl2");
	if (zpsl!=null)
	{
		zpsl=zpsl.trim();
		if (!(zpsl.equals("")))	wheresql+=" and  (crm_heysjl.zpsl<="+zpsl+")";
	...
	khkkzpsl=request.getParameter("khkkzpsl");
	if (khkkzpsl!=null)
	{
		khkkzpsl=khkkzpsl.trim();
		if (!(khkkzpsl.equals("")))	wheresql+=" and  (crm_heysjl.khkkzpsl>="+khkkzpsl+")";
	}
	khkkzpsl=request.getParameter("khkkzpsl2");
	if (khkkzpsl!=null)
	{
		khkkzpsl=khkkzpsl.trim();
		if (!(khkkzpsl.equals("")))	wheresql+=" and  (crm_heysjl.khkkzpsl<="+khkkzpsl+")";
	...
	String bhgzpsl=null;
	bhgzpsl=request.getParameter("bhgzpsl");
	if (bhgzpsl!=null)
	{
		bhgzpsl=bhgzpsl.trim();
		if (!(bhgzpsl.equals("")))	wheresql+=" and  (crm_heysjl.bhgzpsl>="+bhgzpsl+")";
	}
	bhgzpsl=request.getParameter("bhgzpsl2");
	if (bhgzpsl!=null)
	{
		bhgzpsl=bhgzpsl.trim();
		if (!(bhgzpsl.equals("")))	wheresql+=" and  (crm_heysjl.bhgzpsl<="+bhgzpsl+")";
	...
	ls_sql="SELECT crm_heysjl.ysjlh, DECODE(crm_heysjl.sjysjg,'0','合格','1','需整改'), DECODE(crm_heysjl.clbz,'9','录入未完成','0','没问题','1','整改未接收','2','接收','3','已处理','4','已解决','5','验收通过','6','验收未通过'),crm_heysjl.zpsl,crm_heysjl.shzpsl,crm_heysjl.khkkzpsl,crm_heysjl.bhgzpsl,crm_khxx.khxm,crm_khxx.fwdz,crm_heysjl.sjs,sgdmc,crm_heysjl.zjxm,crm_heysjl.gcysxmmc,crm_heysjl.yssj,crm_heysjl.ysr,crm_heysjl.dwjg,crm_heysjl.dwwz,crm_heysjl.dwjd,crm_heysjl.dwwd,crm_heysjl.dwhb, DECODE(crm_heysjl.padlrbz,'0','电脑','1','客户端') ,crm_heysjl.khbh,dwmc,crm_heysjl.lrr,crm_heysjl.lrsj,crm_heysjl.bz";
	ls_sql+=" FROM crm_heysjl,crm_khxx,sq_dwxx,sq_sgd  ";
    ls_sql+=" where crm_heysjl.khbh=crm_khxx.khbh(+)";
    ls_sql+=" and crm_khxx.sgd=sq_sgd.sgd(+)";
    ls_sql+=" and crm_khxx.dwbh=sq_dwxx.dwbh(+)";
    ls_sql+=" and crm_heysjl.zpsl>0";
    ls_sql+=" and (crm_heysjl.shzpsl is null OR crm_heysjl.shzpsl<crm_heysjl.zpsl) ";
    ls_sql+=" and crm_heysjl.clbz!='9'";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_heysjlZpShList.jsp","","","EditCrm_heyszpAll.jsp");
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="2%">&nbsp;</td>
	<td  width="3%">验收记录号</td>
	<td  width="3%">项目验收结果</td>
	<td  width="3%">整改处理状态</td>
	<td  width="3%">照片数量</td>
	<td  width="3%">审核照片数量</td>
	<td  width="3%">客户可看</td>
	<td  width="3%">不合格照片数</td>
	<td  width="3%">客户姓名</td>
	<td  width="7%">房屋地址</td>
	<td  width="3%">设计师</td>
	<td  width="3%">施工队</td>
