...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Dm_heysxmpzList.jsp","","","EditDm_heysxmpz.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Dm_heysxmpzList.jsp?"};//按钮单击调用的网页，可以增加参数
...
//设置列超级连接
	...
	String[] coluKey={"gcysbbh","gcysxmbm"};//设置列参数：coluParm.key的主键
	coluParm.key=coluKey;//设置列参数：coluParm.key的主键
	coluParm.link="ViewDm_heysxmpz.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("gcysxmbm",coluParm);//列参数对象加入Hash表
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from dm_heysxmpz where "+chooseitem;
