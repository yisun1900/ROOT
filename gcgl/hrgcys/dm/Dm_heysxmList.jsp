...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by dm_heysxm.gcysbbh desc,dm_heysxm.gcysxmbm";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Dm_heysxmList.jsp","","","EditDm_heysxm.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Dm_heysxmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from dm_heysxm where "+chooseitem;
		sql[1]="delete from dm_heysxmpz where "+chooseitem;
