...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT ybj_cgsq.sqbh as sqbh, DECODE(ybj_cgsq.clzt,'1','发出申请','2','分配样板间','3','参观完成'),khxm,fwdz,lxfs,sjs,fgflmc,zxjwmc,hxmc,hxmjmc,ysmc, DECODE(ybj_cgsq.sfyqd,'Y','已签单','N','未签单'),sq_dwxx.dwmc,ybj_cgsq.sqr,ybj_cgsq.sqsj";
	ls_sql+=" FROM dm_fgflbm,dm_hxmjbm,dm_hxbm,dm_ysbm,dm_zxjwbm,sq_dwxx,ybj_cgsq,crm_khxx  ";
    ls_sql+=" where ybj_cgsq.dwbh=sq_dwxx.dwbh";
	...
	ls_sql+=" SELECT ybj_cgsq.sqbh as sqbh, DECODE(ybj_cgsq.clzt,'1','发出申请','2','分配样板间','3','参观完成'),khxm,fwdz,lxfs,sjs,fgflmc,zxjwmc,hxmc,hxmjmc,ysmc, DECODE(ybj_cgsq.sfyqd,'Y','已签单','N','未签单'),sq_dwxx.dwmc,ybj_cgsq.sqr,ybj_cgsq.sqsj";
	ls_sql+=" FROM dm_fgflbm,dm_hxmjbm,dm_hxbm,dm_ysbm,dm_zxjwbm,sq_dwxx,ybj_cgsq,crm_zxkhxx  ";
    ls_sql+=" where ybj_cgsq.dwbh=sq_dwxx.dwbh";
	ls_sql+=" and ybj_cgsq.ysbm=dm_ysbm.ysbm(+) and ybj_cgsq.hxbm=dm_hxbm.hxbm(+)";
	ls_sql+=" and ybj_cgsq.hxmjbm=dm_hxmjbm.hxmjbm(+) and ybj_cgsq.zxjwbm=dm_zxjwbm.zxjwbm(+)";
	ls_sql+=" and ybj_cgsq.fgflbm=dm_fgflbm.fgflbm(+)";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Ybj_cgsqList.jsp","SelectYbj_cgsq.jsp","","EditYbj_cgsq.jsp");
//	pageObj.setPageRow(10);//设置每页显示记录数
/*
//设置显示列
	String[] disColName={"sqbh","sq_dwxx_dwmc","ybj_cgsq_sqr","ybj_cgsq_sqsj","ybj_cgsq_fgflbm","ybj_cgsq_zxjwbm","ybj_cgsq_hxbm","ybj_cgsq_hxmjbm","ybj_cgsq_ysbm","ybj_cgsq_sfyqd","ybj_cgsq_clzt","ybj_cgsq_khxm","ybj_cgsq_fwdz","ybj_cgsq_lxfs","ybj_cgsq_sjs"};
...
//设置按钮参数
	...
	String[] buttonLink={"Ybj_cgsqList.jsp?"};//按钮单击调用的网页，可以增加参数
...
//设置列超级连接
	...
	coluParm.link="ViewYbj_cgsq.jsp";//为列参数：coluParm.link设置超级链接
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from ybj_cgsq where "+chooseitem;
