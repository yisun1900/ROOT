...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		fbr=cf.GB2Uni(fbr);
		if (!(fbr.equals("")))	wheresql+=" and  (ybj_ybjsb.fbr='"+fbr+"')";
	}
	String fbsj=null;
	...
	{
		fbsj=fbsj.trim();
		if (!(fbsj.equals("")))	wheresql+="  and (ybj_ybjsb.fbsj>=TO_DATE('"+fbsj+"','YYYY/MM/DD'))";
	}
	...
	ls_sql="SELECT crm_khxx.khbh,crm_khxx.khxm,crm_khxx.fwdz, DECODE(ybj_ybjsb.clzt,'1','未审核','2','已审核','3','发布'), DECODE(ybj_ybjsb.shjl,'Y','合格','N','不合格'),ybj_ybjsb.fbr,ybj_ybjsb.fbsj,ybj_ybjsb.ybjyxq,ybj_ybjsb.fbsm,ybj_ybjsb.shr,ybj_ybjsb.shsj,ybj_ybjsb.shsm,crm_khxx.sjs,sgdmc,crm_khxx.zjxm,sq_dwxx.dwmc ssfgs,ybj_ybjsb.sbr,ybj_ybjsb.sbsj,ybj_ybjsb.lrr,ybj_ybjsb.lrsj,ybj_ybjsb.bz  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Ybj_ybjsbXgFbList.jsp","SelectXgFbYbj_ybjsb.jsp","","XgFbEditYbj_ybjsb.jsp");
...
//设置按钮参数
	String[] buttonName={"删除发布信息"};//按钮的显示名称
	String[] buttonLink={"Ybj_ybjsbXgFbList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update ybj_ybjsb set fbr=null,fbsj=null,fbsm=null,clzt='2' where "+chooseitem;
		sql[1]="update crm_khxx set ybjbz='N' where "+chooseitem;
