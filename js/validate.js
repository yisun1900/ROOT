...
//				FieldName.value=""
	...
	function isMPhone(FieldName,FieldNote)
	{
		...
		if (len!=11)
		{
			alert(FieldNote+"：输入不正确，手机的长度为11位");
			FieldName.focus();
			return false;
		}
		...
		if (str.charAt(1)!='3' && str.charAt(1)!='5' && str.charAt(1)!='8')
		{
			alert(FieldNote+"：第二位必须是3、5、8");
			FieldName.focus();
			return false;
		}
...
//把字符串转换为下拉列表数据，用%、+分割
//不同行%分割，同一行用＋分割
function strToSelectXh(field,str,xh)
{ 
...
function replaceAll(str,oldstr,newstr)//替换字符串
{ 
...
function codeSQL(sql)//替换字符串
{ 
	sql=replaceAll(sql,"select ","qqwweerrtt ");
	sql=replaceAll(sql,"SELECT ","qqwweerrtt ");
	sql=replaceAll(sql,"from ","aassddffgg ");
	sql=replaceAll(sql,"FROM ","aassddffgg ");
	sql=replaceAll(sql,"where ","zzxxccvvbb ");
	sql=replaceAll(sql,"WHERE ","zzxxccvvbb ");
	sql=replaceAll(sql,"group by ","zzaaqqwwssxx ");
	sql=replaceAll(sql,"GROUP BY ","zzaaqqwwssxx ");
	sql=replaceAll(sql,"group BY ","zzaaqqwwssxx ");
	sql=replaceAll(sql,"GROUP by ","zzaaqqwwssxx ");
	sql=replaceAll(sql,"order by ","xxsswweeddcc ");
	sql=replaceAll(sql,"ORDER BY ","xxsswweeddcc ");
	sql=replaceAll(sql,"order BY ","xxsswweeddcc ");
	sql=replaceAll(sql,"ORDER by ","xxsswweeddcc ");
	sql=replaceAll(sql,"and ","ccddeerrffvv ");
	sql=replaceAll(sql,"AND ","ccddeerrffvv ");
	sql=replaceAll(sql,"OR ","nnhhyyuujjmm ");
	sql=replaceAll(sql,"or ","nnhhyyuujjmm ");
	sql=replaceAll(sql,"%","youbolibfh");
