...
function createCldTabFrm(){
	...
	_TD.innerHTML = "<table cellspacing=0 cellpadding=0 class=calendar style='border:0px solid;width:100%'>"
		...
		+ "</td>"
		+ "<td width=30px>"
		+ "<A HREF=# onclick=parent.resetTargetValue()>取消</A></td>"
		+ "<td align=right class=smallFont>优铂利软件公司</td></tr></table>";
...
function fillCldTabFrm(year, month, date){
	...
	_cldTabFrm.rows(0).cells(0).innerHTML
	 = "<table bgcolor=420042 class=calendar style=\"color:white;font-weight:bolder;border:0px solid;width:100%;height:20px;\" cellspacing=0 cellpadding=0>"
		+ "<tr><td>"
		+ "&nbsp;<a style='cursor:hand' title='上个月' onclick=\"parent.switchLastMonth()\">上月</a>"
		+ "</td>"
		+ "<td align=center vAlign=middle>"
		+ "<map name=mapForBtnYears><area title='下一年' onclick=parent.switchLastYear() shape=rect coords=0,0,10,5><area title='上一年' onclick=parent.switchNextYear() shape=rect coords=0,5,10,15></map>"		
		...
		+ "</td>"
		+ "<td align=right>"
		+ "<a style='cursor:hand' title='下个月' onclick=\"parent.switchNextMonth()\">下月</a>&nbsp"
		+ "</td></tr></table>"
...
function whenMouseOverDateItem(){
	...
	if(e.tagName == "TD"){
		if(e.name == "LASTMONTH"){
			...
			e.title = "上个月 : " + _tmpdate.getFullYear() + "-" + (_tmpdate.getMonth() + 1) + "-" + e.innerText;
			return;
		}
		if(e.name == "NEXTMONTH"){
			_tmpdate = getNextDate(_dCurYear, _dCurMonth);
			e.title = "下个月 : " + _tmpdate.getFullYear() + "-" + (_tmpdate.getMonth() + 1) + "-" + e.innerText;
			return;
		}
	
		var sCurDate = _dCurYear + "-" + (_dCurMonth + 1) + "-" + e.innerText;
		e.title = "当前月 : " + sCurDate;
