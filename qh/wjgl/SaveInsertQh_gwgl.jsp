...
String bt=cf.GB2Uni(request.getParameter("bt"));
String fl=cf.GB2Uni(request.getParameter("fl"));
...
/*
	int xh=0;
	ls_sql="select NVL(max(gwbh)+1,1)";
	ls_sql+=" from  qh_gwgl";
...
*/
	...
	ls_sql="insert into qh_gwgl ( gwbh,gwmc,gwnr,fbsj,fbr,fbbm,lrsj,bz,lrr,fbfgs,fbfw,bt,fl ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,? ) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,gwbh);
	ps.setString(2,gwmc);
