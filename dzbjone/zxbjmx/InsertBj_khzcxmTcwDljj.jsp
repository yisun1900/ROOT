...
<%
    ...
    ls_sql+=" and bj_khzcxm.ddcljjfs in('2')";//0：非大店材料；1：套外升级（补差价）；2：套外升级（独立计价）；3：套餐选配
    ls_sql+=" order by bj_khzcxm.cpbm,bj_khzcxm.sxh";


    pageObj.sql=ls_sql;
	pageObj.initPage("","","DeleteBj_khzcxmTcwDljj.jsp?khbh="+khbh,"EditBj_khzcxmTcwDljj.jsp?khbh="+khbh);
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（￥'||dj||'/'||jldw||'）' from bj_tcwzck where ppmc='"+FormName.ppmc.value+"' and jjfs='2' and cpbm not in(select cpbm from bj_khzcxm where khbh='<%=khbh%>' and zclx in('3','4')) and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE)))  order by cpmc,xh";
