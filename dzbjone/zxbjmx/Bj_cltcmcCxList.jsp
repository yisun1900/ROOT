	...
	while (rs.next())
	{
		...
		if (count>0)
		{
			...
			<tr bgcolor="<%=bgcolor%>" align="center"> 
				...
				<td><%=tcjg%></td>
				<td>&nbsp;</td>
				<td><A HREF="Bj_khcltcfjslList.jsp?khbh=<%=khbh%>&dqbm=<%=dqbm%>&lx=<%=lx%>&tcmc=<%=java.net.URLEncoder.encode(tcmc,"UTF-8")%>&lx=<%=lx%>"  target="lrbjframe">选择</A></td>
				<td><A HREF="DeleteBj_khcltcmc.jsp?khbh=<%=khbh%>&tcmc=<%=tcmc%>&lx=<%=lx%>"  target="lrbjframe">删除</A></td>
			</tr>
			<%
		}
		else{
			...
			<tr bgcolor="<%=bgcolor%>" align="center"> 
				...
				<td><A HREF="ViewBj_cltczcmx.jsp?dqbm=<%=dqbm%>&hxbm=<%=hxbm%>&tcmc=<%=tcmc%>" target="_blank"><%=tcmc%></A></td>
				<td><%=tcjg%></td>
				<td><A HREF="SaveInsertBj_khcltcmc.jsp?khbh=<%=khbh%>&dqbm=<%=dqbm%>&hxbm=<%=hxbm%>&tcmc=<%=tcmc%>&lx=<%=lx%>" >选择</A></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
			</tr>
			<%
