...
<tr bgcolor="#FFFFFF">
  ...
  <select name="cpbm" style="FONT-SIZE:12PX;WIDTH:512PX">
    <option value=""></option>
    <%
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（￥'||dj||'/'||jldw||'）' from bj_tcwzck where jjfs in('2') and ppmc='"+ppmc+"' and  tccplbbm='"+tccplbbm+"' order by cpmc,xh";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（￥'||dj||'/'||jldw||'）' from bj_tcwzck where jjfs in('2') and ppmc='"+FormName.ppmc.value+"' and  tccplbbm='"+FormName.tccplbbm.value+"' order by cpmc,xh";
