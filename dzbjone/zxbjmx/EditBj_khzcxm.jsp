...
<tr bgcolor="#FFFFFF">
  ...
  <td><select name="ppmc" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changePp(insertform)">
    ...
    <%
	String sql="";
	sql="select distinct  ppmc c1,ppmc c2 from bj_tczcbj where tccplbbm='"+tccplbbm+"' and  bjjbbm='"+bjjbbm+"' and  dqbm='"+dqbm+"'  and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE))) order by ppmc";
...
<tr bgcolor="#FFFFFF">
  ...
  <select name="cpbm" style="FONT-SIZE:12PX;WIDTH:512PX">
    <option value=""></option>
    <%
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where ppmc='"+ppmc+"' and  dqbm='"+dqbm+"' and  tccplbbm='"+tccplbbm+"' and  bjjbbm='"+bjjbbm+"' and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE))) order by cpmc,xh";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（'||jldw||'）' from bj_tczcbj where ppmc='"+FormName.ppmc.value+"' and  dqbm='<%=dqbm%>' and  tccplbbm='"+FormName.tccplbbm.value+"' and  bjjbbm='"+FormName.bjjbbm.value+"' and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE))) order by cpmc,xh ";
