...
<%
	...
	while (rs.next())
	{
		...
		String khtcmc="";
		int sjmj=0;
		ls_sql="SELECT tcmc,sjmj";
		ls_sql+=" FROM bj_khwntcmc";
		ls_sql+=" where khbh='"+khbh+"' and tcmc='"+tcmc+"' and lx='"+lx+"'";
		...
		if (khtcmc.equals(tcmc))
		{
			...
			<tr bgcolor="<%=bgcolor%>" align="center"> 
				...
				if (zcsfxxz.equals("1"))//1：自动选择；2：人工选择
				{
					...
					%>
					<td><A HREF="Bj_wntczcmxList.jsp?khbh=<%=khbh%>&dqbm=<%=dqbm%>&hxbm=<%=hxbm%>&tcmc=<%=java.net.URLEncoder.encode(tcmc,"UTF-8")%>&lx=<%=lx%>" target="_blank">选择明细</A></td>
					<%
				}
				%>
				<td><%=sjmj%></td>
				<td><A HREF="DeleteBj_khwntcmc.jsp?khbh=<%=khbh%>&tcmc=<%=tcmc%>&lx=<%=lx%>" >删除</A></td>
			</tr>
			<%
		}
		else{
			...
			<tr bgcolor="<%=bgcolor%>" align="center"> 
				...
				<td><%=mjsfkzjmc%></td>
				<td><%=zcsfxxzmc%></td>
				<td><A HREF="InsertBj_khwntcmc.jsp?khbh=<%=khbh%>&dqbm=<%=dqbm%>&hxbm=<%=hxbm%>&tcmc=<%=tcmc%>&lx=<%=lx%>" >选择</A></td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
				<td>&nbsp;</td>
			</tr>
