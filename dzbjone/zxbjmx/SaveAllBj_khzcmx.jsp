...
String[] jgwzbm1=request.getParameterValues("jgwzbm1");
String[] slstr1=request.getParameterValues("sl1");
String[] djstr1=request.getParameterValues("dj1");
String[] ycfstr1=request.getParameterValues("ycf1");
String[] qtfy1=request.getParameterValues("qtfy1");
String[] qtfyjestr1=request.getParameterValues("qtfyje1");
...
//更新已选数据
	if (lrxh!=null)
	{
		for (int i=0;i<lrxh.length ;i++ )
		{
			...
			sl=0;
			try{
				sl=java.lang.Double.parseDouble(slstr1[i].trim());
			}
			catch (Exception e){
				conn.rollback();
				out.println("<BR>修改出错！[数量]输入不正确:"+slstr1[i].trim());
				return;
			}

			dj=0;
			try{
				dj=java.lang.Double.parseDouble(djstr1[i].trim());
			}
			catch (Exception e){
				conn.rollback();
				out.println("<BR>修改出错！[单价]输入不正确:"+djstr1[i].trim());
				return;
			}

			ycf=0;
			if (!ycfstr1[i].trim().equals(""))
			{
				try{
					ycf=java.lang.Double.parseDouble(ycfstr1[i].trim());
				}
				catch (Exception e){
					conn.rollback();
					out.println("<BR>修改出错！[远程费]输入不正确:"+ycfstr1[i].trim());
					return;
				}
			}

			qtfyje=0;
			if (!qtfyjestr1[i].trim().equals(""))
			{
				try{
					qtfyje=java.lang.Double.parseDouble(qtfyjestr1[i].trim());
				}
				catch (Exception e){
					conn.rollback();
					out.println("<BR>修改出错！[其它费]输入不正确:"+qtfyjestr1[i].trim());
					return;
				}
			}


			if (qtfyje!=0 && qtfy1[i].equals(""))
			{
				conn.rollback();
			...
			ps.setString(7,cf.GB2Uni(jgwzbm1[i]));
			ps.setDouble(8,ycf);
			ps.setString(9,qtfy1[i]);
