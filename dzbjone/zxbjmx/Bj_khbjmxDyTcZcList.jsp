...
//	out.println(ls_sql);
	...
	while (rs2.next())
	{
		...
		ls_sql+=" where khbh='"+khbh+"' and jgwzbm='"+jgwzmc+"' and tccplbbm in(select tccplbbm from bj_khzcxmxj where  khbh='"+khbh+"' and jgwzbm='"+jgwzmc+"')";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		{
		...
		{
			//房间内[标准套餐：产品类别]
			ls_sql="SELECT distinct bj_khzcxmxj.tccplbbm,clxlmc,sjjmsl,sjjmje,jmdj";
			ls_sql+=" FROM bj_khzcxmxj,jxc_clxlbm ";
			...
			while (rs1.next())
			{
				...
				//	out.println(ls_sql);
					...
					ls_sql+=" and bj_khzcxm.jgwzbm='"+jgwzmc+"'";
					ls_sql+=" and bj_khzcxm.tccplbbm='"+tccplbbm+"'";
					ls_sql+=" and zclx not in('1','2')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
					ls_sql+=" order by xh";
				//	out.println(ls_sql);
					ps= conn.prepareStatement(ls_sql);
			...
			ls_sql="SELECT distinct bj_khzcxm.tccplbbm,clxlmc";
			ls_sql+=" FROM bj_khzcxm,jxc_clxlbm ";
			ls_sql+=" where bj_khzcxm.tccplbbm=jxc_clxlbm.clxlbm(+) ";
			ls_sql+=" and bj_khzcxm.khbh='"+khbh+"'";
			ls_sql+=" and bj_khzcxm.jgwzbm='"+jgwzmc+"'";
			ls_sql+=" and bj_khzcxm.tccplbbm not in(select tccplbbm from bj_khzcxmxj where khbh='"+khbh+"' and jgwzbm='"+jgwzmc+"')";
			ls_sql+=" and bj_khzcxm.zclx not in('1','2')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
			ls_sql+=" order by bj_khzcxm.tccplbbm";
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			while (rs1.next())
			{
