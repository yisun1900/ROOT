...
try {
	...
	if (!zxzt.equals("3"))//0：未分配店面；1: 分配店面；2: 店面退回客户；3：已签约；4：公司签单失败；5：重新分配店面
	{
...
//	out.println(ls_sql);
	...
	ls_sql+=" FROM bj_khzcxm";
    ls_sql+=" where khbh='"+khbh+"' and zclx='2'";//1：套餐内；2：套餐外收费；3：大店
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql="SELECT count(*)";
	ls_sql+=" FROM bj_khzcxm";
	ls_sql+=" where khbh='"+khbh+"' and zclx='1'";//1：套餐内；2：套餐外收费；3：大店
	ls_sql+=" and tccplbbm=64 and dj=0";//墙漆改壁纸
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
...
//	out.println(ls_sql);
	...
	ls_sql="SELECT sum(ROUND(dj*sl,2))";
	ls_sql+=" FROM bj_khzcxm";
	ls_sql+=" where khbh='"+khbh+"'";//1：套餐内；2：套餐外；3：升级大店；4：大店外
	...
	ls_sql="SELECT sum(ROUND(dj*sl,2))";
	ls_sql+=" FROM bj_khzcxm";
	ls_sql+=" where khbh='"+khbh+"' and zclx='4'";//1：套餐内；2：套餐外；3：升级大店；4：大店外
...
<p class="STYLE2">注：①套餐标准价格；②面积增加金额；③主材升级差价+主材超量加价+需收费主材+主材减免+顶配升级+套餐外施工项造价+工程超量加价；④远程费；⑤套餐总价；⑥管理费；⑦税金；⑧套餐内总价；⑨套餐外增加主材；⑩合同总价</p>
