...
<%
	...
	ls_sql="SELECT jgwzbm,tccplbbm,cpmc,sku,ppmc,xh,jldw,dj,sum(sl) sl,sum(ROUND(dj*sl,2)) je,zclx,decode(zclx,'3','顶配升级','4','独立收费') zclxmc,decode(ddcljjfs,'0','套餐主材','1','顶配升级','2','顶配独立收费','3','套餐选配') ddcljjfs";
	ls_sql+=" FROM bj_khzcxm";
	ls_sql+=" where khbh='"+khbh+"'";//1：套餐内；2：套餐外；3：升级大店；4：大店外
	ls_sql+=" and ddcljjfs='1'";//0：套餐内材料；1：顶配升级；2：顶配独立收费；3：套餐选配
	ls_sql+=" group by jgwzbm,tccplbbm,cpmc,sku,ppmc,xh,jldw,dj,zclx,ddcljjfs";
	ls_sql+=" order by ppmc,cpmc,jgwzbm,tccplbbm,ddcljjfs,xh";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	while (rs.next())
	{
...
<%
	...
	ls_sql="SELECT jgwzbm,cpmc,sku,ppmc,xh,jldw,dj,sum(sl) sl,sum(ROUND(dj*sl,2)) je,zclx,decode(zclx,'3','顶配升级','4','独立收费') zclxmc,decode(ddcljjfs,'0','套餐主材','1','顶配升级','2','顶配独立收费','3','套餐选配') ddcljjfs";
	ls_sql+=" FROM bj_khzcxm";
	ls_sql+=" where khbh='"+khbh+"' and zclx in('4')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
