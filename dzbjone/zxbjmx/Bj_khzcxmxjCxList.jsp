...
<%
	...
	while (rs1.next())
	{
		...
		while (rs2.next())
		{
			...
			double khzdsl=0;
			String jmfl=null;
			ls_sql="SELECT bj_khzcxmxj.jmfl,bj_khzcxmxj.sjsfycx,bj_khzcxmxj.khbjjbbm,bj_khzcxmxj.bjjbbm,bj_khzcxmxj.tccplbbm,clxlmc,sjsl,bzsl,DECODE(bj_khzcxmxj.sfxzsl,'1','超量加价','2','不可超量','3','无限制') sfxzsl,DECODE(sfbxx,'Y','必选','N','') sfbxx,sfyxsj,DECODE(sfyxsj,'Y','收费','N','否') sfyxsjMc";
...
<%
					if (sfkjm.equals("2"))//1：不可减免；2：可减免
					{
						...
						{
							%>
							  <td  ><a href="qxzcjm.jsp?khbh=<%=khbh%>&jgwzbm=<%=jgwzbm%>&jmfl=<%=jmfl%>" target="_blank">取消减免</a></td>
							<%
						}
						else{
							%>
							  <td  ><a href="zcjm.jsp?khbh=<%=khbh%>&jgwzbm=<%=jgwzbm%>&jmfl=<%=jmfl%>" target="_blank">减免</a></td>
							<%
						}
					}
					else{
...
%>							
		...
		double khzdsl=0;
		String jmfl=null;
		ls_sql="SELECT bj_khzcxmxj.jmfl,bj_khzcxmxj.khbjjbbm,bj_khzcxmxj.bjjbbm,bj_khzcxmxj.tccplbbm,clxlmc,sjsl,bzsl,DECODE(bj_khzcxmxj.sfxzsl,'1','超量加价','2','不可超量','3','无限制') sfxzsl,DECODE(sfbxx,'Y','必选','N','') sfbxx,sfyxsj,DECODE(sfyxsj,'Y','收费','N','否') sfyxsjMc";
		ls_sql+=" ,sjjmsl,jldw,bj_khzcxmxj.sfkjm,DECODE(bj_khzcxmxj.sfkjm,'1','不可','2','可减免') sfkjmStr,bj_khzcxmxj.yxjmzdsl,sfkgh,DECODE(sfkgh,'1','不可换','2','可换') sfkghStr,khzdsl";
		ls_sql+=" FROM bj_khzcxmxj,jxc_clxlbm ";
		ls_sql+=" where bj_khzcxmxj.tccplbbm=jxc_clxlbm.clxlbm(+)  ";
