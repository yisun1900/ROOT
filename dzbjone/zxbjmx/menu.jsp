...
<%
	...
	if (lx.equals("2"))//1：非套餐；2：套餐
	{
		...
		%>
		zdyxm00= insFld(foldersTree, gFld("主材", ""));
		insDoc(zdyxm00, gLnk(2, "选择主材", "Bj_khzcmxCxList.jsp?khbh=<%=khbh%>&bjjb=<%=bjjb%>"));
		insDoc(zdyxm00, gLnk(2, "选择主材1", "Bj_khzcmxCxList1.jsp?khbh=<%=khbh%>&bjjb=<%=bjjb%>"));
		insDoc(zdyxm00, gLnk(2, "选择主材2", "XzzcMian2.jsp?khbh=<%=khbh%>&bjjb=<%=bjjb%>"));
		insDoc(zdyxm00, gLnk(2, "自定义", "InsertBj_khzcmxZdy.jsp?khbh=<%=khbh%>&ssfgs=<%=ssfgs%>"));
		<%
	}
