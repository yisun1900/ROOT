...
<%
	...
	ls_sql+=" where khbh='"+khbh+"' and sl>0";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
	...
	if (xzzcsl>0)
	{
		...
		ls_sql="SELECT zcpzwzbm,zcmc,jldwbm,ppmc,xinghao,dj,DECODE(qtfy,'1','安装费','2','加工费','3','其它收费') qtfy,sum(sl) sl,sum(je) je,sum(ycf) ycf,sum(qtfyje) qtfyje";
		ls_sql+=" FROM jc_zcddmx ";
		ls_sql+=" where khbh='"+khbh+"' and sl>0";
		ls_sql+=" group by zcpzwzbm,zcmc,jldwbm,ppmc,xinghao,dj,qtfy";
...
//		out.println(ls_sql);
		...
		while (rs.next())
		{
			...
			qtfy=cf.fillHtml(rs.getString("qtfy"));
			...
			%> 
			  <tr bgcolor="#FFFFFF" align="center"> 
				<td width="6%"><%=row%></td>
				<td width="18%"><%=zcmc%></td>
				<td width="7%"><%=jldwbm%></td>
				<td width="6%"><%=cf.formatDouble(sl)%></td>
				<td width="7%"><%=dj%></td>
				<td width="8%"><%=cf.formatDouble(zcje,"#########.##")%></td>
				<td width="48%" align="left" ><%=ppmc%>,<%=xinghao%>,<%=zcpzwzbm%><%=ycfStr%><%=qtfyjeStr%></td>
			  </tr>
			<%
		}
		rs.close();
