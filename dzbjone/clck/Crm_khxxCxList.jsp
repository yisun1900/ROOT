...
<%
	...
	ls_sql="SELECT '批量导入' pldr,'每单导入' mddr,khbh,hth,khxm,fwdz,a.dwmc sbdm,sgdmc sgd,sjs,zjxm,kgrq,jgrq ";
...
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("","","InsertBj_clckd.jsp","");
...
//设置列超级连接
	...
	coluParm.link="plLoadClckData.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("pldr",coluParm);//列参数对象加入Hash表
	pageObj.setColuLink(coluParmHash);//列参数对象加入Hash表

	coluParm=new ColuParm();//生成一个列参数对象
	String[] coluKey2={"khbh"};//设置列参数：coluParm.key的主键
	coluParm.key=coluKey2;//设置列参数：coluParm.key的主键
	coluParm.link="loadClckData.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("mddr",coluParm);//列参数对象加入Hash表
	pageObj.setColuLink(coluParmHash);//列参数对象加入Hash表
