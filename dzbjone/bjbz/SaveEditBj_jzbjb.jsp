...
try {
	...
	ls_sql="update bj_jzbjb set dqbm=?,xmbh=?,bjjbbm=?,xmmc=?,xmdlbm=?,xmxlbm=?,smbm=?,jldwbm=?    ,bj=?,sgcbj=?,cbenj=?  ,gycl=?,xh=?,lrr=?   ,lrsj=SYSDATE,sfbxx=?,bjbbh=?,glxmbh=?,flmcgg=?,bz=?,sfxycl=?,sfxyyjd=?,xmflbm=?,sfyxdz=?,zdzk=?";
	...
	ps.setString(16,bjbbh);
	ps.setString(17,glxmbh);
	ps.setString(18,flmcgg);
	ps.setString(19,bz);
	ps.setString(20,sfxycl);
	ps.setString(21,sfxyyjd);
	ps.setString(22,xmflbm);
	ps.setString(23,sfyxdz);
	ps.setDouble(24,zdzk);
	ps.executeUpdate();
	ps.close();
