...
try {
	...
	ls_sql="SELECT bj_zdyxmb.dqbm,xmmc,dqmc,xmdlmc,xmxlmc";
...
<table border="1" cellspacing="0" cellpadding="1" width="100%" style='FONT-SIZE: 12px'>
      ...
      <tr> 
        ...
        <td width="34%">报价级别： <%
	cf.radioItem(out,"select bjjbbm,bjjbmc from bdm_bjjbbm order by bjjbbm","",true);
%>、&nbsp;&nbsp;&nbsp;报价版本号： /td>
      </tr>
    </table>
  </div>
  <table border="1" cellspacing="0" cellpadding="1" width="125%" style='FONT-SIZE: 12px'>
	...
	int row=0;
	ls_sql="select glbxh,glfxclbm,glfxclmc,glflbm,dj,jldwbm,xhl,jqsfl,bsfl,tvocsfl,lx,wlllbz,bz ";
	ls_sql+=" FROM bj_zdyglfxb ";
	ls_sql+=" where bj_zdyglfxb.xmlrr='"+lrr+"' and bj_zdyglfxb.xmbh='"+xmbh+"'";
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveEditBj_zdyglfxb.jsp";
	FormName.submit();
	return true;
}
function f_sc(FormName)//参数FormName:Form的名称
{
	...
	if ( confirm("确实要继续吗?") )	{
		...
		if (choo == 0){
			...
			FormName.action="DeleteBj_zdyglfxb.jsp";
			FormName.submit();
			return true;
		}
	}
