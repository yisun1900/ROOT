...
try {
	...
	for (int i=0;i<xmbh.length ;i++ )
	{
		...
		ls_sql+=" values("+glbxh+",'"+bjbbh+"','"+dqbm+"','"+xmbh[i]+"','"+glflbm[i]+"','"+cf.GB2Uni(wlllbz[i])+"','"+lx[i]+"','"+cf.GB2Uni(glfxclbm[i])+"','"+cf.GB2Uni(glfxclmc[i])+"','"+cf.GB2Uni(jldwbm[i])+"',"+xhl+","+dj+",'"+yhmc+"',TRUNC(SYSDATE),'"+bjjbbm+"','"+cf.GB2Uni(bz[i])+"',"+jqsfl+","+bsfl+","+tvocsfl+")";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
