...
<form method="post" action="" name="insertform" >
  <div align="center">修改家装工料分析表</div>
...
//		alert("ddd");
		...
		//获取型号列表
		var actionStr="GetListByAjax.jsp?dqbm=<%=dqbm%>&xmbh=<%=xmbh%>&bjjbbm=<%=bjjbbm%>&bjbbh=<%=bjbbh%>&invalue="+FormName.glfxclmc.value;
...
function f_lrrg(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="InsertBj_jzglfxbRgf.jsp";
	FormName.submit();
	return true;
}
...
function f_lrjx(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="InsertBj_jzglfxbJxf.jsp";
	FormName.submit();
	return true;
}
...
function f_del(FormName,glbxh)//参数FormName:Form的名称
{
	if ( confirm("确实要继续吗?") )	
	{
		...
		FormName.action="DeleteBj_jzglfxb.jsp?glbxh="+glbxh;
		FormName.submit();
		return true;
	}
} 
