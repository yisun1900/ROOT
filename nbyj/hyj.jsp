...
<%
String whereyjzt=cf.GB2Uni(request.getParameter("whereyjzt"));
String wherefjrid=cf.GB2Uni(request.getParameter("wherefjrid"));
String wherefjr=cf.GB2Uni(request.getParameter("wherefjr"));
String yjygbh=cf.GB2Uni(request.getParameter("yjygbh"));
...
ls_sql=" select yhdlm ";
ls_sql+=" from sq_yhxx where ygbh="+yjygbh+" ";
//	out.println(ls_sql);
ps= conn.prepareStatement(ls_sql);
rs=ps.executeQuery();
if (rs.next())
{
	yjyhdlm=cf.fillNull(rs.getString("yhdlm"));
}
}
catch (Exception e) {
...
//	alert(FormName.yjfj.value);
	FormName.action="plfs.jsp?yhdlm=<%=yjyhdlm%>&fjrygbh=<%=yjygbh%>";
	FormName.submit();
	return true;
	}
};
