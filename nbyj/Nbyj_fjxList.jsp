...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String yjnr=null;
	String ygbh=null;
	ygbh=request.getParameter("yjygbh");
	...
	ls_sql="SELECT '<a href=\"Delete_fjx.jsp?whereyjbh='||yjbh||'&yjygbh="+yjygbh+"\">删除</a>' sc,yjbh,DECODE(yjlx,'J','<font color=red>紧急</font>','P','普通') yjlx,DECODE(sfyd,'Y','已读','N','未读') sfyd,'<a href=\"ViewNbyj_fjx.jsp?yjbh='||yjbh||'\">'||yjzt||'</a>' yjzt,a.yhmc sjr,b.yhmc fjr,to_char(fssj,'YYYY-MM-DD'),fjym  ";
	ls_sql+=" FROM nbyj_fjx,sq_yhxx a,sq_yhxx b  ";
    ls_sql+=" where a.ygbh=nbyj_fjx.sjr and b.ygbh=nbyj_fjx.fjr and fjr="+yjygbh+" and fssj<=sysdate ";
    ls_sql+=wheresql;
	ls_sql+=" order by fssj desc,sfyd,yjlx desc ";
//	out.print(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Nbyj_fjxList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"删除选中的邮件"};//按钮的显示名称
	String[] buttonLink={"Nbyj_fjxList.jsp?yjygbh="+ygbh};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from nbyj_fjx where "+chooseitem;
