...
if (rklx.equals("S"))//C：采购入库；T：退货入库；R：调拨入库；S：申购入库；J：代金券退货入库；D：主材订单退货；F：发货单入库；Z：直接入库
{
	...
	ddzt=cf.executeQuery("select ddzt from jxc_gysdd where gysddph='"+lydh+"'");//是否直接入库申购公司
...
<div align="center">申购单－直接入库申购公司</div>
<div align="center">
...
<%
	...
	else if (rklx.equals("C"))
	{
		...
		out.print("<option value=\"\"></option>");
		sql="select gysmc c1,gysmc c2 from jxc_gysxx where sfhz='Y' and cllx in('1','3') order by gysmc";
	}
...
*/	
	...
	FormName.action="SaveInsertJxc_rkdSgd.jsp";
	FormName.submit();
	return true;
}
