...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jxc_fcxsd.xsph, DECODE(jxc_fcxsd.xsdzt,'0','等待提交','2','等待出库','3','已出库'),dqmc,a.dwmc ssfgs, DECODE(jxc_fcxsd.xsdlx,'1','公装销售','2','客户零售','3','物流赔偿','4','库存调整','9','其它'),jxc_fcxsd.xszsl,jxc_fcxsd.xszje,jxc_fcxsd.cbzje, DECODE(jxc_fcxsd.sklx,'1','现金','2','转账','3','支票','9','其它'), DECODE(jxc_fcxsd.sfqk,'1','全款','2','部分款','3','未付款'),jxc_fcxsd.shfmc,jxc_fcxsd.shrlxfs,jxc_fcxsd.shfdz,jxc_fcxsd.xsy,jxc_fcxsd.xsrq,jxc_fcxsd.lrr,jxc_fcxsd.lrsj,b.dwmc lrbm,jxc_fcxsd.bz  ";
	ls_sql+=" FROM jxc_fcxsd,sq_dwxx a,sq_dwxx b,dm_dqbm  ";
    ls_sql+=" where jxc_fcxsd.ssfgs=a.dwbh(+) and jxc_fcxsd.lrbm=b.dwbh(+) and jxc_fcxsd.dqbm=dm_dqbm.dqbm";
    ls_sql+=" and jxc_fcxsd.xsdzt in('0','2')";
     ls_sql+=" and jxc_fcxsd.cllx='0'";//0：主材；1：辅材
    ls_sql+=wheresql;
    ls_sql+=" order by jxc_fcxsd.lrsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_fcxsdCxList.jsp","","","InsertJxc_ckdXsd.jsp?cklx=X");
...
//设置列超级连接
	...
	coluParm.link="/jxcfc/xsd/ViewJxc_fcxsd.jsp";//为列参数：coluParm.link设置超级链接
