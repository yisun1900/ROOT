...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,DECODE(sfck,'1','未出库','2','部分出库','3','已出库') sfck,crm_khxx.khxm,crm_khxx.fwdz,ppbm,jc_zcdd.htshsj,jc_zcdd.jhshsj,jc_zcdd.tzshsj,jc_zcdd.tzshr,jc_zcdd.tzry,crm_khxx.hth,crm_khxx.sgbz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'),jc_zcdd.hkze,jc_zcdd.zjhze,jc_zcdd.shtzsm,jc_zcdd.gys,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知')";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)  ";
    ls_sql+=" and jc_zcdd.clzt in('11','13','15','17')";
	ls_sql+=" and jc_zcdd.ddlx='4' and jc_zcdd.sfck in('1','2')";//1：未出库；2：部分出库；3：已出库
	if (yhjs.equals("G0"))
	{
		ls_sql+=" and crm_khxx.fgsbh in(select ssfgs from sq_gyssqfgs where gysbm='"+ygbh+"' )";
	}
	else{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddCxList.jsp","","","InsertJxc_ckdDd.jsp?cklx=D");
	pageObj.setEditStr("出库");//设置每页显示记录数
