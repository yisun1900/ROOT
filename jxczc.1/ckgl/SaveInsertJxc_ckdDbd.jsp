...
try {
	...
	if (rs.next())
	{
		...
		mddqbm=cf.fillNull(rs.getString("drdq"));
	}
	rs.close();
	ps.close();

	//0：未提交；1：提交等待审核；2：审核通过等待出库；3：已出库等待入库；7：审核未通过；9：入库完成
	if (!getdbdzt.equals("2"))
	{
		out.print("错误！调拨单["+lydh+"]状态不正确，应为【2：审核通过等待出库】！");
		return;
	}
	...
	response.sendRedirect("EditJxc_ckdDbd.jsp?lx=1&ckph="+ckph);
}
catch (Exception e) {
