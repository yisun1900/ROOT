...
try {
	...
	//0：未提交；1：等待审核；2：审核通过；5：部分出库；3：已出库；4：已入库；6：审核未通过
	if (!sgdzt.equals("2"))
	{
		out.print("错误！申购单["+lydh+"]状态不正确，应为【审核通过】！");
		return;
	}

/*
	//N：未转；Y：转为采购计划；M：转为供应商订单
	if (!sfzcgd.equals("N"))//是否转为订单
	{
		out.print("错误！申购单["+lydh+"]状态不正确，是否转为订单应为【未转】！");
		return;
	}
*/
