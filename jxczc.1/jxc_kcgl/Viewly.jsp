...
String bfpzsl="";
String wheresql="";
wherelyph=cf.GB2Uni(request.getParameter("lyph"));
if (wherelyph!=null && !(wherelyph.equals("")))
{
	wherelyph=cf.addZero(wherelyph,13);
	wheresql+=" and jxc_cljgb.lyph='"+wherelyph+"' ";
}
...
try {
	...
	ls_sql="select jxc_cljgb.clbm lyph,dqmc dqbm,jxc_kcb.gysmc gysmc,lsj,xsj,cxhdbl,myqjf,DECODE(sfycx,'N','无','Y','有') sfycx,cxkssj,cxjzsj,cxj,lscxhdbl,cxmyqjf,DECODE(xsfs,'1','代销品','2','现货销售','3','库存无货可销售') xsfs,zcpzsl,dhsl,ycgsl,clmc,xh,gg,'<img src=\"/jxc/jxc_lyph/clzp/'||jxc_kcb.zp||'\" width=\"100px\" height=\"100px\">' zp,scsmc,ppmc,cldlbm,clxlbm,ccpzsl,bfpzsl ";
	ls_sql+=" from  jxc_cljgb,dm_dqbm,jxc_kcb";
	ls_sql+=" where jxc_cljgb.lyph=jxc_kcb.lyph and jxc_cljgb.dqbm=dm_dqbm.dqbm"; 
	ls_sql+=wheresql;
	...
	else 
	{
		...
		<!--
//		alert("没有在库存中找到相应数据！");
		window.location.href="View2.jsp?dqbm=<%=wheredqbm%>&lyph=<%=wherelyph%>";
		window.close();
		//-->
		</SCRIPT>
		<%
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	curPage=1;
	String ls_sql2=null;
	String wheresql2=" and lyph= '"+lyph+"' and jxc_kcb.dqbm='"+wheredqbm+"' ";
	ls_sql2="SELECT dqmc,ckmc,hjmc,hwmc,jxc_kcb.lyph lyph,jxc_kcb.clmc clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh xh,jxc_kcb.nbbm nbbm,'' zp,SUM(jxc_kcb.kcsl) ";
    ...
    ls_sql2+=wheresql2;
	ls_sql2+=" group by dqmc,ckmc,hjmc,hwmc,jxc_kcb.lyph,jxc_kcb.clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh,jxc_kcb.nbbm,'' ";
...
/*
//设置显示列
	String[] disColName={"lsh","jxc_kcb_clmc","jxc_kcb_cllb","jxc_kcb_cldlbm","jxc_kcb_clxlbm","jxc_kcb_scsmc","jxc_kcb_ppmc","jxc_kcb_cllbbm","jxc_kcb_xh","jxc_kcb_gg","jxc_kcb_clysbm","jxc_kcb_jldwbm","jxc_kcb_hwbh","jxc_kcb_pjts","jxc_kcb_sfyh","jxc_kcb_cpjb","jxc_kcb_fgsx","jxc_kcb_cpsm","jxc_kcb_cd","jxc_kcb_zp","jxc_kcb_cgzq","jxc_kcb_bz","jxc_kcb_lyph","jxc_kcb_hjbh","jxc_kcb_ckbh","jxc_kcb_dqbm","jxc_kcb_rkj","jxc_kcb_kcsl","jxc_kcb_kcje"};
