...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
	ls_sql="SELECT dqmc,ckmc,hjmc,hwmc,jxc_kcb.clbm clbm,jxc_kcb.clmc clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh xh,jxc_kcb.nbbm nbbm,'<img src=\"/jxc/jxc_clbm/clzp/'||jxc_kcb.zp||'\" width=\"100px\" height=\"100px\">' zp,SUM(jxc_kcb.kcsl),jldwbm,sum(jxc_kcb.kcje),SUM(jxc_kcb.kcsl*dfgsjg),SUM(jxc_kcb.kcsl*dgzjg),SUM(jxc_kcb.kcsl*xsj) ";
	}
	else
	{*/
		ls_sql="SELECT dqmc,ckmc,jxc_kcb.nbbm,jxc_kcb.clmc clmc,jxc_kcb.gg gg,jldwbm,SUM(jxc_kcb.kcsl),DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.clbm clbm ";
	//}
	ls_sql+=" FROM jxc_kcb,dm_dqbm,jxc_ckmc,jxc_hjmc,jxc_hwmc,jxc_cljgb  ";
    ls_sql+=" where jxc_kcb.dqbm=dm_dqbm.dqbm and jxc_kcb.ckbh=jxc_ckmc.ckbh and jxc_kcb.hjbh=jxc_hjmc.hjbh and jxc_kcb.hwbh=jxc_hwmc.hwbh and jxc_kcb.dqbm=jxc_cljgb.dqbm and jxc_kcb.clbm=jxc_cljgb.clbm ";
    ls_sql+=wheresql;
	/*if (sfxstp!=null && sfxstp!="")
	{
	ls_sql+=" group by dqmc,ckmc,hjmc,hwmc,jxc_kcb.clbm,jxc_kcb.clmc,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),jxc_kcb.xh,jxc_kcb.nbbm,'<img src=\"/jxc/jxc_clbm/clzp/'||jxc_kcb.zp||'\" width=\"100px\" height=\"100px\">',jldwbm";
	}
	else
	{*/
		
		ls_sql+=" group by dqmc,ckmc,jxc_kcb.nbbm,jxc_kcb.clmc,jxc_kcb.gg,DECODE(zljb,'1','<font color=green>正常品</font>','2','<font color=blue>残次品</font>','3','<font color=red>报废品</font>'),'',jldwbm,jxc_kcb.clbm";
		
		//}
//	ls_sql+=ordersql;
	ls_sql+=" order by dqmc,ckmc,jxc_kcb.clmc ";
//	out.print(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_kcbTj.jsp","SelectCxJxc_kcb.jsp","","");
