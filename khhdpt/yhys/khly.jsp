...
<table width="100%" border="0" height="834" style='FONT-SIZE: 13px'>
  <tr>
    ...
    <td width="88%" valign="top"> 目前位置－->有话要说－->我要留言
...
<form method="post" action="" name="insertform" target='_blank'>
        <table  border=0 cellspacing=0 cellpadding=0 width="100%" style='FONT-SIZE: 13px;width:100.0%;background:#F6F6F6'>
          <tr bgcolor="#FFFFFF">
            <td style='border-top:white;border-left:white;border-bottom:#C1C1C1;border-right:#C1C1C1;border-style:solid;border-width:1.0pt;background:#DBDBDB;padding:3.75pt 2.25pt 2.25pt 3.75pt' align="right" height="32" width="19%"><font color="#FF0000">*</font>留言类型</td>
            <td style='border-top:white;border-left:white;border-bottom:#C1C1C1;border-right:#C1C1C1;border-style:solid;border-width:1.0pt;background:#ECECEC;padding:3.75pt 2.25pt 2.25pt 3.75pt' height="32" width="81%">
              <input type="radio" name="lylx" value="1">工程留言
              <input type="radio" name="lylx" value="2">集成留言
            </td>
          </tr>
          <tr bgcolor="#FFFFFF"> 
          ...
          <tr bgcolor="#FFFFFF"> 
            <td style='border-top:white;border-left:white;border-bottom:#C1C1C1;border-right:#C1C1C1;border-style:solid;border-width:1.0pt;background:#DBDBDB;padding:3.75pt 2.25pt 2.25pt 3.75pt' align="right" height="32" width="19%"> 
              <font color="#FF0000">*</font>留 言 内 容&nbsp;<BR>
              <BR>
              <input type="hidden" name="khbh" value="<%=khbh%>" >
              <input type="button" name="tjly" value="提交留言" onclick="f_do(insertform)">
            </td>
            <td style='border-top:white;border-left:white;border-bottom:#C1C1C1;border-right:#C1C1C1;border-style:solid;border-width:1.0pt;background:#ECECEC;padding:3.75pt 2.25pt 2.25pt 3.75pt' height="32" width="81%"> 
...
<%
	...
	ls_sql+=" from  hd_khly";
	ls_sql+=" where khbh='"+khbh+"' and lylx in('1','2')";//1：工程留言；2：集成留言；3：客户投诉；4：客户表扬；5：客户建议；6：总经理信箱；9：其它留言
...
{
	if(	!radioChecked(FormName.lylx)) {
		alert("请选择[留言类型]！");
		FormName.lylx[0].focus();
		return false;
	}
	...
	if(	javaTrim(FormName.lynr)=="" || FormName.lynr.value=="请您填写留言内容！不要超过300个汉字") {
		alert("请您填写留言内容！");
		FormName.lynr.value="请您填写留言内容！不要超过300个汉字";
		FormName.lynr.select();
		return false;
	}

	FormName.action="SaveKhly.jsp";//1：工程留言；2：集成留言；3：客户投诉；4：客户表扬；5：客户建议；6：总经理信箱；9：其它留言
	FormName.submit();
