...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String kdlbz=null;
	kdlbz=request.getParameter("kdlbz");
	if (kdlbz!=null)
	{
		if (!(kdlbz.equals("")))	wheresql+=" and  (crm_khxx.kdlbz='"+kdlbz+"')";
	}
	String yhdlm=null;
	yhdlm=request.getParameter("yhdlm");
	if (yhdlm!=null)
	{
		if (!(yhdlm.equals("")))	wheresql+=" and  (crm_khxx.yhdlm='"+yhdlm+"')";
	}
	...
	if (ckjgbz.equals("Y"))
	{
		ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,DECODE(crm_khxx.kdlbz,'01','可登陆','02','<font color=\"#FF0033\">不可登陆</font>'),crm_khxx.yhdlm,crm_khxx.yhkl,crm_khxx.sfzhm, DECODE(crm_khxx.xb,'M','男','W','女'),crm_khxx.fwdz,crm_khxx.lxfs,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,ywy,a.dwmc qydm,crm_khxx.qyrq,crm_khxx.qye,ysgcjdmc  ";
	}
	else{
		ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,DECODE(crm_khxx.kdlbz,'01','可登陆','02','<font color=\"#FF0033\">不可登陆</font>'),crm_khxx.yhdlm,crm_khxx.yhkl,crm_khxx.sfzhm, DECODE(crm_khxx.xb,'M','男','W','女'),crm_khxx.fwdz,crm_khxx.lxfs,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,ywy,a.dwmc qydm,crm_khxx.qyrq,0,ysgcjdmc  ";
	}
    ...
    ls_sql+=" and crm_khxx.gcjdbm not in('5') and crm_khxx.zt='2'";
    ls_sql+=wheresql;
    ls_sql+=" order by khbh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxCxList.jsp","changePasskhxx.jsp","","Editkhxxpass.jsp");
