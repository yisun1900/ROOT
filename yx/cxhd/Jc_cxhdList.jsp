...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_cxhd.cxhdbm,jc_cxhd.cxhdmc,DECODE(jc_cxhd.zklx,'1','整单打折','2','单项打折','3','部分费用打折','4','无折扣') zklx, sq_dwxx.dwmc,DECODE(jc_cxhd.jsbz,'N','未结束','Y','结束') jsbz,jc_cxhdsj.kssj,jc_cxhdsj.cxjssj,jc_cxhdsj.qdjssj,jc_cxhd.dj,jc_cxhd.fxgck,jc_cxhd.fxzck,jc_cxhd.zdzk,jc_cxhd.gcfzk,jc_cxhd.glfzk,jc_cxhd.sjzk,jc_cxhd.sjfzk,jc_cxhd.qtsfxmzk,DECODE(jc_cxhd.zjxzklx,'1','同合同','2','独立折扣'),DECODE(jc_cxhd.sfybfxmzk,'Y','有','N','无'),'<A HREF=\"/yx/cxhd/file/'||jc_cxhd.fjmc||'\" target=\"_blank\">'||jc_cxhd.fjmc||'</A>' fjmc, DECODE(jc_cxhd.hdlx,'1','无活动','2','公司活动','3','小区活动','4','展会活动','5','市场活动') hdlx, DECODE(jc_cxhd.kgxqthd,'1','不可共享','2','可共享') kgxqthd, DECODE(jc_cxhd.spbz,'N','未审批','Y','审批') spbz,jc_cxhd.bz  ";
	ls_sql+=" FROM jc_cxhd,sq_dwxx,jc_cxhdsj  ";
    ls_sql+=" where jc_cxhd.fgsbh=sq_dwxx.dwbh";
    ls_sql+=" and jc_cxhd.cxhdbm=jc_cxhdsj.cxhdbm";
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cxhd.cxhdbm desc,jc_cxhdsj.kssj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cxhdList.jsp","","","EditJc_cxhd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_cxhdList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from jc_cxhdsj where "+chooseitem;
		sql[1]="delete from jc_cxhdxq where "+chooseitem;
		sql[2]="delete from jc_cxhdyjxs where "+chooseitem;
		sql[3]="delete from jc_cxhddm where "+chooseitem;
		sql[4]="delete from jc_hdzsxx where "+chooseitem;
		sql[5]="delete from jc_hddxzsxx where "+chooseitem;
		sql[6]="delete from jc_hdbfxmdz where "+chooseitem;
		sql[7]="delete from jc_hddjj where "+chooseitem;
		sql[8]="delete from jc_cxhd where "+chooseitem;
...
<CENTER >
  <B><font size="3">促销活动管理－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
