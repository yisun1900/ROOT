...
%> 
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td colspan="3"><%
	cf.radioToken(out, "hdlx","1+无活动&2+公司活动&3+小区活动&4+展会活动",hdlx);
%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
%></td>
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td align="right"><font color="#FF0000">*</font>结束标志</td>
      <td><%
	cf.radioToken(out, "jsbz","N+未结束&Y+结束",jsbz);
%></td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFCC">
  ...
  <input type="button" onClick="window.open('InsertJc_cxhdtczk.jsp?cxhdbm=<%=cxhdbm%>')"  value="套餐折扣"  >
  </td>
  <td width="32%"><strong><font color="#FF0000">标准套餐部分采用独立折扣</font></strong></td>
</tr>
</table>
...
<tr bgcolor="#FFFFCC">
  ...
  <td><input type="text" name="gcfzk" value="<%=gcfzk%>" size="8" maxlength="8" >
      <b><font color="#0000FF">（>0且<=10）</font></b> </td>
  <td align="right"><input type="button" onClick="window.open('InsertJc_cxhdtczk.jsp?cxhdbm=<%=cxhdbm%>')"  value="套餐折扣"  ></td>
  <td><strong><font color="#FF0000">标准套餐部分采用独立折扣</font></strong></td>
</tr>
<tr bgcolor="#FFFFCC">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if (FormName.zklx[0].checked)//无折扣 
	{
	}
	else if (FormName.zklx[1].checked)//整单打折
	{
	...
	if(	!radioChecked(FormName.jsbz)) {
		alert("请选择[结束标志]！");
		FormName.jsbz[0].focus();
		return false;
	}
