...
try {
	...
	ls_sql+=" where fgsbh='"+fgs+"' and csmc='zbjxs'";//质保金系数，质保金可抵中单
	...
	ls_sql=" select csz";
	ls_sql+=" from  kp_pdcsb";
	ls_sql+=" where fgsbh='"+fgs+"' and csmc='ddjdxs'";//最后一名的大单接单系数
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	if (rs1.next())
	{
	...
	ls_sql=" select csz";
	ls_sql+=" from  kp_pdcsb";
	ls_sql+=" where fgsbh='"+fgs+"' and csmc='firstjdexs'";//第一名接单额系数
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	if (rs1.next())
	{
	...
	ls_sql=" select csz";
	ls_sql+=" from  kp_pdcsb";
	ls_sql+=" where fgsbh='"+fgs+"' and csmc='lastjdexs'";//最后一名接单额系数
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	if (rs1.next())
	{
	...
	while (rs1.next())
	{
		...
		ls_sql+=" and sgd='"+sgd+"' and lx='1' and yzcdbm in('01','02') and tsxlbm='2101'";//02:一般投诉次数
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		{
		...
		ls_sql+=" and slsj>=TO_DATE('"+qssj+"','YYYY-MM-DD')";
		ls_sql+=" and slsj<=TO_DATE('"+jzsj+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ls_sql+=" and sgd='"+sgd+"' and lx='1' and yzcdbm='03' and tsxlbm='2101'";//03:严重违章
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		{
