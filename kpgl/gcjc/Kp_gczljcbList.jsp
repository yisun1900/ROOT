...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Kp_gczljcbList.jsp","SelectKp_gczljcb.jsp","","EditKp_gczljcb.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Kp_gczljcbList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] kpxh = request.getParameterValues("kpxh");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(kpxh,"kpxh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[2];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from kp_jcmxb where "+chooseitem;
		sql[1]="delete from kp_gczljcb where "+chooseitem;
