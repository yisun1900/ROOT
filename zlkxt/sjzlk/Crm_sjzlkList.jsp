...
<%
if (cf.check((String)session.getAttribute("yhdlm"),request.getRemoteHost(),"100203")==0) 
{
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (sfqd!=null)
	{
		...
		{
			wheresql+=" and  (crm_zxkhxx.zxzt in('3') )";//0：未分配店面；1: 分配店面；2: 飞单审核未通过；3：已签约；4：飞单；5：飞单审核未通过
		}
	...
	ls_sql="SELECT distinct crm_zxkhxx.khbh,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,sjs,khjl,DECODE(crm_zxkhxx.zxzt,'0','未分配店面','1','分配店面','2','飞单审核未通过','3','已签约','4','飞单','5','飞单审核未通过'),DECODE(shbz,'N','未审核','Y','审核'),DECODE(ysshbz,'N','未审核','Y','通过','M','审核未通过'),zjzbjsj,dzbjze,bjjbmc,qsjhtsj,sjhtje,lfdj,jlfdjsj,cxhdbm,hddj,jhddjsj,sjf,jsjfsj,lfsj,ctsj,cxgtsj,csgtsj,DECODE(crm_zxkhxx.jzbz,'1','家装','2','公装'),b.dwmc as zxdm,ywy,sbyymc,lrsj,a.dwmc zxdjbm,fwlxmc,xxlysm";
	ls_sql+=" FROM crm_zxkhxx,dm_fwlxbm,sq_dwxx a,sq_dwxx b,dm_sbyybm,bdm_bjjbbm,crm_sjzlk ";
	...
	ls_sql+=" and crm_zxkhxx.khbh=crm_sjzlk.khbh";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_sjzlkList.jsp","","","/zlkxt/loadWj.jsp");
...
//设置列超级连接
	...
	coluParm.link="/zlkxt/viewLoadWj.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
