...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	cw_jcfkd_lrbm=request.getParameter("cw_jcfkd_lrbm");
	if (cw_jcfkd_lrbm!=null)
	{
		cw_jcfkd_lrbm=cf.GB2Uni(cw_jcfkd_lrbm);
		if (!(cw_jcfkd_lrbm.equals("")))	wheresql+=" and  (cw_jcfkd.lrbm='"+cw_jcfkd_lrbm+"')";
	}
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_jcfkdList.jsp","","","EditCw_jcfkd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Cw_jcfkdList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from cw_jcfkd where "+chooseitem;
