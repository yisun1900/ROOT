...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" where cw_gdjkjl.khbh=crm_khxx.khbh(+) and cw_gdjkjl.sgd=sq_sgd.sgd(+) and cw_gdjkjl.czlx in('1')";
    ls_sql+=wheresql;
    ls_sql+=" order by cw_gdjkjl.sgd,cw_gdjkjl.khbh,cw_gdjkjl.gdjkjlh";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_gdjkjlList.jsp","SelectCw_gdjkjl.jsp","","EditCw_gdjkjl.jsp");
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update cw_gdjkjl set scbz='Y',scsj=SYSDATE,scr='"+yhmc+"' where scbz='N' and czlx in('1') and gdjsjd='0' and ("+chooseitem+")";
