...
//		if (!(sq_sgd_ssdw.equals("")))	wheresql+=" and  (sq_sgd.ssdw='"+sq_sgd_ssdw+"')";
	...
	ls_sql="SELECT dqmc,a.dwmc ssfgs,sq_sgd.sgd,sgdmc,duihao,sgdjbmc,zbj,bzs,jcf,DECODE(tdbz,'Y','<font color=\"#FF0000\">停单</font>','N','不停'),tdkssj,tdzzsj,DECODE(sq_sgd.cxbz,'Y','<font color=\"#FF0000\">撤销</font>','N','未撤销'),dh,kjdsm,sfzhm,jjlxr,lxrdh,hjszd,DECODE(sgdlx,'S0','非直属','S1','直属队') as sgdlx";
	ls_sql+=" FROM sq_sgd,dm_dqbm,sq_sgdjbbm,sq_dwxx a   ";
    ls_sql+=" where sq_sgd.dqbm=dm_dqbm.dqbm and sq_sgd.sgdjbbm=sq_sgdjbbm.sgdjbbm(+)";
    ls_sql+=" and sq_sgd.ssfgs=a.dwbh(+) ";
    ls_sql+=wheresql;
    ls_sql+=" order by sq_sgd.dqbm,sq_sgd.ssfgs,sq_sgd.cxbz,"+px;
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_sgdQqList.jsp","SelectQqSq_sgd.jsp","","");
