...
try {
	...
	{
		if (!spjl.equals("3"))//3：通过；4：未通过
		{
			out.println("错误！审批未通过");
			return;
		}
	}
	else if (jsjd.equals("1"))//1：结算部结算；2：结算审批；3：财务结算
	{
		out.println("错误！未审批");
		return;
	}
	...
	//借款金额
	ls_sql="update cw_gdjkjl set gdjsjd='1',gdjsjlh='"+gdjsjlh+"'";
	ls_sql+=" where  khbh='"+khbh+"' and gdjsjd='0'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();

	//罚款
	ls_sql=" update  kp_xjgdjl set jsbz='Y',jsjlh='"+gdjsjlh+"'";
	ls_sql+=" where kp_xjgdjl.jsbz='N' and kp_xjgdjl.cfspbz='3' ";//2：未审批；3：审批通过；4：审批未通过
	ls_sql+=" and khbh in(select khbh from crm_khxx where crm_khxx.sgd='"+sgd+"')";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	//扣其它款记录
	ls_sql=" update  cw_kqtkjl set jsbz='Y',jsjlh='"+gdjsjlh+"'";
	ls_sql+=" where sgd='"+sgd+"' and jsbz='N' ";//N：未结算；Y：已结算
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
