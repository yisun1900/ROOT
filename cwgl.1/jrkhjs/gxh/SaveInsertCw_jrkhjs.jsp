...
try {
	...
	ls_sql="insert into cw_jrkhjs ( khbh,htsjf,sssjf,wdzgce,qye,zqljzjx,zhljzjx,zqhj,zhhj,jsze,yjzxk,dj,yjwk,yhje,fqbl,fqje,djfxje,dyq,yhze,shfqje,yjzck,zcddjezq,zcddyh,dgzcjezq,dgzcyh,zcjsje,pck,khbjk,lrr,lrsj,lrbm,bz,zctcze,fxjzck,lx,clzt,czbh,dbbh ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'1','1',0,0 ) ";//1：个性化；2：套餐 ; 1：未审批；2：审批同意；3：审批不同意 
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
