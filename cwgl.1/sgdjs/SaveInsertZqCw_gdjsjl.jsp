...
try {
	...
	if (!gdjsjd.equals("0") && !gdjsjd.equals(""))//0：未结算；1：完工结算； 2：中期结算
	{
		out.println("错误！已结算");
		return;
	}
	...
	ls_sql="update crm_khxx set gdjsjd='2',gdjsjlh=?";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
...
/*
	ls_sql="update cw_gdjkjl set gdjsjd='1',gdjsjlh=?";
	...
	ls_sql="update cw_kqtkjl set jsbz='Y',jsjlh=?";
	ls_sql+=" where khbh='"+khbh+"' and jsbz='N'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,gdjsjlh);
	ps.executeUpdate();
	ps.close();

	ls_sql="update cl_ckd set jsbz='Y',jsjlh=?";
	ls_sql+=" where cl_ckd.khbh='"+khbh+"' and cl_ckd.jsbz='N'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,gdjsjlh);
	ps.executeUpdate();
	ps.close();
*/
	...
	ls_sql="insert into cw_gdjsjl (gdjsjlh,khbh,sgd,khlx,cxhdbm,zkl,wdzgce,qye,zjxje,zhzjxje ";
	ls_sql+=" ,zqwgzj,zhwgzj,khsskze,gdjskze,clf,bzclfbl,psgjgff ";
	ls_sql+=" ,ysf,lbcb,kqtk,jkje,sqgz ";
	ls_sql+=" ,jslx,jsdw,jsr,jssj,bz,jsjd,fkbz,sbgf )";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?   ,?,?,?,?,?   ,?,?,?,?,?,'1','1',0) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,gdjsjlh);
	ps.setString(2,khbh);
