...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT cw_lfbgfcsb.fgsbh,sq_dwxx.dwmc,cw_lfbgfcsb.bfcs,gdjsjdmc, DECODE(cw_lfbgfcsb.bfjs,'1','折前工程费','2','折后工程费','3','实收款','9','由打折力度决定折前折后') bfjs,cw_lfbgfcsb.bfbl||'%' bfbl,cw_lfbgfcsb.dzld,DECODE(sfkclk,'Y','是','N','否') sfkclk,clkzdbl||'%' clkzdbl,DECODE(sfkybgf,'Y','是','N','否') sfkybgf ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_lfbgfcsbList.jsp","","","EditCw_lfbgfcsb.jsp");
//	pageObj.setPageRow(10);//设置每页显示记录数

//设置显示列
	String[] disColName={"dwmc","gdjsjdmc","bfjs","bfbl","dzld","sfkclk","clkzdbl","sfkybgf"};
...
//设置按钮参数
	...
	String[] buttonLink={"Cw_lfbgfcsbList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from cw_lfbgfcsb where "+chooseitem;
...
<CENTER >
  <B><font size="3">拨工费参数表－修改</font></B>
</CENTER>
<%
	pageObj.out=out;
