...
try {
	...
	ls_sql="update cw_jrsgdjs set jsrq=?,ysglk=?,zjsge=?,cfje=?,jsglk=?,rgfbfb=?,yfrgf=?,bcrgf=?,flkbfb=?,ycflk=?,kjflk=?,bcflk=?,gsfl=?,psfl=?,kzbjbl=?,kzbj=?,fk=?,bx=?,rybx=?,pck=?,kqtk=?,kqtksm=?,bcfkhj=?,lrr=?,lrsj=?,lrbm=?,bz=? ";
