...
try {
	...
	ls_sql="select khbh,skr,sksj,glxh";
	...
	if (rs.next())
	{
		...
		getglxh=cf.fillNull(rs.getString("glxh"));
	}
	rs.close();
	ps.close();
	...
	ls_sql="SELECT fkxh,cw_khfkjl.sksj,cw_khfkjl.skr,a.fklxmc fklxbm,fkcsmc,gysbh,cw_khfkjl.fkje,DECODE(skdd,'1','不需','2','需审核') skdd,zffsmc,b.fklxmc zckx,zcgysbh,glxh";
...
//	ls_sql+=" and cw_khfkjl.fklxbm in ('11')  ";
//  ls_sql+=" and cw_khfkjl.skr='"+getskr+"' and cw_khfkjl.sksj=TO_DATE('"+getsksj+"','YYYY-MM-DD') ";
	...
	while (rs.next())
	{
		...
		String mark="";
		if (sksj.equals(getsksj) && skr.equals(getskr) && dysl<7)//确认是否同一天
		{
			mark="checked";
			dysl++;
		}
...
function cf_submit(formName)
{
	...
	if (choo == 0){
		...
		formName.action="PrintAllJzSj.jsp";
		formName.submit();
		return true;
	}
