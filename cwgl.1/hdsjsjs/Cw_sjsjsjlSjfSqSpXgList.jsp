...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT hth,khxm,DECODE(clzt,'1','申请','2','审批同意','3','审批不同意','4','结算'),fwdz,sfyjqk,fkcsmc,qye,zjxje,sfke,sfkl,qyrq,sbdm,sgd,sjs,zjxm,khbh ";
    ls_sql+=" from (";
	ls_sql+=" SELECT cw_sjfjssqb.khbh,hth,cw_sjfjssqb.clzt,khxm,DECODE(sfyjqk,'N','未交','Y','已交') sfyjqk,fkcsmc,qye,zjxje,sfke,ROUND(sfke/(qye+NVL(zjxje,0))*100,2)||'%' sfkl,fwdz,qyrq,sjjgrq,a.dwmc sbdm,sgdmc sgd,sjs,zjxm ";
    ...
    ls_sql+=" and cw_sjfjssqb.clzt in('2','3')";
    ls_sql+=wheresql;
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_sjsjsjlSjfSqSpXgList.jsp","SelectSjfSqSpXgCw_sjsjsjl.jsp","","");
...
//设置按钮参数
	...
	String[] buttonLink={"Cw_sjsjsjlSjfSqSpXgList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update cw_sjfjssqb set clzt='1' where clzt in('2','3') and "+chooseitem;
...
<CENTER >
  <B><font size="3">设计费结算审批－修改</font></B>
</CENTER>
<%
	pageObj.out=out;
