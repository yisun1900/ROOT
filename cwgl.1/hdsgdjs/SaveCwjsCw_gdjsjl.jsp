...
try {
	...
	ls_sql="update cw_gdjsjl set clf=?,zgclf=?,sj=?,ksdf=?,gdjskze=?,kwxf=?,jkje=?,fakuan=?,kqtk=?,sqgz=?,cwjsr=?,cwjssj=?,cwjssm=?,jsjd='3' ";
	ls_sql+=" where  (gdjsjlh='"+gdjsjlh+"')  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,clf);
	ps.setDouble(2,zgclf);
	ps.setDouble(3,sj);
	ps.setDouble(4,ksdf);
	ps.setDouble(5,gdjskze);
	ps.setDouble(6,kwxf);
	ps.setDouble(7,jkje);
	ps.setDouble(8,fakuan);
	ps.setDouble(9,kqtk);
	ps.setDouble(10,sqgz);
	ps.setString(11,cwjsr);
	ps.setDate(12,cwjssj);
	ps.setString(13,cwjssm);
