...
try {
	...
	ls_sql="insert into cw_gdjsjl ( khbh,sgd,jzbz,wdzgce,qye,zkl,zjxje,khjskze,khsskze,sjcb   ,jsr,jssj,jsdw,jsjd,fkbz,bz,gdjsjlh,gckk,sjsfy,sgdfy,zjfy,gsfy,qtfy,gdjskze,dejsje,jsfs,sjsfk,sgdfk ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?,?,?,?,0,0,?,?,? ) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
	ps.setString(2,sgd);
	...
	ps.setString(11,jsr);
	ps.setDate(12,jssj);
	ps.setString(13,jsdw);
	ps.setString(14,"0");//0：未结算；1：结算；2：结算审批；3：财务结算
	ps.setString(15,"1");
	ps.setString(16,bz);
	ps.setString(17,gdjsjlh);
	ps.setDouble(18,gckk);
	ps.setDouble(19,sjsfy);
	ps.setDouble(20,sgdfy);
	ps.setDouble(21,zjfy);
	ps.setDouble(22,gsfy);
	ps.setDouble(23,qtfy);
	ps.setString(24,jsfs);
	ps.setDouble(25,sjsfk);
	ps.setDouble(26,sgdfk);
	ps.executeUpdate();
	ps.close();
