...
try {
	...
	ls_sql="update crm_khxx set khjsbz='Y',khjsje=?,khjssj=?,bxkh=?";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,jsze);
	ps.setDate(2,lrsj);
	...
	ls_sql="insert into cw_jrkhjs ( khbh,htsjf,sssjf,wdzgce,qye,zqljzjx,zhljzjx,zqhj,zhhj,jsze,yjzxk,dj,yjwk,yhje,fqbl,fqje,djfxje,dyq,yhze,shfqje,yjzck,zcddjezq,zcddyh,dgzcjezq,dgzcyh,zcjsje,pck,khbjk,lrr,lrsj,lrbm,bz,zctcze,lx,clzt ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'2','1' ) ";//1：个性化；2：套餐 ; 1：未审批；2：审批同意；3：审批不同意 
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khbh);
