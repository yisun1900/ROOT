...
//		return;
	...
	ls_sql="update cw_khfkjl set sjbh=?,fklxbm=?,fkje=?,jkr=?,skr=?,sksj=?,skdw=?,bz=?,zffs=?,zckx=?,lrr=?,lrsj=?,fkcs=?,yfkbl=?,yfkje=?,sfkbl=?,zcgysbh=?,zcddbh=?,zczjxbh=?,sjr=?,sjsj=?,jzwkqs=?,zzyhkh=? ";
	ls_sql+=" where fkxh='"+fkxh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,sjbh);
	...
	ps.setString(20,skr);
	ps.setDate(21,sksj);
	ps.setString(22,jzwkqs);
	ps.setString(23,zzyhkh);
	ps.executeUpdate();
	ps.close();
	...
	if (glxh!=null)
	{
		if (zffs.equals("31") )//转帐款项，生成关联记录，『支付方式』未发生变化
		{
			...
			ls_sql="update cw_khfkjl set sjbh=?,fklxbm=?,gysbh=?,ddbh=?,zjxbh=?,fkje=?,jkr=?,skr=?,sksj=?,skdw=?,bz=?    ,zffs=?,zckx=?,lrr=?,lrsj=?,fkcs=?,yfkbl=?,yfkje=?,sfkbl=?,sjr=?,sjsj=?,zzyhkh=? ";
