...
function getThis(lx)
{
	...
	if (lx=="sgd")
	{
		if (sgdMark==0)
		{
			sql="select sgd,sgdmc||'（'||DECODE(sq_sgd.sgdlx,'S0','非直属','S1','直属')||'）'||DECODE(cxbz,'N','','Y','（撤销）') from sq_sgd where ssfgs='"+selectform.fgs.value+"' order by cxbz,sgdlx desc,sgdmc";
			var actionStr="/ajax/selectstr.jsp?sql="+sql;
