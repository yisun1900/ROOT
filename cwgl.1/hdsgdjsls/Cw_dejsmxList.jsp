...
<table border="1" width="150%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
    ...
    <tr bgcolor="#FFFFFF"> 
      <td colspan="18" > 
		  <input type=button value='修改结算' onClick="cf_submit(listform)" >
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="button" value='查看材料明细' onClick="window.open('ViewCw_dejsclmx.jsp?khbh=<%=khbh%>')" >
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<input type="button" value='查看基价单价' onClick="window.open('ViewCw_dejsxmclmx.jsp?khbh=<%=khbh%>&zjxxh=<%=zjxxh%>')" >
...
<%
	...
	//输出项目明细
	ls_sql="SELECT xmbh,xh,xmmc,cw_dejsmx.jldwbm,jldwmc,sl,dwbjdj,dwbjje,cbdj,cbje,mll,mle,jsblxs,nbjsdj,nbjsje,gsmll,gsmlr,clfbfb,bjlx,DECODE(xclbz,'N','未处理','Y','处理') xclbz ";
	ls_sql+=" from cw_dejsmx,jdm_jldwbm ";
	ls_sql+=" where cw_dejsmx.jldwbm=jdm_jldwbm.jldwbm ";
	ls_sql+=" and cw_dejsmx.khbh='"+khbh+"'";
	ls_sql+=" order by cw_dejsmx.xmbh";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	while (rs.next())
	{
		...
		xclbz=cf.fillHtml(rs.getString("xclbz"));
...
%>	
  <tr bgcolor="#FFFFFF" align="center"> 
    ...
    <tr bgcolor="#FFFFFF"> 
      <td colspan="18" > 
		  <input type=button value='修改结算' onClick="cf_submit(listform)" name="jsblxs">
      </td>
  </tr>
</table>
</form>
