...
<table border="1" width="720%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
 <tr bgcolor="#FFFFFF" >
      <td colspan="194"> 
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="180px" rowspan="3">记入业绩时间</td>
	<td  width="180px" rowspan="3">合同审核时间</td>
...
try {
	...
	ls_sql="select crm_khxx.yjjzsj,crm_khxx.htshsj,khbh,hth,khxm,fwdz,crm_khxx.sjsjsbz,DECODE(crm_khxx.sjsjsbz,'0','未提成','1','签单提成','2','结算提成') sjsjsbzmc,fgsbh,crm_khxx.dwbh,sjs,zjxm,khjl,clgw,hdbz,hdlxmc,fwmj,crm_khxx.bjjb,bjjbmc,khjsbz,gcjdbm";
	...
	while (rs1.next())
	{
		yjjzsj=cf.fillNull(rs1.getDate("yjjzsj"));
		htshsj=cf.fillNull(rs1.getDate("htshsj"));
...
//			zjzxftcbl=cf.round(zjzxftcbl-0.2,2);
		...
		if (sjsjsbz.equals("0"))//0：未提成；1：签单提成；2：结算提成
		{
			...
			if (!sjs.equals(""))
			{
				...
				sjstchj=cf.round(sjsqysjftc+sjsqyzxtc,2);   //设计师提成合计	    
			}

			if (!zjxm.equals(""))
			{
				zjqyzxtc=cf.round(qye*zjzxftcbl/200,2);  //客户经理签约装修提成		
				zjtcxj=cf.round(zjqyzxtc,2);    //客户经理提成小计			
		...
		jjze=dztcze+wyqyzxtc+khjltcze+sjstchj+zjtchj+clytc+xqzgtc+xqzztc+xqzytc+wlyxzgtc+yxzyqyzxtc+wzchqyzxtc+wysjqyzxtc+wzcxyqyzxtc+dhzyqyzxtc+wztgytc+sjqyzxtc;
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SavePltcQd.jsp";
	FormName.submit();
	return true;
}
//-->
