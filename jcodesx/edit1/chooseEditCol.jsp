...
<table width="100%">
  <tr> 
    <td colspan="2"> 
      <div align="center"><b><font size="4">选择修改列</font></b></div>
    </td>
  </tr>
  <tr> 
    <td colspan="2" height="2"> 
      <div align="center"> 
        <form method='post' action='saveChooseEditCol.jsp' name='selectTableForm'>
	...
	try {
		...
		ls_sql="select iskey,table_name,column_name,data_type,data_length,column_china,nullable,column_type ";
		ls_sql+=",column_id,data_default,isedit ";
...
//		out.println("<BR>ls_sql="+ls_sql);
		...
		while (rs.next())
		{
			...
			isedit=cf.fillNull(rs.getString("isedit"));
			...
			if (isedit.equals("Y"))
			{
				out.println("    <td><input type='checkbox' checked name='isedit' value='"+column_id+"' ></td>\r\n");
			}
			else{
				out.println("    <td><input type='checkbox' name='isedit' value='"+column_id+"' ></td>\r\n");
			}
...
<form name="ref" method="post" action="chooseEditCol.jsp?dw_name=<%=dw_name%>&ref=0">
</form>
...
function checkAll(formName)//
{
	...
	if (<%=row%>==1){
		if (mark==0)
		{
			formName.isedit.checked =false
		}
		else{
			formName.isedit.checked =true
		}
	}
	else{ 
		for (i=0;i <<%=row%>;i++ ){
			if (mark==0)
			{
				formName.isedit[i].checked =false
			}
			else{
				formName.isedit[i].checked =true
			}
		}
	}
	if (mark==0)
