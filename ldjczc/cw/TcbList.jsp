...
String getkhbh=null;
getkhbh=request.getParameter("khbh");
if (getkhbh!=null)
{
	getkhbh=cf.GB2Uni(getkhbh);
	if (!(getkhbh.equals("")))	wheresql+=" and  (crm_khxx.khbh='"+getkhbh+"')";
}

String getkhxm=null;
getkhxm=request.getParameter("khxm");
if (getkhxm!=null)
{
	getkhxm=cf.GB2Uni(getkhxm);
	if (!(getkhxm.equals("")))	wheresql+=" and  (crm_khxx.khxm='"+getkhxm+"')";
}
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center">
			...
			<td style="FONT-WEIGHT: bold;BORDER-RIGHT: #808080 2px solid;BORDER-TOP: black 1px solid; BORDER-BOTTOM: black 1px solid; LEFT: expression(this.parentElement.offsetParent.parentElement.scrollLeft); POSITION: relative; BACKGROUND-COLOR: #efe1ff"><A HREF="/khxx/ViewCwCrm_khxx.jsp?khbh=<%=khbh%>" target='_blank'><%=khxm%></A></td>
...
<tr bgcolor="#FFFFFF"  align="center">
	...
	while (rs2.next())
	{
		...
		ls_sql="select sum(jc_zcdd.zjxcbze+jc_zcdd.cbze)";
		ls_sql+=" from  jc_zcdd,crm_khxx,bdm_bjjbbm";
		ls_sql+=" where jc_zcdd.clzt='99'";//退单
		ls_sql+=" and jc_zcdd.ppbm='"+ppmc+"'";
		ls_sql+=" and jc_zcdd.zcdlbm in('壁纸类','橱柜类','瓷砖类','地板类','吊顶类','木门类','卫浴类','石材')";
		ls_sql+=" and jc_zcdd.khbh=crm_khxx.khbh(+)";
		ls_sql+=" and crm_khxx.bjjb=bdm_bjjbbm.bjjbbm";
