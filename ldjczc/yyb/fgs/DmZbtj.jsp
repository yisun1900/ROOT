...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='170223'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[周报(分公司)]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[周报(分公司)]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
<%
	...
	String[] weeks=cf.getWeeks(sjfw,sjfw2);
	int weeksl=Integer.parseInt(weeks[0]);
	for (int i=0;i<weeksl;i++ )
	{
		sjfw_fd=weeks[i*2+1];//周一
		sjfw2_fd=weeks[i*2+2];//周日
			...
			<%
				if (row==1)
				{
					...
					<tr align="right"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
						<td align="center" rowspan="<%=fgssl%>">周累计:<BR><%=sjfw_fd%><BR>到<BR><%=sjfw2_fd%></td>
					<%
				}
				else{
					%>
...
//		out.println(ls_sql);
		...
		<%
			if (row==1)
			{
				%>
				<tr align="right"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
					<td align="center" rowspan="<%=fgssl%>">合计:<BR><%=sjfw%><BR>到<BR><%=sjfw2%></td>
				<%
			}
			else{
				%>
