...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='170212'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[日报]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[日报]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
<%
	...
	while (rs.next())
	{
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='ViewDmRdkhMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+rdkhs+"</A>";
			rdkhszj[3+i]+=rdkhs;
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='ViewDjkhMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+djs+"</A>";
			djszj[3+i]+=djs;
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='/ldjczc/cztj/sjf/ViewDmsjfMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+sjfkhs+"</A>";
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='/ldjczc/cztj/zhtj/ViewDmGxhYjMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+gxhkhs+"</A>";
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='/ldjczc/cztj/zhtj/ViewDmTcYjMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+tckhs+"</A>";
		...
		for (int i=1;i<=day ;i++ )
		{
			...
			dataStr[row][3+i]="<A HREF='/ldjczc/cztj/zhtj/ViewDmJcMx.jsp?sjfw="+days[i]+"&sjfw2="+days[i]+"&dwbh="+getdwbh+"' target='_blank'>"+zcje+"</A>";
