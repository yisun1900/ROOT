...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='17030307'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[供应商销售统计]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[供应商销售统计]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		<tr align="center"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			...
			<td>
				<A HREF="ViewGysQdMxJs.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&gys=<%=getgys%>&fgsbh=<%=getfgsbh%>" target="_blank">
				<%=cf.formatDouble(cbze)%>
				</A>
			</td>
			<td>
				<A HREF="ViewGysTdMxJs.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&gys=<%=getgys%>&fgsbh=<%=getfgsbh%>" target="_blank">
				<%=cf.formatDouble(tdcbze)%>
				</A>
			</td>
			<td>
				<A HREF="ViewGysZjxMxJs.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&gys=<%=getgys%>&fgsbh=<%=getfgsbh%>" target="_blank">
