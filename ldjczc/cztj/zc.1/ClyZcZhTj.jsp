...
<TR bgcolor="#CCCCCC"  align="center"  >
  ...
  <td  width="7%" rowspan="2" >店面</td>
  <td  width="5%" rowspan="2" >材料员</td>
  <td colspan="7" >套餐</td>
...
<TR bgcolor="#CCCCCC"  align="center"  > 
  ...
  <td  width="4%" >平均成本单值</td>
  <td  width="4%" >退单</td>
  <td  width="4%" >退单(正常品材料费)</td>
  <td  width="4%" >退单(特价品材料费)</td>
  <td  width="4%" >增减项</td>
  </tr>
	...
	while (rs.next())
	{
		...
		while (rs2.next())
		{
			...
			if (   (cxbz.equals("N") && sfzszg.equals("Y")) || (cxbz.equals("N") && sfzszg.equals("N")) || (je!=0) || (ddsl!=0)  )
			{
				...
				<td><%=tcddsl%></td>
				<td>
					<A HREF="ViewClyZcCbQdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getdwbh%>&clgw=<%=getclgw%>" target="_blank">
					<%=cf.formatDouble(tccbze)%>				</td>
				<td><%=cf.formatDouble(tcpjde)%></td>
				<td>
					<A HREF="ViewClyZcCbTdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getdwbh%>&clgw=<%=getclgw%>" target="_blank">
					<%=cf.formatDouble(tctdcbe)%>				</td>
				<td>
					<A HREF="ViewClyZcCbZjxMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getdwbh%>&clgw=<%=getclgw%>" target="_blank">
...
//		out.println(ls_sql);
		...
		<tr align="center"  bgcolor="#E8E8FF"  onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			...
			<td><strong><%=cf.formatDouble(xjqye)%></strong></td>
				<td><%=cf.formatDouble(xjzcpclf)%></td>
				<td><%=cf.formatDouble(xjtjpclf)%></td>
			<td><strong><%=cf.formatDouble(pjde)%></strong></td>
			<td><strong><%=cf.formatDouble(xjtde)%></strong></td>
				<td><%=cf.formatDouble(xjtdzcpclf)%></td>
				<td><%=cf.formatDouble(xjtdtjpclf)%></td>
...
<tr align="center"  bgcolor="#E8E8FF"  onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
	...
	<td><strong><%=cf.formatDouble(zjqye)%></strong></td>
				<td><%=cf.formatDouble(zjzcpclf)%></td>
				<td><%=cf.formatDouble(zjtjpclf)%></td>
	<td><strong><%=cf.formatDouble(pjde)%></strong></td>
	<td><strong><%=cf.formatDouble(zjtde)%></strong></td>
				<td><%=cf.formatDouble(zjtdzcpclf)%></td>
				<td><%=cf.formatDouble(zjtdtjpclf)%></td>
