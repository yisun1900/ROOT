...
//	out.println(ls_sql);
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT sq_dwxx.ssdqbm dwbh,sq_ssdqbm.ssdqmc dwmc,count(*) qys,sum(wdzgce) wdzgce,sum(qye) qye,sum(zqgczjf) zqgczjf,sum(zhgczjf) zhgczjf,sum(NVL(zqsuijin,0)) zqsuijin,sum(NVL(suijin,0)) suijin,sum(NVL(zqguanlif,0)) zqguanlif,sum(NVL(guanlif,0)) guanlif  ,sum(NVL(zqqtsf,0)) zqqtsf,sum(NVL(zhqtsf,0)) zhqtsf ,sum(NVL(zqwjrqyexm,0)) zqwjrqyexm,sum(NVL(zhwjrqyexm,0)) zhwjrqyexm ,sum(NVL(zqsjf,0)+NVL(sjhtsjf,0)) zqsjf,sum(NVL(sjf,0)+NVL(sjhtsjf,0)) sjf    ,sum(fwmj) fwmj";
	...
	ls_sql+=" SELECT sq_dwxx.ssdqbm dwbh,sq_ssdqbm.ssdqmc dwmc,0 qys,0 wdzgce,0 qye,0 zqgczjf,0 zhgczjf,0 zqsuijin,0 suijin,0 zqguanlif,0 guanlif  ,0 zqqtsf,0 zhqtsf ,0 zqwjrqyexm,0 zhwjrqyexm ,0 zqsjf,0 sjf    ,0 fwmj";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		ls_sql+=" and sq_dwxx.ssdqbm=sq_ssdqbm.ssdqbm(+)";
		ls_sql+=" and crm_khxx.zt in('2','5')";
		ls_sql+=" and sq_dwxx.ssdqbm='"+dwbh+"' and jzkscsj is not null and cwjzkscsj is null";//首次付家装款时间
		ls_sql+=" and crm_khxx.qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and crm_khxx.qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		...
		ls_sql+=" and sq_dwxx.ssdqbm=sq_ssdqbm.ssdqbm(+)";
		ls_sql+=" and crm_khxx.zt in('2','5')";
		ls_sql+=" and sq_dwxx.ssdqbm='"+dwbh+"' and cwjzkscsj is not null";//财务首次收家装款时间
		ls_sql+=" and crm_khxx.qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
		ls_sql+=" and crm_khxx.qyrq<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		...
		<tr align="center"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			<td ><%=row%></td>
			<td >
			<A HREF="ViewDqQdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssdqbm=<%=dwbh%>" target="_blank">
			<%=dwmc%>			</A>			</td>
