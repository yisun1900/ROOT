...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='17030345'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[材料员主材统计(综合)]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[材料员主材统计(综合)]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
if (!(clgw.equals("")))	
{
	...
	wheresql1+=" and  sq_yhxx.yhmc='"+clgw+"'";
	wheresql2+=" and  jc_zcdd.jzsjs='"+clgw+"'";
	wheresql3+=" and  cw_khfkjl.sjs='"+clgw+"'";
}
...
<TR bgcolor="#CCCCCC"  align="center"  >
  ...
  <td  width="3%" rowspan="3" >材料员</td>
  <td  width="5%" rowspan="3" >部门</td>
...
<%
	...
	ls_sql=" SELECT distinct clgw";
	ls_sql+=" from ";
	ls_sql+=" ( ";
	ls_sql+=" SELECT distinct crm_khxx.clgw";
	...
	while (rs.next())
	{
		...
		<tr align="center"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			...
			<td><%=tckhsl%></td>
			<td>
				<A HREF="ViewClyZcCbQdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
				<%=cf.formatDouble(tccbze)%>				</td>
			<td><%=cf.formatDouble(tcpjde)%></td>
			<td>
				<A HREF="ViewClyZcCbTdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
				<%=cf.formatDouble(tctdcbe)%>				</td>
			<td>
				<A HREF="ViewClyZcCbZjxMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
			...
			<td><%=ddsl%></td>
			<td>
				<A HREF="ViewClyZcQdMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
			...
			<td><%=jc_ddsl%></td>
			<td>
				<A HREF="ViewClyZcQdMx_ddlx.jsp?ddlx=9&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
			...
			<td><%=sk_ddsl%></td>
			<td>
				<A HREF="ViewClyZcQdMx_ddlx.jsp?ddlx=E&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
			...
			<td><%=xp_ddsl%></td>
			<td>
				<A HREF="ViewClyZcQdMx_ddlx.jsp?ddlx=C&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
			...
			<td><%=tw_ddsl%></td>
			<td>
				<A HREF="ViewClyZcQdMx_ddlx.jsp?ddlx=D&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&ssfgs=<%=fgs%>&clgw=<%=getclgw%>" target="_blank">
