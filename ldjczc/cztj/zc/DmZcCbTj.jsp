...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='17030323'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[店面主材统计(成本)]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[店面主材统计(成本)]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
<TR bgcolor="#CCCCCC"  align="center"  > 
  <td  width="4%" >序号</td>
  <td  width="15%" >店面</td>
  <td  width="13%" >小计成本额</td>

  <td  width="7%" >客户数</td>
  <td  width="8%" >订单数</td>
  <td  width="12%" >成本总额</td>
  <td  width="12%" >折后合同额</td>
  <td  width="9%" >平均成本单值</td>
  <td  width="10%" >退单成本额</td>
  <td  width="10%" >增减项成本额</td>
  </tr>
<%
	//获取最大数
	double je=0;
	...
	while (rs2.next())
	{
		...
		while (rs.next())
		{
			...
			double cbze=0;
			double qye=0;
			ls_sql =" SELECT count(distinct khbh) khsl,count(ddbh) ddsl,sum(cbze) cbze,sum(hkze) qye";
			ls_sql+=" FROM jc_zcdd";
			...
			//退单
			double tde=0;
			ls_sql =" SELECT -1*sum(cbze+zjxcbze) je";
			ls_sql+=" FROM jc_zcdd";
			...
			<tr align="center"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
				...
				<td><%=khsl%></td>
				<td><%=ddsl%></td>
				<td><%=cf.formatDouble((long)cbze)%></td>
				<td>
		...
		<tr align="center"  bgcolor="#E8E8FF"  onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			...
			<td><strong><%=cf.formatDouble((long)xjcbzedm)%></strong></td>
...
<tr align="center"  bgcolor="#E8E8FF"  onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
	...
	<td><strong><%=cf.formatDouble((long)xjcbze)%></strong></td>
