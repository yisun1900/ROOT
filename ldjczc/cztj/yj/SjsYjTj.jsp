...
<%
		...
		ls_sql+=wheresql;
		ls_sql+=" group by crm_khxx.sjs";

		ls_sql+=" UNION ALL";

		ls_sql+=" SELECT sjs,0 qys,0 wdzgce,0 qye,0 zhljzjx,0 zqgczjf,0 zhgczjf,0 zqsuijin,0 suijin,0 zqguanlif,0 guanlif  ,0 zqqtsf,0 zhqtsf ,0 zqwjrqyexm,0 zhwjrqyexm ,0 zqsjf,0 sjf    ,0 fwmj";
		ls_sql+=" FROM crm_khxx";
		ls_sql+=" where crm_khxx.zt='3'";//3：退单客户
		ls_sql+=" and crm_khxx.fgsbh='"+fgs+"' ";//首次付家装款时间
		...
		if (sjs.equals(""))
		{
			...
			ls_sql+=" SELECT yhmc sjs,0 qys,0 wdzgce,0 qye,0 zhljzjx,0 zqgczjf,0 zhgczjf,0 zqsuijin,0 suijin,0 zqguanlif,0 guanlif  ,0 zqqtsf,0 zhqtsf ,0 zqwjrqyexm,0 zhwjrqyexm ,0 zqsjf,0 sjf    ,0 fwmj";
			ls_sql+=" FROM sq_yhxx";
		...
		while (rs.next())
		{
			...
			<tr align="center"  bgcolor="<%=bgcolor%>" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
				...
				<td >
					...
					<A HREF="ViewSjsYjMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&sjs=<%=getsjs%>&ssfgs=<%=fgs%>" target="_blank">
					<%=getsjs%>
					</A>
				</td>
				<td ><%=dwmc%></td>
