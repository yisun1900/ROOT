...
try {
	...
	ls_sql+=" select crm_khxx.sgd,sgdmc,sq_sgd.sgdlx,bzs,count(*) sl,sum(zqgczjf) je";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		//回单奖励
		ls_sql=" select count(*),sum(zqgczjf)";
		ls_sql+=" from crm_khxx";
		...
		ls_sql+=" and jlbz='1'";
		//0+正常派单&1+回单奖励&2+公司奖励&3+设计师指定派单&4+客户指定派单&7+特殊情况&9+同小区派单
...
%>
          ...
          <td align="left"><A HREF="ViewSgdcz.jsp?sgd=<%=sgd%>&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>" target="_blank"><%=je%></A></td>
        </tr>
<%
