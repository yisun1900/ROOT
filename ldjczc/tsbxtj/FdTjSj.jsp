...
try {
	...
	String sbyybm=null;
	String sbyymc=null;
	ls_sql="select distinct crm_qysbdj.sbyybm,sbyymc";
	ls_sql+=" FROM crm_qysbdj,dm_sbyybm,crm_zxkhxx  ";
	ls_sql+=" where crm_qysbdj.sbyybm=dm_sbyybm.sbyybm";
	ls_sql+=" and crm_qysbdj.khbh=crm_zxkhxx.khbh(+)";
	ls_sql+=" and crm_qysbdj.zgshjl='Y'";//接待主管审核结论 Y：通过；N：未通过
	ls_sql+=wheresql;
	...
	while (rs1.next())
	{
		...
		ls_sql+=" and crm_qysbdj.zgshjl='Y'";//接待主管审核结论 Y：通过；N：未通过
			...
			ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+startDate+"','YYYY-MM-DD')";
			ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+endDate+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
			ls_sql+=" and crm_qysbdj.sbyybm='"+sbyybm+"'";
			ps= conn.prepareStatement(ls_sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
...
<tr bgcolor="#CCCCCC"  align="center">
  ...
  %>
	...
	String sbkhlx=null;
	ls_sql="select distinct crm_qysbdj.sbkhlx";
	...
	ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
	ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	ls_sql+=" and crm_qysbdj.sbkhlx is not null";
	ls_sql+=" order by crm_qysbdj.sbkhlx";
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	while (rs1.next())
	{
		sbkhlx=rs1.getString("sbkhlx");
		...
		for (int i=0;i<yuesl ;i++ )
		{
			...
			ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+startDate+"','YYYY-MM-DD')";
			ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+endDate+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
			ls_sql+=" and crm_qysbdj.sbkhlx='"+sbkhlx+"'";
			ps= conn.prepareStatement(ls_sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
