...
try {
	...
	String sbyymc=null;
	ls_sql="select distinct crm_qysbdj.sjhfsbyybm,sbyymc";
	ls_sql+=" FROM crm_qysbdj,dm_sbyybm,crm_zxkhxx  ";
	ls_sql+=" where crm_qysbdj.sjhfsbyybm=dm_sbyybm.sbyybm";
	...
	ls_sql+=" order by sjhfsbyybm";
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	while (rs1.next())
	{
		sjhfsbyybm=rs1.getString("sjhfsbyybm");
		...
		for (int i=0;i<yuesl ;i++ )
		{
			...
			ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+startDate+"','YYYY-MM-DD')";
			ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+endDate+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
			ls_sql+=" and crm_qysbdj.sjhfsbyybm='"+sjhfsbyybm+"'";
			ps= conn.prepareStatement(ls_sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
...
<tr bgcolor="#CCCCCC"  align="center">
  ...
  %>
	...
	ls_sql="select distinct crm_qysbdj.sjhfsbkhlx";
	ls_sql+=" FROM crm_qysbdj,crm_zxkhxx  ";
	ls_sql+=" where crm_qysbdj.khbh=crm_zxkhxx.khbh(+)";
	ls_sql+=wheresql;
	ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
	ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	ls_sql+=" and crm_qysbdj.sjhfsbkhlx is not null";
	ls_sql+=" order by crm_qysbdj.sjhfsbkhlx";
	ps1= conn.prepareStatement(ls_sql);
	rs1 =ps1.executeQuery();
	while (rs1.next())
	{
		sjhfsbkhlx=rs1.getString("sjhfsbkhlx");
		...
		for (int i=0;i<yuesl ;i++ )
		{
			...
			ls_sql+=" and crm_qysbdj.sbsj>=TO_DATE('"+startDate+"','YYYY-MM-DD')";
			ls_sql+=" and crm_qysbdj.sbsj<=TO_DATE('"+endDate+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
			ls_sql+=" and crm_qysbdj.sjhfsbkhlx='"+sjhfsbkhlx+"'";
			ps= conn.prepareStatement(ls_sql);
			rs =ps.executeQuery();
			if (rs.next())
			{
