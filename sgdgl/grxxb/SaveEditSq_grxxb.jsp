...
try {
	...
	ls_sql="update sq_grxxb set sgd=?,bzmc=?,xm=?,xb=?,jg=?,csrq=?,gz=?,rzsj=?,dj=?,lx=?,sfzh=?,zz=?,dh=?,pxsm=?,jjlxr=?,jjlxrdh=?,jjlxrdz=?,sgzbh=?,lzsj=?,yhk=?,sfqdlwpqht=?,sfkstg=? ";
	...
	if (!wherebzmc.equals(""))
	{
		ls_sql="update sq_bzxx set grsl=(select count(*) from sq_grxxb where sq_grxxb.sgd=sq_bzxx.sgd and sq_grxxb.bzmc=sq_bzxx.bzmc and sq_grxxb.lx!='离职') ";
