...
<body bgcolor="#FFFFFF">
<form method="post" action="Kp_gdrcjcjlCxList.jsp" name="selectform">
...
<tr bgcolor="#FFFFFF">
  <td align="right" width="18%">检查时间 从</td> 
...
<tr bgcolor="#FFFFFF">
  ...
  <td width="32%"><%
	cf.radioToken(out, "kp_gdrcjcjl_spbz","1+未审批&2+审批","");
%></td>
...
<tr bgcolor="#FFFFFF">
  ...
  <td align="right" width="18%">录入单位</td> 
...
{
	if(!(isDatetime(FormName.kp_gdrcjcjl_jcsj, "检查时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kp_gdrcjcjl_jcsj2, "检查时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kp_gdrcjcjl_spsj, "审批时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kp_gdrcjcjl_spsj2, "审批时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kp_gdrcjcjl_lrsj, "录入时间"))) {
		return false;
	}
	if(!(isDatetime(FormName.kp_gdrcjcjl_lrsj2, "录入时间"))) {
		return false;
	}
	FormName.submit();
	return true;
