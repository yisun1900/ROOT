...
double yqzbj=0;
ls=request.getParameter("yqzbj");
try{
	if (!(ls.equals("")))  yqzbj=Integer.parseInt(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量yqzbj不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[要求质保金]类型转换发生意外:"+e);
	return;
}
...
try {
	...
	ls_sql="update sq_sgd set dh=?,sgdlx=?,dz=?,cz=?,email=?,bz=?,cxbz=?,jjlxr=?,lxrdh=?,sfzhm=?,hjszd=?,duihao=?,sgdjbbm=?,tdbz=?,tdkssj=?,tdzzsj=?,bzs=?,jcf=?,sgdmc=?,sfqldht=?,kjdsm=?,jjlxrdz=?,grs=?,rzsj=?,lzsj=?,yqzbj=?";
