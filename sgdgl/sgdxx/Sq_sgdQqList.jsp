...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT dqmc,sq_dwxx.dwmc ssfgs,sq_sgd.sgd,sgdmc,duihao,sgdjbmc,zbj,bzs,grs,jcf,DECODE(sfqldht,'N','<font color=\"#FF0000\">否</font>','Y','是'),DECODE(tdbz,'Y','<font color=\"#FF0000\">停单</font>','N','不停'),tdkssj,tdzzsj,rzsj,lzsj,DECODE(sq_sgd.cxbz,'Y','<font color=\"#FF0000\">撤销</font>','N','未撤销'),dh,kjdsm,sfzhm,jjlxr,lxrdh,jjlxrdz,hjszd,DECODE(sgdlx,'S0','非直属','S1','直属队') as sgdlx";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_sgdQqList.jsp","","","EditQqSq_sgd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Sq_sgdQqList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(sgd,"sgd"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[5];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from sq_kjdcq where "+chooseitem;
		sql[1]="delete from sq_kjddm where "+chooseitem;
		sql[2]="delete from sq_grdjb where "+chooseitem;
		sql[3]="delete from sq_bzxx where "+chooseitem;
		sql[4]="delete from sq_sgd where "+chooseitem;
