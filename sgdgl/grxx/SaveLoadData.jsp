...
try {
	...
	for (int i=0;i<xm.length ;i++ )
	{
		...
		sql ="insert into sq_grdjb (sgd,bzmc,xm,xb,jg,csrq ";
		sql+="  ,gz,rzsj,dj,lx,sfzh ";
		sql+="  ,zz,dh,jjlxr,jjlxrdh,jjlxrdz,pxsm,sgzbh )";
		sql+=" values('"+sgd[i]+"','"+cf.GB2Uni(bzmc[i])+"','"+cf.GB2Uni(xm[i])+"','"+cf.GB2Uni(xb[i])+"','"+cf.GB2Uni(jg[i])+"',TO_DATE('"+csrq[i]+"','YYYY-MM-DD')";
		sql+=" ,'"+cf.GB2Uni(gz[i])+"',TO_DATE('"+rzsj[i]+"','YYYY-MM-DD'),'"+cf.GB2Uni(dj[i])+"','"+cf.GB2Uni(lx[i])+"','"+cf.GB2Uni(sfzh[i])+"'";
		sql+=" ,'"+cf.GB2Uni(zz[i])+"','"+cf.GB2Uni(dh[i])+"','"+cf.GB2Uni(jjlxr[i])+"','"+cf.GB2Uni(jjlxrdh[i])+"','"+cf.GB2Uni(jjlxrdz[i])+"','"+cf.GB2Uni(pxsm[i])+"','"+cf.GB2Uni(sgzbh[i])+"')";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
		ps.close();
