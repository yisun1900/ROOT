...
*/
	...
	{
		conn.rollback();
		out.println("<BR存盘失败！已接大单数已超标，可接大单数为："+kjdds+"，目前已接大单数为："+yjdds);
		return;
	}
	if (yjzds>kjzds)
	{
		conn.rollback();
		out.println("<BR>存盘失败！已接中单数已超标，可接中单数为："+kjzds+"，目前已接中单数为："+yjzds);
		return;
	}
	if (yjxds>kjxds)
	{
		conn.rollback();
		out.println("<BR>存盘失败！已接小单数已超标，可接小单数为："+kjxds+"，目前已接小单数为："+yjxds);
		return;
	}
