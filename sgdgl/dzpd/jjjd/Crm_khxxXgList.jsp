...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_khxx.khbh,hth,DECODE(pdclzt,'1','申请派单','2','人工派单','5','电子派单','6','特权派单','7','拒绝接单','8','换施工队','4','不许修改派单') pdclzt,DECODE(kgsjqd,'Y','已确定','N','<font color=\"#FF00FF\">未确定</font>') kgsjqd";
	ls_sql+=" ,pd_pdjl.sgd,sgdmc,crm_khxx.sgbz,crm_khxx.zjxm,DECODE(crm_khxx.jlbz,'0','人工派单','1','特权-回单奖励','2','特权-公司奖励','3','特权-设计师指定派单','4','特权-客户指定派单','5','大单抵小单','6','小单抵大单','7','特权-特殊情况','8','电子派单','9','特权-同小区派单') jlbz,crm_khxx.pdsm,crm_khxx.pdsj,crm_khxx.pdr,DECODE(pd_pdjl.shzzb,'Y','占指标','N','不占') shzzb,pd_pdjl.sgdyxj,jyjdrq,DECODE(hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他') hdbz,khxm,fwdz";
	ls_sql+=" ,sjs,sq_dwxx.dwmc sqdm";
	ls_sql+=" FROM crm_khxx,sq_dwxx,sq_sgd,pd_pdjl ";
    ls_sql+=" where crm_khxx.dwbh=sq_dwxx.dwbh(+) and pd_pdjl.sgd=sq_sgd.sgd(+)  and crm_khxx.zt='2'";
    ls_sql+=" and crm_khxx.pdsj is not null and crm_khxx.gcjdbm!='5' ";
	ls_sql+=" and crm_khxx.khbh=pd_pdjl.khbh(+)";
	ls_sql+=" and pd_pdjl.hfjl='0'";
	ls_sql+=wheresql;
    ls_sql+=" order by crm_khxx.pdsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxXgList.jsp","SelectXgCrm_khxx.jsp","","InsertPd_sgdjdjl.jsp");
	pageObj.setPageRow(15);//设置每页显示记录数

//设置显示列
	String[] disColName={"hth","khxm","pdclzt","sgdmc","sgbz","zjxm","jlbz","pdsj","pdr","shzzb","sgdyxj","pdsm","kgsjqd","jyjdrq","hdbz","fwdz","sjs","sqdm"};
...
<CENTER >
  <B><font size="3">拒绝接单（注意：已完结不能再修改） </font></B>
</CENTER>
<%
	pageObj.out=out;
