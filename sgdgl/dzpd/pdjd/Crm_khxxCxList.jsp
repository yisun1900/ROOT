...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.qye,DECODE(crm_khxx.pdclzt,'1','申请派单','2','人工派单','5','电子派单','6','特权派单','7','拒单','8','换施工队','4','不许修改派单') pdclzt";
	ls_sql+=" ,DECODE(crm_khxx.kgsjqd,'Y','已确定','N','<font color=\"#FF00FF\">未确定</font>') kgsjqd,sgdmc,pd_pdjl.sgbz,pd_pdjl.zjxm";
	ls_sql+=" ,DECODE(pd_pdjl.jlbz,'0','人工派单','1','回单奖励','2','公司奖励','3','设计师指定派单','4','客户指定派单','5','大单抵小单','6','小单抵大单','7','特殊情况','8','电子派单','9','同小区派单') jlbz";
	ls_sql+=" ,pd_pdjl.pdsm,pd_pdjl.pdsj,pd_pdjl.pdr,DECODE(pd_pdjl.shzzb,'Y','占指标','N','不占') shzzb,TO_CHAR(pd_pdjl.sgdyxj) sgdyxj,jyjdrq";
	ls_sql+=" ,DECODE(crm_khxx.hdbz,'1','非回单','2','设计师回单','3','施工队回单','4','老客户回单','5','员工回单','6','班长回单','7','售楼处','8','材料商','9','其他') hdbz";
	ls_sql+=" ,khxm,fwdz,sjs,sq_dwxx.dwmc,DECODE(pd_pdjl.hfjl,'0','接受','3','拒单','5','换队') hfjl,DECODE(pd_pdjl.kppdbz,'1','非考评派单','2','考评派单') kppdbz,TO_CHAR(pd_pdjl.xuhao) xuhao";
	ls_sql+=" ,pd_pdjl.jrpde,pd_pdjl.kcpde,DECODE(pd_pdjl.kczbbl,'0','不扣指标','1','扣除单倍派单额','2','扣除2倍派单额','3','扣除3倍派单额') kczbbl";
	ls_sql+=" ,pd_pdjl.jjyy,pd_pdjl.hfr,pd_pdjl.hfsj,deflmc,pd_pdjl.kpjlh";
	ls_sql+=" FROM crm_khxx,sq_dwxx,sq_sgd,pd_pdjl,kp_deflbm ";
    ls_sql+=" where pd_pdjl.khbh=crm_khxx.khbh and crm_khxx.dwbh=sq_dwxx.dwbh(+) and pd_pdjl.sgd=sq_sgd.sgd(+)  ";
	ls_sql+=" and pd_pdjl.deflbm=kp_deflbm.deflbm(+) and kp_deflbm.fgsbh=crm_khxx.fgsbh";
	ls_sql+=wheresql;
    ls_sql+=" order by pd_pdjl.xuhao desc,pd_pdjl.pdsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
...
//设置显示列
	String[] disColName={"hth","khxm","qye","pdclzt","sgdmc","sgbz","jlbz","shzzb","sgdyxj","hfjl","pdsj","pdr","pdsm","jrpde","deflmc","kpjlh","kczbbl","kcpde","jjyy","hfr","hfsj","kppdbz","kgsjqd","jyjdrq","hdbz","khbh","fwdz","sjs","zjxm","dwmc","xuhao"};
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="4%">大小单</td>
	<td  width="2%">考评记录号</td>
	<td  width="3%">拒单扣除派单指标比例</td>
	<td  width="3%">扣除派单额</td>
	<td  width="9%">拒单原因</td>
	<td  width="2%">拒单录入人</td>
	<td  width="3%">拒单录入时间</td>
	<td  width="3%">考评派单标志</td>
	<td  width="2%">开工时间</td>
	<td  width="3%">交底日期</td>
	<td  width="2%">回单标志</td>
	<td  width="2%">客户编号</td>
