...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		pdr=cf.GB2Uni(pdr);
		if (!(pdr.equals("")))	wheresql+=" and  (crm_khxx.pdr='"+pdr+"')";
	}
    ...
    ls_sql+=" where crm_khxx.dwbh=sq_dwxx.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and crm_khxx.zt='2'";
    ls_sql+=" and crm_khxx.pdclzt in('5','6','8') and crm_khxx.gcjdbm!='5' ";//1：申请派单；2：人工派单；5：电子派单；6：特权派单；7：拒单；8：换施工队；4：不许修改派单
	ls_sql+=wheresql;
    ls_sql+=" order by crm_khxx.pdsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxCxList.jsp","SelectCxCrm_khxx.jsp","","EditCrm_khxxMain.jsp");
	pageObj.setPageRow(15);//设置每页显示记录数

//设置显示列
	String[] disColName={"hth","khxm","pdclzt","sgdmc","sgbz","zjxm","jlbz","pdsj","pdr","pdsm","kgsjqd","jyjdrq","hdbz","fwdz","sjs","sqdm"};
...
<CENTER >
  <B><font size="3">换施工队（注意：已完结不能再修改） </font></B>
</CENTER>
<%
	pageObj.out=out;
