...
<%
	...
	ls_sql =" select *";
	ls_sql+=" from ( ";
	ls_sql+=" select sgdmc,kp_yzfbzpdkpjg.bzmc,(NVL(-1*sq_sgd.sgdpdjf,0)+NVL(sq_sgd.sgdwgjf,0)+NVL(sq_sgd.pdkpf,0)+(NVL(sq_sgd.zdzsgds,0)-NVL(sq_sgd.mqzsgds,0))) dzzf,(NVL(-1*sq_bzxx.bzpdjf,0)+NVL(sq_bzxx.bzwgjf,0)+NVL(sq_bzxx.pdkpf,0)+(NVL(sq_bzxx.zdzsgds,0)-NVL(sq_bzxx.mqzsgds,0))) bzzf";
	...
	while (rs.next())
	{
		...
		txqsgd+="[施工队:"+sgdmc+"、班长:"+bzmc+"]";
	}
	rs.close();
	ps.close();
	if (txqsgd.equals(""))
	{
		txqsgd="小区【"+xqbm+"】没有施工队";
	}
	...
	while (rs.next())
	{
		...
		tcqsgd+="[施工队:"+sgdmc+"、班长:"+bzmc+"]";
	}
	rs.close();
	ps.close();
	if (tcqsgd.equals(""))
	{
		tcqsgd="城区【"+cqmc+"】没有施工队";
	}
...
%>
		  ...
		  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
            ...
            <tr> 
              ...
              <td width="36%" bgcolor="#FFFFFF"> <font color="#000000"><%=cqmc%> </font></td>
...
%></font></td>
            ...
            <tr>
              ...
              <td colspan="3" bgcolor="#FFFFCC">
				<input name="xqpd" type="button" onClick="changePdlx(editform,'1');pdsm.value='同小区派单';xqpd.disabled=true;cqpd.disabled=false;zcpd.disabled=false"  value="选择同小区班长">
				<input name="cqpd" type="button" onClick="changePdlx(editform,'2');pdsm.value='同城区派单';xqpd.disabled=false;cqpd.disabled=true;zcpd.disabled=false"  value="选择同城区班长">
				<input name="zcpd" type="button" onClick="changePdlx(editform,'3');pdsm.value='正常派单';xqpd.disabled=false;cqpd.disabled=false;zcpd.disabled=true"  value="正常派单"></td>
            </tr>
            <tr>
              <td bgcolor="#FFFFCC" align="right">同小区施工队</td>
              <td colspan="3" bgcolor="#FFFFCC"><%=txqsgd%></td>
            </tr>
            <tr>
              <td bgcolor="#FFFFCC" align="right">同城区施工队</td>
              <td colspan="3" bgcolor="#FFFFCC"><%=tcqsgd%></td>
            </tr>
            <tr> 
...
<%
	...
	cf.ajax(out);
%>
function changePdlx(FormName,lx)
{
	FormName.sgbz.length=1;


	var actionStr="getPdSgd.jsp?khbh=<%=khbh%>&lx="+lx;
