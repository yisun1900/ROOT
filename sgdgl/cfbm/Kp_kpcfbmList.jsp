...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Kp_kpcfbmList.jsp","SelectKp_kpcfbm.jsp","","EditKp_kpcfbm.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Kp_kpcfbmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(kpcfbm,"kpcfbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from kp_kpcfbm where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="12%">&nbsp;</td>
	<td  width="24%">违规处罚编码</td>
	<td  width="44%">违规处罚名称</td>
</tr>
<%
	pageObj.displayDate();
	pageObj.displayFoot();
