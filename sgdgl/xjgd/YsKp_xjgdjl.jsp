...
try {
	...
	ls_sql="select xjjlh,khbh,xjr,xjsj,xjlxbm,xjjgbm,fkze,kcjdebl,kfzs,kcjdebl,lrr,lrsj,kp_xjgdjl.bz ,sgdmc,kp_xjgdjl.zpml";
...
<form method="post" action="SaveYsKp_xjgdjl.jsp" name="editform" target="_blank">
<div align="center">整改问题－验收(处理记录号:<%=zgcljlh%>)</div>
...
%></td>
	...
	<tr bgcolor="#FFFFFF">
	  ...
	  <td><%
	cf.radioToken(out, "cljg","5+验收通过&6+验收未通过","");
%></td>
	  <td align="right">&nbsp;</td>
	  <td>&nbsp;</td>
    </tr>
	<tr bgcolor="#FFFFFF"> 
	  <td width="18%" align="right"> 
		<font color="#FF0000">*</font>验收人              </td>
	  <td width="32%"> 
		<input type="text" name="clr" size="20" maxlength="20"  value="<%=yhmc%>" readonly>              </td>
	  <td width="18%" align="right"> 
		<font color="#FF0000">*</font>验收时间              </td>
	  <td width="32%"> 
		<input type="text" name="clsj" size="20" maxlength="10"  value="<%=cf.today()%>" readonly>              </td>
	</tr>
	
	<tr bgcolor="#FFFFFF"> 
	  <td width="18%" align="right"><font color="#FF0000">*</font>验收说明</td>
	  <td colspan="3"> 
		<textarea name="clsm" cols="71" rows="4"></textarea>              </td>
	</tr>
	<tr> 
	  ...
	  <input type="reset"  value="重输">
                <input name="sczp" type="button" disabled onClick="f_sczp(editform)"  value="上传照片">
                <input name="ckzp" type="button" disabled onClick="f_ckzp(editform)"  value="查看照片">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isDatetime(FormName.clsj, "验收时间"))) {
		return false;
	}
	if(	!radioChecked(FormName.cljg)) {
		alert("请选择[验收结果]！");
		FormName.cljg[0].focus();
		return false;
	}
...
function f_sczp(FormName)//参数FormName:Form的名称
{
	...
	{
		var urlStr="XmloadFilePad.jsp?xjjlh=<%=xjjlh%>&lx=5&zpgjz=<%=zgcljlh%>&zpml=<%=zpml%>&zpsm="+encodeURI("验收照片");
		window.open(urlStr);
	}
	else
	{
		var urlStr="XmloadFile.jsp?xjjlh=<%=xjjlh%>&lx=5&zpgjz=<%=zgcljlh%>&zpml=<%=zpml%>&zpsm=验收照片";
		window.open(urlStr);
	}
}
...
{
	var urlStr="viewXmPhoto.jsp?xjjlh=<%=xjjlh%>&lx=5&zpgjz=<%=zgcljlh%>&zpsm="+encodeURI("验收照片");
	window.open(urlStr);
}
