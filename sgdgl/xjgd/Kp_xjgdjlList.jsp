...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT kp_xjgdjl.xjjlh,DECODE(kp_xjgdjl.cfspbz,'1','不需审批','2','未审批','3','审批通过','4','审批未通过','5','审批确认'),DECODE(kp_xjgdjl.wtclbz,'B','无整改','N','整改未接收','C','在处理','Y','已解决'),fkze,kfzs,kp_xjgdjl.kcjdebl||'%',sgdmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,crm_khxx.zjxm,crm_khxx.hth,kp_xjgdjl.xjr,kp_xjgdjl.xjsj,xjlxmc,xjjgmc,kp_xjgdjl.bz  ";
    ...
    ls_sql+=" and kp_xjgdjl.cfspbz in('1','2')";//1：无处罚；2：未审批；3：审批通过；4：审批未通过
    ls_sql+=" and kp_xjgdjl.wtclbz in('B','N')";//B：无整改；N：整改未接收；C：在处理；Y：已解决
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Kp_xjgdjlList.jsp","","","EditKp_xjgdjl.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Kp_xjgdjlList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(xjjlh,"xjjlh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[5];//要执行的SQL
		boolean needCommit=true;//不需要事务处理
		sql[0]="delete from kp_xjgdmx where "+chooseitem;
		sql[1]="delete from kp_xjgdjlzp where "+chooseitem;
		sql[2]="delete from kp_zggdmx where "+chooseitem;
		sql[3]="delete from kp_zgclgdmx where "+chooseitem;
		sql[4]="delete from kp_xjgdjl where "+chooseitem;
...
<CENTER >
  <B><font size="3">项目经理检查记录--维护</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="4%">&nbsp;</td>
	<td  width="4%">检查记录号</td>
	<td  width="4%">奖罚审批标志</td>
	<td  width="4%">整改处理标志</td>
	<td  width="4%">罚款总额</td>
	<td  width="4%">扣分</td>
