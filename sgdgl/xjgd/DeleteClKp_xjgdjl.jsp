...
try {
	...
	for (int i=0;i<zgcljlh.length ;i++ )
	{
		...
		if (!getclzt.equals("3") && !getclzt.equals("4"))//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
		{
			conn.rollback();
		...
		if (zgclsl>0)
		{
			...
			ls_sql="update kp_zggdmx set clzt='"+setclzt+"',zgcljlh=?,clr=?,clsj=?,clsm=? ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
			ls_sql+=" where zgjlh='"+zgjlh+"' ";
			ps= conn.prepareStatement(ls_sql);
			...
			ls_sql="update kp_zggdmx set clzt='"+setclzt+"',zgcljlh=?,clr=null,clsj=null,clsm=null ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
			ls_sql+=" where zgjlh='"+zgjlh+"' ";
			ps= conn.prepareStatement(ls_sql);
			ps.setString(1,maxzgcljlh);
			ps.executeUpdate();
