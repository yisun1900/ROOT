...
try {
	...
	ls_sql="select ygbh";
	ls_sql+=" from sq_yhxx";
	ls_sql+=" where  sfzszg in('Y','N') and dwbh='ZJG25' and yhmc='"+sgdmc+"'  ";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
...
//检查记录号
	ls_sql="select NVL(max(substr(zgjlh,11,2)),0)";
	...
	zgjlh=xjjlh+cf.addZero(count+1,2);
...
%>
<body bgcolor="#FFFFFF">
<form method="post" action="SaveInsertKp_zggdmx.jsp" name="insertform" target="_blank">

          <center>整改记录号:<%=zgjlh%></center>
		  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
            <tr bgcolor="#FFFFFF"> 
              <td width="17%" align="right"><span class="STYLE1">*</span>要求解决时间</td>
              <td width="33%"><input name="yqjjsj" type="text" onDblClick="JSCalendar(this)" value="<%=cf.today()%>" size="20" maxlength="10"></td>
              <td width="19%" align="right"><span class="STYLE1">*</span>检查记录号</td>
              <td width="31%">
				  <input type="text" name="xjjlh" value="<%=xjjlh%>"  size="20" readonly>
					<input type="hidden" name="zgjlh" value="<%=zgjlh%>" >			  </td>
            </tr>
...
%>
              ...
              <td><select name="ygbh" style="FONT-SIZE:12PX;WIDTH:152PX">
                <option value=""></option>
                 <%
//	sql="select ygbh,yhmc||'('||xzzwbm||')' from sq_yhxx where sq_yhxx.sfzszg in('Y','N') and dwbh='ZJG25' and yhmc='"+sgdmc+"' order by yhmc";
	sql="select ygbh,yhmc||'('||xzzwbm||')' from sq_yhxx where sq_yhxx.sfzszg in('Y','N') and yhmc='"+sgdmc+"' order by yhmc";

	cf.selectItem(out,sql,sgdbh);
%>
             </select></td>
            </tr>
            <tr bgcolor="#FFFFFF"> 
              ...
              <td colspan="3"> 
                <textarea name="zgsm" cols="74" rows="5" id="zgsm"></textarea>              </td>
            </tr>
            ...
            <tr align="center"> 
              <td colspan="4"> 
                ...
                <input type="button" onClick="window.location='InsertKp_zggdmx.jsp?xjjlh=<%=xjjlh%>'"  value="录入下一条">
                <input name="zgxm" type="hidden"  value="0" size="75" maxlength="100"></td>
            </tr>
          </table>
</form>
...
function getZrbm(FormName)
{
	...
	if (FormName.fkbm.value=='ZJG25')//工程管理中心
	{
