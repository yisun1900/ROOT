...
try {
	...
	for (int i=0;i<zgcljlh.length ;i++ )
	{
		...
		if (!getclzt.equals("2"))//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
		{
			conn.rollback();
			out.println("错误！整改未接收，不能删除");
			return;
		}
		...
		ls_sql="update kp_zggdmx set clzt='1',zgcljlh=?,jsr=null,jssj=null,jssm=null ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
		ls_sql+=" where zgjlh='"+zgjlh+"' ";
		ps= conn.prepareStatement(ls_sql);
