...
try {
	...
	ls_sql="select zgjlh,xjjlh,cljg,clr,clsj,clsm";
...
<form method="post" action="SaveEditYsKp_xjgdjl.jsp" name="editform" target="_blank">
...
%></td>
	...
	<tr bgcolor="#FFFFFF">
	  ...
	  <td><%
	cf.radioToken(out, "cljg","5+验收通过&6+验收未通过",cljg);
%></td>
	  <td align="right">&nbsp;</td>
	  <td>&nbsp;</td>
    </tr>
	...
	<tr bgcolor="#FFFFFF"> 
	  ...
	  <td colspan="3"> 
		<textarea name="clsm" cols="71" rows="4"><%=clsm%></textarea>              </td>
	</tr>
	<tr> 
	  ...
	  <input type="reset"  value="重输">
				...
				<input name="sczp" type="button" onClick="f_sczp(editform)"  value="上传照片">
                <input name="ckzp" type="button" onClick="f_ckzp(editform)"  value="查看照片">
...
function f_sczp(FormName)//参数FormName:Form的名称
{
	...
	{
		var urlStr="XmloadFilePad.jsp?xjjlh=<%=xjjlh%>&lx=5&zpgjz=<%=zgcljlh%>&zpsm="+encodeURI("验收照片");
		window.open(urlStr);
	}
	else
	{
		var urlStr="XmloadFile.jsp?xjjlh=<%=xjjlh%>&lx=5&zpgjz=<%=zgcljlh%>&zpsm=验收照片";
		window.open(urlStr);
	}
}
