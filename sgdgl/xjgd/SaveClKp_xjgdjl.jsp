...
try {
	...
	if (!getclzt.equals("2") && !getclzt.equals("3"))//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
	{
	...
	ls_sql="update kp_zggdmx set clzt='"+cljg+"',zgcljlh=?,clr=?,clsj=SYSDATE,clsm=?  ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
	ls_sql+=" where zgjlh='"+zgjlh+"' ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,zgcljlh);
	ps.setString(2,clr);
	...
	if (cljg.equals("4"))//4：已解决
	{
		//发送短信
		ybl.dxxt.Dxxt dxxt=new ybl.dxxt.Dxxt("hmy");
		String retStr=dxxt.jcgdClwc(conn,xjjlh,clr);//调用发送短信函数
		if (retStr.equals(""))
		{
			out.println("短信发送成功");
		}
