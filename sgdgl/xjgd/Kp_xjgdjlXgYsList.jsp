...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT zgjlh,decode(kp_zggdmx.clzt,'1','未接收','2','已接受','3','在处理','4','已解决','5','验收通过','6','验收未通过')  clzt,kp_xjgdjl.xjjlh,kp_zggdmx.yqjjsj,kp_zggdmx.zgcljlh,kp_zggdmx.ysr,kp_zggdmx.yssj,sgdmc,crm_khxx.khxm,crm_khxx.fwdz,kp_zggdmx.yssm,kp_zggdmx.zgxm,dwmc,kp_zggdmx.fkbmjsr,crm_khxx.sjs,crm_khxx.zjxm,kp_zggdmx.zgsm,xjlxmc,crm_khxx.khbh";
	ls_sql+=" FROM kp_xjgdjl,crm_khxx,sq_sgd,kp_xjlxbm,kp_zggdmx,sq_dwxx  ";
    ...
    ls_sql+=" and kp_zggdmx.clzt in ('5','6') ";//1:未接收；2：接收；3：在处理；4：已解决；5：验收通过；6：验收未通过
    ls_sql+=" and kp_xjgdjl.wtclbz in('Y','C')  ";//B：无整改；N：整改未接收；C：在处理；Y：已解决
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by kp_zggdmx.yssj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Kp_xjgdjlXgClList.jsp","","","EditYsKp_xjgdjl.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteYsKp_xjgdjl.jsp?"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">整改问题－验收维护</font></B>
</CENTER>
<%
	pageObj.out=out;
