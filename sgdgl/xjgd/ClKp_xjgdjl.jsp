...
<form method="post" action="SaveClKp_xjgdjl.jsp" name="editform" target="_blank">
<div align="center">整改问题－处理(处理记录号:<%=zgcljlh%>)</div>
...
%></td>
	...
	<tr bgcolor="#FFFFFF">
	  <td align="right"><font color="#FF0000">*</font>处理结果</td>
	  <td><%
...
function f_sczp(FormName)//参数FormName:Form的名称
{
	...
	{
		var urlStr="XmloadFilePad.jsp?xjjlh=<%=xjjlh%>&lx=4&zpgjz=<%=zgcljlh%>&zpml=<%=zpml%>&zpsm="+encodeURI("整改处理照片");
		window.open(urlStr);
	}
	else
	{
		var urlStr="XmloadFile.jsp?xjjlh=<%=xjjlh%>&lx=4&zpgjz=<%=zgcljlh%>&zpml=<%=zpml%>&zpsm=整改处理照片";
		window.open(urlStr);
	}
}
...
{
	var urlStr="viewXmPhoto.jsp?xjjlh=<%=xjjlh%>&lx=4&zpgjz=<%=zgcljlh%>&zpsm="+encodeURI("整改处理照片");
	window.open(urlStr);
}
