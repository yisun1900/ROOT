...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Dm_rcjclxbmList.jsp","","","EditDm_rcjclxbm.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Dm_rcjclxbmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] rcjclxbm = request.getParameterValues("rcjclxbm");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(rcjclxbm,"rcjclxbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from dm_rcjclxbm where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="12%">&nbsp;</td>
	<td  width="44%">日常检查类型编码</td>
	<td  width="44%">日常检查类型名称</td>
</tr>
<%
	pageObj.displayDate();
	pageObj.displayFoot();
