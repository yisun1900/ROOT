...
<body bgcolor="#FFFFFF">
<form method="post" action="Cw_sgdwgjsXgShList.jsp" name="selectform">
...
{
	if(!(isFloat(FormName.cw_sgdwgjs_htjcbj, "合同工程基础报价"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_zjxjcbj, "增减项工程基础报价"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_ljqyf, "垃圾清运费"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_djazf, "灯具安装费"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_jsjs, "结算基数"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_jsbl, "结算比例"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_tzjsbl, "调整结算比例"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_sdlje, "水电路金额"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_sdljsbl, "水电路结算比例"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_jsje, "结算总额"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_clf, "材料费"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_clyf, "材料运费"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_gdyp, "工地用品"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_yfgck, "已付工程款"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_kzbjjbl, "应扣质保金比例"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_kzbj, "扣质保金"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_clfk, "材料罚款"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_jfje, "奖罚金额"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_sgdwgjs_yfje, "应付金额"))) {
		return false;
	}
	if(!(isDatetime(FormName.cw_sgdwgjs_jsrq, "结算日期"))) {
		return false;
	}
	if(!(isDatetime(FormName.cw_sgdwgjs_lrsj, "录入时间"))) {
		return false;
	}
	FormName.submit();
	return true;
