...
try {
	...
	ls_sql="update cw_sgdwgjs set jsjlh=?,khbh=?,sgd=?,sgbz=?,jsjs=?,jsbl=?,tzjsbl=?,sdlje=?,sdljsbl=?,jsje=?,clf=?,clyf=?,gdyp=?,yfgck=?,kzbjbl=?,clfk=?,yfje=?,jsrq=?,clzt=?,lrr=?,lrsj=?,lrbm=?,bz=?,kzbj=?,htje=?,zjxje=?,qtf=?,qtfsm=?,ssk=?,jsjsze=?,sdljsfs=?,sfkclk=?,sfkgdyp=?,sfkyfgf=?,sfkzbj=?,kzbjjs=?,fkje=?,jlje=?,qtje=? ";
