...
try {
	...
	ls_sql="update crm_khxx set khjsje=?,bxkh=?";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,jsze);
	ps.setString(2,bxkh);
	...
	ls_sql="update cw_jrkhjs set htsjf=?,sssjf=?,wdzgce=?,qye=?,zqljzjx=?,zhljzjx=?,zqhj=?,zhhj=?,jsze=?,yjzxk=?,dj=?,yjwk=?,yhje=?,fqbl=?,fqje=?,djfxje=?,dyq=?,yhze=?,shfqje=?,yjzck=?,zcddjezq=?,zcddyh=?,dgzcjezq=?,dgzcyh=?,zcjsje=?,pck=?,khbjk=?,lrr=?,lrsj=?,lrbm=?,bz=?,zctcze=? ";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
