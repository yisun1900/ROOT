...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String sfzk=request.getParameter("sfzk");
	if (sfzk.equals("1"))
	{
		wheresql+=" and  (cw_khfkjl.zffs!='31')";
	}
	else if (sfzk.equals("2"))
	{
		wheresql+=" and  (cw_khfkjl.zffs='31')";
	}
	...
	if (!(crm_khxx_hth.equals("")))
	{
		...
		ls_sql+=" and cw_khfkjl.czlx='1'";//1：未导出；2：导出凭证；3：凭证导入
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_khfkjlCxList.jsp","","","/cwgl/qyecx/print.jsp");
...
//设置按钮参数
	String[] buttonName={"对账","批量打印"};//按钮的显示名称
	String[] buttonLink={"Cw_khfkjlCxList.jsp","/cwgl/qyecx/PrintCheckSj.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update cw_khfkjl set czlx='2',dcr='"+yhmc+"',dcsj=SYSDATE where "+chooseitem;
