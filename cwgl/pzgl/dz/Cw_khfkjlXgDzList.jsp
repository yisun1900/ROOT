...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (!(crm_khxx_hth.equals("")))
	{
		ls_sql="SELECT cw_khfkjl.fkxh,DECODE(cw_khfkjl.czlx,'1','未导出','2','导出凭证','3','凭证导入'),cw_khfkjl.dcr,cw_khfkjl.dcsj,a.fklxmc fklxbm,fkcsmc,cw_khfkjl.gysbh,cw_khfkjl.fkje,zffsmc,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,DECODE(zt,'2','家装客户','3','退单客户','4','非家装客户','5','设计客户') zt,b.fklxmc zckx,cw_khfkjl.zcgysbh,NVL2(cw_khfkjl.yfkbl,cw_khfkjl.yfkbl||'%',''),cw_khfkjl.yfkje,DECODE(skdd,'1','不需审核','2','需审核'),DECODE(dsksjbz,'N','未审核','Y',' 审核通过','M','审核未通过'),cw_khfkjl.sjbh,cw_khfkjl.skr,cw_khfkjl.sksj,dwmc,cw_khfkjl.sjr,cw_khfkjl.sjsj,DECODE(scbz,'N','未删除','Y','删除'),scr,scsj,DECODE(cw_khfkjl.gljlbz,'N','非关联','Y','关联'),cw_khfkjl.glxh";
		...
		ls_sql+=" and cw_khfkjl.czlx='2'";//1：未导出；2：导出凭证；3：凭证导入
		...
		ls_sql=" SELECT cw_khfkjl.fkxh,DECODE(cw_khfkjl.czlx,'1','未导出','2','导出凭证','3','凭证导入'),cw_khfkjl.dcr,cw_khfkjl.dcsj,a.fklxmc fklxbm,fkcsmc,cw_khfkjl.gysbh,cw_khfkjl.fkje,zffsmc,crm_zxkhxx.hth,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,DECODE(zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') zt,b.fklxmc zckx,cw_khfkjl.zcgysbh,NVL2(cw_khfkjl.yfkbl,cw_khfkjl.yfkbl||'%',''),cw_khfkjl.yfkje,DECODE(skdd,'1','不需审核','2','需审核'),DECODE(dsksjbz,'N','未审核','Y',' 审核通过','M','审核未通过'),cw_khfkjl.sjbh,cw_khfkjl.skr,cw_khfkjl.sksj,dwmc,cw_khfkjl.sjr,cw_khfkjl.sjsj,DECODE(scbz,'N','未删除','Y','删除'),scr,scsj,DECODE(cw_khfkjl.gljlbz,'N','非关联','Y','关联'),cw_khfkjl.glxh";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_khfkjlXgDzList.jsp","","","");
	pageObj.setPageRow(18);
/*
//设置显示列
...
//设置按钮参数
	String[] buttonName={"删除对账"};//按钮的显示名称
	String[] buttonLink={"Cw_khfkjlXgDzList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(fkxh,"fkxh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="update cw_khfkjl set czlx='1',dcr=null,dcsj=null where "+chooseitem;
