...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (!(crm_khxx_hth.equals("")))
	{
		ls_sql="SELECT crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,DECODE(zt,'2','家装客户','3','退单客户','4','非家装客户','5','设计客户') zt,a.fklxmc fklxbm,cw_khfkjl.fkxh,fkcsmc,cw_khfkjl.ddbh,cw_khfkjl.gysbh,cw_khfkjl.fkje,zffsmc,b.fklxmc zckx,cw_khfkjl.zcgysbh,NVL2(cw_khfkjl.yfkbl,cw_khfkjl.yfkbl||'%',''),cw_khfkjl.yfkje,DECODE(skdd,'1','不需审核','2','需审核'),DECODE(dsksjbz,'N','未审核','Y',' 审核通过','M','审核未通过'),cw_khfkjl.sjbh,cw_khfkjl.skr,cw_khfkjl.sksj,a.dwmc,cw_khfkjl.sjr,cw_khfkjl.sjsj,DECODE(scbz,'N','未删除','Y','删除'),scr,scsj,DECODE(cw_khfkjl.gljlbz,'N','非关联','Y','关联'),cw_khfkjl.glxh,cw_khfkjl.shyj,cw_khfkjl.bz  ";
		ls_sql+=" FROM cw_khfkjl,crm_khxx,sq_dwxx a,sq_dwxx b,cw_fklxbm a,cw_fklxbm b,cw_zffsbm,cw_fkcs  ";
		ls_sql+=" where cw_khfkjl.khbh=crm_khxx.khbh(+) and cw_khfkjl.fklxbm=a.fklxbm";
		...
		ls_sql=" SELECT crm_zxkhxx.hth,crm_zxkhxx.khxm,crm_zxkhxx.fwdz,DECODE(zxzt,'0','未分配店面','1','分配店面','2','设计师提交飞单','3','已签约','4','飞单','5','飞单审核未通过') zt,a.fklxmc fklxbm,cw_khfkjl.fkxh,fkcsmc,cw_khfkjl.ddbh,cw_khfkjl.gysbh,cw_khfkjl.fkje,zffsmc,b.fklxmc zckx,cw_khfkjl.zcgysbh,NVL2(cw_khfkjl.yfkbl,cw_khfkjl.yfkbl||'%',''),cw_khfkjl.yfkje,DECODE(skdd,'1','不需审核','2','需审核'),DECODE(dsksjbz,'N','未审核','Y',' 审核通过','M','审核未通过'),cw_khfkjl.sjbh,cw_khfkjl.skr,cw_khfkjl.sksj,a.dwmc,cw_khfkjl.sjr,cw_khfkjl.sjsj,DECODE(scbz,'N','未删除','Y','删除'),scr,scsj,DECODE(cw_khfkjl.gljlbz,'N','非关联','Y','关联'),cw_khfkjl.glxh,cw_khfkjl.shyj,cw_khfkjl.bz  ";
		ls_sql+=" FROM cw_khfkjl,crm_zxkhxx,sq_dwxx a,sq_dwxx b,cw_fklxbm a,cw_fklxbm b,cw_zffsbm,cw_fkcs  ";
		ls_sql+=" where cw_khfkjl.khbh=crm_zxkhxx.khbh(+) and cw_khfkjl.fklxbm=a.fklxbm(+)";
		ls_sql+=" and cw_khfkjl.zffs=cw_zffsbm.zffsbm(+) and cw_khfkjl.zckx=b.fklxbm(+)";
		ls_sql+=" and cw_khfkjl.fkcs=cw_fkcs.fkcs(+)";
		ls_sql+=" and cw_khfkjl.skdw=a.dwbh(+)";
    ...
    ls_sql+=" order by cw_khfkjl.khbh,cw_khfkjl.ddbh,fkxh";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cw_khfkjlFklxList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"修改付款类型"};//按钮的显示名称
	String[] buttonLink={"UpdateFklxCw_khfkjl.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">把【支付方式】转换成【付款类型】</font></B>
</CENTER>
<%
	pageObj.out=out;
