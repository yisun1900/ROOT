...
try {
	...
	ls_sql="update crm_khqye set djjje=?";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,fqje);
	ps.executeUpdate();
	...
	ls_sql="update cw_khjsty set htsjf=?,sssjf=?,wdzgce=?,qye=?,zqljzjx=?,zhljzjx=?,zqhj=?,zhhj=?,jsze=?,yjzxk=?,dj=?,yjwk=?,yhje=?,fqbl=?,fqje=?,djfxje=?,dyq=?,yhze=?,shfqje=?,yjzck=?,zcddjezq=?,zcddyh=?,dgzcjezq=?,dgzcyh=?,zcjsje=?,pck=?,khbjk=?,lrr=?,lrsj=?,lrbm=?,bz=?,zctcze=?,fxjzck=?,shsgfq=?,shzcfq=? ";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setDouble(34,shsgfq);
	ps.setDouble(35,shzcfq);
	ps.executeUpdate();
	ps.close();
