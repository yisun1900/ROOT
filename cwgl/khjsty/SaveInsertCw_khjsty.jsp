...
try {
	...
	ls_sql="update crm_khxx set khjsbz='Y',khjsje=?,khjssj=?,kbxbz='Y',bxkssj=?,bxjzsj=ADD_MONTHS(?,24)";//Y：保修；M：部分保修；N：不保修；0：未办理；1：预办理保修
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,zhgchj);
	ps.setDate(2,lrsj);
	ps.setDate(3,lrsj);
	ps.setDate(4,lrsj);
	ps.executeUpdate();
