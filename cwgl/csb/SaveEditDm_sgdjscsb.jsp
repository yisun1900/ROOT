...
String wherefgsbh=request.getParameter("wherefgsbh");
String wherebfcs=request.getParameter("wherebfcs");
...
try {
	...
	ls_sql="update dm_sgdjscsb set fgsbh=?,bfcs=?,bfjs=?,bfbl=?,sdlbfbl=?,kzbjjs=?,sfwgjs=?,sdljsfs=?,sfkclk=?,sfkgdyp=?,sfkyfgf=?,sfkzbj=?,sgdjbxg=?,kzbjbl=?,zxxs=?,jxxs=?,lkhjbfb=?,zjxjffs=?,ljzjxxs=?,sfkyqpc=?,sfkclfk=?,sfkclyf=?,sfkzckk=?,sfkfkje=?,sfjjlje=?,sfjqtje=?,zjxbhfw=?,zjxsjd=?,sdlqz=?,sfjzcjl=?,zcsfjs=?,zcjehqfs=?,zcjsblkg=?,zcjsbl=?,fkbl=?,sksh=?,gcjdmc=? ";
	ls_sql+=" where fgsbh='"+wherefgsbh+"' and bfcs="+wherebfcs;
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,fgsbh);
	ps.setLong(2,bfcs);
