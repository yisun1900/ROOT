...
String[] kzbjbl=request.getParameterValues("kzbjbl");
...
try {
	...
	ls_sql="delete from dm_kzbjbl  ";
	ls_sql+=" where fgsbh='"+fgsbh+"' and bfcs='"+bfcs+"' ";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();

	for (int i=0;i<kzbjbl.length ;i++ )
	{
		if (kzbjbl[i].equals(""))
		{
			continue;
		}
		ls_sql="insert into dm_kzbjbl (fgsbh,bfcs,zbjqd,zbjzd,kzbjbl)";
		ls_sql+=" values(?,?,?,?,?)";
		ps= conn.prepareStatement(ls_sql);
