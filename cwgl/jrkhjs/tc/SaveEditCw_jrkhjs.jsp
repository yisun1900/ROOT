...
try{
	...
	out.println("<BR>变量[dbbh]不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[地板补货]类型转换发生意外:"+e);
	return;
}
...
try {
	...
	ls_sql="update cw_jrkhjs set htsjf=?,sssjf=?,wdzgce=?,qye=?,zqljzjx=?,zhljzjx=?,zqhj=?,zhhj=?,jsze=?,yjzxk=?,dj=?,yjwk=?,yhje=?,fqbl=?,fqje=?,djfxje=?,dyq=?,yhze=?,shfqje=?,yjzck=?,zcddjezq=?,zcddyh=?,dgzcjezq=?,dgzcyh=?,zcjsje=?,pck=?,khbjk=?,lrr=?,lrsj=?,lrbm=?,bz=?,zctcze=?,fxjzck=?,czbh=?,dbbh=? ";
	ls_sql+=" where  khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	ps.setDouble(34,czbh);
	ps.setDouble(35,dbbh);
	ps.executeUpdate();
	ps.close();
