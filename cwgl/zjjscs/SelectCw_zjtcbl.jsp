...
<tr bgcolor="#FFFFFF">
  ...
  <td align="right" width="15%">质检类型</td> 
  <td width="35%"> 
    <select name="cw_zjtcbl_qdlx" style="FONT-SIZE:12PX;WIDTH:152PX">
    <option value=""></option>
<%
	cf.selectToken(out,"1+主任监理&2+监理","");
%>
    </select>
  </td>
</tr>
...
{
	if(!(isNumber(FormName.cw_zjtcbl_xh, "序号"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_zjtcbl_qd, "起点（>）"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_zjtcbl_zd, "终点（<=）"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_zjtcbl_tcbfb, "提成百分比"))) {
		return false;
	}
	FormName.submit();
	return true;
