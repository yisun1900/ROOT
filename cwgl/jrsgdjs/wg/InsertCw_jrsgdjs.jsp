...
try {
	...
	ls_sql+=" where khbh='"+khbh+"' and lx in('1','2')";//1：工程增减项；2：工程增减项无明细；3：套餐退补货；4：套餐调换产品
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql="select clf,fwmjcz,ysglk,zccz";
	...
	if (rs.next())
	{
		ysclf=rs.getDouble("clf");//材料费
		ysfwmjcz=rs.getDouble("fwmjcz");//地锢金额
		yszccz=rs.getDouble("zccz");//工长套餐主材充值
		ysglk=rs.getDouble("ysglk");//预算工料款
	}
	rs.close();
	ps.close();

	clf=ysclf+yszccz+ysfwmjcz;
	clf=cf.round(clf,0);
	...
	if (lx.equals("2"))//1：非套餐；2：套餐
	{
		...
		zjxje=cf.round(jsglk-zqgczjf,0);
	}
	else{
		jsglk=cf.round(zqgczjf+zjxje,0);
	}

	bcflk=cf.round((jsglk-cfje)*sgebfb/100+fwmj*0.8-clf+yszccz,0);
	...
	kzbj=cf.round(jsglk*kzbjbl/100,0);
	...
	bcrgf=cf.round(jsglk*rgfbfb/100+cfje*0.23-yfrgf,0);

	bcfkhj=bcrgf-kzbj-czbh;
...
<tr bgcolor="#FFFFFF">
  <td align="right"><font color="#FF0000">*</font><span class="STYLE6">扣质保金比例</span></td>
  <td colspan="3"><input type="text" name="kzbjbl" value="<%=kzbjbl%>" size="10" maxlength="9" readonly>
% 
<BR>
<span class="STYLE8">质保金>=10万：扣质保金比例=0 <BR>
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><p>
    <input type="text" name="kzbj" value="<%=kzbj%>" size="20" maxlength="17" readonly>
    <span class="STYLE8">扣质保金=结算工料款*扣质保金比例</span></p>    </td>
  </tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td align="right" width="18%"><font color="#FF0000">*</font><font color="#0000CC">磁砖补货</font></td> 
  <td width="32%"><input name="czbh" type="text" value="<%=czbh%>" size="20" maxlength="17" readonly>
    <strong>
