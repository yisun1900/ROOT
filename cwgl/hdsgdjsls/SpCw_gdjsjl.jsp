...
try {
	...
	ls_sql+=" ,jsr,jssj,sgdfk,sjsfk,dejsje,xbclk,jsfs,cbje,sgdglftd,gdjskze";
	...
	if (rs.next())
	{
		...
		cbje=cf.fillNull(rs.getString("cbje"));
		sgdglftd=cf.fillNull(rs.getString("sgdglftd"));
		gdjskze=cf.fillNull(rs.getString("gdjskze"));
...
%></td>
            ...
            <tr bgcolor="#FFFFFF">
              <td><div align="right">提点基数</div></td>
              <td><select name="tdjs" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="f_jsglf(editform)">
                  <option value=""></option>
                  <%
...
%>
              ...
              <td><div align="right">提点基数金额</div></td>
              <td><input type="text" name="tdjsje" size="20" maxlength="17"  value="<%=tdjsje%>" >
              </td>
            </tr>
            <tr bgcolor="#FFFFFF">
              ...
              <td><input type="text" name="sgdglftd" value="<%=sgdglftd%>" size="8" maxlength="8" onChange="calValue(editform)">
            ...
            <tr bgcolor="#FFFFFF">
              <td align="right">&nbsp;</td> 
              <td>&nbsp;</td>
              <td width="21%" align="right"><font color="#990000">奖励金额</font></td>
              <td width="29%">&nbsp;</td>
            </tr>
...
function f_wxj(FormName)//参数FormName:Form的名称
{
	if (FormName.jsfs[0].checked)
	{
		...
		FormName.wxj.value=round(<%=dejsje%>*FormName.wxjbfb.value/100.0,2);
		FormName.glftd.value=round(100-FormName.jsglftd.value);
	}
}
function f_do(FormName)//参数FormName:Form的名称
{
   ...
   if (FormName.jsfs[1].checked) {
		if (FormName.glftd.value<=0 || FormName.glftd.value>=100)
		{
			alert("错误！[管理费提点]应该在0和100之间,请修改[基价金额]或是[施工队管理费率]");
			FormName.cbje.select();
			return false;
		}
	}
