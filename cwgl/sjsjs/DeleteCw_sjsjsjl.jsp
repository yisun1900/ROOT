...
try {
	...
	for (int i=0;i<jsjlh.length ;i++ )
	{
		...
		while (rs.next())
		{
			...
			if (rs1.next())
			{
				sjsjsjlh=cf.fillNull(rs1.getString("sjsjsjlh"));
			}
			rs1.close();
			ps1.close();
			...
			{
				ls_sql=" update crm_khxx set sjsjsbz='0',sjstcbl=null,qdsyjsje=null,sjsjsjlh=null";
			...
			{
				ls_sql=" update crm_khxx set sjsjsbz='1',sjswgtcbl=null,wgtcje=null,wgjsjlh=null";
				ls_sql+=" where crm_khxx.khbh='"+khbh+"' ";//0：未结算；1：签单结算；2：完工结算
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
			...
			{
				ls_sql=" update crm_khxx set sjsjsbz='"+sjsjsbz+"',sjswgtcbl=null,wgtcje=null,wgjsjlh=null";
				ls_sql+=" where crm_khxx.khbh='"+khbh+"' ";//0：未结算；1：签单结算；2：完工结算
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
