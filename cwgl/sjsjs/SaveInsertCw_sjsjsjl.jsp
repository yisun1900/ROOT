...
try {
	...
	if (khbh!=null)
	{
		for (int i=0;i<khbh.length ;i++ )
		{
			...
			ls_sql="select zt,jzbz,ywy,sjsjsbz,sjstcbl,qdsyjsje,wdzgce,qye,zjxje,zhzjxje,cxhdbm,fgsbh";
			...
			if (rs.next())
			{
				...
				ytcbl=rs.getDouble("sjstcbl");
				ytcje=rs.getDouble("qdsyjsje");
			...
			{
				conn.rollback();
				out.println("错误！设计师结算标志不正确："+khbh[i]);
				return;
			}
...
/*
			if (cxhdbm.equals(""))//无活动
			{
				...
				if (rs.next())
				{
					sjszkl=rs.getDouble("sjszkl");
				}
				else{
					conn.rollback();
					out.println("错误！促销活动："+cxhdbm+"无设计师提成系数");
					return;
				}
				rs.close();
				ps.close();

				tcjs=wdzgce*sjszkl/100;
			}
*/
			...
			{
				qdlx="1";//1：非市场部家装；2：市场部家装；3：工装
			}
			else
			{
				qdlx="2";//1：非市场部家装；2：市场部家装；3：工装
			}
				tcjs=qye;
...
//				return;
			...
			ls_sql=" insert into cw_sjsjsmx (jsjlh,khbh,qdlx,tclx,sjfksbl,tcjs,ytcbl,ytcje,bctcbl  ,bctcje,zjqtfy,sjtcje) ";
			ls_sql+=" values ( ?,?,?,?,0,?,?,?,?  ,?,0,?) ";
			ps= conn.prepareStatement(ls_sql);
			...
			ls_sql=" update crm_khxx set sjsjsbz='1',sjstcbl=?,qdsyjsje=?,sjsjsjlh=?";
			ls_sql+=" where crm_khxx.khbh='"+khbh[i]+"' ";//0：未结算；1：签单结算；2：完工结算
			ps= conn.prepareStatement(ls_sql);
	...
	if (khbh1!=null)
	{
		for (int i=0;i<khbh1.length ;i++ )
		{
			...
			{
				conn.rollback();
				out.println("错误！设计师结算标志不正确："+khbh1[i]);
				return;
			}
			...
			ls_sql=" update crm_khxx set sjsjsbz='2',sjswgtcbl=?,wgtcje=?,wgjsjlh=?";
			ls_sql+=" where crm_khxx.khbh='"+khbh1[i]+"' ";//0：未结算；1：签单结算；2：完工结算
			ps= conn.prepareStatement(ls_sql);
			ps.setDouble(1,bctcbl);
			ps.setDouble(2,bctcje);
	...
	ls_sql="insert into cw_sjsjsjl ( jsjlh,fgsbh,dwbh,sjsbh,sjs,xdsskbfb,sjfw,sjfw2,tcze,lrr  ,lrsj,lrbm,spbz,bz) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?  ,?,?,?,?) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,jsjlh);
