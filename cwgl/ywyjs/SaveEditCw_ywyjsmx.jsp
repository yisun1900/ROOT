...
try {
	...
	for (int i=0;i<sjsxmbm.length ;i++ )
	{
		if (!zjbfbstr[i].trim().equals(""))
		{
			...
			ls_sql=" insert into cw_ywyqtfymx (jsjlh,khbh,sjsxmmc,zjbfb,zjje,bz)";
	...
	ls_sql=" update cw_ywyjsmx set zjqtfy=?,sjtcje=bctcje+?";
	ls_sql+=" where jsjlh='"+jsjlh+"' and khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
	...
	{
		ls_sql=" update crm_khxx set ywyqdjsje=?";
	...
	{
		ls_sql=" update crm_khxx set ywywgjsje=?";
		ls_sql+=" where crm_khxx.khbh='"+khbh+"' ";
		ps= conn.prepareStatement(ls_sql);
		ps.setDouble(1,sjtcje);
		ps.executeUpdate();
