...
//		return;
	...
	if (glxh!=null)
	{
		...
		{
			ls_sql="update cw_khfkjl set sjbh=?,fklxbm=?,gysbh=?,ddbh=?,zjxbh=?,fkje=?,jkr=?,skr=?,sksj=?,skdw=?,bz=?   ,zffs=?,zckx=?,sjfkzc=?,sjr=?,sjsj=?,zzyhkh=? ";
	...
	ls_sql="update cw_khfkjl set sjbh=?,fklxbm=?,fkje=?,jkr=?,skr=?,sksj=?,skdw=?,bz=?,zffs=?,zckx=?   ,sjfkzc=?,zcgysbh=?,zcddbh=?,zczjxbh=?,sjr=?,sjsj=?,zzyhkh=? ";
	ls_sql+=" where fkxh='"+fkxh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,sjbh);
