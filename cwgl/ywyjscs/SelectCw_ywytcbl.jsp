...
{
	if(!(isNumber(FormName.cw_ywytcbl_xh, "序号"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_ywytcbl_qd, "起点（>）"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_ywytcbl_zd, "终点（<=）"))) {
		return false;
	}
	if(!(isFloat(FormName.cw_ywytcbl_tcbfb, "提成百分比"))) {
		return false;
	}
	FormName.submit();
	return true;
