...
String oldwybh=request.getParameter("oldwybh");
...
try {
	...
	{
		int aa=0;
		int len=oldxmbh.length();
		ls_sql="select count(*)";
		ls_sql+=" from  bj_gzbjb";
		ls_sql+=" where dqbm='"+dqbm+"'  and substr(xmbh,1,"+len+")='"+oldxmbh+"'";
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery(ls_sql);
		if (rs.next())
		{
...
*/	
	...
	ls_sql="insert into bj_gzbjb ( dqbm,xmbh,xmmc,xmdlbm,xmxlbm,smbm,jldwbm,bj,gycl,bbj,bgycl,cbj,cgycl,xh,wybh,bjlx,lrr,lrsj,shbz,cbenj,sgdbj,clf,bclf,cclf,ckxm) ";
	ls_sql+=" select ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,cbenj,sgdbj,clf,bclf,cclf,?   ";
	ls_sql+=" from  bj_gzbjb";
	ls_sql+=" where  wybh='"+oldwybh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,dqbm);
	ps.setString(2,xmbh);
	ps.setString(3,xmmc);
