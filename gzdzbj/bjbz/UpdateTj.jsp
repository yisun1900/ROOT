...
String jgzd = request.getParameter("jgzd");
...
try {
	...
	if (jgzd.equals("bj"))
	{
		out.println("修改价格："+dzbjzjmc+"<BR>");
	}
	else if (jgzd.equals("bbj"))
	{
		out.println("修改价格："+dzbjzgjmc+"<BR>");
	}
	else if (jgzd.equals("cbj"))
	{
		out.println("修改价格："+dzbjgjmc+"<BR>");
	}
	...
	for (int i=0;i<wybh.length ;i++ )
	{
		...
		ls_sql+= " set "+jgzd+"=ROUND("+jggs+"),lrr='"+yhdlm+"',lrsj=trunc(SYSDATE)";
		ls_sql+= " where wybh='"+wybh[i]+"'";
//		out.println(ls_sql);
		ps= conn.prepareStatement(ls_sql);
