...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT hy_hyxxb.hybh as hybh, DECODE(hy_hyxxb.sfzf,'Y','作废','N','未作废') as hy_hyxxb_sfzf,hy_hyxxb.hykh as hy_hyxxb_hykh,hy_hyxxb.hyxm as hy_hyxxb_hyxm,hy_hyxxb.hydz as hy_hyxxb_hydz,hy_hyxxb.dh as hy_hyxxb_dh,sq_dwxx.dwmc as sq_dwxx_dwmc,hy_hyxxb.rhrq as hy_hyxxb_rhrq,hy_hyxxb.yxrq as hy_hyxxb_yxrq,hy_hyjbb.hyjb as hy_hyjbb_hyjb, DECODE(hy_hyxxb.fwlx,'1','基本服务','2','增值服务') as hy_hyxxb_fwlx  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Hy_hyxxbList.jsp","SelectHy_hyxxb.jsp","","EditHy_hyxxb.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Hy_hyxxbList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	try {
		...
		for (int i=0;i<hybh.length ;i++ )
		{
			//取水质分析卡号
			String hykh=null;
			ls_sql = "select hykh from hy_hyxxb where hybh='"+hybh[i]+"'";
			...
			//还原
			ls_sql="update hy_khxxb set sfysy='N',hybh=null,syrq=null where hykh='"+hykh+"'";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();

			ls_sql="delete from hy_hyxxb where hykh='"+hybh[i]+"'";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
		}
