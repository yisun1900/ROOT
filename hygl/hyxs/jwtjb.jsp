...
<%
	int[] mark={1,2,2,2,2,0};//设置显示风格参数
	//0:空;1：合并列，无小计；2：数值列，需小计、总计；3：合并列，有小计；4：不合并列；5：列名在小计中显示

	ls_sql="select jwmc,count(*),sum(lsj),sum(bqj),sum(sxj)";
	ls_sql+="  from cp_cpxx1,xt_jwbm ";
	ls_sql+=wheresql;
	ls_sql+=" and cp_cpxx1.jwbm=xt_jwbm.jwbm";
	ls_sql+=" group by jwmc,cp_cpxx1.jwbm";
	ls_sql+=" order by cp_cpxx1.jwbm";
...
<%
//	out.println("<BR>szdw="+szdw);
//	out.println("<BR>pq="+pq);
//	out.println("<BR>szdw"=szdw);
//	oc.printXsJwTable(szdw,xsrq,xsrq2,wheresql,"1",out);
/*
*/
%>
</body>
