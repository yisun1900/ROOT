...
<body bgcolor="#FFFFFF">
<center><B><%=dwmc%>销售（品类）统计表
  <BR>（<%=xsrq%>--<%=xsrq2%>）</B></center>
<table style='FONT-SIZE:12px' width="100%" border="1" cellspacing="0" cellpadding="1">
  <tr> 
    <td width="10%" align="center">品类</td>
    <td width="10%" align="center">数量</td>
    <td width="13%" align="center">零售金额</td>
    <td width="13%" align="center">标签金额</td>
    <td width="13%" align="center">实销金额</td>
    <td width="12%" align="center">金重</td>
  </tr>
<%
	int[] mark={1,2,2,2,2,2};//设置显示风格参数
	//0:空;1：合并列，无小计；2：数值列，需小计、总计；3：合并列，有小计；4：不合并列；5：列名在小计中显示
	ls_sql="select xt_pmbm.pmmc,count(*),sum(lsj),sum(bqj),sum(sxj),sum(jlzl)";
	ls_sql+="from cp_cpxx1,xt_pmbm,xt_plbm,xt_cpdlbm ";
	ls_sql+=wheresql;
	ls_sql+=" and cp_cpxx1.plbm=xt_plbm.plbm(+) ";
	ls_sql+=" and xt_plbm.cpdlbm=xt_cpdlbm.cpdlbm ";
	ls_sql+=" and cp_cpxx1.wlzt='05' and xt_pmbm.pmbm=xt_plbm.pmbm";
	ls_sql+=" group by xt_pmbm.pmmc";
	ls_sql+=" order by xt_pmbm.pmmc";
	/*ls_sql="select xt_pmbm.pmmc,count(*),sum(lsj),sum(bqj),sum(sxj),sum(jlzl)";
	ls_sql+="  from cp_cpxx1,xt_pmbm,v_bk";
	ls_sql+=wheresql;
	ls_sql+=" and cp_cpxx1.pmbm=xt_pmbm.pmbm(+) and cp_cpxx1.bkbh=v_bk.bkbh(+)";
	ls_sql+=" and xt_pmbm.pmmc=xt_pmbm.pmbm";
