...
%>
<center><B><%=dwmc%>销售（款号）统计表
  <BR>（<%=xsrq%>--<%=xsrq2%>）</B><BR></center>
<table style='FONT-SIZE:12px' width="100%" border="1" cellspacing="0" cellpadding="1">
  <tr> 
    <td width="11%" align="center">价位</td>
    <td width="9%" align="center">款号</td>
    <td width="35%" align="center">图  片</td>
    <td width="8%" align="center">数量</td>
    <td width="10%" align="center">零售金额</td>
    <td width="10%" align="center">标签金额</td>
    <td width="10%" align="center">实销金额</td>
    <td width="10%" align="center">备注</td>
  </tr>
<%

	int[] mark={3,1,1,2,2,2,2,0};
	//0:空;1：合并列，无小计；2：数值列，需小计、总计；3：合并列，有小计；4：不合并列；5：列名在小计中显示

	ls_sql="select jwmc,cp_cpxx1.bkbh,'<IMG SRC=\"/bkgl/images/'||UPPER(zpml)||'\" width=\"200\">',count(*),sum(lsj),sum(bqj),sum(sxj)";
	ls_sql+=" from cp_cpxx1,xt_jwbm,v_bk";
	ls_sql+=wheresql;
	ls_sql+=" and cp_cpxx1.jwbm=xt_jwbm.jwbm";
	ls_sql+=" and cp_cpxx1.bkbh=v_bk.bkbh(+)";
	ls_sql+=" group by xt_jwbm.jwbm,jwmc,cp_cpxx1.bkbh,zpml";
	ls_sql+=" order by xt_jwbm.jwbm,count(*) desc";
//	out.println(ls_sql);
	so.addData(ls_sql);
