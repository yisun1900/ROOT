...
//	out.println(ls_sql);
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT crm_khxx.fgsbh dwbh,sq_dwxx.dwmc,count(*) qys,sum(wdzgce) wdzgce,sum(qye) qye,sum(zhljzjx) zhljzjx,sum(zqgczjf) zqgczjf,sum(zhgczjf) zhgczjf,sum(NVL(zqsuijin,0)) zqsuijin,sum(NVL(suijin,0)) suijin,sum(NVL(zqguanlif,0)) zqguanlif,sum(NVL(guanlif,0)) guanlif  ,sum(NVL(zqqtsf,0)) zqqtsf,sum(NVL(zhqtsf,0)) zhqtsf ,sum(NVL(zqwjrqyexm,0)) zqwjrqyexm,sum(NVL(zhwjrqyexm,0)) zhwjrqyexm ,sum(NVL(zqsjf,0)+NVL(sjhtsjf,0)) zqsjf,sum(NVL(sjf,0)+NVL(sjhtsjf,0)) sjf    ,sum(fwmj) fwmj";
	ls_sql+=" FROM crm_khxx,sq_dwxx";
	...
	ls_sql+=" SELECT dwbh,dwmc,0 qys,0 wdzgce,0 qye,0 zhljzjx,0 zqgczjf,0 zhgczjf,0 zqsuijin,0 suijin,0 zqguanlif,0 guanlif  ,0 zqqtsf,0 zhqtsf ,0 zqwjrqyexm,0 zhwjrqyexm ,0 zqsjf,0 sjf    ,0 fwmj";
	ls_sql+=" FROM sq_dwxx";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		ls_sql+=" and crm_khxx.fgsbh='"+dwbh+"' and yjjzsj is not null ";//首次付家装款时间
		...
		<tr align="center"  bgcolor="#FFFFFF" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			<td ><%=row%></td>
			<td >
			<A HREF="ViewFgsYjMx.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=dwbh%>" target="_blank">
			<%=dwmc%>			</A>			</td>
