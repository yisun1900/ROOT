...
<%
	...
	String yhqx=cf.executeQuery("select count(*)  from v_sqxx where yhdlm='"+xtrzyhdlm+"' and cdbh='17030203'");
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[店面业绩统计]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[店面业绩统计]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
<TR bgcolor="#CCCCCC"  align="center"  > 
  ...
  <td  width="3%" >折后－记入业绩其它费</td>

  <td  width="4%" >折前－不记业绩其它费</td>
  <td  width="3%" >折后－不记业绩其它费</td>

  <td  width="3%" >折前－设计费</td>
  <td  width="3%" >折后－设计费</td>
  </tr>
<%
	//获取最大数
	String getfgs=null;
...
//	out.println(ls_sql);
	...
	ls_sql+=" FROM cw_khfqdj,crm_khxx";
	ls_sql+=" where cw_khfqdj.khbh=crm_khxx.khbh";
	ls_sql+=" and crm_khxx.zt in('2','3','5') ";//N：否；Y：是
	ls_sql+=wheresql;
	ls_sql+=" and cw_khfqdj.scbz='N'";
	ls_sql+=wheresql;
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
		ALLfqje=rs.getDouble(1);
	}
	rs.close();
	ps.close();
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT b.dwmc fgs,crm_khxx.dwbh dwbh,a.dwmc dm,count(distinct crm_khxx.khbh) qys,sum(wdzgce) wdzgce,sum(qye) qye,sum(zhljzjx) zhljzjx,sum(zqgczjf) zqgczjf,sum(zhgczjf) zhgczjf,sum(NVL(zqsuijin,0)) zqsuijin,sum(NVL(suijin,0)) suijin,sum(NVL(zqguanlif,0)) zqguanlif,sum(NVL(guanlif,0)) guanlif  ,sum(NVL(zqqtsf,0)) zqqtsf,sum(NVL(zhqtsf,0)) zhqtsf ,sum(NVL(zqwjrqyexm,0)) zqwjrqyexm,sum(NVL(zhwjrqyexm,0)) zhwjrqyexm ,sum(NVL(crm_khxx.zqsjf,0)+NVL(crm_zxkhxx.sjhtje,0)) zqsjf,sum(NVL(crm_khxx.sjf,0)+NVL(crm_zxkhxx.sjhtje,0)) sjf    ,sum(crm_khxx.fwmj) fwmj";
	ls_sql+=" FROM crm_khxx,crm_zxkhxx,sq_dwxx a,sq_dwxx b";
