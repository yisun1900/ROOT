...
<%
	...
	if (yhqx.equals("0"))
	{
		...
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'警告！非法侵入[品牌结算统计]："+xtrzyhmc+"') ";
		...
		String sql="insert into sq_ckxxrz ( xh,ip,yhdlm,yhmc,dlsj,bz ) ";
		sql+=" values ( (select NVL(max(xh),0)+1 from sq_ckxxrz),'"+xtrzip+"','"+xtrzyhdlm+"','"+xtrzyhmc+"',SYSDATE,'进入[品牌结算统计]："+xtrzyhmc+"') ";
		cf.executeUpdate(sql);
	}
%>
...
<TR bgcolor="#CCCCCC"  align="center"  > 
  ...
  <td  width="5%" bgcolor="#66FFFF" ><strong>平均成本</strong></td>
  <td  width="5%" bgcolor="#66FFFF" ><strong>平均结算比例</strong></td>
  <td  width="7%" bgcolor="#66FFFF" ><strong>签单成本</strong></td>
  <td  width="7%" bgcolor="#66FFFF" ><strong>退单成本</strong></td>
  <td  width="7%" bgcolor="#66FFFF" ><strong>增减项成本</strong></td>

  <td  width="7%" >销售小计<BR>(签单-退单+增减项)</td>
  <td  width="4%" >客户数</td>
  <td  width="4%" >订单数</td>
