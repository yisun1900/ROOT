...
{
	sfyx=cf.GB2Uni(sfyx);
	if (!(sfyx.equals("")))	wheresql+=" and  (bj_khzcxm.sfyx='"+sfyx+"')";
}
...
if (ppmc2!=null)
{
	...
	if (!(ppmc2.equals("")))	wheresql+=" and  (bj_khzcxm.ppmc like '%"+ppmc2+"%')";
}
...
<TR bgcolor="#CCCCCC"  align="center"  > 
	...
	<td  width="19%" >产品名称</td>
	<td  width="6%" >是否有效</td>
	<td  width="7%" >有效开始时间</td>
	<td  width="7%" >有效结束时间</td>

	<td  width="5%" >客户数量</td>
	<td  width="5%" >订货数量</td>
	<td  width="147%" >销售金额</td>
</tr>
...
<%
	...
	ls_sql="SELECT bj_khzcxm.ppmc,bj_khzcxm.xh,bj_khzcxm.gg,bj_khzcxm.cpmc,DECODE(sfyx,'1','永远有效','2','无效','3','阶段有效') sfyx,yxkssj,yxjzsj,count(distinct bj_khzcxm.khbh) khsl,sum(bj_khzcxm.xdsl) xdsl,sum(bj_khzcxm.jsj*bj_khzcxm.xdsl) xdje";
	ls_sql+=" FROM crm_khxx,bj_khzcxm";
	...
	ls_sql+=wheresql;
	ls_sql+=" group by bj_khzcxm.ppmc,bj_khzcxm.xh,bj_khzcxm.gg,bj_khzcxm.cpmc,sfyx,yxkssj,yxjzsj";
	ls_sql+=" order by bj_khzcxm.sfyx";
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		getxh=cf.fillHtml(rs.getString("xh"));
		getgg=cf.fillHtml(rs.getString("gg"));
		getcpmc=cf.fillHtml(rs.getString("cpmc"));
		getsfyx=cf.fillHtml(rs.getString("sfyx"));
		getyxkssj=cf.fillHtml(rs.getDate("yxkssj"));
		getyxjzsj=cf.fillHtml(rs.getDate("yxjzsj"));
		...
		<tr bgcolor="#FFFFFF"  align="center">
			<td ><%=row%></td>
			<td ><A HREF="ViewPpmcCrm_khxx.jsp?ppmc=<%=getppmc%>&ssfgs=<%=fgs%>&wheresql=<%=wheresql%>" target="_blank"><%=getppmc%></A></td>
			<td ><%=getxh%></td>
			<td ><%=getgg%></td>
			<td ><%=getcpmc%></td>
			<td ><%=getsfyx%></td>
...
<tr bgcolor="#FFFFFF"  align="center">
	...
	<td  ><%=cf.formatDouble(xjxdsl)%></td>
	<td  ><%=cf.formatDouble(xjxdje)%></td>
</tr>
</table>
