...
try {
	...
	ls_sql=" select count(distinct jc_zcdd.khbh),count(distinct jc_zcddmx.ddbh),sum(jc_zcddmx.sl),sum(jc_zcddmx.je),sum(jc_zcddmx.ycf),sum(jc_zcddmx.qtfyje),sum(NVL(jc_zcddmx.je,0)+NVL(jc_zcddmx.ycf,0)+NVL(jc_zcddmx.qtfyje,0)),sum(ROUND(jc_zcddmx.cbj*jc_zcddmx.sl,2)),sum(jc_zcddmx.cxhdje),sum(jc_zcddmx.zjhsl),sum(jc_zcddmx.zjhje),sum(jc_zcddmx.zjhycf),sum(jc_zcddmx.zjhqtfy),sum(NVL(jc_zcddmx.zjhje,2)+NVL(jc_zcddmx.zjhycf,2)+NVL(jc_zcddmx.zjhqtfy,2)),sum(ROUND(jc_zcddmx.cbj*jc_zcddmx.zjhsl,2)),sum(jc_zcddmx.zjhcxhdje) ";
...
//	out.println(ls_sql);
	...
	if (rs.next())
	{
		...
		htzj=rs.getDouble(7);
		htcb=rs.getDouble(8);
		cxhdje=rs.getDouble(9);

		zjhsl=rs.getDouble(10);
		zjhje=rs.getDouble(11);
		zjhzjhycf=rs.getDouble(12);
		zjhzjhqtfy=rs.getDouble(13);
		zjhzj=rs.getDouble(14);
		zjhcb=rs.getDouble(15);
		zjhcxhdje=rs.getDouble(16);
	}
	rs.close();
	ps.close();
}
...
<%
	...
	if (groupsl==0)
	{
		...
		selectgroupsql=" select "+selectgroupsql.substring(1)+",count(distinct jc_zcdd.khbh),count(distinct jc_zcddmx.ddbh),sum(jc_zcddmx.sl),sum(jc_zcddmx.je),sum(jc_zcddmx.ycf),sum(jc_zcddmx.qtfyje),sum(NVL(jc_zcddmx.je,0)+NVL(jc_zcddmx.ycf,0)+NVL(jc_zcddmx.qtfyje,0)),sum(jc_zcddmx.cxhdje),sum(jc_zcddmx.zjhsl),sum(jc_zcddmx.zjhje),sum(jc_zcddmx.zjhycf),sum(jc_zcddmx.zjhqtfy),sum(NVL(jc_zcddmx.zjhje,2)+NVL(jc_zcddmx.zjhycf,2)+NVL(jc_zcddmx.zjhqtfy,2)),sum(jc_zcddmx.zjhcxhdje) ";
	}
%>
...
%>
	...
	<td  width="6%" >合同材料费</td>
	<td  width="6%" >合同远程费</td>
	<td  width="6%" >合同其它费用</td>
	<td  width="6%" >合同总额</td>
	<td  width="6%" >合同计入活动金额</td>
	<td  width="5%" >增减后数量</td>
	<td  width="6%" >增减后材料费</td>
	<td  width="6%" >增减后远程费</td>
	<td  width="6%" >增减后其它费用</td>
	<td  width="6%" >增减后总额</td>
	<td  width="6%" >增减后计入活动金额</td>
</tr>
<%
