	...
	while (rs.next())
	{
		...
		if (sxbz.equals("2"))//手写合同
		{
			...
			<tr bgcolor="#FFFFFF"  align="center">
				...
				<td >
					<input type="text" name="qtfyje" size="8" maxlength="16"  value="<%=qtfyjeStr%>" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid">			
				</td>
				<td >
...
function f_do(FormName)//参数FormName:Form的名称
{
	if (<%=row%>==1)
	{
		...
		if (FormName.dpzk.value<0 || FormName.dpzk.value>20)
		{
		...
		if (FormName.cxhdbz.value=="Y")
		{
			...
			{
				alert("主材编码["+FormName.zcbm.value+"]出错！[计入促销活动比率]应在0％和300％之间！");
		...
		for (var i=0;i< <%=row%> ;i++ )
		{
			...
			if (FormName.dpzk[i].value<0 || FormName.dpzk[i].value>20)
			{
			...
			if (FormName.cxhdbz[i].value=="Y")
			{
				...
				{
					alert("主材编码["+FormName.zcbm[i].value+"]出错！[计入促销活动比率]应在0％和300％之间！");
