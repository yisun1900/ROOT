...
<TR bgcolor="#CCCCCC"  align="center" style="FONT-WEIGHT: bold; Z-INDEX: 5; POSITION: relative;TOP: expression(this.offsetParent.scrollTop)"> 
	...
	<td  width="2%" bgcolor="#CC99FF" style="POSITION: relative; BACKGROUND-COLOR: #CC6600"><span class="STYLE1">计入促销活动比率</span></td>
	...
	while (rs.next())
	{
		...
		if (sxbz.equals("2"))//手写合同
		{
			...
			<tr bgcolor="#E8E8FF"  align="center">
				...
				<td ><%=sxhtyy%></td>
				<td bgcolor="#FFFFFF">
					<A HREF="EditJc_zcddmxMain.jsp?ddbh=<%=ddbh%>&xh=<%=xh%>" target="_blank">修改手写合同</A>
				</td>
				<td ><%=bz%></td>
				<td ><%=zclbbm%></td>
			</tr>
			...
			<tr bgcolor="#FFFFFF"  align="center">
				...
				<td bgcolor="#CC99FF" >
					...
					%>			</td>
				<td bgcolor="#CC99FF" >
					<input type="text" name="cxhdbl" size="3" maxlength="9"  value="<%=cxhdbl%>" onChange="f_jsjg(editform,<%=row%>)" readonly style="BACKGROUND-COLOR:#CC99FF;border-color: #CC99FF #CC99FF #CC99FF #CC99FF;border-style:solid">%			</td>
				<td bgcolor="#CC99FF">
				...
				<td >
					<A HREF="EditJc_zcddmxMain.jsp?ddbh=<%=ddbh%>&xh=<%=xh%>" target="_blank">修改合同</A>
				</td>
				<td >
	...
	if (<%=row%>==1)
	{
		...
		for (var i=0;i< <%=row%> ;i++ )
		{
			...
			if(	javaTrim(FormName.zqdj[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[折前实销单价]！");
			...
			if(	javaTrim(FormName.dpzk[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[折扣率]！");
			...
			{
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！[折扣率]应该在0和10之间！");
				FormName.dpzk[i].select();
				return false;
			}

			if(	javaTrim(FormName.dj[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[折后实销单价]！");
				FormName.dj[i].focus();
				return false;
			}
			...
			if(	javaTrim(FormName.sl[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[订货数量]！");
				FormName.sl[i].focus();
				return false;
			}
			...
			{
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！[订货数量]不能<=0！");
				FormName.sl[i].select();
				return false;
			}

			if(	javaTrim(FormName.zqje[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[折前材料费]！");
			...
			if(	javaTrim(FormName.je[i])=="") {
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[折后材料费]！");
				FormName.je[i].focus();
				return false;
			}
			...
			{
				alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请选择[其它费用]！");
			...
			if (FormName.qtfy[i].value!="W")
			{
				if(	javaTrim(FormName.qtfyje[i])=="") {
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[其它费金额]！");
				...
				{
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！[其它费金额]不能为0！");
				...
				{
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！没有收[其它费]！");
					FormName.qtfyje[i].select();
					return false;
				}
			}
			...
			if (FormName.cxhdbz[i].value=="Y")
			{
				if(	javaTrim(FormName.cxhdbl[i])=="") {
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！请输入[计入促销活动比率]！");
					FormName.cxhdbl.focus();
					return false;
				}
				...
				{
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！[计入促销活动比率]应在0％和100％之间！");
				...
				if(	javaTrim(FormName.cxhdbl[i])!="" && FormName.cxhdbl[i].value!="0") {
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！不参加促销活动，[计入促销活动比率]应为0！");
					FormName.cxhdbl[i].select();
					return false;
				}
				if(	javaTrim(FormName.cxhdje[i])!="" && FormName.cxhdje[i].value!="0") {
					alert("主材编码["+FormName.zcbm[<%=row%>+i].value+"]出错！不参加促销活动，[计入促销活动金额]应为0！");
					FormName.cxhdbl[i].select();
					return false;
				}
			}
