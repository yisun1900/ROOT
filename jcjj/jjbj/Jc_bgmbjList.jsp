...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT xcmbm,xcmlxmc,bklxxh,mbcl,xbpz,DECODE(jjfs,'1','平米单价','2','平米单价+封边延米单价') jjfs,pmdj,ymdj,zpmc,zsxt,bz,lrr,lrsj  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_bgmbjList.jsp","","","EditJc_bgmbj.jsp");
	pageObj.setPageRow(100);//设置每页显示记录数


//设置主键
	String[] keyName={"xcmbm"};
	pageObj.setKey(keyName);
//设置按钮参数
	String[] buttonName={"删除选中的信息"};//按钮的显示名称
	String[] buttonLink={"Jc_bgmbjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] xcmbm = request.getParameterValues("xcmbm");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(xcmbm,"xcmbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from jc_bgmbj where "+chooseitem;
