...
<%
	...
	ls_sql="SELECT cpbm,jgwzbm,cpmc,cpbm,xh,gg,ppmc,jldw,ydj,tcdj,sl,sfbpx,DECODE(sfbpx,'Y','标配','N','否') sfbpxmc";
	...
	while (rs.next())
	{
		...
		sfbpxmc=cf.fillNull(rs.getString("sfbpxmc"));
		...
		<tr bgcolor="#FFFFF"  align="center">
			<td><A HREF="DeleteBj_khcltcmx.jsp?zjxxh=<%=zjxxh%>&khbh=<%=khbh%>&cpbm=<%=cpbm%>&jgwzbm=<%=jgwzbm%>&tcmc=<%=tcmc%>" target="_blank">删除</A></td>
			<td ><%=jgwzbm%>
