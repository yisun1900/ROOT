...
try {
	...
	double xjsl=0;
	double xjxdsl=0;
	ls_sql="SELECT sum(sl),sum(xdsl)";
	ls_sql+=" FROM bj_khzcxmh  ";
    ls_sql+=" where zjxxh='"+zjxxh+"' and jgwzbm='"+jgwzbm+"' and tccplbbm='"+tccplbbm+"' and cpbm='"+getcpbm+"'";
    ls_sql+=" and zclx in('1','2')";//主材类型  1：套餐内；2：套餐外收费；3：大店
	...
	if (sl!=0)
	{
		ls_sql=" insert into bj_khzcxmh (sxh,zjxxh,khbh,jgwzbm,cpbm,sku,tccpdlbm,tccplbbm,tcsjflbm,cpmc,ppmc,gysmc,xh,gg,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,tcndj,tccldj,dj    ,jsfs,jsj,jsbl,ddcljjfs,sh,sfcscp,bz,sfkgdj,zclx,sl,xdsl,ptcpsm,bjjbbm,oldsxh,lx,ywdhslbz,newzjxxh) ";
		ls_sql+=" select                 ?  ,zjxxh,khbh,jgwzbm,cpbm,sku,tccpdlbm,tccplbbm,tcsjflbm,cpmc,ppmc,gysmc,xh,gg,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,tcndj,tccldj,dj    ,jsfs,jsj,jsbl,ddcljjfs,sh,sfcscp,bz,sfkgdj,zclx,? ,?   ,ptcpsm,bjjbbm,sxh   ,? ,'Y'     ,zjxxh";
		ls_sql+=" from bj_khzcxmh";
		ls_sql+=" where zjxxh='"+zjxxh+"' and sxh="+oldsxh;
		ps= conn.prepareStatement(ls_sql);
		ps.setLong(1,sxh);
	...
	while (rs.next())
	{
		...
		if (sfxzsl.equals("1"))//1:超量需加价;2:不可超量;3:无数量限制
		{
			...
			else if (sjsl<bzsl)//不收费数量<标准数量，(bzsl-sjsl)部分转为套餐内不收费
			{
				...
				ls_sql=" insert into bj_khzcxmh (sxh,zjxxh,khbh,jgwzbm,cpbm,sku,tccpdlbm,tccplbbm,tcsjflbm,cpmc,ppmc,gysmc,xh,gg,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,tcndj,tccldj,dj   ,jsfs,jsj,jsbl,ddcljjfs,sh,sfcscp,bz,sfkgdj,zclx,sl,ptcpsm,bjjbbm,fjddbh,oldsxh,lx,ywdhslbz,newzjxxh) ";
				ls_sql+=" select                 ?+1,zjxxh,khbh,jgwzbm,cpbm,sku,tccpdlbm,tccplbbm,tcsjflbm,cpmc,ppmc,gysmc,xh,gg,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,tcndj,tccldj,tcndj,'1' ,jsj,0   ,'0'     ,sh,sfcscp,bz,sfkgdj,?   ,? ,ptcpsm,bjjbbm,fjddbh,oldsxh,lx,'Y'     ,zjxxh";
				ls_sql+=" from bj_khzcxmh";
				ls_sql+=" where zjxxh='"+zjxxh+"' and sxh='"+getsxh+"'";
				ps1= conn.prepareStatement(ls_sql);
