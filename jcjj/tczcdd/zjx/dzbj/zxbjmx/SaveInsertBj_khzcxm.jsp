...
//		return;
	...
	double xdb=0;
	double sh=0;
	ls_sql="select sfkgdj,dj,tccldj,ppmc,sfxclxs,xdb,sh,tccplbbm";
	...
	if (rs.next())
	{
		...
		sh=rs.getDouble("sh");
		gettccplbbm=rs.getInt("tccplbbm");
	}
	rs.close();
	ps.close();


	if (gettccplbbm==64)//墙漆改壁纸，取当时价格
	{
		//取同级别单价
		double sjdj=0;
		ls_sql="select dj";
		ls_sql+=" from  bj_khzcxmh";
		ls_sql+=" where zjxxh='"+zjxxh+"' and bjjbbm='"+bjjbbm+"' and tccplbbm="+gettccplbbm;
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		{
			sjdj=rs.getDouble("dj");
...
//		return;
	...
	if (sfxzsl.equals("1"))//是否限制数量  1:超量需加价；2：不可超量；3：无数量限制
	{
		if (tcnsjsl>=bzsl)
		{
			...
			//超出标准数量限制，全部按套餐外收费
			ls_sql=" insert into bj_khzcxmh (sxh,zjxxh,khbh,jgwzbm,cpbm,sku,tccpdlbm,tccplbbm,tcsjflbm,cpmc,ppmc,gysmc,xh,gg,jldw,xdjldw,xdb,sfxclxs,sfyx,yxkssj,yxjzsj,tcndj,tccldj,dj    ,jsfs,jsj,jsbl,ddcljjfs,sh,sfcscp,bz,sfkgdj,zclx,sl,xdsl,ptcpsm,bjjbbm,lx ,ywdhslbz,newzjxxh) ";
