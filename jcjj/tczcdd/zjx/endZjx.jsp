...
try {
	...
	double zqtczjje=0;
	double tczjje=0;
	ls_sql="SELECT sum(wdzje),sum(hkze)";
	ls_sql+=" FROM jc_zcdd";
    ls_sql+=" where khbh='"+khbh+"'";
	...
	if (rs.next())
	{
		zqtczjje=rs.getDouble(1);
		tczjje=rs.getDouble(2);
	}
	rs.close();
	ps.close();

	ls_sql="update crm_zjxdj set zqtczjje=?,tczjje=?";
	ls_sql+=" where zjxxh='"+zjxxh+"' ";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,zqtczjje);
	ps.setDouble(2,tczjje);
	ps.executeUpdate();
	ps.close();
