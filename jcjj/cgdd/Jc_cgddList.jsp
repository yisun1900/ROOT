...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and jc_cgdd.clzt in('00','01')";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddList.jsp","","","EditJc_cgdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_cgddList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[1]="delete from jc_cgwjyddmx where "+chooseitem;
		sql[2]="delete from jc_cgyddmx where "+chooseitem;
		sql[3]="delete from jc_cgdd where "+chooseitem;
