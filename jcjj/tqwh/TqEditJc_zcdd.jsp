...
try {
	...
	ls_sql="select sfjs,qtycf,qtf,ycf,ddlx,sfkgsl,dzyy,lrsj,sfyyh,yhkssj,yhjzsj,yhnr,zczkl,jjsjs,htcxhdje,wdzje,zcdlbm,zcxlbm,ppbm,ppmc,gys,ppbm,khbh,clgw,ztjjgw,jkxz,jkdd,ddshbz,kjxsj,xclbz,htshsj,xmzy,qhtsj,jhclsj,bz,hkze,jhtzshsj ";
	ls_sql+=" from  jc_zcdd";
...
%></td>
            ...
            <tr bgcolor="#FFFFCC"> 
              ...
              <td width="18%" align="right" bgcolor="#FFFFCC"><font color="#FF0000">*</font>订单类型</td>
              <td width="32%">
			  <select name="ddlx" style="FONT-SIZE:12PX;WIDTH:152PX">
                <option value=""></option>
                <%
	cf.selectToken(out,"1+代销品&4+有库存订单&5+标准套餐&6+家具健康套餐&7+温暖静音套餐&8+窗帘套餐&9+代购主材&A+顶配升级&B+顶配独立收费&C+选配主材&D+套餐外主材&E+散客",ddlx);
%>
              </select></td>
            </tr>
            <tr bgcolor="#FFFFFF"> 
            ...
            <tr bgcolor="#FFFFFF">
              ...
              <td><select name="ppbm" style="FONT-SIZE:12PX;WIDTH:152PX">
                  <option value=""></option>
                  <%
	cf.selectItem(out,"select distinct jxc_ppgysdzb.ppmc c1,jxc_ppgysdzb.ppmc c2 from jxc_ppgysdzb,jxc_ppxx,jxc_gysxx where jxc_ppxx.ppbm=jxc_ppgysdzb.ppbm and jxc_ppgysdzb.gysbm=jxc_gysxx.gysbm and jxc_ppxx.pplb='1' and jxc_gysxx.cllx='1' and jxc_ppgysdzb.ssfgs='"+ssfgs+"' order by jxc_ppgysdzb.ppmc",ppbm);
%>
                </select>              </td>
              ...
              <td><select name="ppmc" style="FONT-SIZE:12PX;WIDTH:152PX">
                <option value=""></option>
                <%
	cf.selectItem(out,"select distinct jxc_ppxx.scsmc c1,jxc_ppxx.scsmc c2 from jxc_ppgysdzb,jxc_ppxx,jxc_gysxx where jxc_ppxx.ppbm=jxc_ppgysdzb.ppbm and jxc_ppgysdzb.gysbm=jxc_gysxx.gysbm and jxc_ppxx.pplb='1' and jxc_gysxx.cllx='1' and jxc_ppgysdzb.ssfgs='"+ssfgs+"' order by jxc_ppxx.scsmc",ppmc);
%>
              </select></td>
            </tr>
            <tr bgcolor="#FFFFFF">
              ...
              <td><select name="gys" style="FONT-SIZE:12PX;WIDTH:152PX">
                <option value=""></option>
                <%
	cf.selectItem(out,"select distinct jxc_gysxx.gysmc c1,jxc_gysxx.gysmc c2 from jxc_ppgysdzb,jxc_ppxx,jxc_gysxx where jxc_ppxx.ppbm=jxc_ppgysdzb.ppbm and jxc_ppgysdzb.gysbm=jxc_gysxx.gysbm and jxc_ppxx.pplb='1' and jxc_gysxx.cllx='1' and jxc_ppgysdzb.ssfgs='"+ssfgs+"' order by jxc_gysxx.gysmc",gys);
%>
              </select></td>
...
%>
            ...
            <tr bgcolor="#FFFFFF"> 
              <td width="19%"> 
                <div align="right">是否等待送货通知</div>              </td>
              <td width="31%"> 
...
%> 
            ...
            <tr bgcolor="#FFFFFF"> 
              ...
              <td width="18%" align="right"><font color="#FF0000">*</font>是否可改数量</td>
              <td width="32%"><%
	cf.radioToken(out,"sfkgsl","N+否&Y+是",sfkgsl);
%></td>
            </tr>
            
            <tr bgcolor="#FFFFFF">
              <td align="right"><font color="#FF0000">*</font>套餐远程费</td>
              <td colspan="3"><input name="qtycf" type="text" value="<%=qtycf%>" size="20" maxlength="20" ></td>
            </tr>
            <tr bgcolor="#FFFFFF"> 
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(	javaTrim(FormName.qtycf)=="") {
		alert("请选择[套餐远程费]！");
		FormName.qtycf.focus();
		return false;
	}
	if(!(isFloat(FormName.qtycf, "套餐远程费"))) {
		return false;
	}
