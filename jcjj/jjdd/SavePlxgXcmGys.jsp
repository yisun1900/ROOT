...
try {
	...
	for (int i=0;i<ddbh.length ;i++ )
	{
		...
		if (rs.next())
		{
			...
			oldpdgcmc=rs.getString("xcmgcmc");
		}
		rs.close();
		ps.close();

		ls_sql="update jc_jjdd set xcmgc=?,xcmgcmc=?,xcmgys=?,xcmppmc=?";
		ls_sql+=" where  ddbh='"+ddbh[i]+"' ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,pdgc);
		...
		ls_sql="update jc_jjzjx set xcmgc=?,xcmgcmc=?,xcmgys=?,xcmppmc=?";
		ls_sql+=" where  ddbh='"+ddbh[i]+"' ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,pdgc);
		ps.setString(2,pdgcmc);
