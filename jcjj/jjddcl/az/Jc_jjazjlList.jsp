...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","EditJc_jjazjl.jsp");
	pageObj.setPageRow(100);//设置每页显示记录数
/*
//设置显示列
	String[] disColName={"ddbh","smrq","jdm_zcsmyybm_zcsmyy","jc_jjazjl_azqk","jc_jjazjl_lrr","jc_jjazjl_lrsj"};
	pageObj.setDisColName(disColName);
*/

//设置主键
	String[] keyName={"ddbh","smrq"};
	pageObj.setKey(keyName);
//设置按钮参数
	String[] buttonName={"删除选中的信息"};//按钮的显示名称
	String[] buttonLink={"Jc_jjazjlList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from jc_jjazjl where "+chooseitem;
