...
//	}
	...
	ls_sql+=" and lydh='"+whereddbh+"'";
	ls_sql+=" and cklx='D'";//D：主材订单出库；B：报废出库；T：退货出库； S：申购出库；P：材料单出库；J：代金券出库；X：销售出库；R:调拨出库
	ls_sql+=" and ckdzt='9'";//9：提交；A：转凭证
	ls_sql+=" and sfth='N'";//Y：提货；N：未提货
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
...
<%
	...
	String getshsj=null;
	String getckph=null;
	ls_sql="select  shdbh,shr,shsj,ckph";
	...
	while (rs.next())
	{
		...
		getckph=cf.fillNull(rs.getString("ckph"));
		...
		while (rs1.next())
		{
			...
			if (row==1)
			{
				...
				<tr bgcolor="#FFFFFF" align="center"> 
				  ...
				  <td rowspan="<%=hbsl%>"><A HREF="/jxczc/ckgl/ViewJxc_ckd2.jsp?ckph=<%=getckph%>" target="_blank"><%=getckph%></A></td>
				  <td><%=getzcmc%></td>
				  <td><%=getxinghao%></td>
				  <td><%=getgg%></td>
				  <td><%=getshsl%></td>
...
<div align="center"><strong>填写送货明细（订单编号：<%=ddbh%>） </strong></div>
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="32%"> <%=tzry%> </td>
      <td width="18%" align="right">出库批号</td>
      <td width="32%"><A HREF="/jxczc/ckgl/ViewJxc_ckd2.jsp?ckph=<%=ckph%>" target="_blank"><%=ckph%></A></td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
<%
	...
	while (rs.next())
	{
		...
		ls_sql+=" where ckph='"+ckph+"' and  clbm='"+zcbm+"'";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		{
		...
		if (cksl==0)
		{
			...
			<tr bgcolor="#FFFFFF"  align="center" height="25">
				...
				<td >
					...
					<input type="text" name="shsl" size="7" maxlength="16"  value="<%=cksl%>" readonly style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid">			</td>
				<td >
					  <%
						cf.selectToken(out,"Y+赠品&N+否",sfzp,true);
					  %>			</td>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.target="";
	FormName.action="SaveShJc_zcddYkc.jsp";
	FormName.submit();
	return true;
}
