...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jsjlh,DECODE(shbz,'N','未审核','Y','审核','F','付款完成','S','部分付款'),gys,ppbm,jssj,jsr,fkbl||'%',fkje,jsje,cbzj,ycfzj,qtfzj,kcfy,fk,kzbjbl||'%',kzbjje,qtkk,jssm,shr,shsj,fkr,fksj,fksm,dwmc ";
	ls_sql+=" FROM jc_gysjs,sq_dwxx  ";
    ls_sql+=" where  jc_gysjs.ssfgs=sq_dwxx.dwbh and jc_gysjs.lx='1'";//1：订单；2：代购主材
	ls_sql+=" and shbz in('Y','S') ";
...
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jc_gysjsFkList.jsp","","","InsertJc_gysjsfkjl.jsp");
...
//设置主键
	...
	pageObj.setEditStr("付款");
//设置按钮参数
	String[] buttonName={"全额付款"};//按钮的显示名称
