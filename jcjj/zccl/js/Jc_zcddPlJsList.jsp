...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (khjsbz!=null)
	{
		if (khjsbz.equals("N"))
		{
			wheresql+=" and  (crm_khxx.khjsbz not in('Y','2'))";
		}
		else{
    ...
    ls_sql=" select * ";
    ls_sql+=" FROM ( ";
	ls_sql+=" SELECT jc_zcdd.ddbh,'<A HREF=\"/jcjj/zcdd/ShJsjJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">审核',DECODE(jc_zcdd.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,DECODE(jc_zcdd.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),clztmc,jc_zcdd.sshsj,ppbm,jc_zcdd.gys,crm_khxx.khxm,crm_khxx.khbh,crm_khxx.fwdz,DECODE(crm_khxx.khjsbz,'N','未结算','Y','已结算','2','审批同意','3','审批不同意'),jc_zcdd.clgw,jc_zcdd.xmzy";
    ...
    ls_sql+=" and jc_zcdd.ddlx not in('9','C')";
	...
	ls_sql+=" SELECT jc_zczjx.zjxbh ddbh,'<A HREF=\"/jcjj/zccl/zjx/ShJsjJc_zczjx.jsp?zjxbh='||jc_zczjx.zjxbh||'\" target=\"_blank\">审核',DECODE(jc_zczjx.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,DECODE(jc_zczjx.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),DECODE(jc_zczjx.clzt,'00','录入未完成','01','录入已完成','02','已确认','04','厂家已接收','03','已送货','05','部分送货','30','单项完结','99','退单') clztmc,jc_zczjx.sshsj,ppbm,jc_zczjx.gys,crm_khxx.khxm,crm_khxx.khbh,crm_khxx.fwdz,DECODE(crm_khxx.khjsbz,'N','未结算','Y','已结算','2','审批同意','3','审批不同意'),jc_zczjx.clgw,jc_zczjx.xmzy";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddPlJsList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"批量结算","设置选择标志","删除设置标志"};//按钮的显示名称
	String[] buttonLink={"InsertJc_gysjs.jsp?ssfgs="+fgs,"chooseJs.jsp","DeleteBZJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单－－厂商批量结算（<font color="#FF0000" size="4">注意：订单和增减项只有完结后才能结算</font>）</font></B>
</CENTER>
<%
	pageObj.out=out;
