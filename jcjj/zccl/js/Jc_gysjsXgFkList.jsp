...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		fkr=cf.GB2Uni(fkr);
		if (!(fkr.equals("")))	wheresql+=" and  (jc_gysjsfkjl.fkr='"+fkr+"')";
	}
	String fksj=null;
	...
	{
		fksj=fksj.trim();
		if (!(fksj.equals("")))	wheresql+="  and (jc_gysjsfkjl.fksj>=TO_DATE('"+fksj+"','YYYY/MM/DD'))";
	}
	fksj=request.getParameter("fksj2");
	if (fksj!=null)
	{
		fksj=fksj.trim();
		if (!(fksj.equals("")))	wheresql+="  and (jc_gysjsfkjl.fksj<=TO_DATE('"+fksj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}
	...
	ls_sql="SELECT TO_CHAR(jc_gysjsfkjl.fkxh) fkxh,jc_gysjsfkjl.fkr,jc_gysjsfkjl.fksj,jc_gysjsfkjl.fkbl||'%',jc_gysjsfkjl.fkje,jc_gysjsfkjl.bz,TO_CHAR(jc_gysjs.jsjlh) jsjlh,jc_gysjs.gys,jc_gysjs.ppbm,TO_CHAR(jsje),TO_CHAR(cbzj),TO_CHAR(ycfzj),TO_CHAR(qtfzj),TO_CHAR(kcfy),TO_CHAR(fk),kzbjbl||'%',TO_CHAR(kzbjje),TO_CHAR(qtkk),jssm,dwmc ";
	ls_sql+=" FROM jc_gysjsfkjl,jc_gysjs,sq_dwxx  ";
    ls_sql+=" where  jc_gysjs.ssfgs=sq_dwxx.dwbh and jc_gysjs.lx='1'";//1：订单；2：代购主材
    ls_sql+=" and  jc_gysjsfkjl.jsjlh=jc_gysjs.jsjlh(+) "; 
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_gysjs.ssfgs,jc_gysjsfkjl.fksj desc,jc_gysjs.jsjlh";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="4%">远程费</td>
	<td  width="4%">其它费</td>
	<td  width="4%">扣除费用总额 </td>
	<td  width="3%">罚款 </td>
	<td  width="3%">扣质保金比例 </td>
	<td  width="4%">扣质保金金额 </td>
	<td  width="3%">其它扣款 </td>
	<td  width="16%">结算说明</td>
	<td  width="5%">所属公司</td>
</tr>
