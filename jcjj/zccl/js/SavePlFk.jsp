...
try {
	...
	for (int i=0;i<jsjlh.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>错误，【"+jsjlh[i]+"】未审核或已付款");
			return;
		}

		ls_sql="update jc_gysjs set shbz='F',fksj=SYSDATE,fkr='"+yhmc+"',fkbl=100,fkje=jsje";
		ls_sql+=" where jsjlh='"+jsjlh[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql+=" select                  ?   ,jsjlh,gys,100 ,jsje,?  ,SYSDATE,SYSDATE,?  ,ssfgs,''";
		ls_sql+=" from jc_gysjs";
		ls_sql+=" where jsjlh='"+jsjlh[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.setInt(1,fkxh);
		...
		while (rs1.next())
		{
			...
			if (ddbh.length()==9)
			{
				...
				ls_sql="update jc_zcdd set sfjs='F' ";//N：未结算；C：已选择结算；Y：已结算；M：结算已审核；F：结算付款
				ls_sql+=" where ddbh='"+ddbh+"'";
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
				...
				ls_sql="update jc_zczjx set sfjs='F' ";
				ls_sql+=" where zjxbh='"+ddbh+"'";
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
