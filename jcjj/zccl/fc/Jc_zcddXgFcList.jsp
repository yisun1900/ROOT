...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_zcdd_jhfcsj=null;
	jc_zcdd_jhfcsj=request.getParameter("jc_zcdd_jhfcsj");
	if (jc_zcdd_jhfcsj!=null)
	{
		jc_zcdd_jhfcsj=jc_zcdd_jhfcsj.trim();
		if (!(jc_zcdd_jhfcsj.equals("")))	wheresql+="  and (jc_zcdd.jhfcsj>=TO_DATE('"+jc_zcdd_jhfcsj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_jhfcsj=request.getParameter("jc_zcdd_jhfcsj2");
	if (jc_zcdd_jhfcsj!=null)
	{
		jc_zcdd_jhfcsj=jc_zcdd_jhfcsj.trim();
		if (!(jc_zcdd_jhfcsj.equals("")))	wheresql+="  and (jc_zcdd.jhfcsj<=TO_DATE('"+jc_zcdd_jhfcsj+"','YYYY/MM/DD'))";
	}
	String jc_zcdd_fcsj=null;
	jc_zcdd_fcsj=request.getParameter("jc_zcdd_fcsj");
	if (jc_zcdd_fcsj!=null)
	{
		jc_zcdd_fcsj=jc_zcdd_fcsj.trim();
		if (!(jc_zcdd_fcsj.equals("")))	wheresql+="  and (jc_zcdd.fcsj>=TO_DATE('"+jc_zcdd_fcsj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_fcsj=request.getParameter("jc_zcdd_fcsj2");
	if (jc_zcdd_fcsj!=null)
	{
		jc_zcdd_fcsj=jc_zcdd_fcsj.trim();
		if (!(jc_zcdd_fcsj.equals("")))	wheresql+="  and (jc_zcdd.fcsj<=TO_DATE('"+jc_zcdd_fcsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,jc_zcdd.jhfcsj,jc_zcdd.fcsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'),jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品')";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)";
    ls_sql+=" and jc_zcdd.xclbz='2'";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.fcsj desc";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgFcList.jsp","","","XgFcJc_zcdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除复尺"};//按钮的显示名称
	String[] buttonLink={"DeleteFcJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改复尺</font></B>
</CENTER>
<%
	pageObj.out=out;
