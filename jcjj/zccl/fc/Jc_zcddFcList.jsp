...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_zcdd_tzfcsj=null;
	jc_zcdd_tzfcsj=request.getParameter("jc_zcdd_tzfcsj");
	if (jc_zcdd_tzfcsj!=null)
	{
		jc_zcdd_tzfcsj=jc_zcdd_tzfcsj.trim();
		if (!(jc_zcdd_tzfcsj.equals("")))	wheresql+="  and (jc_zcdd.tzfcsj>=TO_DATE('"+jc_zcdd_tzfcsj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_tzfcsj=request.getParameter("jc_zcdd_tzfcsj2");
	if (jc_zcdd_tzfcsj!=null)
	{
		jc_zcdd_tzfcsj=jc_zcdd_tzfcsj.trim();
		if (!(jc_zcdd_tzfcsj.equals("")))	wheresql+="  and (jc_zcdd.tzfcsj<=TO_DATE('"+jc_zcdd_tzfcsj+"','YYYY/MM/DD'))";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.jhfcsj";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddFcList.jsp","","","FcJc_zcdd.jsp");
	pageObj.setEditStr("复尺");//设置每页显示记录数
...
<CENTER >
  <B><font size="3">主材订单处理－－复尺</font></B>
</CENTER>
<%
	pageObj.out=out;
