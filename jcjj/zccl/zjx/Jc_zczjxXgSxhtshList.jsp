...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (sxhtsfysh!=null)
	{
		if (!(sxhtsfysh.equals("")))	wheresql+=" and  (jc_zczjx.sxhtsfysh='"+sxhtsfysh+"')";
	}


	ls_sql="SELECT jc_zczjx.zjxbh,DECODE(jc_zczjx.clzt,'00','录入未完成','01','录入已完成','02','已确认','04','厂家已接收','03','已送货','99','退单'),DECODE(sxhtsfysh,'M','非手写合同','Y','已审核','N','未审核'),jc_zczjx.sxhtshr,jc_zczjx.sxhtshsj,crm_khxx.khxm,crm_khxx.fwdz,jc_zczjx.ddbh,jc_zczjx.zcxlbm,jc_zczjx.ppbm,jc_zczjx.zqzjxze,jc_zczjx.zjxze,TO_CHAR(jc_zczjx.zjxzkl) zjxzkl,jc_zczjx.zjxcxhdje,jc_zczjx.zjxfssj,jc_zczjx.lrsj,crm_khxx.hth,crm_khxx.khbh,jc_zczjx.gys,jc_zczjx.bz";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zczjxXgSxhtshList.jsp","","","");
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_zczjxXgSxhtshList.jsp"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(zjxbh,"zjxbh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="update jc_zczjx set sxhtsfysh='N',sxhtshr=null,sxhtshsj=null where ("+chooseitem+")";
