...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,DECODE(jc_zcdd.ddlx,'1','代销品','3','补录合同','4','有库存订单','5','标准套餐','6','家具健康套餐','7','温暖静音套餐','8','窗帘套餐','9','代购主材','A','顶配升级','B','顶配独立收费','C','选配主材','D','套餐外主材'),jc_zcdd.qhtsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'),jc_zcdd.hkze,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品')";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)";
    ls_sql+=" and jc_zcdd.clzt not in('00','02','03','05','07','09','25','26','98','99')";
	ls_sql+=" and jc_zcdd.ddlx in('1','3','4','A','D')";//1：代销品；3：补录合同；4：有库存订单；9：代购主材
