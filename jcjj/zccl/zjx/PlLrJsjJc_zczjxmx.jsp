...
<body bgcolor="#ffffff" text="#000000" style='FONT-SIZE: 14px'>
 ...
 <div align="center">审核手写合同－批量录入结算价（订单编号：<%=ddbh%>）</div>
...
<%
	...
	ls_sql="select jc_zcddmx.xh,zcbm,zcmc,zclbbm,xinghao,gg,zqdj,dpzk,dj,sl,zqje,je,ycf,zcpzwzbm,zcysbm,DECODE(cxhdbz,'Y','参加','N','不参加') cxhdbz ";
	ls_sql+=" ,jldwbm,DECODE(qtfy,'1','安装费','2','加工费','3','其它收费','W','无') qtfy,xzjg,DECODE(sfbhpj,'Y','包含','N','不包含') sfbhpj,pjts,lsj,yhj";
	ls_sql+=" ,sfycx,cxj,cxkssj,cxjzsj,qtfyje,cxhdbl,cxhdje,gys,ppmc,cbj,bzcbj,bzcxhdbl,cxcbj ";
	ls_sql+=" ,lscxhdbl,DECODE(sftjcp,'Y','是','N','否') sftjcp,jsbl,jc_zcddmx.cbj ";
    ...
    ls_sql+=" and jc_zczjxmx.lx='2'  and jc_zcddmx.sxbz='2'";//1：已有项目；2：新增项目
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#E8E8FF"  align="center">
			...
			<td>
				<A HREF="LrJsjJc_zczjxmx.jsp?zjxbh=<%=zjxbh%>&ddbh=<%=ddbh%>&xh=<%=xh%>" target="_blank">修改</A>
			...
			<td bgcolor="#CC99FF" >
				<input type="text" name="cbj" size="5" maxlength="9"  value="<%=cbj%>"   style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid">
			</td>
			<td><%=zcysbm%></td>
			<td><%=zcpzwzbm%></td>
			<td ><%=jldwbm%></td>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SavePlLrJsjJc_zczjxmx.jsp";
	FormName.submit();
	return true;
}
