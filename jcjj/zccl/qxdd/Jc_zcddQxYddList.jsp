...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		lrsj=lrsj.trim();
		if (!(lrsj.equals("")))	wheresql+="  and (jc_zcdd.lrsj>=TO_DATE('"+lrsj+"','YYYY-MM-DD'))";
	}
	lrsj=request.getParameter("lrsj2");
	if (lrsj!=null)
	{
		lrsj=lrsj.trim();
		if (!(lrsj.equals("")))	wheresql+="  and (jc_zcdd.lrsj<=TO_DATE('"+lrsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	ls_sql="SELECT jc_zcdd.ddbh,clztmc,jc_zcdd.lrsj,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,zcxlbm,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),DECODE(jc_zcdd.xclbz,'1','不需测量','2','等待测量通知','3','测量已通知','4','已测量'),jc_zcdd.htshsj,jc_zcdd.jhshsj,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'),a.dwmc lrbm,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys";
	...
	ls_sql+=" and jc_zcdd.ddlx in('1','3','4','9')";//1：代销品；3：补录合同；4：有库存订单；9：代购主材
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddQxYddList.jsp","","","QxYdd.jsp");
...
<CENTER >
  <B><font size="3">主材订单处理－－取消预订单</font></B>
</CENTER>
<%
	pageObj.out=out;
