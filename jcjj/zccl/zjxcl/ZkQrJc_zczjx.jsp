...
String zktkbz="";//转款、退款标志，1：转款；2：退款；3：不需操作；4：预付款不足
...
//		return ;
	...
	{
		if ( yfkze < (hkze+zjhze-11-zcfk))//预付款不足，退出
		{
			zktkbz="4";//转款、退款标志，1：转款；2：退款；3：不需操作；4：预付款不足

			out.println("<strong><font color=\"#FF0000\">提醒！付款不足，不能确认！</font></strong>");
			out.println("<BR>订单实付款＝￥"+zcfk+",可用家居预付款＝￥"+yfkze);
			out.println("<BR>订单金额＝￥"+cf.round(hkze+zjhze,2)+"，应补交货款＝￥"+cf.round(hkze+zjhze-zcfk,2));
			return ;
		}
		else{//付款不足,需预付款转换
			zktkbz="1";//转款、退款标志，1：转款；2：退款；3：不需操作；4：预付款不足
		}
	}
	else if (zcfk > (hkze+zjhze))//减项
	{
		zktkbz="2";//转款、退款标志，1：转款；2：退款；3：不需操作；4：预付款不足

		jxzk=zcfk-(hkze+zjhze);
	}
	else{
		zktkbz="3";//转款、退款标志，1：转款；2：退款；3：不需操作；4：预付款不足
	}
}
catch (Exception e) {
...
<table border="1" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
  ...
  <%
	...
	%> 
      ...
      <td width="26%" align="right" bgcolor="#FFFFCC"><b><font color="#0000FF">订货金额</font></b></td>
      <td width="24%" bgcolor="#FFFFCC"><%=cf.round(hkze+zjhze,2)%>（合同＋增减项）</td>
    </tr>
    <tr bgcolor="#FFFFFF">
...
<%
	...
	else  if (zktkbz.equals("2"))//1：转款；2：退款；3：不需操作；4：预付款不足
	{
	  ...
	  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
		<tr bgcolor="#FFFFFF">
		  <td width="25%" align="right" bgcolor="#FFFFCC"><b><font color="#0000FF">订货金额</font></b></td>
		  <td width="25%" bgcolor="#FFFFCC"><%=cf.round(hkze+zjhze,2)%>（合同＋增减项）</td>
		  <td width="26%" align="right" bgcolor="#FFFFCC"><b><font color="#0000FF">订单已付款</font></b></td>
		  <td width="24%" bgcolor="#FFFFCC"><%=zcfk%><input type="hidden" name="sfkje" value="<%=jxzk%>"></td>
		</tr>
		<tr bgcolor="#FFFFFF">
		  <td colspan="4" align="center" bgcolor="#FFFFCC" ><strong><font color="#FF0000">提醒！本次为减项，剩余金额【<%=jxzk%>】将自动转为主材预付款</font></strong></td>
		</tr>
	  </TABLE>
		<%
	}
	else  if (zktkbz.equals("3"))//1：转款；2：退款；3：不需操作；4：预付款不足
	{
		%>
		<BR>	 
	  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
		<tr bgcolor="#FFFFFF">
		  ...
		  <td width="25%" bgcolor="#FFFFCC"><%=cf.round(hkze+zjhze,2)%>（合同＋增减项）</td>
		  <td width="26%" align="right" bgcolor="#FFFFCC"><b><font color="#0000FF">订单已付款</font></b></td>
		  <td width="24%" bgcolor="#FFFFCC"><%=zcfk%><input type="hidden" name="sfkje" value="0"></td>
		</tr>
	  </TABLE>
		<%
	}
...
%>
  ...
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
  <tr bgcolor="#FFFFFF">
    <td width="25%" align="right"> 增减项确认人</td>
    <td width="25%"><input type="text" name="qrr" value="<%=yhmc%>" size="20" maxlength="20" readonly></td>
    <td width="26%" align="right">增减项确认时间 </td>
    <td width="24%"><input type="text" name="qrsj" value="<%=cf.today()%>" size="20" maxlength="10" readonly></td>
  </tr>
  <tr>
    <td colspan="4" height="2"><div align="center">
        <input type="hidden" name="zjxbh" value="<%=zjxbh%>" >
        <input type="hidden" name="zktkbz" value="<%=zktkbz%>" >
...
if (zktkbz.equals("1"))//1：转款；2：退款；3：不需操作；4：预付款不足
{
	...
	{
		alert("[预付款转出额]不能小于[<%=cf.round(hkze+zjhze-zcfk-11,2)%>]，或大于[<%=cf.round(hkze+zjhze-zcfk+11,2)%>]！");
...
%>
	...
	FormName.action="SaveZkQrJc_zczjx.jsp";
	FormName.submit();
	return true;
}
