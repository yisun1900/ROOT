...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,jc_zcdd.ddqrsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),jc_zcdd.htshsj,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,jc_zcdd.hkze,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品')";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)";
    ls_sql+=" and jc_zcdd.ddlx!='4'";//1：系统存在品牌订单；2：系统不存在品牌订单；3：补录合同；4：有库存订单；5：套餐分解订单
	ls_sql+=" and (jc_zcdd.clzt='11' OR (jc_zcdd.clzt='99' and jc_zcdd.tdqrbz='Y'))";
	...
	if (yhjs.equals("G0"))
	{
		...
		{
			ls_sql+=" and (jc_zcdd.clgw='"+yhmc+"' OR jc_zcdd.ztjjgw='"+yhmc+"' )";
		}
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.ppbm,jc_zcdd.ddqrsj desc,jc_zcdd.ddbh";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddGcjsList.jsp","","","GcjsJc_zcdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"SaveAllGcjsJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－供应商接收</font></B>
</CENTER>
<%
	pageObj.out=out;
