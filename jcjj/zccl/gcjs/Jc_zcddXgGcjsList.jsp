...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		gysjslrsj=gysjslrsj.trim();
		if (!(gysjslrsj.equals("")))	wheresql+="  and (jc_zcdd.gysjslrsj<=TO_DATE('"+gysjslrsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	ls_sql="SELECT jc_zcdd.ddbh,clztmc,jc_zcdd.gysjslrr,jc_zcdd.gysjslrsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),jc_zcdd.htshsj,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,jc_zcdd.hkze,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品')";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)";
	ls_sql+=" and jc_zcdd.gysjslrr is not null";
	ls_sql+=" and jc_zcdd.sfjs in('N','C','P')";
	ls_sql+=" and (jc_zcdd.clzt='13' OR (jc_zcdd.clzt='99' and jc_zcdd.tdqrbz='J'))";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.gysjslrsj desc,jc_zcdd.ddbh";

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgGcjsList.jsp","","/jcjj/dygl/gyszcfj.jsp","");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteGcjsJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
