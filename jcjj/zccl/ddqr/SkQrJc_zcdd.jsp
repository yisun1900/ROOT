...
try {
	...
	ls_sql="select clzt,djqje,zjxxh,ddlx,sxhtsfysh,wdzje,hkze,zczkl,dzyy,htcxhdje,zjxcxhdje,zqzjhze,zjhze   ,ztjjgw,zcdlbm,ppbm,ddbh,khbh,clgw,jkxz,jkdd,ddshbz,kjxsj,xclbz,htshsj,xmzy,qhtsj,jhclsj,bz ";
	ls_sql+=" from  jc_zcdd";
...
//		return ;
	...
	if (ddlx.equals("C") || ddlx.equals("9") || ddlx.equals("5"))//C:选配主材 9:集成家居主材 5：标准套餐 1:代销品
	{
...
//		ls_sql+=" and (  skdd='1' OR (skdd='2' and dsksjbz='Y')  )";
		...
		{
			out.println("提醒！付款不足，不能确认！");
			out.println("<BR>实付款＝￥"+zcfk);
			out.println("<BR>订单金额＝￥"+cf.formatDouble(hkze+zjhze)+"，应补交货款＝￥"+cf.formatDouble(hkze+zjhze-zcfk));
			return ;
		}
		...
		if (!zjxxh.equals(""))
		{
			...
			ls_sql="select sum(fkje)";
			ls_sql+=" from  cw_khfkjl";
			ls_sql+=" where gysbh='主材变更' and scbz='N' and fklxbm='23'";//21:木门；22:橱柜；23:主材；24：家具
//			ls_sql+=" and (  skdd='1' OR (skdd='2' and dsksjbz='Y')  )";
			...
			if ( zcfk < (gethkze-11))
			{
				out.println("提醒！付款不足，不能确认！请在[收集成款]中收取[主材变更款]");
				out.println("<BR>实付款＝￥"+zcfk);
	...
	else if (ddlx.equals("E"))//E：散客
	{
		...
		if (!zjxxh.equals(""))
		{
			...
			if ( zcfk < (gethkze-11))
			{
				out.println("提醒！付款不足或财务未审核，不能确认！请在[收集成款]中收取[主材变更款]");
				out.println("<BR>实付款＝￥"+zcfk);
				out.println("<BR>订单金额＝￥"+cf.formatDouble(gethkze)+"，应补交货款＝￥"+cf.formatDouble(gethkze-zcfk));
				return ;
			}
...
%></td>
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <td><input type="text" name="ddqrr" value="<%=yhmc%>" size="20" maxlength="20" readonly>
      </td>
      <td align="right"><font color="#FF0000">*</font><font color="#0000CC">订单确认时间</font> </td>
      <td><input type="text" name="ddqrsj" value="<%=cf.today()%>" size="20" maxlength="10" readonly>
      </td>
    </tr>
    ...
    <tr> 
      <td height="2" colspan="4" align="center">
          ...
          <input type="button" onClick="f_do(editform)"  value="存盘" >
          <input type="reset"  value="重输" name="reset">
          <input type="button" value="查看客户电话" onClick="window.open('/khxx/ViewDh.jsp?khbh=<%=khbh%>')">
          <input name="button" type="button" onClick="window.open('/jcjj/zcdd/Cw_khfkjlCxList.jsp?ddbh=<%=ddbh%>')" value="付款信息">
          <input type="button"  value="确认并打印主材款凭证" onClick="f_do1(editform)" ></td>
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveSkQrJc_zcdd.jsp";
	FormName.submit();
	return true;
}
...
function f_do1(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="DYSkQrJc_zcdd.jsp";
	FormName.submit();
	return true;
}
