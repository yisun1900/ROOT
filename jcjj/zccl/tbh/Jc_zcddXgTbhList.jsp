...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String stbhsj=null;
	stbhsj=request.getParameter("stbhsj");
	if (stbhsj!=null)
	{
		stbhsj=stbhsj.trim();
		if (!(stbhsj.equals("")))	wheresql+="  and (jc_zcdd.stbhsj>=TO_DATE('"+stbhsj+"','YYYY-MM-DD'))";
	}
	stbhsj=request.getParameter("stbhsj2");
	if (stbhsj!=null)
	{
		stbhsj=stbhsj.trim();
		if (!(stbhsj.equals("")))	wheresql+="  and (jc_zcdd.stbhsj<=TO_DATE('"+stbhsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	ls_sql="SELECT jc_zcdd.ddbh,clztmc,zcxlbm,ppbm,crm_khxx.khxm,crm_khxx.lxfs||','||crm_khxx.qtdh,crm_khxx.sgbz||'('||sq_bzxx.dh||')' sgbz,jc_zcdd.stbhsj,jc_zcdd.jhtbhsj,jc_zcdd.tbhtzry,jc_zcdd.tbhqk,jc_zcdd.sshsj,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,jc_zcdd.hkze,jc_zcdd.zjhze,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.stbhsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgTbhList.jsp","","","XgTbhJc_zcdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除退补货"};//按钮的显示名称
	String[] buttonLink={"DeleteTbhJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改退补货</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="5%">客户电话</td>
	<td  width="6%">班长</td>
	<td  width="3%">实退补货时间</td>
	<td  width="3%">约定退补货时间</td>
	<td  width="3%">退补货通知人员</td>
	<td  width="12%">退补货情况</td>
	<td  width="3%">实送货时间</td>
	<td  width="3%">合同号</td>
	<td  width="8%">房屋地址</td>
