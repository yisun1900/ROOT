...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveInsertBj_khzcxmTcwDljj.jsp" name="insertform">
<div align="center">录入－顶配独立计价主材</div>
	...
	while (rs.next())
	{
		...
		if (lx.equals("4"))//1：原项目；2：增项；3：减项；4：新增项
		{
			...
			<tr bgcolor="#FFFFF"  align="center">
				<td>
					<A HREF="EditNewBj_khzcxmTcwDljj.jsp?zjxxh=<%=zjxxh%>&sxh=<%=sxh%>" target="_blank">修改</A>  
					<A HREF="DeleteBj_khzcxmTcwDljj.jsp?zjxxh=<%=zjxxh%>&sxh=<%=sxh%>" target="_blank">删除</A>
				</td>
				<td><%=lxmc%></td>
				<td><%=jldw%></td>
				<td><%=sl%></td>
		...
		else if (lx.equals("1"))//1：原项目；2：增项；3：减项；4：新增项
		{
			...
			<tr bgcolor="#FFFFF"  align="center">
				<td>
					<A HREF="EditBj_khzcxmTcwDljj.jsp?zjxxh=<%=zjxxh%>&sxh=<%=sxh%>" target="_blank">修改</A>  
				</td>
				<td><%=lxmc%></td>
				<td><%=jldw%></td>
				<td><%=sl%></td>
...
<tr bgcolor="#FFFFFF">
  ...
  <td><select name="ppmc" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changePp(insertform)">
    ...
    <%
	String sql="";
	sql="select distinct  ppmc c1,ppmc c2 from bj_tcwzck where jjfs='2' and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE))) order by ppmc";
...
function changePp(FormName)
{
	...
	var sql;
	sql="select cpbm,cpmc||'（'||xh||'）（'||gg||'）'||'（￥'||dj||'/'||jldw||'）' from bj_tcwzck where ppmc='"+FormName.ppmc.value+"' and jjfs='2' and cpbm not in(select cpbm from bj_khzcxmh where zjxxh='<%=zjxxh%>' and zclx in('3','4')) and ((sfyx='1') OR (sfyx='3' and yxkssj<=TRUNC(SYSDATE) and yxjzsj>=TRUNC(SYSDATE)))  order by cpmc,xh";
