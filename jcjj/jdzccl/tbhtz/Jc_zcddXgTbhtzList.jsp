...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jhtbhsj=null;
	jhtbhsj=request.getParameter("jhtbhsj");
	if (jhtbhsj!=null)
	{
		jhtbhsj=jhtbhsj.trim();
		if (!(jhtbhsj.equals("")))	wheresql+="  and (jc_zcdd.jhtbhsj>=TO_DATE('"+jhtbhsj+"','YYYY-MM-DD'))";
	}
	jhtbhsj=request.getParameter("jhtbhsj2");
	if (jhtbhsj!=null)
	{
		jhtbhsj=jhtbhsj.trim();
		if (!(jhtbhsj.equals("")))	wheresql+="  and (jc_zcdd.jhtbhsj<=TO_DATE('"+jhtbhsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}


	ls_sql="SELECT jc_zcdd.ddbh,clztmc,jc_zcdd.jhtbhsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'),jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品')";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.jhtbhsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgTbhtzList.jsp","","","XgTbhtzJc_zcdd.jsp");
//	pageObj.setEditStr("通知");//设置每页显示记录数
/*
//设置显示列
...
//设置按钮参数
	String[] buttonName={"删除退补货通知"};//按钮的显示名称
	String[] buttonLink={"DeleteTbhtzJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改退补货通知</font></B>
</CENTER>
<%
	pageObj.out=out;
