...
if (DBpageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (gysjslrsj!=null)
	{
		...
		if (!(gysjslrsj.equals("")))	wheresql+="  and (SEOrder.gysjslrsj>=TO_DATE('"+gysjslrsj+"','YYYY-MM-DD'))";
	}
	gysjslrsj=request.getParameter("gysjslrsj2");
	if (gysjslrsj!=null)
	{
		gysjslrsj=gysjslrsj.trim();
		if (!(gysjslrsj.equals("")))	wheresql+="  and (SEOrder.gysjslrsj<=TO_DATE('"+gysjslrsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	ls_sql="SELECT SEOrder.fbillno,clztmc,SEOrder.gysjslrr,CONVERT(char(10),SEOrder.gysjslrsj,111 ),ppbm,t_Organization.fnumber khbh,t_Organization.FName,t_Organization.FAddress,SEOrder.clgw,SEOrder.xmzy ,CONVERT(char(10),SEOrder.FDate,111 ) ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by SEOrder.gysjslrsj desc";
    DBpageObj.sql=ls_sql;
//进行对象初始化
	DBpageObj.initPage("Jc_zcddShsjList.jsp","","","ShsjJc_zcdd.jsp","sqlserver","sa","");
//	DBpageObj.setPageRow(1000);//设置每页显示记录数
	DBpageObj.setEditStr("确定");//设置每页显示记录数
/*
//设置显示列
	String[] disColName={"fbillno","crm_khxx_khxm","crm_khxx_fwdz","crm_khxx_hth","jc_zcdd_clgw","jc_zcdd_jkxz","jc_zcdd_jkdd","jc_zcdd_ddshbz","jc_zcdd_hkze","jc_zcdd_xclbz","jc_zcdd_clzt","jc_zcdd_lrsj","jc_zcdd_dwbh","jc_zcdd_bz","jc_zcdd_xmzy","crm_khxx_lxfs"};
...
<CENTER >
  <B><font size="3">主材订单处理－－确定送货时间</font></B>
</CENTER>
<%
	DBpageObj.out=out;
