...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (jc_zcjgb_zcdlbm!=null)
	{
		jc_zcjgb_zcdlbm=cf.GB2Uni(jc_zcjgb_zcdlbm);
		if (!(jc_zcjgb_zcdlbm.equals("")))	wheresql+=" and  (jc_zcjgb.zcdlbm='"+jc_zcjgb_zcdlbm+"')";
	}
	...
	if (jc_zcjgb_dwbh!=null)
	{
		jc_zcjgb_dwbh=cf.GB2Uni(jc_zcjgb_dwbh);
		if (!(jc_zcjgb_dwbh.equals("")))	wheresql+=" and  (jc_zcjgb.dwbh='"+jc_zcjgb_dwbh+"')";
	}
	...
	ls_sql="SELECT zclbbm,jc_zcjgb.zcmc,jc_zcjgb.xh,jc_zcjgb.gg,zcysbm,jldwbm,jc_zcjgb.lsj,jc_zcjgb.yhj,DECODE(sfyh,'Y','有货','N','<font color=\"#FF0000\">无货</font>'),DECODE(sfycx,'N','无','Y','<font color=\"#FF0000\">有促销</font>'),cxj,cxkssj,cxjzsj,cxhdbl||'%',DECODE(jc_zcjgb.sfbhpj,'Y','<font color=\"#FF0000\">包含</font>','N','否'),jc_zcjgb.pjts,jc_zcjgb.zcbm,jc_zcjgb.bz";
...
//设置按钮参数
	String[] buttonName={"选中打勾主材"};//按钮的显示名称
