...
<form method="post" action="" name="selectform" target="_blank">
  ...
  <table width="100%" border="0" style="FONT-SIZE:12;" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr bgcolor="#CCCCFF" style="FONT-SIZE:12;HEIGHT:26"> 
      <td > 
		...
		<input type="text" name="jc_zcjgb_xh" id="txt" size="50" maxlength="100" style='position:absolute; WIDTH:300;left:335; top:20; z-index:2' onKeyUp="changheIn(selectform);">
  ...
  <table width="100%" border="0" name="firsttable" id="firsttable" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr align="center" bgcolor="#E8E8FF" name="ff" id="ff"> 
      <td colspan="2" height="22"><b>增减项编号：<%=zjxbh%></b></td>
    </tr>
    <tr bgcolor="#FFFFFF">
      <td width="21%" align="right"><font color="#0000CC">主材编码</font></td>
      <td width="79%" id="zcbm"></td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="79%"><div id="zclbbm"></div></td>
    </tr>
    ...
    <tr bgcolor="#FFFFCC"> 
      ...
      <td width="79%"> 
        ...
        促销价&nbsp;&nbsp;<font color="#0000CC">实销价</font> 
        <input type="text" name="dj" size="8" maxlength="16"  value=""  style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" readonly>
      </td>
    </tr>
    <tr bgcolor="#FFFFCC"> 
      ...
      <td width="79%"> 
        <input type="text" name="zcysbm" size="20" maxlength="50"  value="" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onKeyUp="keyGo(zcpzwzbm)" >
      </td>
    </tr>
    <tr bgcolor="#FFFFCC"> 
...
%> </td>
    ...
    <tr bgcolor="#E8E8FF"> 
      ...
      <td width="79%"> 
        <textarea name="bz" cols="42" rows="3"></textarea>
      </td>
    </tr>
    <tr> 
      <td colspan="4" height="2"> 
		...
		<input type="hidden" name="zcbm"  value="" >
...
function getAjax(ajaxRetStr) 
{
	...
	if(ajaxlx==1)
	{
		...
		//单品显示信息开始＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋
		document.getElementById('zcbm').innerHTML="";
		txtNode=document.createTextNode(xhsz2[1]);//主材编码
		document.getElementById('zcbm').appendChild(txtNode);
		selectform.zcbm.value=xhsz2[1];
		...
		document.getElementById('zcmc').innerHTML="";
		txtNode=document.createTextNode(xhsz2[3]);//主材名称
		document.getElementById('zcmc').appendChild(txtNode);
		...
		document.getElementById('zclbbm').innerHTML="";
		txtNode=document.createTextNode(xhsz2[4]+"(计入活动比例:"+xhsz2[14]+"%)");//类别/系别
		document.getElementById('zclbbm').appendChild(txtNode);
...
{   
	if(event.keyCode==13)//获取型号详细信息
	{
		selectform.xhlist.style.display='none';

		ajaxlx=2;
		actionStr="/jcjj/zccl/zjx/GetCpxxByAjax.jsp?jc_zcjgb_xh="+FormName.jc_zcjgb_xh.value+"&dqbm=<%=dqbm%>&ppbm=<%=ppbm%>";
...
//		window.open(actionStr);
	...
	else if(event.keyCode==38)
	{
		...
		oldtext=FormName.jc_zcjgb_xh.value;

		FormName.zcbm.value       =FormName.xhlist.options[FormName.xhlist.length-1].value;
		FormName.jc_zcjgb_xh.value=FormName.xhlist.options[FormName.xhlist.length-1].text;
	}
	else if(event.keyCode==40)
	{
		...
		FormName.zcbm.value       =FormName.xhlist.options[0].value;
		FormName.jc_zcjgb_xh.value=FormName.xhlist.options[0].text;
	}
	else{
		if(FormName.jc_zcjgb_xh.value=="") 
		{
			selectform.xhlist.style.display='none';
			return;
		}
		...
		//获取型号列表
		ajaxlx=1;
		actionStr="/jcjj/zccl/zjx/GetXhListByAjax.jsp?jc_zcjgb_xh="+FormName.jc_zcjgb_xh.value+"&dqbm=<%=dqbm%>&ppbm=<%=ppbm%>";
//		window.open(actionStr);
		openAjax(actionStr);  
	}
...
function changeItem(FormName,field)
{
	if(event.keyCode==38)
	{
		...
		FormName.zcbm.value       =field.options[field.selectedIndex].value;
		FormName.jc_zcjgb_xh.value=field.options[field.selectedIndex].text;
		...
		if (field.selectedIndex==0)
		{
			if (mark=="first")
			{
				...
				FormName.zcbm.value       ="";
				FormName.jc_zcjgb_xh.value=oldtext;
	...
	else if(event.keyCode==13)
	{
		...
		ajaxlx=2;
		actionStr="/jcjj/zccl/zjx/GetCpxxByAjax.jsp?zcbm="+FormName.zcbm.value;
//		window.open(actionStr);
		openAjax(actionStr);
	}
}
