...
if (DBpageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT SEOrder.fbillno,clztmc,CONVERT(char(10),SEOrder.tzshsj,111 ),ppbm,t_Organization.fnumber khbh,t_Organization.FName,t_Organization.FAddress,SEOrder.clgw,SEOrder.xmzy ,CONVERT(char(10),SEOrder.FDate,111 ) ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by SEOrder.tzshsj desc";
    DBpageObj.sql=ls_sql;
//进行对象初始化
	DBpageObj.initPage("Jc_zcddXgShtzList.jsp","","","","sqlserver","sa","");
	DBpageObj.setEditStr("通知");//设置每页显示记录数
...
//设置按钮参数
	String[] buttonName={"删除送货通知"};//按钮的显示名称
	String[] buttonLink={"DeleteShtzJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－删除送货通知</font></B>
</CENTER>
<%
	DBpageObj.out=out;
