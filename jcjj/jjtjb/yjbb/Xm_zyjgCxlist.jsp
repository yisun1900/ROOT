...
<center><B>项目专员定单统计表<br>(统计时间：<%=cxrq%>--<%=cxrq2%>)</B></center><BR>
<%
...
//		out.print(ls_sql);
		...
		while (rs.next())
		{
				...
				ls_sql+=" from jc_mmydd,crm_khxx where crm_khxx.khbh=jc_mmydd.khbh  and jc_mmydd.htrq<=TO_DATE('"+cxrq2+"','YYYY-MM-DD') and jc_mmydd.htrq>=TO_DATE('"+cxrq+"','YYYY-MM-DD') and jc_mmydd.xmzy='"+yhmc+"'";
				...
				ls_sql+=" from jc_cgdd,crm_khxx where crm_khxx.khbh=jc_cgdd.khbh and jc_cgdd.qhtsj<=TO_DATE('"+cxrq2+"','YYYY-MM-DD') and jc_cgdd.qhtsj>=TO_DATE('"+cxrq+"','YYYY-MM-DD') and jc_cgdd.xmzy='"+yhmc+"'";
				ps1= conn.prepareStatement(ls_sql);
				rs1 =ps1.executeQuery(ls_sql);
				if (rs1.next())
				{
				...
				ls_sql+=" from jc_jjdd,crm_khxx where crm_khxx.khbh=jc_jjdd.khbh  and jc_jjdd.qhtsj<=TO_DATE('"+cxrq2+"','YYYY-MM-DD') and jc_jjdd.qhtsj>=TO_DATE('"+cxrq+"','YYYY-MM-DD') and jc_jjdd.xmzy='"+yhmc+"'";
				ps1= conn.prepareStatement(ls_sql);
				rs1 =ps1.executeQuery(ls_sql);
				if (rs1.next())
				{
				...
				ls_sql+=" from jc_zcdd,crm_khxx where crm_khxx.khbh=jc_zcdd.khbh and jc_zcdd.qhtsj<=TO_DATE('"+cxrq2+"','YYYY-MM-DD') and jc_zcdd.qhtsj>=TO_DATE('"+cxrq+"','YYYY-MM-DD') and jc_zcdd.xmzy='"+yhmc+"'";
...
<tr  align="center">
	...
	<td  width="6%"><font color="red"><%=cf.formatDouble(htje5)%></td>
