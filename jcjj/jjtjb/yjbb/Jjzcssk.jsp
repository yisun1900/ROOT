...
<center><B>家居顾问外主材实收款<br>(统计时间：<%=cxrq%>--<%=cxrq2%>)</B></center><BR>
<%
...
try {
		...
		//主材实收款总额(注:以签合同时间为准)
		ls_sql="select sum(jc_zcdd.hkze+jc_zcdd.zjhze) from crm_KHXX,jc_zcdd where  CRM_khxx.khbh=jc_zcdd.khbh   ";
		...
		ls_sql="select clgw,sum(jc_zcdd.hkze+jc_zcdd.zjhze) from crm_KHXX,jc_zcdd where  CRM_khxx.khbh=jc_zcdd.khbh   ";
		ls_sql+="and jc_zcdd.qhtsj<=TO_DATE('"+cxrq2+"','YYYY-MM-DD') and jc_zcdd.qhtsj>=TO_DATE('"+cxrq+"','YYYY-MM-DD') ";
		ls_sql+=wheresql;
