...
ls=(new java.sql.Date(java.sql.Date.valueOf(ls).getTime()-24*3600*1000)).toString();
...
//	out.println(ls_sql);
	...
	ls_sql=" SELECT b.dwmc fgs,crm_khxx.dwbh,a.dwmc dm,count(*) qys,sum(TRUNC(qye)) zqye,sum(TRUNC(wdzgce)) wdzgce,sum(TRUNC(sjf)) sjf,sum(TRUNC(fwmj)) fwmj";
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		<tr align="center">
			...
			<td  align="left"><A HREF="ViewDmcz.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getdwbh%>" target="_blank"><img src="/images/ColuGif.gif" width="<%=zqye*140/maxqye%>" height="14"></A></td>
		</tr>
		<%	
	}
	rs.close();
...
<%
	...
	ls_sql=" SELECT b.dwmc fgs,count(*) qys,sum(TRUNC(qye)) zqye,sum(TRUNC(wdzgce)) wdzgce,sum(TRUNC(sjf)) sjf,sum(TRUNC(fwmj)) fwmj";
	ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_dwxx b";
	ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and a.ssdw=b.dwbh(+)";
	ls_sql+=" and crm_khxx.zt='2'";
	ls_sql+=wheresql;
