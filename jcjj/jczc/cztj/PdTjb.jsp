...
try {
	...
	ls_sql+=" where crm_khxx.sgd=sq_sgd.sgd(+) and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
	ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	if (fgs!=null)
	{
		if (!(fgs.equals("")))	ls_sql+=" and  (crm_khxx.fgsbh='"+fgs+"')";
...
//	out.println(ls_sql);
	...
	ls_sql="select sgd,sgdmc,bzs,sum(sl),sum(je)";
	ls_sql+=" from";
	ls_sql+=" (";

	ls_sql+=" select crm_khxx.sgd,sgdmc,bzs,count(*) sl,sum(qye) je";
	ls_sql+=" from crm_khxx,sq_sgd";
	ls_sql+=" where crm_khxx.sgd=sq_sgd.sgd(+) and pdsj>=TO_DATE('"+sjfw+"','YYYY-MM-DD') ";
	ls_sql+=" and pdsj<=TO_DATE('"+sjfw2+" 23:59:59','YYYY-MM-DD HH24:MI:SS')";
	if (fgs!=null)
	...
	ls_sql+=" group by crm_khxx.sgd,sgdmc,bzs";

	ls_sql+=" union";

	ls_sql+=" select sgd,sgdmc,bzs,0 sl,0 je";
	ls_sql+=" from sq_sgd";
	ls_sql+=" where cxbz='N'";
	if (fgs!=null)
	{
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		ls_sql+=" from crm_khxx";
		ls_sql+=" where crm_khxx.hdr='"+dwmc+"' and qyrq>=TO_DATE('"+sjfw+"','YYYY-MM-DD')";
...
%>
          ...
          <td align="left"><A HREF="ViewSgdcz.jsp?sgd=<%=sgd%>&sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>" target="_blank"><img src="/images/ColuGif.gif" width="<%=je*100/maxje%>" height="14"></A></td>
        </tr>
<%
