...
<%
	...
	ls_sql="SELECT jc_zcdd.ddbh, DECODE(jc_zcdd.clzt,'00','订单未完成','01','订单已完成','02','订单已确认','03','已送货'),DECODE(jc_zcdd.xclbz,'1','不需测量','2','等待测量通知','3','测量已通知','4','已测量'),jc_zcdd.jhclsj,jc_zcdd.tzclsj,crm_khxx.khxm,crm_khxx.lxfs,jc_zcdd.clgw,jc_zcdd.xmzy,jc_zcdd.hkze,dwmc,jc_zcdd.bz";
	ls_sql+=" FROM crm_khxx,jc_zcdd,sq_dwxx  ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh and jc_zcdd.dwbh=sq_dwxx.dwbh ";
	ls_sql+=" and (jhclsj<SYSDATE or tzclsj is not null)";
	ls_sql+=" and xclbz in('2','3')";
	ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.jhclsj,jc_zcdd.ddbh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","");
...
<CENTER >
  <B><font size="3">主材测量延期（项目专员：<%=xmzymc%>）</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
