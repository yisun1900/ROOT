...
<CENTER >
  <B><font size="3">设计师选配主材主材统计</font></B>
</CENTER>
...
try {
    ...
    ls_sql+=wheresql;
	ls_sql+=" group by dwmc,crm_khxx.dwbh,crm_khxx.sjs";
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" height="23">
			<td><%=getdwmc%></td>
			<td><A HREF="ViewSjsJc_zcdd.jsp?wheresql=<%=java.net.URLEncoder.encode(wheresql,"UTF-8")%>&dwbh=<%=getdwbh%>&sjs=<%=getsjs%>" target="_blank"><%=getsjs%></A></td>
			<td><%=wdzje%></td>
			<td><%=hkze%></td>
			<td><%=cbze%></td>
			<td><%=tcze%></td>
