...
<CENTER >
  <B><font size="3">材料顾问选配主材主材统计</font></B>
</CENTER>
...
try {
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" height="23">
			<td><%=getdwmc%></td>
			<td><A HREF="ViewClgwJc_zcdd.jsp?wheresql=<%=java.net.URLEncoder.encode(wheresql,"UTF-8")%>&dwbh=<%=getdwbh%>&clgw=<%=getclgw%>" target="_blank"><%=getclgw%></A></td>
			<td><%=wdzje%></td>
			<td><%=hkze%></td>
			<td><%=cbze%></td>
			<td><%=tcze%></td>
