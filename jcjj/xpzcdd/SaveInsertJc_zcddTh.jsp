...
try {
	...
	if (rs.next())
	{
		...
		xpjsbl=rs.getDouble("fdbl");
		xpcxjsbl=rs.getDouble("tjpfdbl");
		xptcjrbl=rs.getDouble("tcjrbl");
		xptccxjrbl=rs.getDouble("tjptcjrbl");
		xpkhfdbl=rs.getDouble("khfdbl");
		xpkhcxfdbl=rs.getDouble("tjpkhfdbl");
	}
	rs.close();
	ps.close();
	...
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?    ,?,?,?,?,?,?,?,?,?,?";
	ls_sql+=" ,?,?,?,?,SYSDATE,?,?,?    ";
	ls_sql+=" ,?,?,?,?   ,0,0,0,0   ,0,0,0,'N','09','C','1',0,0,0,0,'N',0,'N' ";//9：集成家居主材
	ls_sql+=" ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,0) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,ddbh);
	ps.setString(2,khbh);
