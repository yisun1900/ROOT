...
<CENTER >
  <B><font size="3">客户选配主材主材统计</font></B>
</CENTER>
...
try {
	...
	ls_sql="SELECT crm_khxx.dgkh,crm_khxx.khbh,crm_khxx.khxm,crm_khxx.fwdz,sum(jc_zcdd.wdzje) wdzje,sum(round(jc_zcdd.zcpclf*jc_zcdd.fdbl/100+jc_zcdd.tjpclf*jc_zcdd.tjpfdbl/100,2)+round(jc_zcdd.ycf*jc_zcdd.ycfjsbl/100,2)+round(jc_zcdd.qtf*jc_zcdd.azfjsbl/100,2)) cbze,sum(round(jc_zcdd.zcpclf*jc_zcdd.khfdbl/100+jc_zcdd.tjpclf*jc_zcdd.tjpkhfdbl/100,2)) hkze,sum(round(jc_zcdd.zcpclf*jc_zcdd.tcjrbl/100+jc_zcdd.tjpclf*jc_zcdd.tjptcjrbl/100,2)) tcze";
	ls_sql+=" FROM jc_zcdd,crm_khxx";
	ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh(+)  ";//9：选配主材主材
	ls_sql+=" and jc_zcdd.ddlx in('C')";//9：选配主材主材
    ls_sql+=wheresql;
	ls_sql+=" group by crm_khxx.dgkh,crm_khxx.khbh,crm_khxx.khxm,crm_khxx.fwdz";
	ls_sql+=" order by crm_khxx.dgkh,crm_khxx.khbh,crm_khxx.khxm,crm_khxx.fwdz";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	while (rs.next())
	{
		getdgkh=cf.fillNull(rs.getString("dgkh"));
		...
		<tr bgcolor="#FFFFFF"  align="center" height="23">
			<td><%=getdgkh%></td>
			<td><A HREF="ViewKhJc_zcdd.jsp?wheresql=<%=java.net.URLEncoder.encode(wheresql,"UTF-8")%>&khbh=<%=getkhbh%>" target="_blank"><%=getkhxm%></A></td>
			<td><%=getfwdz%></td>
			<td><%=wdzje%></td>
			<td><%=hkze%></td>
			<td><%=cbze%></td>
