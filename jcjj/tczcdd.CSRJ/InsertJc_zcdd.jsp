...
try {
	...
	ls_sql="SELECT khxm,lxfs,fwdz,sjs,bjjbmc,ssfgs,fwmj,fjtcbz";
	...
	if (fjtcbz.equals("Y"))//Y：分解；N：未分解；M：非精装
	{
		out.println("<font color=\"#FF0000\">错误！已分解订单</font>");
		return;
	}
	else if (fjtcbz.equals("M"))//Y：分解；N：未分解；M：非精装
	{
		out.println("<font color=\"#FF0000\">错误！非精装客户</font>");
		return;
	}
...
<table border="0" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 14px'>
  ...
  <tr bgcolor="#FFFFFF" align="center"> 
    <td colspan="8" class="STYLE2" ><B>全装修精装主材明细
      <label>
...
<%
	...
	while (rs1.next())
	{
		...
		int count=0;
		ls_sql="SELECT count(*)";
		ls_sql+=" FROM tc_csrjkhzcqd,jxc_clxlbm";
		ls_sql+=" where tc_csrjkhzcqd.tccplbbm=jxc_clxlbm.clxlbm(+) and tc_csrjkhzcqd.khbh='"+khbh+"' and tc_csrjkhzcqd.jgwzbm='"+jgwzmc+"'";
	//	out.println(ls_sql);
		ps= conn.prepareStatement(ls_sql);
		rs =ps.executeQuery();
		if (rs.next())
		...
		double xdb=0;
		String sfxclxs=null;
		ls_sql="SELECT clxlmc,ppmc,gysmc,xh,gg,jldw,sl,ptcpsm,bjjbmc,xdjldw,xdb,DECODE(sfxclxs,'1','向上取整','2','向下取整','3','4舍5入','4','不处理') sfxclxs";
		ls_sql+=" FROM tc_csrjkhzcqd,jxc_clxlbm,bdm_bjjbbm ";
