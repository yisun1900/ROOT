...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (bj_tczcbj_xh!=null)
	{
		...
		if (!(bj_tczcbj_xh.equals("")))	wheresql+=" and  (bj_tczcbj.xh like '%"+bj_tczcbj_xh+"%')";
	}
	bj_tczcbj_gg=request.getParameter("bj_tczcbj_gg");
	if (bj_tczcbj_gg!=null)
	{
		bj_tczcbj_gg=cf.GB2Uni(bj_tczcbj_gg);
		if (!(bj_tczcbj_gg.equals("")))	wheresql+=" and  (bj_tczcbj.gg like '%"+bj_tczcbj_gg+"%')";
	}
	...
	ls_sql+=" and (bj_tczcbj.bjjbbm='"+bjjbbm+"' OR bj_tczcbj.bjjbbm in";
 	ls_sql+=" ( ";
	ls_sql+=" select distinct bjjbbm";
	ls_sql+=" from  bj_khzcxmxj";
	ls_sql+=" where khbh='"+khbh+"'  and tccplbbm in";

 	ls_sql+=" ( ";
	ls_sql+=" select distinct tccplbbm ";
	ls_sql+=" FROM bj_khzcxm ";
	ls_sql+=" where khbh='"+khbh+"' and ppmc='"+ppmc+"' ";
 	ls_sql+=" ) ";
 	ls_sql+=" )) ";
...
//设置按钮参数
	...
	String[] buttonLink={"ChooseBj_tczcbj.jsp?ddbh="+ddbh};//按钮单击调用的网页，可以增加参数
	int[] buttonNew={1};//设置按钮单击是否打开新的窗口，0：不打开；1：打开
	pageObj.setButton(buttonName,buttonLink,buttonNew);

//设置显示列
	String[] disColName={"bjjbmc","cpbm","sfcscp","sku","cpmc","xh","gg","jldw","xdjldw","xdb","sh","sfxclxs","dj","xuhao","cldlmc","clxlmc","tcsjflmc","gysmc","lrr","lrsj","bz","dqmc","ppmc"};
