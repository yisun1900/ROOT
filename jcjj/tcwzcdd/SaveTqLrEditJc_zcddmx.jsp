...
try {
	...
	ls_sql ="update jc_zcddmx set zcmc=?,zcdlbm=?,zcxlbm=?,zclbbm=?,xinghao=?,gg=?,zcysbm=?,jldwbm=?,zqdj=?,dpzk=?   ,dj=?,sl=?,zqje=?,je=?,ycf=?,qtfy=?,qtfyje=?,zjhsl=?,zqzjhje=?,zjhje=?   ,zjhycf=?,zjhqtfy=?,zcpzwzbm=?,cxhdbz=?,cxhdbl=?,cxhdje=?,sfycx=?,sxhtyy=?,bz=?,lrr=?,sftjcp=?,lrsj=SYSDATE";
	ls_sql+=" where xh="+xh;
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,zcmc);
