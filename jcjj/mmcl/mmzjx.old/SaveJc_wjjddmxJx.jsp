...
try {
	...
	{
		ls_sql="update jc_wjjddmx set jxbz='Y'";
		ls_sql+=" where lrxh="+lrxh[i]+" and jxbz='N'";
		ps= conn.prepareStatement(ls_sql);
		if (ps.executeUpdate()!=1)
		{
			conn.rollback();
			out.print("存盘失败,序号为【"+lrxh[i]+"】该项不存在，或已被减项");
			return;
		}
		ps.close();


		ls_sql="insert into jc_wjzjxmx ( xh,yddbh,zjxxh) ";
		ls_sql+=" values ( ?,?,?) ";
		ps= conn.prepareStatement(ls_sql);
	...
	ls_sql+=" where yddbh='"+yddbh+"' and lrxh in(select xh from jc_wjzjxmx where yddbh='"+yddbh+"' and zjxxh="+zjxxh+")";
	ps= conn.prepareStatement(ls_sql);
	...
	ls_sql="select mmzjxje+blzjxje+"+wjzjxje;
	...
	ls_sql="update jc_mmzjx set wjzjxje=?,zjxze=?";
	ls_sql+=" where  (yddbh='"+yddbh+"' and zjxxh="+zjxxh+")  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setDouble(1,wjzjxje);
	...
	<!--
	alert("存盘成功！五金减项金额=<%=wjzjxje%>，减项总额=<%=zjxze%>");
	window.close();
	//-->
	</SCRIPT>
	<%
