...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_mmydd_skqrsj=null;

	jc_mmydd_skqrsj=request.getParameter("jc_mmydd_skqrsj");
	if (jc_mmydd_skqrsj!=null)
	{
		jc_mmydd_skqrsj=jc_mmydd_skqrsj.trim();
		if (!(jc_mmydd_skqrsj.equals("")))	wheresql+="  and (jc_mmydd.skqrsj>=TO_DATE('"+jc_mmydd_skqrsj+"','YYYY-MM-DD'))";
	}
	jc_mmydd_skqrsj=request.getParameter("jc_mmydd_skqrsj2");
	if (jc_mmydd_skqrsj!=null)
	{
		jc_mmydd_skqrsj=jc_mmydd_skqrsj.trim();
		if (!(jc_mmydd_skqrsj.equals("")))	wheresql+="  and (jc_mmydd.skqrsj<=TO_DATE('"+jc_mmydd_skqrsj+"','YYYY-MM-DD'))";
	...
	ls_sql="SELECT yddbh,crm_khxx.khxm,crm_khxx.hth,clztmc,jc_mmydd.skqrsj,jc_mmydd.skqrr,jc_mmydd.pdgcmc,mmcljs,jc_mmydd.htje,jc_mmydd.mmhtje,jc_mmydd.wjhtje,jc_mmydd.blhtje,jc_mmydd.fwf, DECODE(jc_mmydd.wjqk,'1','客户自购','2','公司购买') wjqk,jc_mmydd.xmzy,jc_mmydd.clgw,jc_mmydd.ztjjgw,jc_mmydd.lrsj,jc_mmydd.bz,jc_mmydd.khbh  ";
    ...
    ls_sql+=" and jc_mmydd.wjqk='1' and wjsgsbz='N'";//1：客户自购；2：公司购买
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_mmydd.skqrsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_mmyddJsList.jsp","","","JsJc_mmydd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"SaveAllJsJc_mmydd.jsp"};//按钮单击调用的网页，可以增加参数
