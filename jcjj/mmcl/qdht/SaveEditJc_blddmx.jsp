...
try {
	...
	ls_sql="update jc_blddmx set blxhbm=?,bzdj=?,dj=ROUND(?,"+jcddmxblxs+"),sl=?,zjhsl=?,bz=?,blxh=?,bllxbm=?,blytbm=? ";
	ls_sql+=" where  lrxh="+lrxh;
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,blxhbm);
	ps.setDouble(2,dj);
	ps.setDouble(3,dj*blzkl/10);
	ps.setLong(4,sl);
	ps.setLong(5,sl);
	ps.setString(6,bz);
	ps.setString(7,blxh);
	ps.setString(8,bllxbm);
	ps.setString(9,blytbm);
	ps.executeUpdate();
	ps.close();
