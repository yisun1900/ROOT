...
try {
	...
	ls_sql+=" ,cgzkl=?,tmzkl=?,wjzkl=?,dqzkl=?,dzyy=?,dqpp=?,dqxh=?";
	ls_sql+="  where  ddbh='"+ddbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setDate(1,qhtsj);
	ps.setDate(2,jhazrq);
	...
	ps.setString(38,dqpp);
	ps.setString(39,dqxh);
	ps.executeUpdate();
	ps.close();
	...
	ls_sql+=" where  gjz='"+ddbh+"' and lx='2' and czlx='1'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	ls_sql+=" select ddbh,'2','1',ddbh,khbh,fgsbh,qddm,jzsjs,clgw,ztjjgw,xmzy,cgsjs            ,'',''        ,pdgc,pdgcmc,gys,ppmc  ,wdzje,htze,cbze,qhtsj";
	ls_sql+=" from jc_cgdd";
	ls_sql+=" where  ddbh='"+ddbh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
