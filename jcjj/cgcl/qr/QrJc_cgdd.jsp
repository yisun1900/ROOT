...
try {
	...
	if ( cgfk < (htze-11))
	{
		if ( yfkze < (htze-11-cgfk))
		{
			...
			out.println("<BR>订单实付款＝￥"+cgfk+",可用家居预付款＝￥"+yfkze);
			out.println("<BR>订单金额＝￥"+cf.formatDouble(htze)+"，应补交货款＝￥"+cf.formatDouble(htze-cgfk));
			return ;
		}
		else{
			mark="2";//付款不足,需预付款转换
...
if (mark.equals("2"))
{
	...
	{
		alert("[预付款转出额]不能小于[<%=cf.round(htze-cgfk-11,2)%>]，或大于[<%=cf.round(htze-cgfk+11,2)%>]！");
