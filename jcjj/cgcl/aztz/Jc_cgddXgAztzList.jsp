...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_kazsj=null;
	jc_cgdd_kazsj=request.getParameter("jc_cgdd_kazsj");
	if (jc_cgdd_kazsj!=null)
	{
		jc_cgdd_kazsj=jc_cgdd_kazsj.trim();
		if (!(jc_cgdd_kazsj.equals("")))	wheresql+="  and (jc_cgdd.kazsj>=TO_DATE('"+jc_cgdd_kazsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_kazsj=request.getParameter("jc_cgdd_kazsj2");
	if (jc_cgdd_kazsj!=null)
	{
		jc_cgdd_kazsj=jc_cgdd_kazsj.trim();
		if (!(jc_cgdd_kazsj.equals("")))	wheresql+="  and (jc_cgdd.kazsj<=TO_DATE('"+jc_cgdd_kazsj+"','YYYY/MM/DD'))";
	...
	String jc_cgdd_aztzsj=null;
	jc_cgdd_aztzsj=request.getParameter("jc_cgdd_aztzsj");
	if (jc_cgdd_aztzsj!=null)
	{
		jc_cgdd_aztzsj=jc_cgdd_aztzsj.trim();
		if (!(jc_cgdd_aztzsj.equals("")))	wheresql+="  and (jc_cgdd.aztzsj>=TO_DATE('"+jc_cgdd_aztzsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_aztzsj=request.getParameter("jc_cgdd_aztzsj2");
	if (jc_cgdd_aztzsj!=null)
	{
		jc_cgdd_aztzsj=jc_cgdd_aztzsj.trim();
		if (!(jc_cgdd_aztzsj.equals("")))	wheresql+="  and (jc_cgdd.aztzsj<=TO_DATE('"+jc_cgdd_aztzsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,crm_khxx.khxm,crm_khxx.fwdz,jc_cgdd.pdgcmc,jc_cgdd.kazsj,jc_cgdd.jhazrq,jc_cgdd.aztzsj,jc_cgdd.azsjbz,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.khbh  ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.aztzsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgAztzList.jsp","","","XgAztzJc_cgdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteAztzJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改安装通知</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="13%">房屋地址</td>
	<td  width="9%">派单工厂</td>
	<td  width="5%">协商安装时间</td>
	<td  width="5%">计划安装日期</td>
	<td  width="5%">发安装通知时间</td>
	<td  width="22%">安装提醒</td>
