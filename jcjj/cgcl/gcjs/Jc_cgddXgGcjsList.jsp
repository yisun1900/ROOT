...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_gcjsr=null;
	jc_cgdd_gcjsr=request.getParameter("jc_cgdd_gcjsr");
	if (jc_cgdd_gcjsr!=null)
	{
		jc_cgdd_gcjsr=cf.GB2Uni(jc_cgdd_gcjsr);
		if (!(jc_cgdd_gcjsr.equals("")))	wheresql+=" and  (jc_cgdd.gcjsr='"+jc_cgdd_gcjsr+"')";
	}
	...
	String jc_cgdd_gcjssj=null;
	jc_cgdd_gcjssj=request.getParameter("jc_cgdd_gcjssj");
	if (jc_cgdd_gcjssj!=null)
	{
		jc_cgdd_gcjssj=jc_cgdd_gcjssj.trim();
		if (!(jc_cgdd_gcjssj.equals("")))	wheresql+="  and (jc_cgdd.gcjssj>=TO_DATE('"+jc_cgdd_gcjssj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_gcjssj=request.getParameter("jc_cgdd_gcjssj2");
	if (jc_cgdd_gcjssj!=null)
	{
		jc_cgdd_gcjssj=jc_cgdd_gcjssj.trim();
		if (!(jc_cgdd_gcjssj.equals("")))	wheresql+="  and (jc_cgdd.gcjssj<=TO_DATE('"+jc_cgdd_gcjssj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,jc_cgdd.gcjssj,jc_cgdd.gcjsr,jc_cgdd.pdgcmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,jc_cgdd.htze,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.jhazrq,sdks,jc_cgdd.khbh  ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.gcjssj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgGcjsList.jsp","","","XgGcjsJc_cgdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除工厂接收信息"};//按钮的显示名称
	String[] buttonLink={"DeleteGcjsJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改工厂接收</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
