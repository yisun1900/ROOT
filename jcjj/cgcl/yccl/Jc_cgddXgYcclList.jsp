...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_ychssj=null;
	jc_cgdd_ychssj=request.getParameter("jc_cgdd_ychssj");
	if (jc_cgdd_ychssj!=null)
	{
		jc_cgdd_ychssj=jc_cgdd_ychssj.trim();
		if (!(jc_cgdd_ychssj.equals("")))	wheresql+="  and (jc_cgdd.ychssj>=TO_DATE('"+jc_cgdd_ychssj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_ychssj=request.getParameter("jc_cgdd_ychssj2");
	if (jc_cgdd_ychssj!=null)
	{
		jc_cgdd_ychssj=jc_cgdd_ychssj.trim();
		if (!(jc_cgdd_ychssj.equals("")))	wheresql+="  and (jc_cgdd.ychssj<=TO_DATE('"+jc_cgdd_ychssj+"','YYYY/MM/DD'))";
	...
	String jc_cgdd_aztzr=null;
	jc_cgdd_aztzr=request.getParameter("jc_cgdd_aztzr");
	if (jc_cgdd_aztzr!=null)
	{
		jc_cgdd_aztzr=cf.GB2Uni(jc_cgdd_aztzr);
		if (!(jc_cgdd_aztzr.equals("")))	wheresql+=" and  (jc_cgdd.aztzr='"+jc_cgdd_aztzr+"')";
	}

	String jc_cgdd_ksazsj=null;
	jc_cgdd_ksazsj=request.getParameter("jc_cgdd_ksazsj");
	if (jc_cgdd_ksazsj!=null)
	{
		jc_cgdd_ksazsj=jc_cgdd_ksazsj.trim();
		if (!(jc_cgdd_ksazsj.equals("")))	wheresql+="  and (jc_cgdd.ksazsj>=TO_DATE('"+jc_cgdd_ksazsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_ksazsj=request.getParameter("jc_cgdd_ksazsj2");
	if (jc_cgdd_ksazsj!=null)
	{
		jc_cgdd_ksazsj=jc_cgdd_ksazsj.trim();
		if (!(jc_cgdd_ksazsj.equals("")))	wheresql+="  and (jc_cgdd.ksazsj<=TO_DATE('"+jc_cgdd_ksazsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,DECODE(jc_cgdd.ycclzt,'0','未延迟','1','延迟未核实','2','延迟原因属实','3','延迟原因不属实') as ycclzt,b.mmycyymc zsycyybm,crm_khxx.khxm,crm_khxx.fwdz,jc_cgdd.pdgcmc,jc_cgdd.ychssj,jc_cgdd.ychsr,jc_cgdd.kazsj,jc_cgdd.aztzsj,jc_cgdd.ksazsj,jc_cgdd.sazrq,a.mmycyymc,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.khbh  ";
	ls_sql+=" FROM jc_cgdd,jdm_cgddzt,crm_khxx,jdm_mmycyybm a,jdm_mmycyybm b";
    ls_sql+=" where jc_cgdd.khbh=crm_khxx.khbh(+) and jc_cgdd.clzt=jdm_cgddzt.clzt(+)";
    ls_sql+=" and jc_cgdd.ycyybm=a.mmycyybm(+) and jc_cgdd.zsycyybm=b.mmycyybm(+)";
    ls_sql+=" and jc_cgdd.ycclzt in('2','3')";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.ychssj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgYcclList.jsp","","","XgHsJc_cgdd.jsp");
//	pageObj.setEditStr("延迟处理");
...
//设置按钮参数
	String[] buttonName={"删除处理信息"};//按钮的显示名称
	String[] buttonLink={"DeleteYcclJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改延迟处理</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="5%">处理状态</td>
	<td  width="6%">延迟处理</td>
	<td  width="9%">真实延迟原因</td>
	<td  width="4%">客户姓名</td>
	<td  width="11%">房屋地址</td>
	<td  width="9%">派单工厂</td>
	<td  width="4%">延迟核实时间</td>
	<td  width="3%">延迟核实人</td>
	<td  width="4%">协商安装时间</td>
	<td  width="4%">发安装通知时间</td>
	<td  width="4%">开始安装时间</td>
	<td  width="4%">安装完成时间</td>
	<td  width="10%">延迟原因</td>
