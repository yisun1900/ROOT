...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,crm_khxx.khxm,crm_khxx.fwdz,jc_cgdd.pdgcmc,jc_cgdd.kazsj,jc_cgdd.jhazrq,jc_cgdd.aztzsj,jc_cgdd.ksazsj,jc_cgdd.sazrq,mmycyymc,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.khbh  ";
	ls_sql+=" FROM jc_cgdd,jdm_cgddzt,crm_khxx,jdm_mmycyybm";
    ls_sql+=" where jc_cgdd.khbh=crm_khxx.khbh(+) and jc_cgdd.clzt=jdm_cgddzt.clzt(+)";
    ls_sql+=" and jc_cgdd.ycyybm=jdm_mmycyybm.mmycyybm(+)";
    ls_sql+=" and jc_cgdd.clzt in('31','33') and jc_cgdd.ycclzt in('0','1')";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.clzt,jc_cgdd.ksazsj";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgAzList.jsp","","","XgAzJc_cgdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteAzJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改安装</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
