...
try {
	...
	for (int i=0;i<ddbh.length ;i++ )
	{
		...
		ls_sql+=" where khbh='"+khbh+"' and ddbh='"+ddbh[i]+"' and scbz='N' and fklxbm='22'";//22:橱柜款
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();

		ls_sql="update cw_khfkjl set zcgysbh=null";
		ls_sql+=" where khbh='"+khbh+"' and zcddbh='"+ddbh[i]+"' and scbz='N' and zckx='22'";//22:橱柜款
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		ls_sql+=" where  ddbh='"+ddbh[i]+"' and lx='2'";//1：木门；2：橱柜；3：家具；4：主材
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
	}
