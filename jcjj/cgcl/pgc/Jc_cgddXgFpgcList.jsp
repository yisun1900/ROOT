...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_pgcr=null;
	jc_cgdd_pgcr=request.getParameter("jc_cgdd_pgcr");
	if (jc_cgdd_pgcr!=null)
	{
		jc_cgdd_pgcr=cf.GB2Uni(jc_cgdd_pgcr);
		if (!(jc_cgdd_pgcr.equals("")))	wheresql+=" and  (jc_cgdd.pgcr='"+jc_cgdd_pgcr+"')";
	}
	...
	String jc_cgdd_pgcsj=null;
	jc_cgdd_pgcsj=request.getParameter("jc_cgdd_pgcsj");
	if (jc_cgdd_pgcsj!=null)
	{
		jc_cgdd_pgcsj=jc_cgdd_pgcsj.trim();
		if (!(jc_cgdd_pgcsj.equals("")))	wheresql+="  and (jc_cgdd.pgcsj>=TO_DATE('"+jc_cgdd_pgcsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_pgcsj=request.getParameter("jc_cgdd_pgcsj2");
	if (jc_cgdd_pgcsj!=null)
	{
		jc_cgdd_pgcsj=jc_cgdd_pgcsj.trim();
		if (!(jc_cgdd_pgcsj.equals("")))	wheresql+="  and (jc_cgdd.pgcsj<=TO_DATE('"+jc_cgdd_pgcsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,jc_cgdd.pgcsj,jc_cgdd.pgcr,jc_cgdd.pdgcmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,jc_cgdd.htze,jc_cgdd.gtbfje,jc_cgdd.tmbfje,jc_cgdd.wjhtze,jc_cgdd.dqhtze,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.jhazrq,sdks,xtbc,cghs,tmpp,tmhs,blcz,jiaolian,jc_cgdd.khbh  ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.pgcsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgFpgcList.jsp","","","XgFpgcJc_cgdd.jsp");
//	pageObj.setPageRow(1000);//设置每页显示记录数
//	pageObj.setEditStr("派工厂");//设置每页显示记录数
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteFpgcJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改工厂</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
