...
try {
	...
	String khbh=null;
	String fgsbh=null;
	ls_sql =" select clzt,gys,ppmc,khbh,fgsbh";
	...
	ls_sql ="insert into jc_zcysdmx (xh,ddbh,zcbm,zcmc,zcdlbm,zcxlbm,dwbh,zclbbm,xinghao,gg   ,zcysbm,jldwbm,cbj,jsbl,lsj,yhj,xzjg,zqdj,dpzk,dj   ,sl,zqje,je,ycf,qtfy,qtfyje,zjhsl,zqzjhje,zjhje,zjhycf   ,zjhqtfy,zcpzwzbm,cxhdbz,cxhdbl,cxhdje,sfbhpj,pjts,sfycx,cxj,bz   ,lrr,lrsj,lx,sxbz,sxhtyy,gys,ppmc,khbh,fgsbh,zjhcxhdje,sftjcp) ";
	ls_sql+=" values(?,?,?,?,?,?,?,?,?,?                                                     ,?,?,?,?,?,?,?,?,?,?                                ,?,?,?,?,?,?,?,?,?,?                                     ,?,?,?,?,?,?,?,?,?,?                                              ,?,SYSDATE,'1','2',?,?,?,?,?,?,?)";
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,xh);
	ps.setString(2,ddbh);
	...
	ps.setDouble(47,cxhdje);
	ps.setString(48,sftjcp);
	ps.executeUpdate();
	ps.close();
