...
<TR bgcolor="#CCCCCC"  align="center" style="FONT-WEIGHT: bold; Z-INDEX: 5; POSITION: relative;TOP: expression(this.offsetParent.scrollTop)"> 
	...
	<td  width="2%" style="POSITION: relative; BACKGROUND-COLOR: #CC6600"><span class="STYLE1">折前材料费</span></td>
...
<%
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center">
			...
			<td >
				<input name="bz" type="text" value="<%=bz%>" size="64" maxlength="300" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid">			</td>
...
function f_do(FormName)//参数FormName:Form的名称
{
	if (<%=row%>==1)
	{
		...
		{
			alert("主材编码["+FormName.zcbm.value+"]出错！[折扣率]不能小于0！");
		...
		for (var i=0;i< <%=row%> ;i++ )
		{
			...
			if (FormName.dpzk[i].value<0)
			{
				alert("主材编码["+FormName.zcbm[i].value+"]出错！[折扣率]不能小于0！");
