...
<%
	...
	if (!clzt.equals("07"))
	{
		out.println("<BR>提醒，不能再进行修改！合同已完成");
		return;
	}
%>
...
<form method="post" action="" name="selectform" target="_blank">
  ...
  <table width="100%" border="0" name="firsttable" id="firsttable" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    ...
    <tr bgcolor="#FFFFCC">
      <td align="right" bgcolor="#FFFFCC"><font color="#0000CC">折扣率</font></td>
      <td><input type="text" name="dpzk" size="10" maxlength="8"  value="10" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" readonly >
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<font color="#FF0000">*</font>折后实销价
        <input type="text" name="dj" size="8" maxlength="16"  value=""  style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onChange="f_jsjg(selectform)"></td>
    </tr>
    <tr bgcolor="#FFFFCC"> 
    ...
    <tr bgcolor="#FFFFCC"> 
      <td colspan="2"><div id="kcslStr"></div>
	  </td>
    </tr>
...
%> </td>
    ...
    <tr bgcolor="#FFFFCC"> 
      <td width="21%" align="right"><font color="#FF0000">*</font>计入比率</td>
      <td width="79%"> 
        <input type="text" name="cxhdbl" id="cxhdbl2" size="10" maxlength="9"  style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onChange="f_jsjg(selectform)">
...
%>
    ...
    <tr> 
      <td colspan="4" height="2"> 
		...
		<input type="hidden" name="xsfs"  value="" >
		<input type="hidden" name="xsxh"  value="" >
		<input type="hidden" name="zxqdl"  value="" >
		<input type="hidden" name="zcpzsl"  value="" >
		<input type="hidden" name="dhsl"  value="" >
		<input type="hidden" name="zdxsj"  value="" >
...
function getAjax(ajaxRetStr) 
{
	...
	if(ajaxlx==1)
	{
		...
		txtNode=document.createTextNode(xhsz2[13]+"(市场标价:"+xhsz2[12]+")"+"(最低销售价:"+xhsz2[36]+")");//销售限价
		...
		document.getElementById('kcslStr').innerHTML="";
		if (xhsz2[33]<xhsz2[35])
		{
			txtNode=document.createTextNode("最小起订:"+xhsz2[32]+"、库存:"+xhsz2[33]+"、已订货:"+xhsz2[34]+"、★★报警最低库存:"+xhsz2[35]);//备注
		}
		else{
			txtNode=document.createTextNode("最小起订:"+xhsz2[32]+"、库存:"+xhsz2[33]+"、已订货:"+xhsz2[34]);//备注
		}
		document.getElementById('kcslStr').appendChild(txtNode);
		...
		document.all.xsfs.value=xhsz2[30];
		document.all.xsxh.value=xhsz2[31];
		document.all.zxqdl.value=xhsz2[32];
		document.all.zcpzsl.value=xhsz2[33];
		document.all.dhsl.value=xhsz2[34];
		document.all.zdxsj.value=xhsz2[36];
...
{   
	...
	if(event.keyCode==13)
	{
		...
		ajaxlx=2;
		actionStr="/jcjj/kczcdd/GetCpxxByAjax.jsp?srxh="+FormName.srxh.value+"&dqbm=<%=dqbm%>&ppbm=<%=ppbm%>";
...
//		window.open(actionStr);
	...
	else if(event.keyCode==40)
	{
		...
		//获取型号列表
		ajaxlx=1;
		actionStr="/jcjj/kczcdd/GetXhListByAjax.jsp?srxh="+FormName.srxh.value+"&dqbm=<%=dqbm%>&ppbm=<%=ppbm%>";
//		window.open(actionStr);
		openAjax(actionStr);  
	}
...
function changeItem(FormName,field)
{
	...
	else if(event.keyCode==13)
	{
		...
		ajaxlx=2;
		actionStr="/jcjj/kczcdd/GetCpxxByAjax.jsp?clbm="+FormName.clbm.value;
//		window.open(actionStr);
		openAjax(actionStr);
	}
}
...
function f_do2(FormName)//参数FormName:Form的名称
{
	...
	if(!(isFloat(FormName.dj, "折后实销价"))) {
		return false;
	}

	if (parseFloat(FormName.dj.value) < parseFloat(FormName.zdxsj.value))
	{
		alert("错误！[折后实销价]不能小于最低销售限价【"+FormName.zdxsj.value+"】！");
		FormName.dj.select();
		return false;
	}
	...
	{
		var kdhsl=round(parseFloat(FormName.zcpzsl.value)-parseFloat(FormName.dhsl.value),2);
