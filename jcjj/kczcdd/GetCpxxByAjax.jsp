...
String srxh="";
srxh=cf.GB2Uni(cf.getParameter(request,"srxh"));
String dqbm=request.getParameter("dqbm");
String ppbm="";
ppbm=cf.GB2Uni(cf.getParameter(request,"ppbm"));
...
//	out.println(ls_sql);
	...
	if (rs.next())
	{
		...
		xsfs=cf.fillNull(rs.getString("xsfs"));//1：代销品；2：现货销售；3：库存无货可销售
		zxqdl=rs.getDouble("zxqdl");//最小起订量
		zcpzsl=rs.getDouble("zcpzsl");//正常品总数量
		dhsl=rs.getDouble("dhsl");//客户订货数量
		zdkcsl=rs.getDouble("zdkcsl");//最低库存数量
		zdxsj=rs.getDouble("zdxsj");//最低销售限价
		...
		getStr2+="+"+xsfs;//销售方式//30
		getStr2+="+"+nbbm;//内部编码
		getStr2+="+"+zxqdl;//最小起订量
		getStr2+="+"+zcpzsl;//正常品总数量
		getStr2+="+"+dhsl;//客户订货数量
		getStr2+="+"+zdkcsl;//最低库存数量
		getStr2+="+"+zdxsj;//最低销售限价
		getStr2+="+";
	}
	rs.close();
	ps.close();
