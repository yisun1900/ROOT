...
try {
	...
	double yjjrbl=0;
	double cxyjjrbl=0;
	ls_sql="select ppmc,gysmc,scsmc,yjjrbl,cxyjjrbl";
	...
	if (mark==0)
	{
		...
		double htyjjrje=0;
		if (xzjg.equals("2"))//0:手写项目；1:销售限价；2:促销价；3: 市场标价
		{
			htyjjrje=cxyjjrbl/100*je;
		}
		else{
			htyjjrje=yjjrbl/100*je;
		}

		
		ls_sql ="insert into jc_zcddmx (xh,ddbh,khbh,zcbm,zcmc,zcdlbm,zcxlbm,dwbh,gys,ppmc,zclbbm,xinghao,gg                                                                                      ,zcysbm,jldwbm,sfbhpj,pjts,lsj,yhj                                                 ,bzcbj,bzjsbl,bzcxhdbl,sfycx,cxkssj,cxjzsj,cxj,cxcbj,cxjsbl,lscxhdbl                                                                                                ,xzjg,zqdj,dpzk,dj,cbj,jsbl,sl,zqje,je,ycf,qtfy,qtfyje,zjhsl,zqzjhje,zjhje,zjhycf,zjhqtfy  ,zcpzwzbm,cxhdbz,cxhdbl,cxhdje,zjhcxhdje,lx,lrr,lrsj  ,sxbz,sxhtyy,fgsbh,qddm,jzsjs,bz,sftjcp  ,zxqdl          ,xsxh         ,cksl,ckje,sfzp,zsdj,shsl         ,jsfs,zdxsj,bzyjjrbl,cxyjjrbl,yjjrbl  ,htyjjrje,zjhyjjrje)"; 
		ls_sql+="                select ?,?,?,?          ,jxc_clbm.clmc,jxc_clbm.cldlbm,jxc_clbm.clxlbm,jxc_clbm.ppmc,jxc_cljgb.gysmc,jxc_clbm.scsmc,jxc_clbm.cllbbm,jxc_clbm.xh,jxc_clbm.gg      ,?     ,jxc_clbm.jldwbm,jxc_clbm.sfbhpj,jxc_clbm.pjts,jxc_cljgb.lsj,jxc_cljgb.xsj  ,jxc_cljgb.cbj,jxc_cljgb.jsbl,jxc_cljgb.cxhdbl,jxc_cljgb.sfycx,jxc_cljgb.cxkssj,jxc_cljgb.cxjzsj,jxc_cljgb.cxj,jxc_cljgb.cxcbj,jxc_cljgb.cxjsbl,jxc_cljgb.lscxhdbl  ,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?                                                         ,?,?,?,?,?,'1',?,SYSDATE                              ,?,?,?,?,?,?,sftjcp                      ,jxc_cljgb.zxqdl,jxc_clbm.nbbm,0   ,0   ,?   ,?   ,0            ,jsfs,zdxsj,yjjrbl  ,cxyjjrbl,cxyjjrbl,?       ,?";//xzjg 1:销售限价；2:促销价
		ls_sql+=" FROM jxc_clbm,jxc_cljgb";
		ls_sql+=" where jxc_clbm.clbm=jxc_cljgb.clbm";
		ls_sql+=" and jxc_clbm.clbm='"+clbm+"' and jxc_cljgb.dqbm='"+dqbm+"'";
		ps= conn.prepareStatement(ls_sql);
		...
		ps.setDouble(37,htyjjrje);
		ps.setDouble(38,htyjjrje);
		ps.executeUpdate();
		ps.close();
