...
try {
	...
	for (i=0;i<xuhao.length ;i++ )
	{
		...
		sql+=" ,cpsm,cd,gys,ppmc,zp,dqbm,lrr,lrsj,lrbm,bz,sftjcp)";
		...
		sql+=" ,'"+cf.GB2Uni(cpsm[i])+"','"+cf.GB2Uni(cd[i])+"','"+cf.GB2Uni(gys[i])+"','"+cf.GB2Uni(ppmc[i])+"','"+cf.GB2Uni(zp[i])+"','"+dqbm+"','"+yhmc+"',SYSDATE,'"+lrbm+"','"+cf.GB2Uni(bz[i])+"','"+sftjcp[i]+"'";
		sql+="  )";
//		out.println(sql);
		ps= conn.prepareStatement(sql);
