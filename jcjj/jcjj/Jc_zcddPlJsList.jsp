...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		ddlx=cf.GB2Uni(ddlx);
		if (!(ddlx.equals("")))	wheresql2+=" and  (jc_zcdd.ddlx='"+ddlx+"')";
	}
	...
	{
		qhtsj=qhtsj.trim();
		if (!(qhtsj.equals("")))	wheresql2+="  and (jc_zcdd.qhtsj>=TO_DATE('"+qhtsj+"','YYYY/MM/DD'))";
		if (!(qhtsj.equals("")))	wheresql3+="  and (jc_zczjx.qhtsj>=TO_DATE('"+qhtsj+"','YYYY/MM/DD'))";
	}
	...
	if (ddqrsj!=null)
	{
		...
		if (!(qhtsj.equals("")))	wheresql3+="  and (jc_zczjx.ddqrsj>=TO_DATE('"+ddqrsj+"','YYYY/MM/DD'))";
	}
	...
	ls_sql=" SELECT jc_zcdd.ddbh,crm_khxx.khxm,decode(jc_zcdd.ddlx,'9','集成','E','散客'),DECODE(jc_zcdd.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,clztmc,jc_zcdd.ddqrsj,jc_zcdd.wdzje,jc_zcdd.zcpclf,jc_zcdd.tjpclf,jc_zcdd.fdbl||'%',jc_zcdd.tjpfdbl||'%',jc_zcdd.cbze+round(jc_zcdd.ycf*jc_zcdd.ycfjsbl/100,2)+round(jc_zcdd.qtf*jc_zcdd.azfjsbl/100,2) cbze,jc_zcdd.qtf,jc_zcdd.ycf,jc_zcdd.azfjsbl||'%',jc_zcdd.ycfjsbl||'%',ppbm,jc_zcdd.gys,crm_khxx.khbh,crm_khxx.fwdz,jc_zcdd.clgw,dwmc ";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt,sq_dwxx ";
	...
	ls_sql+=" and jc_zcdd.clzt not in('07','09','98','99')";
...
//设置按钮参数
	String[] buttonName={"批量结算","设置选择标志","打印","删除设置选择标志"};//按钮的显示名称
	String[] buttonLink={"InsertJc_gysjs.jsp?ssfgs="+fgs,"chooseJs.jsp","DyJc_gysjs.jsp","DeleteDGJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单－－厂商批量结算</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="5%">安装费</td>
	<td  width="4%">远程费</td>
	<td  width="3%">安装费结算比例</td>
	<td  width="3%">远程费结算比例</td>
	<td  width="5%">子品牌</td>
	<td  width="10%">供应商</td>
	<td  width="3%">客户编号</td>
	<td  width="10%">房屋地址</td>
