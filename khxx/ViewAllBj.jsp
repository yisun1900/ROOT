...
<table border="1" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
  <tr bgcolor="#FFFFFF" align="center"> 
	...
	<td  width="7%" ><strong>施工成本价百分比</strong></td>
...
<%
	...
	//	out.println(ls_sql);
		...
		while (rs.next())
		{
			...
			if (zqje!=0)
			{
				sgcbjbfb=cf.round(sgcbj*100/zqje,2);
			}
		...
		if (xjzqje!=0)
		{
			sgcbjbfb=cf.round(xjsgcbj*100/xjzqje,2);
		}
	...
	if (allzqje!=0)
	{
		sgcbjbfb=cf.round(allsgcbj*100/allzqje,2);
	}
%> 
...
<table border="1" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
  <tr bgcolor="#FFFFFF" align="center"  >
	...
	<td  width="5%" ><strong>是否可打折</strong></td>
  ...
  <%  
	...
	ls_sql="SELECT bj_sfxmmx.sfxmlx,DECODE(bj_sfxmmx.cbjjsfs,'1','折前','2','折后','3','不记入') cbjjsfs,DECODE(bj_sfxmmx.sfjrhtjz,'Y','记入','N','否') sfjrhtjz,DECODE(sfxmlx,'1','间接工程费','2','工程相关费','3','设计费','4','公司购主材间接费','5','工程管理费','6','工程税金','7','公司购主材管理费','8','公司购主材税金','A','自购主材管理费','B','自购主材税金','C','自购主材间接费','D','水电路','E','水电路预收','9','其它') sfxmlxmc,sfcysgdjs,DECODE(sfcysgdjs,'Y','是','N','否') sfcysgdjsmc,sl,cbjbfb,sgcbjbfb,gcjcbjbfb,dj,ROUND(DECODE(cbjjsfs,'1',zqsfje*cbjbfb/100,sfje*cbjbfb/100),2) cbj,ROUND(DECODE(cbjjsfs,'1',zqsfje*sgcbjbfb/100,sfje*sgcbjbfb/100),2) sgcbj,DECODE(cbjjsfs,'1',zqsfje*gcjcbjbfb/100,sfje*gcjcbjbfb/100) gcjcbj,sfxmbm,sfxmmc,DECODE(sflx,'1','工程直接费百分比（不可改）','A','工程直接费百分比（可改）','2','固定金额(可改)','3','税金（折后按基数计算）','8','税金（折后按折扣计算）','4','固定金额(不可改)','5','管理费','6','公司购主材百分比','7','自购主材百分比','9','按百分比收设计费') sflx,sfbfb,sfje,bz,DECODE(sfbxx,'1','必选','2','非必选') sfbxx,DECODE(sfjrqye,'1','记入','2','不记入') sfjrqye,DECODE(jrsjbz,'1','记入','2','不记入') jrsjbz,DECODE(jrglfbz,'1','记入','2','不记入') jrglfbz,zqsfje,dxzkl,DECODE(sfkdz,'N','不可打折','Y','可打折') sfkdz,zdzk";
...
<%
	...
	ls_sql+=" FROM bj_sfxmmx  ";
    ls_sql+=" where bj_sfxmmx.khbh='"+khbh+"'";
    ls_sql+=" and sfxmlx not in('D','E','4','7','8','A','B','C')";//D：水电路；E：水电路预收
    ls_sql+=" order by bj_sfxmmx.sfxmbm";
//	out.println(ls_sql);
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
...
%> 
  ...
  <tr bgcolor="#FFFFFF"> 
    <td align="left" colspan="20"><strong>总 计&nbsp;&nbsp;&nbsp;&nbsp;折前：<%=cf.formatDouble(allzqje,"#########.##")%>&nbsp;&nbsp;&nbsp;&nbsp;折后：<%=cf.formatDouble(allzhje,"#########.##")%></strong></td>
  </tr>
</table>
