...
<%
	...
	while(rs1.next())
	{
		...
		ls_sql="select count(*) sjsrs,sum(yy_sjbsjf_byjh),sum(yy_sjbsjf_bylj) ";
		...
		{
			sjsrs=rs.getInt(1);
			yy_sjbsjf_byjh=rs.getDouble(2);
			yy_sjbsjf_bylj=rs.getDouble(3);

			yy_sjbsjf_szjh=rs.getDouble(4);
			yy_sjbsjf_szjh=rs.getDouble(5);
			yy_sjbsjf_bzyj=rs.getDouble(6);
			yy_sjbgccz_bylj=rs.getDouble(7);
		}
		rs.close();
		ps.close();
...
<tr bgcolor="#FFFFFF" align="center">
	<td>小计</td>
	<td><%=xjsjsrs%></td>
	<td><%=xjyy_sjsjb_tqsjf%></td>

	<td><%=xjyy_sjbsjf_byjh%></td>
	...
	<td><%=xjyy_sjbsjf_szjh%></td>
	<td><%=xjyy_sjbsjf_szwc%></td>
