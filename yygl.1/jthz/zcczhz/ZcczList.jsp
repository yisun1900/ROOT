...
<%
	...
	while(rs1.next())
	{
		...
		ls_sql="select count(*) jjgwrs,sum(yy_jjb.yy_jjbqntq),sum(yy_jjb.yy_jjbbyjh),sum(yy_jjb.yy_jjbylj)";
		...
		if(rs.next())
		{
			...
			yy_jjbbyjh=rs.getDouble(3);
			yy_jjbylj=rs.getDouble(4);

			yy_jjbszjh=rs.getDouble(5);
			yy_jjbszwc=rs.getDouble(6);
			yy_jjbbzyj=rs.getDouble(7);
			yy_jjbdmbylj=rs.getDouble(8);
		}
		rs.close();
		ps.close();
