...
try {
	...
	ls_sql="insert into call_qdjl (xh,ldsj,ldhm,ldlxbm,khbh,khlx,bjhm,lywjml,lywj,jtzt,lrr,lrsj,lrbm,ssfgs) ";
	ls_sql+=" values(?,SYSDATE,?,?,?,?,?,?,?,?,?,SYSDATE,?,?) ";//1：未接通；2：接通
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,xh);
	...
	if (rs.next())
	{
		...
		khlx="2";
		lywj=lywjml+"\\"+"out-"+xh+"-"+khbh+"-"+khxm+"（"+xb+"）（签单）"+".wav";
		...
		if (rs.next())
		{
			...
			khlx="1";
			lywj=lywjml+"\\"+"out-"+xh+"-"+khbh+"-"+khxm+"（"+xb+"）（未签单）"+".wav";
	...
	ls_sql="update call_qdjl set khlx=?,lywj=? ";
	ls_sql+=" where xh="+xh;
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,khlx);
	ps.setString(2,lywj);
