<%@ page contentType="text/html;charset=GBK" %>
<jsp:useBean id="cf" scope="page" class="ybl.common.CommonFunction"/>
<jsp:setProperty name="cf" property="*" />
<%@ include file="/getlogin.jsp" %>

<html>
<head>
<title>录入数据</title>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
</head>

<body bgcolor="#FFFFFF">
<form method="post" action="SaveInsertNblt_bkgl.jsp" name="insertform" target="_blank">
<div align="center">请录入数据</div>
<table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
<tr bgcolor="#FFFFFF">
  <td align="right">板块编号</td>
  <td width="35%"><input type="text" name="bkbh" value="" size="20" maxlength="2" >（两位数字）</td>
  <td width="15%" align="right">是否匿名帖</td>
  <td width="35%"><select name="sfnmt" style="FONT-SIZE:12PX;WIDTH:152PX" >
	<option value=""></option>
	<option value="Y">匿名帖</option>
	<option value="N">非匿名帖</option>
	</select></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%">板块名称</td> 
  <td colspan="3"><input type="text" name="bkmc" value="" size="60" maxlength="50" ></td>
  </tr>
    <tr align="center"> 
      <td colspan="4"> 
	<input type="button"  value="存盘" onClick="f_do(insertform)">
	<input type="reset"  value="重输" name="reset">      </td>
    </tr>
</table>
关于匿名贴：<br>
匿名帖：板块内所有发贴和回复全部匿名显示！
</form>
</body>
</html>
<SCRIPT language=javascript src="/js/validate.js"></SCRIPT>
<SCRIPT language=javascript >
<!--
function f_do(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.bkbh)=="") {
		alert("请输入[板块编号]！");
		FormName.bkbh.focus();
		return false;
	}
	if(	javaTrim(FormName.sfnmt)=="") {
		alert("请输入[是否匿名帖]！");
		FormName.sfnmt.focus();
		return false;
	}
	if(	javaTrim(FormName.bkmc)=="") {
		alert("请输入[板块名称]！");
		FormName.bkmc.focus();
		return false;
	}
	FormName.submit();
	return true;
}
//-->
</SCRIPT>
