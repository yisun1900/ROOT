...
String ssnr=cf.GB2Uni(cf.getParameter(request,"ssnr"));
...
String wheress="";
//out.print(ssnr);
if (ssfw!=null && !(ssfw.equals("")))
{
	if (ssfw.equals("1"))//主题
	{wheress=" and ftzt like '%"+ssnr+"%' ";}
	else if (ssfw.equals("2"))//内容
	{wheress=" and ftnr like '%"+ssnr+"%' ";}
	else if (ssfw.equals("3"))//回复
	{}
}
else
{out.print("请选择搜索范围！");}
...
<table width="98%" id="table2" border=0 cellpadding=0 cellspacing=1 bgcolor="#BBE9FF">
	...
	<td><span style="FONT-SIZE:12">我的：&nbsp;<a href="/nblt/fwlt/mytz.jsp?yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>">[贴子]</a>&nbsp;||&nbsp;<a href="/nblt/fwlt/myhf.jsp?yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>">[回复]</a>&nbsp;||&nbsp;<a href="/nblt/grzlgl/editgrzl.jsp?yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>">[资料]</a></span></td>
	</tr></table>
	</div>
	</td>
	</tr>
...
<div id="div1" >
		...
		<tr>
		<td width="90%">
		<span style="FONT-SIZE:12">&nbsp;&nbsp;<a href="ktz.jsp?yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>">首页</a></span>
		</td>
...
try {
	...
	ls_sql+=" where a.ltid=nblt_ftgl.ftr and b.ltid=nblt_ftgl.zhhfr ";
	ls_sql+=wheress;
...
//		}
		...
		<tr>
		<td width="70%">
		<span style="FONT-SIZE:14"><%=sfzd%>&nbsp;<a href="cktz.jsp?tzbh=<%=tzbh%>&yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>"><%=ftzt%></a></span>
		</td>
		<td width="100px" align="center">
...
{
//	alert(djys+<%=tzbh%>);
	FormName.action="mytz.jsp?ys="+djys;
	FormName.submit();
	return true;
}
function f_srtz(FormName)//参数FormName:Form的名称
{
	if (event.keyCode==13)
	{
		...
		{
			FormName.action="mytz.jsp?ys="+FormName.rddqys2.value+"&yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>";
		}
		else
		{
			FormName.action="mytz.jsp?ys="+FormName.rddqys.value+"&yhdlm=<%=yhdlm%>&ygbh=<%=ygbh%>";
		}
	FormName.submit();
	return true;
	}
