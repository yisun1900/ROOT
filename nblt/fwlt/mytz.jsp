...
try {
	...
	ls_sql+=" where a.ltid=nblt_ftgl.ftr and b.ltid=nblt_ftgl.zhhfr and ftr='"+dlygbh+"' ";
...
//		}
		...
		<tr>
		<td width="70%">
		<span style="FONT-SIZE:14">[<A href=# onclick="f_sc(insertform,'<%=tzbh%>')">删除</A>]<%=sfzd%>&nbsp;<a href="cktz.jsp?tzbh=<%=tzbh%>"><%=ftzt%></a></span>
		</td>
		<td width="100px" align="center">
...
function f_sc(FormName,tzbh)//参数FormName:Form的名称
{
	if (!confirm("确实要删除吗?") )	
	{
		return;
	...
	FormName.action="deletemytz.jsp?tzbh="+tzbh;
	FormName.submit();
	return true;
}
function f_srtz(FormName)//参数FormName:Form的名称
{
	if (event.keyCode==13)
	{
		...
		{
			FormName.action="mytz.jsp?ys="+FormName.rddqys2.value;
		}
		else
		{
			FormName.action="mytz.jsp?ys="+FormName.rddqys.value;
		}
	FormName.submit();
	return true;
	}
