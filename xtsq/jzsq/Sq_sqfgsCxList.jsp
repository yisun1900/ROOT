...
<%
if (cf.check((String)session.getAttribute("yhdlm"),request.getRemoteHost(),"010212")==0) 
{
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String sqfgs=null;
	sqfgs=request.getParameter("sqfgs");
	if (sqfgs!=null)
	{
		sqfgs=cf.GB2Uni(sqfgs);
		if (!(sqfgs.equals("")))	wheresql+=" and  (sq_sqfgs.ssfgs='"+sqfgs+"')";
	}
	...
	ls_sql="SELECT b.dwmc ssfgs,a.dwmc as ssdw,yhmc,DECODE(sq_yhxx.yhjs,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','部门小组','S0','施工队','G0','供应商','J0','加盟商') as yhjs,xzzwbm,zwmc,c.dwmc sqfgs";
	ls_sql+=" FROM sq_yhxx,sq_dwxx a,sq_dwxx b,sq_dwxx c,dm_zwbm,sq_sqfgs";
	...
	ls_sql+=" and sq_yhxx.ygbh=sq_sqfgs.ygbh and sq_sqfgs.ssfgs=c.dwbh";
	ls_sql+=wheresql;
    ls_sql+=" order by sq_yhxx.ssfgs,sq_yhxx.dwbh,sq_yhxx.yhmc,sq_sqfgs.ssfgs";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_sqfgsCxList.jsp","SelectCxSq_sqfgs.jsp","","");
