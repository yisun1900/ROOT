...
try {
	...
	ls_sql="update sq_dwxx set dwbh=?,dwmc=?,dwjc=?,ssdw=?,dwlx=?,dwdz=?,dwfzr=?,dwdh=?,dwcz=?,email=?,bz=?,dqbm=?,sfjms=?,hthqz=?,cxbz=?,ssfgs=?,bianma=UPPER(?),cwdm=?,ssdqbm=?,dwflbm=?,ssfw=?,dwjb=?,dmxs=?,bzrs=?,jryjbfb=?,jcppbz=?,zxkhlrjc=?,zxkhlrts=?,qdkhbljc=?,qdkhblts=?,dzbjjc=?,hfsjzdsz=?,xmzyglbz=?,jjgwglbz=?,khxxglsj=?,jcddblxs=?,sjjryjbfb=?,kfdh=?,jjsjsglbz=?,jcddmxblxs=?,zcmmbz=?,mmbz=?,cgjjbz=?,jjbz=?,djlrfs=?,kgcxhdbl=?,xuhao=?,bzlybl=?,lyblxx=?,lyblsx=?,dhqh=?";
	ls_sql+=" where  (dwbh='"+dwbh+"')  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,dwbh);
	ps.setString(2,dwmc);
