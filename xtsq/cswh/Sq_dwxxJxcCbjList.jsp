...
<%
if (cf.check((String)session.getAttribute("yhdlm"),request.getRemoteHost(),"010133")==0) 
{
...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT sq_dwxx.dwbh,dwmc,DECODE(fccbj,'1','入库价','2','加权平均价') fccbj,DECODE(zccbj,'1','入库价','2','加权平均价') zccbj,DECODE(sq_dwxx.sfqyyssh,'Y','启用','N','不启用'),DECODE(sq_dwxx.zdyxmshbz,'Y','启用','N','不启用'),sq_dwxx.zdyxmyxq,sq_dwxx.clfbfb||'%',sq_dwxx.cbbfb||'%',sq_dwxx.bianma,sq_dwxx.dhqh,DECODE(dwlx,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','设计小组','F4','市场小组','F5','其它小组') as dwlx,DECODE(sfjms,'Y','加盟','N','直营公司','Q','其它公司'),DECODE(sq_dwxx.cxbz,'Y','撤销','N','否'),dwdh ";
	ls_sql+=" FROM sq_dwxx  ";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_dwxxJxcCbjList.jsp","","","EditJxcCbjSq_dwxx.jsp");
...
<CENTER >
  <B><font size="3">进销存成本价－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
