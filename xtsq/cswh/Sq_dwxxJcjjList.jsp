...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT sq_dwxx.dwbh,dwmc,DECODE(jcppbz,'Y','无订单无法录入','N','无订单可录入'),DECODE(zcmmbz,'Y','无订单不能收款','N','无订单可收款','M','可空'),DECODE(mmbz,'Y','无订单不能收款','N','无订单可收款','M','可空'),DECODE(cgjjbz,'Y','无订单不能收款','N','无订单可收款','M','可空'),DECODE(jjbz,'Y','无订单不能收款','N','无订单可收款','M','可空'),DECODE(jcddblxs,2,'2位小数',1,'1位小数',0,'保留个位',-1,'保留十位',-2,'保留百位',-3,'保留千位'),DECODE(jcddmxblxs,2,'2位小数',1,'1位小数',0,'保留个位',-1,'保留十位',-2,'保留百位',-3,'保留千位'),DECODE(sq_dwxx.xmzyglbz,'Y','启用','N','不启用'),DECODE(sq_dwxx.jjgwglbz,'Y','启用','N','不启用'),DECODE(sq_dwxx.jjsjsglbz,'Y','启用','N','不启用'),sq_dwxx.bianma,sq_dwxx.dhqh,DECODE(dwlx,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','设计小组','F4','市场小组','F5','其它小组') as dwlx,DECODE(sfjms,'Y','加盟','N','直营公司','Q','其它公司'),DECODE(sq_dwxx.cxbz,'Y','撤销','N','否')";
	ls_sql+=" FROM sq_dwxx  ";
    ls_sql+=" where (1=1)";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_dwxxJcjjList.jsp","","","EditJcjjSq_dwxx.jsp");
...
<CENTER >
  <B><font size="3">集成家居参数－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="7%">投诉报修订单检查</td>
	<td  width="7%">主材收款检查</td>
	<td  width="7%">木门收款检查</td>
	<td  width="7%">橱柜收款检查</td>
	<td  width="7%">家具收款检查</td>
	<td  width="6%">集成订单保留小数</td>
	<td  width="6%">集成订单明细保留小数</td>
	<td  width="5%">集成订单对项目专员过滤</td>
	<td  width="5%">集成订单对家居顾问过滤</td>
	<td  width="5%">集成订单对家居设计师过滤</td>
