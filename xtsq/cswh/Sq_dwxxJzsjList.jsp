...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT sq_dwxx.dwbh,dwmc,NVL2(sq_yjjzsj.jzrq,sq_yjjzsj.jzrq||'日',''),NVL2(sq_yjjzsj.jzsj,sq_yjjzsj.jzsj||'时',''),sq_dwxx.bianma,sq_dwxx.dhqh,DECODE(dwlx,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','设计小组','F4','市场小组','F5','其它小组') as dwlx,DECODE(sfjms,'Y','加盟','N','直营公司','Q','其它公司'),DECODE(sq_dwxx.cxbz,'Y','撤销','N','否'),dwdh,dwcz,sq_dwxx.bz";
	ls_sql+=" FROM sq_dwxx,sq_yjjzsj  ";
    ls_sql+=" where sq_dwxx.dwbh=sq_yjjzsj.dwbh(+)";
    ls_sql+=wheresql;
	...
	ls_sql+=" order by dwbh,sq_yjjzsj.jzrq";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_dwxxJzsjList.jsp","","","EditJzsjSq_dwxx.jsp");
...
<CENTER >
  <B><font size="3">业绩结转时间－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
