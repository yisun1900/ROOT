...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT sq_dwxx.dwbh,dwmc,DECODE(zxkhlrjc,'Y','启用','N','不启用'),zxkhlrts,DECODE(qdkhbljc,'Y','启用','N','不启用'),qdkhblts,sq_dwxx.djbhts,sq_dwxx.scyjts,sq_dwxx.bianma,sq_dwxx.dhqh,DECODE(dwlx,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','设计小组','F4','市场小组','F5','其它小组') as dwlx,DECODE(sfjms,'Y','加盟','N','直营公司','Q','其它公司'),DECODE(sq_dwxx.cxbz,'Y','撤销','N','否'),dwdh";
	ls_sql+=" FROM sq_dwxx  ";
    ls_sql+=" where (1=1)";
...
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_dwxxLrgfList.jsp","","","EditLrgfSq_dwxx.jsp");
...
<CENTER >
  <B><font size="3">录入信息规范－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="4%">单位编号</td>
	<td  width="10%">单位名称</td>
	<td  width="6%">启用咨询客户录入检查</td>
	<td  width="6%">咨询客户提前录入天数</td>
	<td  width="6%">启用签单客户补录检查</td>
	<td  width="6%">签单客户补录允许天数</td>
	<td  width="6%">交定金客户保护天数</td>
	<td  width="6%">市场客户移交有效天数</td>
	<td  width="4%">编码</td>
	<td  width="4%">电话区号</td>
	<td  width="4%">单位类型</td>
	<td  width="5%">公司类型</td>
