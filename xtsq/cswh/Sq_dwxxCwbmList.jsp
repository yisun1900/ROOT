...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT sq_dwxx.dwbh,dwmc,sq_dwxx.cwdm,(select dwmc from sq_dwxx b where b.dwbh=sq_dwxx.ssfgs) ssfgsmc,sq_dwxx.bianma,sq_dwxx.dhqh,DECODE(dwlx,'A0','总部','A1','总部部门','F0','分公司','F1','分公司部门','F2','店面','F3','设计小组','F4','市场小组','F5','其它小组') as dwlxmc,DECODE(sfjms,'Y','加盟','N','直营公司','Q','其它公司'),DECODE(sq_dwxx.cxbz,'Y','撤销','N','否'),dwdh,dwcz,sq_dwxx.bz";
	ls_sql+=" FROM sq_dwxx  ";
    ls_sql+=" where (1=1)";
    ...
    ls_sql+=wheresql;
	ls_sql+=" order by ssfgs,dwlx,dwbh";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Sq_dwxxCwbmList.jsp","","","EditCwbmSq_dwxx.jsp");
...
//设置显示合并列
	...
	spanColHash.put("ssfgsmc","1");//列参数对象加入Hash表
	spanColHash.put("dwlxmc","1");//列参数对象加入Hash表
...
<CENTER >
  <B><font size="3"> 部门财务编码－维护</font></B>
</CENTER>
<%
	pageObj.out=out;
