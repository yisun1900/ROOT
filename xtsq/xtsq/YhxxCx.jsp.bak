...
try {
	...
	//	out.println(ls_sql);
		...
		//供应商
		ls_sql ="select sq_gysbm.yhdlm,sq_gysbm.yhdlm||yhmc||'（供应商用户）' ";
		ls_sql+=" from sq_gysbm,sq_yhssz ";
		ls_sql+=" where sq_yhssz.yhdlm=sq_gysbm.yhdlm and yhzbh='"+yhzbh+"' ";
...
//		out.println(ls_sql);
		...
		ls_sql+=" from sq_gysbm,sq_gyssqfgs  ";
		ls_sql+=" where sq_gysbm.gysbm=sq_gyssqfgs.gysbm  ";
		ls_sql+=" and yhdlm not in(select yhdlm from sq_yhssz where yhzbh='"+yhzbh+"') ";

		if (xzfgs!=null)
		{
			if (!(xzfgs.equals("")))	ls_sql+=" and  (sq_gyssqfgs.ssfgs='"+xzfgs+"')";
		}
		ls_sql+=" order by yhmc";
	//	out.println(ls_sql);
		ps= conn.prepareStatement(ls_sql);
