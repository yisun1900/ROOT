...
try {
	...
	for (int i=0;i<ygbh.length ;i++ )
	{
		...
		if (kfgssq.equals("3"))//3：授权单个店面
		{
			...
			{
				conn.rollback();
				out.println("员工编号"+ygbh[i]+"错误！您选择了『授权单个店面』，确没有选择该用户所属部门："+yhdwmc);
				return;
			}
		}
		else{
			...
			{
				conn.rollback();
				out.println("员工编号"+ygbh[i]+"错误！您选择了『授权某一分公司多个店面』，可实际授权了["+fgssl+"]个分公司的店面");
				return;
			}
			else if (kfgssq.equals("4") && dmsl==1)
			{
				conn.rollback();
				out.println("员工编号"+ygbh[i]+"错误！您选择了『授权某一分公司多个店面』，可实际只授权了1个店面");
				return;
			}
			else if (kfgssq.equals("5") && fgssl==1)
			{
				conn.rollback();
				out.println("员工编号"+ygbh[i]+"错误！您选择了『授权多个分公司多个店面』，可实际只授权了1个分公司的店面");
				return;
			}
		}
