...
try {
	...
	ls_sql="insert into bj_jzbjb ( dqbm,xmbh,bjjbbm,xmmc,xmdlbm,xmxlbm,smbm,jldwbm,clcb,rgcb    ,cbenj,clbj,rgbj,bj,gycl,sfbxx,bjbbh,glxmbh,flmcgg,sfxycl   ,sfxyyjd,xh,lrr,lrsj,bz,xmflbm,sfyxdz,zdzk,sgcbj   ,jxcb,jxbj,zcf,shf,ysf,qtf,sgdbj,shbz) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?   ,0,0,0,0,0,0,0,'N') ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,dqbm);
	ps.setString(2,xmbh);
	...
	ps.setString(27,sfyxdz);
	ps.setDouble(28,zdzk);
	ps.setDouble(29,sgcbj);
	ps.executeUpdate();
	ps.close();
