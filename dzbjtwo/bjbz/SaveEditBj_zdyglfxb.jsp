...
try {
	...
	for (int i=0;i<glflbm.length ;i++ )
	{
		...
		if (glbxh[i].equals(""))
		{
			...
			//家装工料分析表
			ls_sql="insert into bj_zdyglfxb(glbxh,xmlrr,xmbh,glflbm,glfxclbm,glfxclmc,dj,jldwbm,xhl,jqsfl,bsfl,tvocsfl,lx,wlllbz,lrr,lrsj,bz)";
			ls_sql+=" values("+newglbxh+",'"+lrr+"','"+xmbh+"','"+glflbm[i]+"','"+cf.GB2Uni(glfxclbm[i])+"','"+cf.GB2Uni(glfxclmc[i])+"',"+dj+",'"+jldwbm[i]+"',"+xhl+","+jqsfl+","+bsfl+","+tvocsfl+",'"+cf.GB2Uni(lx[i])+"','"+wlllbz[i]+"','"+yhmc+"',TRUNC(SYSDATE),'"+cf.GB2Uni(bz[i])+"')";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
		}
			...
			//家装工料分析表
			ls_sql="update bj_zdyglfxb set glflbm='"+glflbm[i]+"',glfxclbm='"+cf.GB2Uni(glfxclbm[i])+"',glfxclmc='"+cf.GB2Uni(glfxclmc[i])+"'";
	...
	ls_sql+=" from  bj_zdyglfxb";
	ls_sql+=" where xmlrr='"+lrr+"' and xmbh='"+xmbh+"' and glfxclbm is not null";
