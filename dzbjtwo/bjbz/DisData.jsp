...
<div align="center">
  ...
  <table width="310%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr bgcolor="#CCCCCC" align="center"> 
		...
		<td width="3%"><font color="#CC0000">成本价＝材料成本＋人工成本*</font></td>
		<td width="3%"><font color="#CC0000">材料成本*</font></td>
		<td width="3%"><font color="#CC0000">人工成本*</font></td>
		<td width="3%"><font color="#CC0000">施工队报价*</font></td>
		<td width="3%"><font color="#CC0000">客户报价＝材料报价＋人工报价*</font></td>

		<td width="3%"><font color="#CC0000">材料报价*</font></td>
		<td width="3%"><font color="#CC0000">人工报价*</font></td>
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		clcbStr=outStr[i][9].trim();
		rgcbStr=outStr[i][10].trim();

		sgcbjStr=outStr[i][11].trim();
		bjStr=outStr[i][12].trim();
		clbjStr=outStr[i][13].trim();
		rgbjStr=outStr[i][14].trim();

		sfyxdzStr=outStr[i][15].trim();
		zdzkStr=outStr[i][16].trim();

		gycl=outStr[i][17].trim();
		flmcgg=outStr[i][18].trim();
		bz=outStr[i][19].trim();
		glxmbh=outStr[i][20].trim();
		xh=outStr[i][21].trim();
		sfxyclstr=outStr[i][22].trim();
		sfxyyjdstr=outStr[i][23].trim();
		...
		if (bj!=cf.round(clbj+rgbj,2))
		{
			out.println("<P>！！！错误，项目编号["+xmbh+"][工程预算价："+bj+"]不等于(辅材费+人工费:"+cf.round(clbj+rgbj,2)+")");
			return;
		}
		...
		if (clcbStr.equals(""))
		{
			...
			try{
				clcb=Double.parseDouble(clcbStr.trim());
			}
			catch (Exception e){
				out.println("<BR>[材料成本:"+clcbStr+"]不是数字:"+e);
				return;
			}
		}
		if (rgcbStr.equals(""))
		{
			...
			try{
				rgcb=Double.parseDouble(rgcbStr.trim());
			}
			catch (Exception e){
				out.println("<BR>[人工成本:"+rgcbStr+"]不是数字:"+e);
				return;
			}
		}

		if (cbenj!=cf.round(clcb+rgcb,2))
		{
			out.println("<P>！！！错误，项目编号["+xmbh+"][成本价："+cbenj+"]不等于(辅材费+人工费:"+cf.round(clcb+rgcb,2)+")");
			return;
		}
    ...
    <tr bgcolor="<%=bgcolor%>" align="center"> 
      ...
      <td> <%=clcb%>
        <input name="clcb" type="hidden" value="<%=clcb%>" >
      </td>
      <td> <%=rgcb%>
        <input name="rgcb" type="hidden" value="<%=rgcb%>" >
      </td>
        <td> <%=sgcbj%>
