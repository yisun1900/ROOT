...
String[] clcb =request.getParameterValues("clcb");
String[] rgcb =request.getParameterValues("rgcb");
...
try {
	...
	for (int i=0;i<xmbh.length ;i++ )
	{
		...
		sql+=" ,jldwbm,bj,gycl,lrr,lrsj,xh,sfbxx,glxmbh,clbj,rgbj,sfyxdz,zdzk,flmcgg,bz,sfxycl,sfxyyjd,clcb,rgcb,cbenj,sgcbj   ,jxcb,jxbj,zcf,shf,ysf,qtf,sgdbj,shbz)";
		sql+=" values('"+bjbbh+"','"+dqbm+"','"+xmbh[i]+"','"+bjjbbm+"','"+cf.GB2Uni(xmmc[i])+"','"+xmdlbm[i]+"','"+xmxlbm[i]+"','"+xmflbm[i]+"','"+smbm[i]+"'";
		sql+=" ,'"+jldwbm[i]+"',"+bj[i]+",'"+cf.GB2Uni(gycl[i])+"','"+yhmc+"',TRUNC(SYSDATE),'"+xh[i]+"','"+cf.GB2Uni(sfbxx[i])+"','"+glxmbhStr+"',"+clbj[i]+","+rgbj[i]+",'"+sfyxdz[i]+"',"+zdzk[i]+",'"+cf.GB2Uni(flmcgg[i])+"','"+cf.GB2Uni(bz[i])+"','"+cf.GB2Uni(sfxycl[i])+"','"+cf.GB2Uni(sfxyyjd[i])+"',"+clcb[i]+","+rgcb[i]+","+cbenj[i]+","+sgcbj[i]+"   ,0,0,0,0,0,0,0,'N')";
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
		ps.close();
