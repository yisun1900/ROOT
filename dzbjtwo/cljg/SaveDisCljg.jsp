...
String[] jsjldwbm=request.getParameterValues("jsjldwbm");
...
try {
	...
	for (int i=0;i<wlbm.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>存盘失败！序号["+(i+1)+"]的[物流材料编码]为空");
			return;
		}
		//[工料名称]是否为空
		if (glmc[i].equals(""))
		...
		//家装工料分析表
		ls_sql="insert into bj_jzcljgb(dqbm,cldlbm,wlbm,glmc,pp,xhgg,jldwbm,dj,jswlbm,jsjldwbm,wljsbz,qrbz,lrr,lrsj)";
		ls_sql+=" values('"+dqbm+"','"+cldlbm[i]+"','"+cf.GB2Uni(wlbm[i])+"','"+cf.GB2Uni(glmc[i])+"','"+cf.GB2Uni(pp[i])+"','"+cf.GB2Uni(xhgg[i])+"','"+jldwbm[i]+"',"+dj+",'"+cf.GB2Uni(jswlbm[i])+"','"+cf.GB2Uni(jsjldwbm[i])+"','"+cf.GB2Uni(wljsbz[i])+"','N','"+yhmc+"',TRUNC(SYSDATE))";
