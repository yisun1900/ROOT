...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql+=" FROM bj_tcsjflbm  ";
    ls_sql+=" where (1=1)";
    ls_sql+=wheresql;
    ls_sql+=" order by tcsjflbm";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Bj_tcsjflbmList.jsp","","","EditBj_tcsjflbm.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Bj_tcsjflbmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(tcsjflbm,"tcsjflbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from bj_tcsjflbm where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="12%">&nbsp;</td>
	<td  width="24%">套餐升级分类编码</td>
	<td  width="64%">套餐升级分类名称</td>
</tr>
<%
	pageObj.displayDate();
	pageObj.displayFoot();
