...
<%
	...
	if (bjtyMark.compareTo("0")>0)//电子报价版本是否已停用
	{
		...
		if (xmmc!=null)
		{
			...
			wheresql1+=" and  (bj_jzbjb1.xmmc like'%"+xmmc+"%')";
		}
		...
		if (smbm!=null)
		{
			...
			wheresql1+=" and  (bj_jzbjb1.smbm='"+smbm+"')";
		}
		...
		if (xmmc!=null)
		{
			...
			wheresql1+=" and  (bj_jzbjb.xmmc like'%"+xmmc+"%')";
		}
		smbm=request.getParameter("smbm");
		if (smbm!=null)
		{
			...
			wheresql1+=" and  (bj_jzbjb.smbm='"+smbm+"')";
		}
		xmdlbm=request.getParameter("xmdlbm");
		if (xmdlbm!=null)
		{
...
%>
<FONT COLOR=red><b>[<%=jgwzbm%>]已选择的项目（<%=xmxlmc%>）：</b></font>
...
<%
	...
	{
		ls_sql="SELECT bj_jzbjb1.xmbh,bj_jzbjb1.xmmc,jdm_jldwbm.jldwmc,bj_jzbjb1.bj,gycl";
		...
		ls_sql+=" and bj_jzbjb1.xmbh in(select xmbh from bj_gclmx where khbh='"+khbh+"' and jgwzbm='"+jgwzbm+"')";
		ls_sql+=wheresql1;
		ls_sql+=" order by bj_jzbjb1.dqbm,bj_jzbjb1.xh,bj_jzbjb1.xmbh";
	}
	else{
		ls_sql="SELECT bj_jzbjb.xmbh,bj_jzbjb.xmmc,jdm_jldwbm.jldwmc,bj_jzbjb.bj,gycl";
		...
		ls_sql+=" and bj_jzbjb.xmbh in(select xmbh from bj_gclmx where khbh='"+khbh+"' and jgwzbm='"+jgwzbm+"')";
		ls_sql+=wheresql1;
...
<FONT COLOR=red><b>[<%=jgwzbm%>]未选项目（<%=xmxlmc%>）：</b></font>
<table border="1" width="150%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
<%
...
<%
	...
	if (bjtyMark.compareTo("0")>0)//已停用，从表[bj_jzbjb1]取数据而非表[bj_jzbjb]
	{
		ls_sql="SELECT dqbm,bj_jzbjb1.xmbh as xmbh,bj_jzbjb1.xmmc,gycl,jdm_jldwbm.jldwmc,bj_jzbjb1.bj";
		...
		ls_sql+=" and bj_jzbjb1.dqbm='"+dqbm+"' and bj_jzbjb1.bjjbbm='"+bjjb+"' and bj_jzbjb1.bjbbh='"+bjbbh+"'";
		ls_sql+=" and bj_jzbjb1.sfbxx!='9'";//1：必报项；2：必列项；3：可选项；9：作废项
		ls_sql+=" and bj_jzbjb1.xmbh not in(select xmbh from bj_gclmx where khbh='"+khbh+"' and jgwzbm='"+jgwzbm+"')";
		ls_sql+=wheresql;
		ls_sql+=" order by bj_jzbjb1.dqbm,bj_jzbjb1.xh,bj_jzbjb1.xmbh";
	}
	else{
		ls_sql="SELECT dqbm,bj_jzbjb.xmbh as xmbh,bj_jzbjb.xmmc,gycl,jdm_jldwbm.jldwmc,bj_jzbjb.bj";
		ls_sql+=" FROM bj_jzbjb,bdm_smbm,jdm_jldwbm  ";
		ls_sql+=" where bj_jzbjb.smbm=bdm_smbm.smbm(+) and bj_jzbjb.jldwbm=jdm_jldwbm.jldwbm";
		ls_sql+=" and bj_jzbjb.dqbm='"+dqbm+"' and bj_jzbjb.bjjbbm='"+bjjb+"' and bj_jzbjb.bjbbh='"+bjbbh+"'";
		ls_sql+=" and bj_jzbjb.sfbxx!='9'";//1：必报项；2：必列项；3：可选项；9：作废项
		ls_sql+=" and bj_jzbjb.xmbh not in(select xmbh from bj_gclmx where khbh='"+khbh+"' and jgwzbm='"+jgwzbm+"')";
		ls_sql+=wheresql;
		ls_sql+=" order by bj_jzbjb.dqbm,bj_jzbjb.xh,bj_jzbjb.xmbh";
	}
//	out.print(ls_sql);
...
//设置显示列
	String[] disColName={"xmbh","xmmc","jldwmc","bj","gycl"};
...
//设置按钮参数
	...
	String[] buttonLink={"SaveInsertBj_bjxmmxFj.jsp?khbh="+khbh+"&jgwzbm="+jgwzbm+"&jzbz="+jzbz+"&dqbm="+dqbm};//按钮单击调用的网页，可以增加参数
