...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	jc_mmydd_swjsj=request.getParameter("jc_mmydd_swjsj");
	if (jc_mmydd_swjsj!=null)
	{
		jc_mmydd_swjsj=jc_mmydd_swjsj.trim();
		if (!(jc_mmydd_swjsj.equals("")))	wheresql+="  and (jc_mmydd.swjsj>=TO_DATE('"+jc_mmydd_swjsj+"','YYYY-MM-DD'))";
	}
	jc_mmydd_swjsj=request.getParameter("jc_mmydd_swjsj2");
	if (jc_mmydd_swjsj!=null)
	{
		jc_mmydd_swjsj=jc_mmydd_swjsj.trim();
		if (!(jc_mmydd_swjsj.equals("")))	wheresql+="  and (jc_mmydd.swjsj<=TO_DATE('"+jc_mmydd_swjsj+"','YYYY-MM-DD'))";
	...
	ls_sql="SELECT yddbh,crm_khxx.khxm,crm_khxx.hth,clztmc,jc_mmydd.swjsj,jc_mmydd.wjjsr,jc_mmydd.pdgcmc,mmcljs,jc_mmydd.htje,jc_mmydd.mmhtje,jc_mmydd.wjhtje,jc_mmydd.blhtje,jc_mmydd.fwf, DECODE(jc_mmydd.wjqk,'1','客户自购','2','公司购买') wjqk,jc_mmydd.xmzy,jc_mmydd.clgw,jc_mmydd.ztjjgw,jc_mmydd.lrsj,jc_mmydd.bz,jc_mmydd.khbh  ";
    ...
    ls_sql+=" and jc_mmydd.wjqk='1' and jc_mmydd.wjsgsbz='Y'";//1：客户自购；2：公司购买
    ls_sql+=" and jc_mmydd.clzt in('04','23','05','06','07')";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_mmydd.swjsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_mmyddXgJsList.jsp","","","XgJsJc_mmydd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除接收情况"};//按钮的显示名称
	String[] buttonLink={"DeleteJsJc_mmydd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">木门预订单---接收客户自购五金件</font></B>
</CENTER>
<%
	pageObj.out=out;
