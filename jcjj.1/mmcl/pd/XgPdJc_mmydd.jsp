...
try {
	...
	if (rs.next())
	{
		...
		jc_mmydd_ztjjgw=cf.fillNull(rs.getString("ztjjgw"));
...
<body bgcolor="#FFFFFF">
<div align="center">木门预订单---修改派单 </div>
  
<form method="post" action="SaveXgPdJc_mmydd.jsp" name="selectform">
  <table width="100%" border="0" style="FONT-SIZE:14" bgcolor="#999999" cellpadding="2" cellspacing="2">
    <tr bgcolor="#E8E8FF">
      <td align="right" width="22%">派单工厂</td>
      <td width="28%"> 
        <select name="pdgc" style="FONT-SIZE:12PX;WIDTH:152PX">
		  <option value=""></option>
<%
	cf.selectItem(out,"select sq_gysxx.gysbh,sq_gysxx.gysmc from sq_gysxx,sq_ppgyssqfgs where sq_gysxx.gysbh=sq_ppgyssqfgs.gysbh and sq_ppgyssqfgs.ssfgs='"+ssfgs+"' and sq_gysxx.gyslb='1' order by sq_gysxx.gysmc",jc_mmydd_pdgc);
%>
		</select>
	  </td>
      <td width="23%" bgcolor="#E8E8FF" align="right">项目专员</td>
      <td width="27%">
        <input type="text" name="xmzy" size="20" maxlength="10" value="<%=xmzy%>" readonly>
      </td>
    </tr>
    <tr bgcolor="#E8E8FF"> 
      <td align="right" width="22%">派单时间</td>
      <td width="28%"> 
        <input type="text" name="paidsj" size="20" maxlength="10" value="<%=cf.today()%>" readonly>
      </td>
      <td width="23%" bgcolor="#E8E8FF" align="right">派单人</td>
      <td width="27%"> 
        <input type="text" name="paidr" size="20" maxlength="20" value="<%=yhmc%>" readonly>
      </td>
    </tr>
    <tr bgcolor="#E8E8FF"> 
...
%>    </td>
    <td align="right"> 木门定金 </td>
    <td><%=jc_mmydd_sqdj%> </td>
  </tr>
  <tr bgcolor="#99CCFF">
    <td align="right" bgcolor="#99CCFF"> 驻店家居顾问 </td>
    <td><%=jc_mmydd_clgw%> </td>
    <td align="right">展厅家居顾问 </td>
    <td><%=jc_mmydd_ztjjgw%></td>
  </tr>
  <tr bgcolor="#99CCFF">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(	javaTrim(FormName.paidr)=="") {
		alert("请输入[派单人]！");
		FormName.paidr.focus();
		return false;
	}
	if(	javaTrim(FormName.paidsj)=="") {
		alert("请输入[派单时间]！");
		FormName.paidsj.focus();
		return false;
	}
	if(!(isDatetime(FormName.paidsj, "派单时间"))) {
		return false;
	}
