...
<%
	...
	//	out.println(ls_sql);
		...
		double xdb=0;
		String sfxclxs=null;
		ls_sql="SELECT sxh,clxlmc,ppmc,gysmc,xh,gg,jldw,sl,ptcpsm,bjjbmc,xdjldw,xdb,DECODE(sfxclxs,'1','向上取整','2','向下取整','3','4舍5入','4','不处理') sfxclxs";
		...
		if (rs.next())
		{
			...
			%> 
			  <tr bgcolor="#FFFFFF" align="center"> 
				...
				<td><%=bjjbmc%></td>
				<td><A HREF="EditBj_tczcbj.jsp?sxh=<%=sxh%>&khbh=<%=khbh%>" target="_blank">修改</A></td>
				<td><%=tccplbmc%></td>
				<td><%=xh%></td>
				<td><%=gg%></td>
				<td><%=jldw%></td>
