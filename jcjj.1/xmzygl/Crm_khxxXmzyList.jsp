...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	crm_khxx_csrq=request.getParameter("crm_khxx_csrq");
	if (crm_khxx_csrq!=null)
	{
		crm_khxx_csrq=crm_khxx_csrq.trim();
		if (!(crm_khxx_csrq.equals("")))	wheresql+="  and (crm_khxx.csrq>=TO_DATE('"+crm_khxx_csrq+"','YYYY/MM/DD'))";
	}
	crm_khxx_csrq=request.getParameter("crm_khxx_csrq2");
	if (crm_khxx_csrq!=null)
	{
		crm_khxx_csrq=crm_khxx_csrq.trim();
		if (!(crm_khxx_csrq.equals("")))	wheresql+="  and (crm_khxx.csrq<=TO_DATE('"+crm_khxx_csrq+"','YYYY/MM/DD'))";
	...
	{
		ls_sql="SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,a.dwmc qydm,crm_khxx.qyrq,gcjdmc,'木门' as lx,'<A HREF=\"/jcjj/mmdd/ViewJc_mmydd.jsp?yddbh='||jc_mmydd.yddbh||'\" target=\"_blank\">'||jc_mmydd.yddbh||'</A>' ddbhlink,jc_mmydd.yddbh ddbh,jc_mmydd.xmzy,crm_khxx.ykhlxsj,DECODE(crm_khxx.jslxfs,'1','电话联系','2','发短信','3','不受限制') jslxfs,'联系' as lianxi ";
		ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_sgd,dm_gcjdbm,jc_mmydd";
		ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and crm_khxx.gcjdbm=dm_gcjdbm.gcjdbm(+)";
		ls_sql+=" and crm_khxx.khbh=jc_mmydd.khbh and jc_mmydd.clzt!='06'";
		ls_sql+=wheresql;
		ls_sql+=mmwheresql;
	...
	{
		ls_sql=" SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,a.dwmc qydm,crm_khxx.qyrq,gcjdmc,'橱柜' as lx,'<A HREF=\"/jcjj/cgdd/ViewJc_cgdd.jsp?ddbh='||jc_cgdd.ddbh||'\" target=\"_blank\">'||jc_cgdd.ddbh||'</A>' ddbhlink,jc_cgdd.ddbh ddbh,jc_cgdd.xmzy,crm_khxx.ykhlxsj,DECODE(crm_khxx.jslxfs,'1','电话联系','2','发短信','3','不受限制') jslxfs,'联系' as lianxi  ";
		ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_sgd,dm_gcjdbm,jc_cgdd";
		ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and crm_khxx.gcjdbm=dm_gcjdbm.gcjdbm(+)";
		ls_sql+=" and crm_khxx.khbh=jc_cgdd.khbh";
		ls_sql+=wheresql;
		ls_sql+=cgwheresql;
		ls_sql+=" order by khbh";
	}
	...
	{
		ls_sql=" SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,a.dwmc qydm,crm_khxx.qyrq,gcjdmc,'家具' as lx,'<A HREF=\"/jcjj/jjdd/ViewJc_jjdd.jsp?ddbh='||jc_jjdd.ddbh||'\" target=\"_blank\">'||jc_jjdd.ddbh||'</A>' ddbhlink,jc_jjdd.ddbh ddbh,jc_jjdd.xmzy,crm_khxx.ykhlxsj,DECODE(crm_khxx.jslxfs,'1','电话联系','2','发短信','3','不受限制') jslxfs,'联系' as lianxi  ";
		ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_sgd,dm_gcjdbm,jc_jjdd";
		ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and crm_khxx.gcjdbm=dm_gcjdbm.gcjdbm(+)";
		ls_sql+=" and crm_khxx.khbh=jc_jjdd.khbh";
		ls_sql+=wheresql;
		ls_sql+=jjwheresql;
		ls_sql+=" order by khbh";
	}
	...
	{
		ls_sql=" SELECT crm_khxx.khbh,crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_khxx.zjxm,a.dwmc qydm,crm_khxx.qyrq,gcjdmc,'主材' as lx,'<A HREF=\"/jcjj/zcdd/ViewJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">'||jc_zcdd.ddbh||'</A>' ddbhlink,jc_zcdd.ddbh ddbh,jc_zcdd.xmzy,crm_khxx.ykhlxsj,DECODE(crm_khxx.jslxfs,'1','电话联系','2','发短信','3','不受限制') jslxfs,'联系' as lianxi  ";
		ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_sgd,dm_gcjdbm,jc_zcdd";
		ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) and crm_khxx.gcjdbm=dm_gcjdbm.gcjdbm(+)";
		ls_sql+=" and crm_khxx.khbh=jc_zcdd.khbh";
		ls_sql+=wheresql;
		ls_sql+=zcwheresql;
		ls_sql+=" order by khbh";
	}
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khxxXmzyList.jsp","ChooseXmzy.jsp","","");
//	pageObj.setPageRow(10);//设置每页显示记录数
//设置显示列
	String[] disColName={"lx","ddbhlink","xmzy","hth","khxm","fwdz","sjs","sgd","zjxm","qydm","gcjdmc","qyrq"};
	pageObj.setDisColName(disColName);
/*
*/

//设置主键
	String[] keyName={"ddbh","lx"};
	pageObj.setKey(keyName);
//设置按钮参数
	String[] buttonName={"更换项目专员"};//按钮的显示名称
	String[] buttonLink={"NewXmzy.jsp"};//按钮单击调用的网页，可以增加参数
...
//设置显示合并列
	...
	spanColHash.put("gcjdmc","1");//列参数对象加入Hash表
