...
<%
	...
	String sfkgsl=cf.executeQuery("select sfkgsl from jc_zcdd where ddbh='"+ddbh+"'");
	
	if (clzt!=null && clzt.equals("07"))//M：非手写合同；Y：已审核；N：未审核
	{
		out.println("错误！合同未完成，不能打印");
		return;
	}
	if (sxhtsfysh!=null && sxhtsfysh.equals("N"))//M：非手写合同；Y：已审核；N：未审核
	{
		if (sfkgsl!=null && sfkgsl.equals("N"))//是否可改数量 Y：可以；N：否
		{
...
//			return;
	...
	if (yhjs.equals("G0"))
	{
		if (clzt.compareTo("13")<0 || clzt.equals("98"))//13：工厂已接收
		{
//			out.println("错误！非定制式产品工厂未接收的合同不能打印:"+clzt.compareTo("13"));
//			return;
		}
	}
    ...
    <tr> 
      <td colspan="2"> 
        <div align="center"> 
          ...
          <input type="button" name="Button" value="打印预定单" onClick="f_do(selectform,'dydd')">
          ...
          <input type="button" value="打印附件-厂商" onClick="f_do(selectform,'dycs')" disabled>
          <input type="button" onClick="f_do(selectform,'dycld')" value="打印测量单">
        </div>
      </td>
    </tr>
  </table>
...
<%
	...
	String ls_sql=null;
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,zcdlbm,zcxlbm,jc_zcdd.ppbm,jc_zcdd.ppmc,jc_zcdd.gys,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,jc_zcdd.clgw,jc_zcdd.xmzy,jc_zcdd.jjsjs, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'), DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),DECODE(jc_zcdd.xclbz,'1','非定制产品','2','定制式产品'),jc_zcdd.lrsj,a.dwmc lrdw,jc_zcdd.bz";
