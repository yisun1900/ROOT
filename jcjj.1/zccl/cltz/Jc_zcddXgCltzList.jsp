...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	jc_zcdd_tzclsj=request.getParameter("jc_zcdd_tzclsj");
	if (jc_zcdd_tzclsj!=null)
	{
		jc_zcdd_tzclsj=jc_zcdd_tzclsj.trim();
		if (!(jc_zcdd_tzclsj.equals("")))	wheresql+="  and (jc_zcdd.tzclsj>=TO_DATE('"+jc_zcdd_tzclsj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_tzclsj=request.getParameter("jc_zcdd_tzclsj2");
	if (jc_zcdd_tzclsj!=null)
	{
		jc_zcdd_tzclsj=jc_zcdd_tzclsj.trim();
		if (!(jc_zcdd_tzclsj.equals("")))	wheresql+="  and (jc_zcdd.tzclsj<=TO_DATE('"+jc_zcdd_tzclsj+"','YYYY/MM/DD'))";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.tzclsj desc";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgCltzList.jsp","","","XgCltzJc_zcdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除测量通知"};//按钮的显示名称
	String[] buttonLink={"DeleteCltzJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改测量通知</font></B>
</CENTER>
<%
	pageObj.out=out;
