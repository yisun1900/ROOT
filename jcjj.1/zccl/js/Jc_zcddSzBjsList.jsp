...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql=" select * ";
    ls_sql+=" FROM ( ";
	ls_sql+=" SELECT jc_zcdd.ddbh,DECODE(jc_zcdd.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核','P','不需结算') sfjs,jc_zcdd.bjsyy,jc_zcdd.bjsszr,jc_zcdd.bjsszsj,clztmc,crm_khxx.khxm,ppbm,jc_zcdd.gys,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.xmzy,DECODE(jc_zcdd.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),jc_zcdd.sshsj";
	ls_sql+=" FROM crm_khxx,jc_zcdd,jdm_zcddzt ";
    ls_sql+=" where jc_zcdd.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zcdd.clzt=jdm_zcddzt.clzt(+)";
    ls_sql+=" and jc_zcdd.sfjs in('N','C','P')";//N：未结算；C：已选择结算；Y：已结算；M：结算已审核
	...
	ls_sql+=" SELECT jc_zczjx.zjxbh ddbh,DECODE(jc_zczjx.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核','P','不需结算') sfjs,jc_zczjx.bjsyy,jc_zczjx.bjsszr,jc_zczjx.bjsszsj,DECODE(jc_zczjx.clzt,'00','录入未完成','01','录入已完成','02','已确认','04','厂家已接收','03','已送货','05','部分送货','30','单项完结','99','退单') clztmc,crm_khxx.khxm,ppbm,jc_zczjx.gys,crm_khxx.hth,crm_khxx.fwdz,jc_zczjx.clgw,jc_zczjx.xmzy,DECODE(jc_zczjx.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),jc_zczjx.sshsj";
	ls_sql+=" FROM crm_khxx,jc_zczjx ";
    ls_sql+=" where jc_zczjx.khbh=crm_khxx.khbh";
    ls_sql+=" and jc_zczjx.sfjs in('N','C','P')";//N：未结算；C：已选择结算；Y：已结算；M：结算已审核
	if (kfgssq.equals("1") || kfgssq.equals("2"))//0：不授权；1：授权单个分公司；2：授权多个分公司；3：授权单个店面；4：授权某一分公司多个店面；5：授权多个分公司多个店面
	{
		ls_sql+=" and crm_khxx.fgsbh in(select ssfgs from sq_sqfgs where ygbh='"+ygbh+"' )";
	}
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddSzBjsList.jsp","","","");
/*
//设置显示列
...
//设置按钮参数
	String[] buttonName={"设置","删除设置"};//按钮的显示名称
	String[] buttonLink={"SzBjs.jsp","ScSzBjs.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">设置－－不需结算订单</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="5%">结算状态</td>
	<td  width="9%">不结算原因</td>
	<td  width="4%">不结算设置人</td>
	<td  width="5%">不结算设置时间</td>
	<td  width="6%">处理状态</td>
	<td  width="6%">客户姓名</td>
	<td  width="8%">子品牌</td>
	<td  width="15%">供应商</td>
