...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql=" select * ";
    ls_sql+=" FROM ( ";
	ls_sql+=" SELECT '<A HREF=\"/jcjj/zcdd/ViewJeJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">'||jc_zcdd.ddbh||'</A>' ddbh,'<A HREF=\"ShJsjJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">修改</A>',DECODE(jc_zcdd.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,DECODE(jc_zcdd.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),clztmc,jc_zcdd.sshsj,ppbm,jc_zcdd.gys,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy";
	...
	ls_sql+=" SELECT '<A HREF=\"/jcjj/zccl/zjx/ViewJeJc_zczjx.jsp?zjxbh='||jc_zczjx.zjxbh||'\" target=\"_blank\">'||jc_zczjx.zjxbh||'</A>' ddbh,'<A HREF=\"ShJsjJc_zczjx.jsp?zjxbh='||jc_zczjx.zjxbh||'\" target=\"_blank\">修改</A>',DECODE(jc_zczjx.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,DECODE(jc_zczjx.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),DECODE(jc_zczjx.clzt,'00','录入未完成','01','录入已完成','02','已确认','04','厂家已接收','03','已送货','30','单项完结','99','退单') clztmc,jc_zczjx.sshsj,ppbm,jc_zczjx.gys,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,jc_zczjx.clgw,jc_zczjx.ztjjgw,jc_zczjx.xmzy";
	ls_sql+=" FROM crm_khxx,jc_zczjx ";
    ls_sql+=" where jc_zczjx.khbh=crm_khxx.khbh";
    ls_sql+=" and (jc_zczjx.sfjs in('N','C') OR  jc_zczjx.sfjs is null)";//N：未结算；C：已选择结算；Y：已结算；M：结算已审核
	if (kfgssq.equals("1") || kfgssq.equals("2"))//0：不授权；1：授权单个分公司；2：授权多个分公司；3：授权单个店面；4：授权某一分公司多个店面；5：授权多个分公司多个店面
	{
		ls_sql+=" and crm_khxx.fgsbh in(select ssfgs from sq_sqfgs where ygbh='"+ygbh+"' )";
	}
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgJsjList.jsp","","","");
...
<CENTER >
  <B><font size="3">主材订单－－修改结算价</font></B>
</CENTER>
<%
	pageObj.out=out;
