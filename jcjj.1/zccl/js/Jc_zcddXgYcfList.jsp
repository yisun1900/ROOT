...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql=" select * ";
    ls_sql+=" FROM ( ";
	ls_sql+=" SELECT '<A HREF=\"/jcjj/zcdd/ViewJeJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">'||jc_zcdd.ddbh||'</A>' ddbh,'<A HREF=\"XgYcfJc_zcdd.jsp?ddbh='||jc_zcdd.ddbh||'\" target=\"_blank\">修改</A>',DECODE(jc_zcdd.sfjs,'N','未结算','C','设置标志','Y','已结算','M','结算已审核') sfjs,DECODE(jc_zcdd.sxhtsfysh,'M','非手写','Y','已审核','N','未审'),clztmc,jc_zcdd.sshsj,ppbm,jc_zcdd.gys,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgYcfList.jsp","","","");
...
<CENTER >
  <B><font size="3">主材订单－－修改远程费</font></B>
</CENTER>
<%
	pageObj.out=out;
