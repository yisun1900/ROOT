...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		jhshsj=jhshsj.trim();
		if (!(jhshsj.equals("")))	wheresql+="  and (jc_zcdd.jhshsj>=TO_DATE('"+jhshsj+"','YYYY-MM-DD'))";
	}
	jhshsj=request.getParameter("jhshsj2");
	if (jhshsj!=null)
	{
		jhshsj=jhshsj.trim();
		if (!(jhshsj.equals("")))	wheresql+="  and (jc_zcdd.jhshsj<=TO_DATE('"+jhshsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}

	ls_sql="SELECT jc_zcdd.ddbh,clztmc,crm_khxx.khxm,crm_khxx.fwdz,ppbm,jc_zcdd.htshsj,jc_zcdd.jhshsj,jc_zcdd.tzshsj,jc_zcdd.tzshr,jc_zcdd.tzry,crm_khxx.lxfs||','||crm_khxx.qtdh,crm_khxx.hth,crm_khxx.sgbz,sq_bzxx.dh,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'),jc_zcdd.hkze,jc_zcdd.zjhze,jc_zcdd.shtzsm,jc_zcdd.gys,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知')";
    ...
    ls_sql+=" and (  (jc_zcdd.ddshbz!='1' and jc_zcdd.clzt in('13','15','17','20'))";//等待送货通知标志  1：不需待电；2：待电未通知；3：待电已通知
    ls_sql+=" OR     (jc_zcdd.ddshbz='1' and jc_zcdd.clzt in('13','15','17','20'))  )";
	if (yhjs.equals("G0"))
	{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.jhshsj";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jc_zcddShList.jsp","","","ShJc_zcdd.jsp");
	pageObj.setEditStr("送货");//设置每页显示记录数
/*
//设置显示列
...
//设置按钮参数
	String[] buttonName={"批量送货"};//按钮的显示名称
	String[] buttonLink={"SaveAllShJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－送货</font></B>
</CENTER>
<%
	pageObj.out=out;
