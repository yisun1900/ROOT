...
try {
	...
	ls_sql="select shqk,sshr,sshsj,stbhsj,stbhr,tbhtzry,jhtbhsj,tbhqk,tztbhsj,tztbhr,shtzsm,jhshsj,tzry,ztjjgw,htcxhdje,zcdlbm,ppbm,ddbh,khbh,clgw,jkxz,jkdd,ddshbz,kjxsj,xclbz,htshsj,xmzy,qhtsj,jhclsj,bz,hkze,zjhze,ddqrr,ddqrsj,cltzr,tzclsj,sclr,sclsj,tzshr,tzshsj ";
	ls_sql+=" from  jc_zcdd";
...
<div align="center">完结登记（订单编号：<%=ddbh%>） </div>
...
<%
		...
		double zqzjxze=0;
		double zjxze=0;
		String zjxhtshsj=null;
		String qrsj=null;
		String lrsj=null;

		ls_sql="SELECT zjxbh,DECODE(jc_zczjx.clzt,'00','录入未完成','01','录入已完成','02','已确认','04','厂家已接收','03','已送货','05','部分送货','30','单项完结','99','退单') clzt,DECODE(jc_zczjx.sfjs,'N','未结算','C','已选择结算','Y','已结算','M','结算已审核') sfjs,zqzjxze,zjxze,htshsj,qrsj,lrsj";
		ls_sql+=" from jc_zczjx";
		ls_sql+=" where ddbh='"+ddbh+"' and clzt not in('00','99','30')";
		...
		while (rs.next())
		{
			...
			zjxhtshsj=cf.fillNull(rs.getDate("htshsj"));
			qrsj=cf.fillNull(rs.getDate("qrsj"));
			lrsj=cf.fillNull(rs.getDate("lrsj"));

			%>
			<tr align="center" bgcolor="#FFFFFF">
				<td><A HREF='/jcjj/zccl/zjxcl/WjJc_zczjx.jsp?zjxbh=<%=zjxbh%>' target='_blank'>点击完结增减项</A></td>
				<td><A HREF='/jcjj/zccl/zjx/ViewJc_zczjx.jsp?zjxbh=<%=zjxbh%>' target='_blank'><%=zjxbh%></A></td>
				<td><%=clzt%></td>
				<td><%=sfjs%></td>
				<td><%=zqzjxze%></td>
				<td><%=zjxze%></td>
...
%>  
  ...
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    ...
    <tr bgcolor="#FFFFFF"> 
      <td width="18%" align="right"> 
        实退补货人      </td>
      <td width="32%"> <%=stbhr%> </td>
      <td width="18%" align="right"> 
        实退补货时间      </td>
      <td width="32%"> <%=stbhsj%> </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%" align="right"> 
        退补货情况      </td>
    ...
    <tr bgcolor="#CCCCFF"> 
      <td width="18%" align="right"> 
        <span class="STYLE1">*</span><span class="STYLE2">完结录入人</span> </td>
      <td width="32%"> 
        <input type="text" name="wjlrr" value="<%=yhmc%>" size="20" maxlength="20" readonly>
      </td>
      <td width="18%" align="right"> 
        <span class="STYLE1">*</span>完结时间      </td>
      <td width="32%"> 
        <input type="text" name="wjsj" value="<%=cf.today()%>" size="20" maxlength="10" onDblClick="JSCalendar(this)">
      </td>
    </tr>
    <tr> 
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveWjdjJc_zcdd.jsp";
	FormName.submit();
	return true;
}
