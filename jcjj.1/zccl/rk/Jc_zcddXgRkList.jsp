...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_zcdd_rksj=null;
	jc_zcdd_rksj=request.getParameter("jc_zcdd_rksj");
	if (jc_zcdd_rksj!=null)
	{
		jc_zcdd_rksj=jc_zcdd_rksj.trim();
		if (!(jc_zcdd_rksj.equals("")))	wheresql+="  and (jc_zcdd.rksj>=TO_DATE('"+jc_zcdd_rksj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_rksj=request.getParameter("jc_zcdd_rksj2");
	if (jc_zcdd_rksj!=null)
	{
		jc_zcdd_rksj=jc_zcdd_rksj.trim();
		if (!(jc_zcdd_rksj.equals("")))	wheresql+="  and (jc_zcdd.rksj<=TO_DATE('"+jc_zcdd_rksj+"','YYYY/MM/DD'))";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.rksj desc";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgRkList.jsp","","","XgRkJc_zcdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteRkJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改入库</font></B>
</CENTER>
<%
	pageObj.out=out;
