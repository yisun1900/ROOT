...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	jc_zcdd_sclsj=request.getParameter("jc_zcdd_sclsj");
	if (jc_zcdd_sclsj!=null)
	{
		jc_zcdd_sclsj=jc_zcdd_sclsj.trim();
		if (!(jc_zcdd_sclsj.equals("")))	wheresql+="  and (jc_zcdd.sclsj>=TO_DATE('"+jc_zcdd_sclsj+"','YYYY/MM/DD'))";
	}
	jc_zcdd_sclsj=request.getParameter("jc_zcdd_sclsj2");
	if (jc_zcdd_sclsj!=null)
	{
		jc_zcdd_sclsj=jc_zcdd_sclsj.trim();
		if (!(jc_zcdd_sclsj.equals("")))	wheresql+="  and (jc_zcdd.sclsj<=TO_DATE('"+jc_zcdd_sclsj+"','YYYY/MM/DD'))";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_zcdd.sclsj desc";

//	out.println(ls_sql);

	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddXgClList.jsp","","","XgClJc_zcdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除测量"};//按钮的显示名称
	String[] buttonLink={"DeleteClJc_zcdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">主材订单处理－－修改测量</font></B>
</CENTER>
<%
	pageObj.out=out;
