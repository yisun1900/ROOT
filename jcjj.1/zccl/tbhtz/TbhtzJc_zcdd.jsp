...
try {
	...
	ls_sql="select shqk,sshsj,sshr,tbhqk,tztbhsj,tztbhr,shtzsm,jhshsj,tzry,ztjjgw,htcxhdje,zcdlbm,ppbm,ddbh,khbh,clgw,jkxz,jkdd,ddshbz,kjxsj,xclbz,htshsj,xmzy,qhtsj,jhclsj,bz,hkze,zjhze,ddqrr,ddqrsj,cltzr,tzclsj,sclr,sclsj,tzshr,tzshsj ";
	ls_sql+=" from  jc_zcdd";
...
<div align="center">退补货通知（订单编号：<%=ddbh%>） </div>
  <table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
    ...
    <tr bgcolor="#FFFFFF"> 
      ...
      <td width="32%"> 
        <input type="text" name="tbhtzry" value="" size="20" maxlength="20" >
      </td>
      <td width="18%"> 
        <div align="right">约定退补货时间</div>
      </td>
      <td width="32%"> 
        <input type="text" name="jhtbhsj" value="<%=cf.today()%>" size="20" maxlength="10" ondblclick="JSCalendar(this)">
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%"> 
      ...
      <td width="32%"> 
        <input type="text" name="tztbhr" value="<%=yhmc%>" size="20" maxlength="20" readonly >
      </td>
      <td width="18%"> 
        <div align="right">发退补货通知时间</div>
      </td>
      <td width="32%"> 
        <input type="text" name="tztbhsj" value="<%=cf.today()%>" size="20" maxlength="10" readonly>
      </td>
    </tr>
    <tr bgcolor="#FFFFFF"> 
      <td width="18%"> 
      ...
      <td colspan="3"> 
        <textarea name="tbhqk" rows="3" cols="72"></textarea>
      </td>
    </tr>
    <tr> 
      <td colspan="4" >
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(	javaTrim(FormName.tztbhr)=="") {
		alert("请选择[发退补货通知人]！");
		FormName.tztbhr.focus();
		return false;
	}
	if(	javaTrim(FormName.tztbhsj)=="") {
		alert("请选择[发退补货通知时间]！");
		FormName.tztbhsj.focus();
		return false;
	}
	if(!(isDatetime(FormName.tztbhsj, "发退补货通知时间"))) {
		return false;
	}
	if(	javaTrim(FormName.jhtbhsj)=="") {
		alert("请选择[约定退补货时间]！");
		FormName.jhtbhsj.focus();
		return false;
	}
	if(!(isDatetime(FormName.jhtbhsj, "约定退补货时间"))) {
		return false;
	}
	if(	javaTrim(FormName.tbhtzry)=="") {
		alert("请选择[退补货通知人员]！");
		FormName.tbhtzry.focus();
		return false;
	}
	if(	javaTrim(FormName.tbhqk)=="") {
		alert("请选择[退补货情况]！");
		FormName.tbhqk.focus();
		return false;
	}

	FormName.action="SaveTbhtzJc_zcdd.jsp";
	FormName.submit();
	return true;
}
