...
try 
{
	...
	%> 
<form method="post" action="SaveUpdateData.jsp" name="editform" target="_blank">
<input name="dqbm" type="hidden" value="<%=dqbm%>" >
<div align="center">更新主材报价（地区：<%=dqmc%>）
<BR>
    注意：<font color="#FF0033">红色表示该[主材编码]不存在</font> <BR>
    注意：<font color="#0000CC">蓝色表示该[品牌、名称、型号]与[主材编码]不一致</font></div>
  <table width="340%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		if (zcbm!=null && !zcbm.equals(""))
		{
			sql=" select count(*)" ;
			sql+=" from jc_zcjgb " ;
			sql+=" where zcbm='"+zcbm+"' and dqbm='"+dqbm+"'" ;
			ps= conn.prepareStatement(sql);
			rs=ps.executeQuery();
			if(rs.next())
			{         
			...
			out.println("<BR>！！！序号["+xuhao+"]错误，[主材编码]不能为空");
			return;
		}
		//主材大类
		...
		if (xh.equals(""))
		{
			sql=" select count(*)" ;
			sql+=" from jc_zcjgb " ;
			sql+=" where dqbm='"+dqbm+"' and zcbm='"+zcbm+"' and zcmc='"+zcmc+"' and dwbh='"+dwbh+"' and xh is null" ;
		}
		else{
			sql=" select count(*)" ;
			sql+=" from jc_zcjgb " ;
			sql+=" where dqbm='"+dqbm+"' and zcbm='"+zcbm+"' and zcmc='"+zcmc+"' and dwbh='"+dwbh+"' and xh='"+xh+"'" ;
		}
//		out.println(sql);
		ps= conn.prepareStatement(sql);
		rs=ps.executeQuery();
...
%> 
	...
	<tr bgcolor="<%=bgcolor%>" align="center"> 
      ...
      <td ><%=yhjstr%>
        <input name="yhj" type="hidden" value="<%=yhj%>" >
      </td>
      <td><%=cbjstr%>
