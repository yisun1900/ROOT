...
if (DBpageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		tzshsj=tzshsj.trim();
		if (!(tzshsj.equals("")))	wheresql+="  and (SEOrder.tzshsj>=TO_DATE('"+tzshsj+"','YYYY-MM-DD'))";
	}
	tzshsj=request.getParameter("tzshsj2");
	if (tzshsj!=null)
	{
		tzshsj=tzshsj.trim();
		if (!(tzshsj.equals("")))	wheresql+="  and (SEOrder.tzshsj<=TO_DATE('"+tzshsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}
	...
	{
		sshsj=sshsj.trim();
		if (!(sshsj.equals("")))	wheresql+="  and (SEOrder.sshsj>=TO_DATE('"+sshsj+"','YYYY-MM-DD'))";
	}
	sshsj=request.getParameter("sshsj2");
	if (sshsj!=null)
	{
		sshsj=sshsj.trim();
		if (!(sshsj.equals("")))	wheresql+="  and (SEOrder.sshsj<=TO_DATE('"+sshsj+" 23:59:59','YYYY-MM-DD hh24:mi:ss'))";
	}


	ls_sql="SELECT SEOrder.fbillno,clztmc,CONVERT(char(10),SEOrder.sshsj,111 ),ppbm,t_Organization.fnumber khbh,t_Organization.FName,t_Organization.FAddress,SEOrder.clgw,SEOrder.xmzy ,CONVERT(char(10),SEOrder.FDate,111 ) ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by SEOrder.sshsj desc";
    DBpageObj.sql=ls_sql;
//进行对象初始化
	DBpageObj.initPage("Jc_zcddXgShList.jsp","","","","sqlserver","sa","");
/*
//设置显示列
...
//设置按钮参数
	String[] buttonName={"删除送货信息"};//按钮的显示名称
