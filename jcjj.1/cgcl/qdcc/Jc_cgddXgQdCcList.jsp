...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_jhccxgsj=null;
	jc_cgdd_jhccxgsj=request.getParameter("jc_cgdd_jhccxgsj");
	if (jc_cgdd_jhccxgsj!=null)
	{
		jc_cgdd_jhccxgsj=jc_cgdd_jhccxgsj.trim();
		if (!(jc_cgdd_jhccxgsj.equals("")))	wheresql+="  and (jc_cgdd.jhccxgsj>=TO_DATE('"+jc_cgdd_jhccxgsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_jhccxgsj=request.getParameter("jc_cgdd_jhccxgsj2");
	if (jc_cgdd_jhccxgsj!=null)
	{
		jc_cgdd_jhccxgsj=jc_cgdd_jhccxgsj.trim();
		if (!(jc_cgdd_jhccxgsj.equals("")))	wheresql+="  and (jc_cgdd.jhccxgsj<=TO_DATE('"+jc_cgdd_jhccxgsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,jc_cgdd.pdsj,jc_cgdd.jhccsj,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,crm_khxx.sjs,DECODE(jc_cgdd.sfpsjs,'1','是','2','否') sfpsjs, jc_cgdd.cgsjs,sq_dwxx.dwmc dm,jc_cgdd.xmzy,jc_cgdd.clgw,jc_cgdd.ztjjgw,jc_cgdd.khbh ";
	...
	ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.jhccxgsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgQdCcList.jsp","","","XgQdCcJc_cgdd.jsp");

//设置主键
	String[] keyName={"ddbh"};
	pageObj.setKey(keyName);
//	pageObj.setEditStr("时间");

//设置按钮参数
	String[] buttonName={"删除初测时间"};//按钮的显示名称
	String[] buttonLink={"DeleteQdCcJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--确定初测时间</font></B>
</CENTER>
<%
	pageObj.out=out;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="6%">订单编号</td>
	<td  width="8%">处理状态</td>
	<td  width="6%">派设计师时间</td>
	<td  width="6%">计划初测时间</td>
	<td  width="6%">客户姓名</td>
	<td  width="6%">合同号</td>
	<td  width="15%">房屋地址</td>
	<td  width="5%">家装设计师</td>
