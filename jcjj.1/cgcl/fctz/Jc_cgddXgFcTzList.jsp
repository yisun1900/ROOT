...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,jc_cgdd.sccsj,jc_cgdd.jhfcsj,crm_khxx.khxm,crm_khxx.hth,crm_khxx.fwdz,crm_khxx.sjs,DECODE(jc_cgdd.sfpsjs,'1','是','2','否') sfpsjs, jc_cgdd.cgsjs,sq_dwxx.dwmc dm,jc_cgdd.xmzy,jc_cgdd.clgw,jc_cgdd.ztjjgw,jc_cgdd.khbh ";
	...
	ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.jhfcxgsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgFcTzList.jsp","","","XgFcTzJc_cgdd.jsp");

//设置主键
	String[] keyName={"ddbh"};
	pageObj.setKey(keyName);
//	pageObj.setEditStr("复测通知");

//设置按钮参数
	String[] buttonName={"删除复测通知"};//按钮的显示名称
	String[] buttonLink={"DeleteCcJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--复测通知</font></B>
</CENTER>
<%
	pageObj.out=out;
