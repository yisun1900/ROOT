...
try {
	...
	if (count>0)
	{
		ls_sql="update jc_cgazsjjl set lrsj=?,lrr=?,bz=?";
		ls_sql+=" where ddbh='"+ddbh+"' and kazsj=?";
		ps= conn.prepareStatement(ls_sql);
		ps.setDate(1,lrsj);
		...
		ls_sql="insert into jc_cgazsjjl (ddbh,kazsj,khbh,lrsj,lrr,bz)";
		ls_sql+=" values(?,?,?,?,?,?) ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,ddbh);
		ps.setDate(2,kazsj);
	...
	ls_sql="update jc_cgdd set kazsj=?,kazsjlrr=?,azsjbz=?,clzt='27'";
