...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	jc_cgdd_rkr=request.getParameter("jc_cgdd_rkr");
	if (jc_cgdd_rkr!=null)
	{
		jc_cgdd_rkr=cf.GB2Uni(jc_cgdd_rkr);
		if (!(jc_cgdd_rkr.equals("")))	wheresql+=" and  (jc_cgdd.rkr='"+jc_cgdd_rkr+"')";
	}
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,jc_cgdd.jhazrq,jc_cgdd.rksj,jc_cgdd.rkr,jc_cgdd.pdgcmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,jc_cgdd.htze,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,sdks,jc_cgdd.khbh  ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.rksj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgRkList.jsp","","","XgRkJc_cgdd.jsp");
...
//设置按钮参数
	String[] buttonName={"删除入库信息"};//按钮的显示名称
	String[] buttonLink={"DeleteRkJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改入库</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
