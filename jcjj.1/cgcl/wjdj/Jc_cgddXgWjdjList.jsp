...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String jc_cgdd_wjsj=null;
	jc_cgdd_wjsj=request.getParameter("jc_cgdd_wjsj");
	if (jc_cgdd_wjsj!=null)
	{
		jc_cgdd_wjsj=jc_cgdd_wjsj.trim();
		if (!(jc_cgdd_wjsj.equals("")))	wheresql+="  and (jc_cgdd.wjsj>=TO_DATE('"+jc_cgdd_wjsj+"','YYYY/MM/DD'))";
	}
	jc_cgdd_wjsj=request.getParameter("jc_cgdd_wjsj2");
	if (jc_cgdd_wjsj!=null)
	{
		jc_cgdd_wjsj=jc_cgdd_wjsj.trim();
		if (!(jc_cgdd_wjsj.equals("")))	wheresql+="  and (jc_cgdd.wjsj<=TO_DATE('"+jc_cgdd_wjsj+"','YYYY/MM/DD'))";
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,DECODE(jc_cgdd.ycclzt,'0','未延迟','1','延迟未核实','2','延迟原因属实','3','延迟原因不属实') as ycclzt,b.mmycyymc zsycyybm,crm_khxx.khxm,crm_khxx.fwdz,jc_cgdd.pdgcmc,jc_cgdd.wjsj,jc_cgdd.wjlrr,jc_cgdd.kazsj,jc_cgdd.aztzsj,jc_cgdd.ksazsj,jc_cgdd.sazrq,a.mmycyymc,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,jc_cgdd.khbh  ";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.wjsj desc";
//	out.println(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgWjdjList.jsp","","","XgWjdjJc_cgdd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"DeleteWjdjJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜订单处理--修改完结登记</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
