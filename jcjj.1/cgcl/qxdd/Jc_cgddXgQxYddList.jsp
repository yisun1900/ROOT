...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		jc_cgdd_tdsj=jc_cgdd_tdsj.trim();
	...
	ls_sql="SELECT jc_cgdd.ddbh,clztmc,crm_khxx.khxm,crm_khxx.fwdz,jc_cgdd.tdsj,jc_cgdd.tdblr,jctdyymc,jc_cgdd.kclf,jc_cgdd.tdsm,TO_CHAR(jc_cgdd.cgzkl) cgzkl,jc_cgdd.htze,jc_cgdd.gtbfje,jc_cgdd.tmbfje,jc_cgdd.wjhtze,jc_cgdd.dqhtze,clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.cgsjs,sdks,jc_cgdd.khbh  ";
	ls_sql+=" FROM jc_cgdd,jdm_cgddzt,crm_khxx,jdm_jctdyybm";
    ls_sql+=" where jc_cgdd.khbh=crm_khxx.khbh(+) and jc_cgdd.clzt=jdm_cgddzt.clzt(+) and jc_cgdd.jctdyybm=jdm_jctdyybm.jctdyybm(+)";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.tdsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgddXgQxYddList.jsp","","","XgQxYddJc_cgdd.jsp");
//	pageObj.setPageRow(1000);//设置每页显示记录数
//	pageObj.setEditStr("退单");
...
//设置按钮参数
	String[] buttonName={"删除『取消预订单』信息"};//按钮的显示名称
	String[] buttonLink={"DeleteQxYddJc_cgdd.jsp"};//按钮单击调用的网页，可以增加参数
...
<CENTER >
  <B><font size="3">橱柜--修改取消预订单</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
