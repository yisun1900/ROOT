...
try {
	...
	ls_sql="update jc_cgccdmx set cgbm=?,dgcd=?,dggd=?,dgjs=?,dgdj=?,dgfbxs=?,tdcd=?,tggd=?,tgjs=?,tgdj=?,tgfbxs=?,tmbm=?,tmcd=?,tmdj=?,tmfbxs=?,mbmj=?,mbdj=?,mbbm=?,mbfbbm=?,mbfbjg=?,xtbm=?,xtjg=?,lsbm=?,lsjg=?,tjbbm=?,tjbjg=?,ctbm=?,ctjg=?,jlbm=?,jljg=?,sjzj=?,bz=? ";
