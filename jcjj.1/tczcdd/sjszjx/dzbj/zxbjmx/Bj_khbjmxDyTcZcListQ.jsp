...
try {
    ...
    ls_sql+=" where bj_khzcxmxjq.tccplbbm=jxc_clxlbm.clxlbm(+) and bj_khzcxmxjq.zjxxh='"+zjxxh+"' and bj_khzcxmxjq.sfbxx='Y' and bj_khzcxmxjq.sjsl=0";
	ls_sql+=" order by jgwzbm,bj_khzcxmxjq.tcsjflbm";
...
<table border="0" width="100%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 14px'>
  ...
  <tr bgcolor="#FFFFFF"> 
    <td colspan="8" align="center" class="STYLE2" ><B>快捷家装主材明细&nbsp;LWTC-02</B></td>
  </tr>
  <tr bgcolor="#FFFFFF"> 
...
<%
	...
	double sjjmje=0;
	double jmdj=0;
	ls_sql="SELECT distinct bj_khzcxmxjq.jgwzbm ";
	ls_sql+=" FROM bj_khzcxmxjq ";
    ls_sql+=" where bj_khzcxmxjq.zjxxh='"+zjxxh+"' ";
	ls_sql+=" order by bj_khzcxmxjq.jgwzbm ";
//	out.println(ls_sql);
	...
	while (rs2.next())
	{
		...
		ls_sql+=" where zjxxh='"+zjxxh+"' and jgwzbm='"+jgwzmc+"' and tccplbbm in(select tccplbbm from bj_khzcxmxjq where  zjxxh='"+zjxxh+"' and jgwzbm='"+jgwzmc+"')";
		ps1= conn.prepareStatement(ls_sql);
		rs1 =ps1.executeQuery();
		if (rs1.next())
		{
		...
		int fjxmsl=0;
		ls_sql="SELECT count(*)";
		ls_sql+=" FROM bj_khzcxmq,jxc_clxlbm";
		ls_sql+=" where bj_khzcxmq.tccplbbm=jxc_clxlbm.clxlbm(+)";
		ls_sql+=" and bj_khzcxmq.zjxxh='"+zjxxh+"' and bj_khzcxmq.jgwzbm='"+jgwzmc+"'";
		...
		{
			//房间内[标准套餐：产品类别]
			ls_sql="SELECT distinct bj_khzcxmxjq.tccplbbm,clxlmc,sjjmsl,sjjmje,jmdj";
			ls_sql+=" FROM bj_khzcxmxjq,jxc_clxlbm ";
			ls_sql+=" where bj_khzcxmxjq.tccplbbm=jxc_clxlbm.clxlbm(+) ";
			ls_sql+=" and bj_khzcxmxjq.zjxxh='"+zjxxh+"'";
			ls_sql+=" and bj_khzcxmxjq.jgwzbm='"+jgwzmc+"'";
			ls_sql+=" order by bj_khzcxmxjq.tccplbbm";
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			while (rs1.next())
			{
			...
			while (rs1.next())
			{
				...
				if (lbzcsl==0)//类别内[主材项目]未选择
				{
					...
					ls_sql+=" FROM bj_khzcxmq,bdm_bjjbbm";
					ls_sql+=" where bj_khzcxmq.bjjbbm=bdm_bjjbbm.bjjbbm(+)";
					ls_sql+=" and bj_khzcxmq.zjxxh='"+zjxxh+"' and bj_khzcxmq.jgwzbm='"+jgwzmc+"' and bj_khzcxmq.tccplbbm='"+tccplbbm+"'";
					ls_sql+=" and zclx in('1','2')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
					ls_sql+=" order by xh";
				//	out.println(ls_sql);
					ps= conn.prepareStatement(ls_sql);
					...
					ls_sql+=" FROM bj_khzcxmq,bdm_bjjbbm";
					ls_sql+=" where bj_khzcxmq.bjjbbm=bdm_bjjbbm.bjjbbm";
					ls_sql+=" and bj_khzcxmq.zjxxh='"+zjxxh+"'";
					ls_sql+=" and bj_khzcxmq.jgwzbm='"+jgwzmc+"'";
					ls_sql+=" and bj_khzcxmq.tccplbbm='"+tccplbbm+"'";
					ls_sql+=" and zclx not in('1','2')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
					ls_sql+=" order by xh";
				//	out.println(ls_sql);
					ps= conn.prepareStatement(ls_sql);
			...
			ls_sql="SELECT distinct bj_khzcxmq.tccplbbm,clxlmc";
			ls_sql+=" FROM bj_khzcxmq,jxc_clxlbm ";
			ls_sql+=" where bj_khzcxmq.tccplbbm=jxc_clxlbm.clxlbm(+) ";
			ls_sql+=" and bj_khzcxmq.zjxxh='"+zjxxh+"'";
			ls_sql+=" and bj_khzcxmq.jgwzbm='"+jgwzmc+"'";
			ls_sql+=" and bj_khzcxmq.tccplbbm not in(select tccplbbm from bj_khzcxmxjq where zjxxh='"+zjxxh+"' and jgwzbm='"+jgwzmc+"')";
			ls_sql+=" and bj_khzcxmq.zclx not in('1','2')";//1：套餐内；2：套餐外；3：升级大店；4：大店外
			ls_sql+=" order by bj_khzcxmq.tccplbbm";
			ps1= conn.prepareStatement(ls_sql);
			rs1 =ps1.executeQuery();
			while (rs1.next())
			{
