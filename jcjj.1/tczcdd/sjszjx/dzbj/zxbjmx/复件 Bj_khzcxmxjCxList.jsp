					...
					if (row==1)//房间内第一行
					{
						...
						<tr bgcolor="<%=bgcolor%>" align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
						  ...
						  <td><%=tccplbmc%></td>
							  ...
							  <td>&nbsp;</td>
						  <td><%=sfbxx%></td>
						  <td><a href="InsertBj_khzcxm.jsp?zjxxh=<%=zjxxh%>&jgwzbm=<%=jgwzbm%>&tccplbbm=<%=tccplbbm%>" target="_blank">套餐内更换</a></td>
							<%
					...
					while (rs3.next())
					{
						...
						if (row==1)//房间内第一行
						{
							...
							%>
							  ...
							  <td rowspan="<%=xlcpsl%>"><a href="InsertBj_khzcxm.jsp?zjxxh=<%=zjxxh%>&jgwzbm=<%=jgwzbm%>&tccplbbm=<%=tccplbbm%>" target="_blank">套餐内更换</a></td>
								<%
								if (sfkgh.equals("2"))//1：不可换；2：可换
								{
									%>
