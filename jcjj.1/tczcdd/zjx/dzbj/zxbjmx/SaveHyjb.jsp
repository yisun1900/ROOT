...
try {
	...
	if (bjjbbm.equals(bjjbbmQ))//恢复原来级别，采用原价格，删除已做增建项
	{
		...
		while (rs1.next())
		{
			...
			if (!sfjs.equals("N"))//N：未结算；C：已选择结算；Y：已结算；M：结算已审核；F：结算付款；P：不需结算
			{
				conn.rollback();
				out.println("错误！【订单:"+fjddbh+"】已结算或已选择结算，不能还原，型号:"+getxh);
				return;
			}
			...
			if (xclbz.equals("2"))//1：非定制式产品；2：定制式产品
			{
				...
				{
					conn.rollback();
					out.println("错误！【定制产品订单:"+fjddbh+"】供应商已接收，不能还原，型号:"+getxh);
					return;
				}
			}
			else{
