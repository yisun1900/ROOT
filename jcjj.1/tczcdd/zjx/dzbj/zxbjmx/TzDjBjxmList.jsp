...
<CENTER>调整项目单价（注意：按项目编号排序）</CENTER>
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
        ...
        <tr bgcolor="<%=bgcolor%>" align="center">
          ...
          <td align="left">
			<input type="text" name="xmmc" value="<%=xmmc%>" size="50" maxlength="50" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onKeyUp="keyGo(xmmc[<%=row%>])">
		</td>
          <td align="left"><%=fjmc%></td>
          <td  ><%=jldwmc%></td>
          <td> <%=slstr%></td>
          <td >
			<input type="text" name="dj" value="<%=dj%>" size="11" maxlength="11" style="border-color: #FFFFFF #FFFFFF #000000;border-style:solid" onKeyUp="keyGo(dj[<%=row%>])">
		  </td>
        </tr>
        <%
	}
...
{
	formName.action="SaveTzDjBjxm.jsp";
	formName.submit();
	return true;
} 
function KeyDown()
