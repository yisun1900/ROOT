...
try 
{
	...
	getzcdlbm =up.getParameter("zcdlbm");
	getzcxlbm =up.getParameter("zcxlbm");
	getdwbh =up.getParameter("dwbh");
...
<form method="post" action="SaveImpAddData.jsp" name="editform" target="_blank">
  <div align="center">增加新的主材报价（地区：<%=dqmc%>），注意：<font color="#FF0000">红色记录表示该[品牌供应商、主材名称、型号]已存在</font></div>
<input name="dqbm" type="hidden" value="<%=dqbm%>" >
  <table width="340%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		dwbh=outStr[i][1].trim();
		zclbbm=outStr[i][2].trim();
		zcbm=outStr[i][3].trim();
		zcmc=outStr[i][4].trim();
		xh=outStr[i][5].trim();
		gg=outStr[i][6].trim();
		zcysbm=outStr[i][7].trim();
		jldwbmstr=outStr[i][8].trim();
		lsjstr=outStr[i][9].trim();
		yhjstr=outStr[i][10].trim();

		cbjstr=outStr[i][11].trim();
		jsblstr=outStr[i][12].trim();
		cxhdblstr=outStr[i][13].trim();
		sfycxstr=outStr[i][14].trim();
		cxkssjstr=outStr[i][15].trim();
		cxjzsjstr=outStr[i][16].trim();
		cxjstr=outStr[i][17].trim();
		cxcbjstr=outStr[i][18].trim();
		lscxhdblstr=outStr[i][19].trim();
		sfbhpjstr=outStr[i][20].trim();

		pjtsstr=outStr[i][21].trim();
		sfyhstr=outStr[i][22].trim();
		cpjb=outStr[i][23].trim();
		fgsx=outStr[i][24].trim();
		cpsm=outStr[i][25].trim();
		cd=outStr[i][26].trim();
		sftjcpstr=outStr[i][27].trim();
		zp=outStr[i][28].trim();
		bz=outStr[i][29].trim();
		...
		if (!getdwbh.equals(dwbh))
		{
			out.println("<BR>！！！序号["+xuhao+"]错误，[品牌供应商名称]不一致，选择【"+getdwbh+"】而导入【"+dwbh+"】");
			return;
		}
		...
		if (dwbh!=null && !dwbh.equals(""))
		{
			...
			sql=" select zcdlmc" ;
			sql+=" from jdm_zcxlbm " ;
			sql+=" where zcxlmc='"+zcxlbm+"'" ;
//			out.println(sql);
			ps= conn.prepareStatement(sql);
			rs=ps.executeQuery();
			if(rs.next())
			{         
				zcdlbm = rs.getString("zcdlmc");			    
			}
			rs.close();
			ps.close();
		...
		if (sftjcpstr!=null && !sftjcpstr.equals(""))
		{
			...
			out.println("<BR>！！！序号["+xuhao+"]错误，[是否推荐产品]不能为空，只能是【是】或【否】");
			return;
		}
		...
		if (xh.equals(""))
		{
			sql=" select count(*)" ;
			sql+=" from jc_zcjgb " ;
			sql+=" where zcmc='"+zcmc+"' and dwbh='"+dwbh+"' and dqbm='"+dqbm+"' and xh is null" ;
		}
		else{
			sql=" select count(*)" ;
			sql+=" from jc_zcjgb " ;
			sql+=" where zcmc='"+zcmc+"' and dwbh='"+dwbh+"' and dqbm='"+dqbm+"' and xh='"+xh+"'" ;
		}
		ps= conn.prepareStatement(sql);
		rs=ps.executeQuery();
		if(rs.next())
