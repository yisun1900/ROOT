...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT '打印' dy,jc_zcdd.ddbh,clztmc,jc_zcdd.lrsj,ppbm,DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),crm_khxx.hth,crm_khxx.khxm,crm_khxx.fwdz,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'),jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys,zcxlbm,DECODE(jc_zcdd.xclbz,'1','非定制式产品','2','定制式产品'),jc_zcdd.khbh";
	...
	if (yhjs.equals("G0"))
	{
		...
		ls_sql+=" and jc_zcdd.clzt in('02','03','05')";
	}
	else{
...
//	out.println(ls_sql);
	...
	pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_zcddCxList.jsp","","","");
...
//设置列超级连接
	...
	coluParm.link="/jcjj/dygl/zcydddy.jsp";//为列参数：coluParm.link设置超级链接
	coluParm.bolt="target='_blank'";//为列参数：coluParm.link设置超级链接
	coluParmHash.put("dy",coluParm);//列参数对象加入Hash表
	pageObj.setColuLink(coluParmHash);//列参数对象加入Hash表
}
