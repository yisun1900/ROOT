...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_zcdd.ddbh,clztmc,zcxlbm,jc_zcdd.ppbm,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,jc_zcdd.jjsjs, DECODE(jc_zcdd.jkxz,'1','未付款','2','全款','3','材料商收余款','4','预付款减出','5','退货款减出'), DECODE(jc_zcdd.jkdd,'1','材料商现场收余款','2','店面收款','3','公司财务收款'), DECODE(jc_zcdd.ddshbz,'1','不需待电','2','待电未通知','3','待电已通知'),jc_zcdd.hkze,DECODE(jc_zcdd.xclbz,'1','非定制产品','2','定制式产品'),jc_zcdd.lrsj,a.dwmc lrdw,jc_zcdd.bz,jc_zcdd.ppmc,jc_zcdd.gys";
	...
	ls_sql+=" and jc_zcdd.xclbz='2'";//1：非定制式产品；2：定制式产品
    ls_sql+=wheresql;
...
//设置按钮参数
	String[] buttonName={"删除预订单"};//按钮的显示名称
	String[] buttonLink={"Jc_zcddList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from jc_zcdd where clzt='00' and ("+chooseitem+")";
