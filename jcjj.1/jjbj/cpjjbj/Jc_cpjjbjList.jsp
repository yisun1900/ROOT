...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_cpjjbj.cpjjbh,jc_cpjjbj.cpjjcz,jc_cpjjbj.dj,jc_cpjjbj.gg,jc_cpjjbj.xh,jc_cpjjbj.cpjjxl,jc_cpjjbj.cpjjmc,jc_cpjjbj.cpjjfg,jc_cpjjbj.cpsm,jc_cpjjbj.bz,dm_dqbm.dqmc,'<IMG SRC=\"/jcjj/jjbj/cpjjbj/images/'||UPPER(jc_cpjjbj.cktp)||'\" >'  cktp,'<IMG SRC=\"/jcjj/jjbj/cpjjbj/images/'||UPPER(jc_cpjjbj.cptp)||'\" >' cptp,jc_cpjjbj.lrr,jc_cpjjbj.lrsj  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cpjjbjList.jsp","SelectJc_cpjjbj.jsp","","EditJc_cpjjbj.jsp");
	pageObj.setPageRow(Integer.parseInt(myxssl));
/*
//设置显示列
	String[] disColName={"cpjjbh","jc_cpjjbj_cpjjfg","jc_cpjjbj_cpjjmc","jc_cpjjbj_xh","jc_cpjjbj_gg","jc_cpjjbj_cpjjxl","jc_cpjjbj_cpjjcz","jc_cpjjbj_dj","jc_cpjjbj_cktp","jc_cpjjbj_cptp","jc_cpjjbj_cpsm","sq_dwxx_dwmc","jc_cpjjbj_bz"};
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_cpjjbjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
//设置显示合并列
	...
	spanColHash.put("cpsm","1");//列参数对象加入Hash表
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(cpjjbh,"cpjjbh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from jc_cpjjbj where "+chooseitem;
