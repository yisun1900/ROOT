...
try {
	...
	for (int i=0;i<jsjlh.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>¥ÌŒÛ£¨°æ"+jsjlh[i]+"°ø“—…Û∫À");
			return;
		}

		ls_sql="update jc_gysjs set shbz='Y',shsj=SYSDATE,shr='"+yhmc+"'";
		ls_sql+=" where jsjlh='"+jsjlh[i]+"'";
		ps= conn.prepareStatement(ls_sql);
		ps.executeUpdate();
		ps.close();
		...
		while (rs1.next())
		{
			...
			if (ddbh.length()==9)
			{
				...
				{
					conn.rollback();
					out.println("<BR>¥ÌŒÛ£¨∂©µ•°æ"+ddbh+"°øŒ¥Ω·À„");
					return;
				}

				ls_sql="update jc_zcdd set sfjs='M' ";
				ls_sql+=" where ddbh='"+ddbh+"'";
				ps= conn.prepareStatement(ls_sql);
				ps.executeUpdate();
				ps.close();
				...
				{
					conn.rollback();
					out.println("<BR>¥ÌŒÛ£¨‘ˆºıœÓ°æ"+ddbh+"°øŒ¥Ω·À„");
					return;
				}
