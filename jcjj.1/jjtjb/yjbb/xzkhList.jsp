...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT  khbh,lx,khxm,fwdz,lxfs,clgw,ztjjgw,xmzy,qs  FROM ";
	ls_sql+="( select crm_khxx.khbh,'木门' as lx ,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.lxfs,jc_mmydd.clgw,jc_mmydd.ztjjgw,jc_mmydd.xmzy,jc_mmydd.htje as qs "; ls_sql+=" from jc_mmydd,crm_khxx where crm_khxx.khbh=jc_mmydd.khbh";//+mmwheresql;
	ls_sql+=mmwheresql;

	ls_sql+=" union all";

	ls_sql+=" select crm_khxx.khbh,'橱柜' as lx,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.lxfs,jc_cgdd.clgw,jc_cgdd.ztjjgw,jc_cgdd.xmzy,jc_cgdd.htze as qs  "; 
	ls_sql+=" from jc_cgdd,crm_khxx where crm_khxx.khbh=jc_cgdd.khbh " ;//+cgwheresql;
	ls_sql+=cgwheresql;

	ls_sql+=" union all";

	ls_sql+=" select crm_khxx.khbh,'家具' as lx, crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.lxfs,jc_jjdd.clgw,jc_jjdd.ztjjgw,jc_jjdd.xmzy,jc_jjdd.htze as qs  "; 
	ls_sql+=" from jc_jjdd,crm_khxx where crm_khxx.khbh=jc_jjdd.khbh ";//+jjwheresql;
	ls_sql+=jjwheresql;

	ls_sql+=" union all";

	ls_sql+=" select crm_khxx.khbh,'主材' as lx,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.lxfs,jc_zcdd.clgw,jc_zcdd.ztjjgw,jc_zcdd.xmzy,jc_zcdd.hkze as qs  "; 
	ls_sql+=" from jc_zcdd,crm_khxx where crm_khxx.khbh=jc_zcdd.khbh ";//+zcwheresql;
	ls_sql+=zcwheresql;
	ls_sql+=" ) ";    
...
//进行对象初始化
	...
	pageObj.initPage("xzkhList.jsp","SelectRq.jsp","","");
