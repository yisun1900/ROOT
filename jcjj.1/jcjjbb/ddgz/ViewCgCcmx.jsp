...
<%
	...
	ls_sql="SELECT jc_cgdd.ddbh as ddbh,jc_cgdd.xmzy,jc_cgdd.khbh as khbh,jhccsj,jc_cgdd.dj as jc_cgdd_dj,jc_cgdd.cgsjs as jc_cgdd_cgsjs, DECODE(jc_cgdd.clzt,'00','预订单未完成','01','预订单已完成','02','已派单','03','已初测','04','已复测','05','签订合同','06','合同已确认','07','已入库','08','已安装') as jc_cgdd_clzt, DECODE(jc_cgdd.khlx,'1','装修客户','2','橱柜客户') as jc_cgdd_khlx,jc_cgdd.khxm as jc_cgdd_khxm,jc_cgdd.fwdz as jc_cgdd_fwdz,jc_cgdd.hth as jc_cgdd_hth,sq_dwxx.dwmc as sq_dwxx_dwmc  ";
	ls_sql+=" FROM jc_cgdd,sq_dwxx   ";
    ls_sql+=" where jc_cgdd.dwbh=sq_dwxx.dwbh ";
	ls_sql+=" and jhccsj<SYSDATE";
	ls_sql+=" and clzt<'03'";
	ls_sql+=wheresql;
    ls_sql+=" order by jc_cgdd.jhccsj";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("","","","");
	pageObj.setPageRow(10000);//设置每页显示记录数
//设置显示列
	String[] disColName={"ddbh","jc_cgdd_clzt","jhccsj","jc_cgdd_khxm","jc_cgdd_fwdz","jc_cgdd_hth","jc_cgdd_dj","xmzy","jc_cgdd_cgsjs","jc_cgdd_khlx","sq_dwxx_dwmc"};
...
<CENTER >
  <B><font size="3">橱柜初测延期（项目专员：<%=xmzymc%>）</font></B>
</CENTER>
<%
	pageObj.out=out;
	pageObj.getDate(1);
