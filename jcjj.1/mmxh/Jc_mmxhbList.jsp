...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jc_mmxhb.mmxh,jdm_mxbm.mxmc,jdm_mytbm.myt,bzjg,jc_mmxhb.zp,jc_mmxhb.bz,jc_mmxhb.lrr,jc_mmxhb.lrsj,dwmc,dqmc  ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_mmxhbList.jsp","SelectJc_mmxhb.jsp","","EditJc_mmxhb.jsp?dqbm="+dqbm);
//	pageObj.setPageRow(10);//设置每页显示记录数
/*
//设置显示列
	String[] disColName={"mmxh","jdm_mxbm_mxmc","jdm_mytbm_myt","jc_mmxhb_zp","jc_mmxhb_bz"};
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_mmxhbList.jsp?dqbm="+dqbm};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(mmxh,"mmxh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from jc_mmxhb where dqbm='"+dqbm+"' and  "+chooseitem;
