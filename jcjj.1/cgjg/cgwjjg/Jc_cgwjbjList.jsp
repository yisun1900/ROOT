...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jc_cgwjbjList.jsp","SelectJc_cgwjbj.jsp","","EditJc_cgwjbj.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Jc_cgwjbjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] wjbm = request.getParameterValues("wjbm");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(wjbm,"wjbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from jc_cgwjbj where "+chooseitem;
