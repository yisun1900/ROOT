...
try {
	...
	if (mqhkze!=oldhkze)
	{
		out.println("<BR>错误，存盘失败！合同总额已发生变化,请刷新后重新保存");
		return;
	}

	ls_sql="update jc_zcdd set clgw=?,ztjjgw=?,ddshbz=?,kjxsj=?,htshsj=?,xmzy=?,qhtsj=?,bz=?,hkze=?,jhtzshsj=?,wdzje=?,htcxhdje=?,jjsjs=?,zczkl=?,dzyy=?,jmje=? ";
	ls_sql+=" where  (ddbh='"+ddbh+"')  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,clgw);
	...
	ps.setDouble(14,zczkl);
	ps.setString(15,dzyy);
	ps.setDouble(16,jmje);
