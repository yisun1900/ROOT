...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Hdm_hfdxflbmList.jsp","","","EditHdm_hfdxflbm.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Hdm_hfdxflbmList.jsp?"};//按钮单击调用的网页，可以增加参数
...
{
	String[] hfdxflbm = request.getParameterValues("hfdxflbm");
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(hfdxflbm,"hfdxflbm"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="delete from hdm_hfdxflbm where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="12%">&nbsp;</td>
	<td  width="24%">回访对象分类编码</td>
	<td  width="64%">回访对象分类名称</td>
</tr>
<%
	pageObj.displayDate();
	pageObj.displayFoot();
