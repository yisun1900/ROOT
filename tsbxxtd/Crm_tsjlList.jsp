...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT '<a href=\"InsertCrm_tsbxxtd.jsp?tsjlh='||crm_tsjl.tsjlh||'&khbh='||crm_khxx.khbh||'\">录入协调单</a>' lrpfd,crm_tsjl.tsjlh as tsjlh,crm_khxx.hth,crm_khxx.khbh,slfsmc,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.sjs,sgdmc sgd,crm_tsjl.slsj,crm_tsjl.hfsj,crm_tsjl.tsnr,DECODE(crm_tsjl.clzt,0,'未受理',1,'已受理',2,'在处理',3,'结案','9','不立案') as clzt,DECODE(crm_tsjl.sfxtz,'Y','需通知','N','不需通知') as sfxtz";
...
//进行对象初始化
	pageObj.initPage("Crm_tsjlList.jsp","SelectCrm_tsjl.jsp","","");
//	pageObj.setPageRow(10);//设置每页显示记录数
//设置显示列
	String[] disColName={"lrpfd","hth","khxm","fwdz","sjs","sgd","tsjlh","sfxtz","clzt","slfsmc","tsnr","slsj","hfsj"};
...
//		sql[0]="delete from crm_tsjlhfmx where tsjlh in(select tsjlh from crm_tsjl where "+chooseitem+")";
//		sql[1]="delete from crm_tswthfmx where tsjlh in(select tsjlh from crm_tsjl where "+chooseitem+")";
//		sql[2]="delete from crm_tsbm where "+chooseitem;
//		sql[3]="delete from crm_tsjl where "+chooseitem;
