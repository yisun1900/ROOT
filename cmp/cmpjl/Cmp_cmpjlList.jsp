...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and cmp_cmpjl.cmpzt='1'";//1：未填写；2：已填写
    ls_sql+=wheresql;
    ls_sql+=" order by cmp_cmpjl.lrsj";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cmp_cmpjlList.jsp","","","");
...
//设置按钮参数
	...
	String[] buttonLink={"Cmp_cmpjlList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from cmp_cmpjl where cmpzt='1' and "+chooseitem;
		sql[1]="update crm_khxx set cmpzt=0,cmpbh=null where "+chooseitem;
