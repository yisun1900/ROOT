...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (spjry!=null)
	{
		...
		if (!(spjry.equals("")))	wheresql+=" and  (cmp_khpjb.spjry='"+spjry+"')";
	}
	...
	ls_sql="SELECT crm_khxx.hth,crm_khxx.khxm,dwmc,crm_khxx.sjs,crm_khxx.zjxm,sgdmc ,cmp_khpjb.xmfl,cmp_khpjb.pjxm,cmp_khpjb.pjjg,cmp_khpjb.lrsj,crm_khxx.khbh ";
	ls_sql+=" FROM crm_khxx,sq_dwxx,sq_sgd,cmp_khpjb ";
    ls_sql+=" where crm_khxx.dwbh=sq_dwxx.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) ";
    ls_sql+=" and crm_khxx.khbh=cmp_khpjb.khbh";
    ls_sql+=wheresql;
    ls_sql+=" order by cmp_khpjb.lrsj desc,crm_khxx.khbh ,cmp_khpjb.xmflbm,cmp_khpjb.pjxmbm";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Cmp_khpjbCxList.jsp","","","");
	pageObj.setPageRow(88);
/*
//设置显示列
...
//设置列超级连接
	...
	coluParm.link="ViewCmp_khpjb.jsp";//为列参数：coluParm.link设置超级链接
