...
try {
	...
	{
		out.print("<font color=red>您没有录入总部联系人！</font>");return;
	}
	rs.close();
	ps.close();
	//查询分公司打印数据
...
<table width="100%" border="1" cellspacing="0" style="border-color: #000000 #000000 #FFFFFF #000000 ;border-style:solid"  style="FONT-SIZE: 14px">
  <tr width="25%" align="center" bgcolor="#FFFFFF"> 
    <td width="20%" rowspan="6"><img width="260px" src="logo.jpg"></td>
    <td colspan="3"><strong>松下亿达分公司商品订单</strong></td>
 ...
 <tr align="center" bgcolor="#FFFFFF"> 
    ...
    <td width="15%"><b>规格及型号</b></td>
	<td width="13%"><b>单位</b></td>
    <td width="6%"><b>数量</b></td>
    <td width="10%"><b>重量 </td>
    <td width="12%"><b>采购单价（元）</b></td>
	<td width="13%"><b>采购总价（元）</b></td>
 </tr>
 <%
 String clmc=null;
 String xh=null;
 ...
 try {
	conn=cf.getConnection();
	//查询商品详细信息
	ls_sql="select clmc,xh,gg,jldwbm,shsl,cbj dfgsjg,cbj*shsl zjg,djzl*shsl zzl,djzl||' * '||shsl xszl";
	ls_sql+=" from jxc_clsgmx,jxc_cljgb,jxc_clbm ";
	ls_sql+=" where jxc_clsgmx.clbm=jxc_cljgb.clbm and jxc_clsgmx.clbm=jxc_clbm.clbm and jxc_clsgmx.sgph='"+sgph+"' and jxc_cljgb.dqbm='"+dqbm+"'";
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	while (rs.next())
	{
		...
		xszl=cf.fillNull(rs.getString("xszl"));

		shsl=rs.getDouble("shsl");
		sumzsl+=shsl;
		dfgsjg=rs.getDouble("dfgsjg");
		zjg=rs.getDouble("zjg");
		sumzjg+=zjg;
		zzl=rs.getDouble("zzl");
		sumzzl+=zzl;
		%>
		<tr align="center" bgcolor="#FFFFFF"> 
			<td width="31%"><%=clmc%>&nbsp;</td>
			<td width="15%"><%=gg%>/<%=xh%>&nbsp;</td>
			<td width="13%"><%=jldwbm%>&nbsp;</td>
			<td width="6%"><%=shsl%>&nbsp;</td>
			<td width="10%"><%=xszl%>&nbsp; </td>
			<td width="12%"><%=dfgsjg%>&nbsp; </td>
			<td width="13%"><%=zjg%>&nbsp;</td>
		 </tr>
		<%
	}
...
%>
   ...
   <tr align="center" bgcolor="#FFFFFF"> 
    <td width="31%" align="right">小记:</td>
    <td width="15%">&nbsp;</td>
	<td width="13%">&nbsp;</td>
    <td width="6%"><%=cf.formatDouble(sumzsl)%>&nbsp;</td>
    <td width="10%"><%=cf.formatDouble(sumzzl)%>&nbsp; </td>
    <td width="12%">&nbsp; </td>
	<td width="13%"><%=cf.formatDouble(sumzjg)%>&nbsp; </td>
 </tr>
 <tr align="center" bgcolor="#FFFFFF"> 
