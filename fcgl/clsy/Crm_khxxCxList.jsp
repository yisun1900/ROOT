...
//	String zjxm=null;
	...
	else if (row==1)//只有一条，不需选择
	{
		...
		//只有一条，不需选择
		response.sendRedirect("InsertCl_clsymx.jsp?khbh="+getkhbh);
	}
	else{
		%>
		<CENTER >
		  <B><font size="3">录入材料使用明细－有多个符合条件客户，请选择</font></B>
		</CENTER>
		...
		ls_sql="SELECT khbh,hth,khxm,fwdz,lxfs||','||qtdh lxfs,sq_dwxx.dwmc,sgdmc,sjs,zjxm,kgrq,jgrq ";
		ls_sql+=" FROM crm_khxx,sq_dwxx,sq_sgd ";
		ls_sql+=" where crm_khxx.dwbh=sq_dwxx.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+) ";
		if (kfgssq.equals("1") || kfgssq.equals("2"))//0：不授权；1：授权单个分公司；2：授权多个分公司；3：授权单个店面；4：授权某一分公司多个店面；5：授权多个分公司多个店面
		{
		...
		while (rs.next())
		{
			...
			if (row>200)
			{
				out.println("数据太多超过200条被截掉，下面不再显示");
				return;
			}

			%>
			<tr bgcolor="#FFFFFF"  align="center">
				<td><A HREF="InsertCl_clsymx.jsp?khbh=<%=khbh%>">录入</A></td>
				<td><%=hth%></td>
