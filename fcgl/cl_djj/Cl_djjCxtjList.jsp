...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT cl_djj.czxh,crm_khxx.khxm,cl_djj.sqlqsl,cl_djj.sqlqje,cl_djj.sjsl,cl_djj.sjje, DECODE(cl_djj.sfjm,'1','不申请','2','申请减免'), DECODE(cl_djj.shzt,'1','未审核','2','一级审核','3','二级审核'),DECODE(shjl,'1','同意','2','不同意','3','需修改'),cl_djj.shr,cl_djj.shsj,cl_djj.shyj, DECODE(cl_djj.czzt,'1','未提交','2','已提交','3','已打印'),cl_djj.khbh,crm_khxx.fwdz,cl_djj.qye,sq_dwxx.dwmc  ,cl_djj.lrr,cl_djj.lrsj,cl_djj.bz";
	ls_sql+=" FROM cl_djj,crm_khxx,sq_dwxx  ";
    ls_sql+=" where cl_djj.dwbh=sq_dwxx.dwbh and cl_djj.khbh=crm_khxx.khbh";
    ls_sql+=" and (cl_djj.shzt='1' or cl_djj.shzt is null )";
    ls_sql+=wheresql;
    ls_sql+=" order by cl_djj.czxh";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cl_djjCxtjList.jsp","SelectCxtjCl_djj.jsp","","");
...
//设置按钮参数
	String[] buttonName={"撤销提交"};//按钮的显示名称
	String[] buttonLink={"Cl_djjCxtjList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	String chooseitem =cf.GB2Uni(cf.arrayToInSQL(czxh,"czxh"));
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		String[] sql=new String[1];//要执行的SQL
		boolean needCommit=false;//不需要事务处理
		sql[0]="update cl_djj set czzt='1' where "+chooseitem;
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="3%">一级审核结论</td>
	<td  width="3%">一级审核人</td>
	<td  width="4%">一级审核时间</td>
	<td  width="15%">一级审核意见</td>
	<td  width="3%">操作状态</td>
	<td  width="3%">客户编号</td>
	<td  width="10%">房屋地址</td>
	<td  width="4%">签约额</td>
