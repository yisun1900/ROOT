...
<div align="center">
  <p><font size="5"><b>代金券作废</b></font></p>
<form method="post" action="SaveZfdjj.jsp" name="insertform" >
    <table border="1" cellspacing="0" cellpadding="1" width="100%" style='FONT-SIZE: 15px' >
      <tr> 
        <td width="20%" align="right">作废原因</td>
        <td colspan="3">
          <textarea name="zfyy" rows="3" cols="71"></textarea>
        </td>
      </tr>
      <tr> 
        <td width="20%" align="right">作废录入人</td>
        <td width="30%">
                <input type="text" name="zflrr" size="20" maxlength="13" value="<%=yhmc%>" readonly>
        </td>
        <td width="17%" align="right">作废录入时间</td>
        <td width="33%"> 
                <input type="text" name="zflrsj" size="20" maxlength="10" value="<%=cf.today()%>" readonly>
        </td>
      </tr>
      <tr align="center"> 
        <td colspan="4">
          <input type="hidden" name="czxh"  value="<%=czxh%>" >
		 <input type="button"  value=" 作废 " name="bc" onClick="f_do(insertform)">
        </td>
      </tr>
    </table>
</form>
...
{
	if(	javaTrim(FormName.zfyy)=="") {
		alert("请输入[作废原因]！");
		FormName.zfyy.focus();
		return false;
	}

	if(	javaTrim(FormName.zflrr)=="") {
		alert("请输入[作废录入人]！");
		FormName.zflrr.focus();
		return false;
	}
	if(	javaTrim(FormName.zflrsj)=="") {
		alert("请输入[作废录入时间]！");
		FormName.zflrsj.focus();
		return false;
	}
	if(!(isDatetime(FormName.zflrsj, "作废录入时间"))) {
		return false;
	}
