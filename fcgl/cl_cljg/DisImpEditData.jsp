...
try 
{
	...
	%> 
<form method="post" action="SaveImpEditData.jsp" name="editform" target="_blank">
  <div align="center">更新辅材报价（分公司：<%=dwmc%>）</div>
<input name="dwbh" type="hidden" value="<%=dwbh%>" >
  <table width="140%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		sql=" select count(*)" ;
		sql+=" from cl_jgmx " ;
		sql+=" where dwbh='"+dwbh+"' and clbm='"+clbm+"'" ;
		ps= conn.prepareStatement(sql);
		rs=ps.executeQuery();
		if(rs.next())
		{         
...
%> 
    <tr bgcolor="#CCCCCC" > 
      <td colspan="45"> 
        <input type="button"  value="更新" name="bc" onClick="f_do(editform)">
      </td>
    </tr>
  </table>
</form>
