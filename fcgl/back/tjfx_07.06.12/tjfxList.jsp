...
ls_sql+=" from cl_rkmx,cl_rkd,cl_clbm,sq_dwxx  ";
ls_sql+=" where cl_rkd.rkph=cl_rkmx.rkph and cl_rkmx.clbm=cl_clbm.clbm ";
...
|		显示库存数据												|
\***************************************************************/
ls_sql="select cl_rkmx.clbm,cl_rkmx.clmc,cl_rkmx.clgg,jldwmc,nvl(sum(rksl),0) sl ";
ls_sql+=" from cl_rkmx,cl_rkd,sq_dwxx  ";
ls_sql+=" where cl_rkd.rkph=cl_rkmx.rkph ";
ls_sql+=" and cl_rkd.dwbh=sq_dwxx.dwbh and rkzt='2' ";
ls_sql+=wheresql;
ls_sql+=whererk;
ls_sql+=" group by cl_rkmx.clbm,cl_rkmx.clmc,cl_rkmx.clgg,jldwmc ";
ls_sql+=" order by cl_rkmx.clbm,cl_rkmx.clmc,cl_rkmx.clgg ";
ps= conn.prepareStatement(ls_sql);
rs =ps.executeQuery(ls_sql);
while (rs.next())
	{
		...
		sql="select cl_ckmx.clbm,cl_ckmx.clmc,cl_ckmx.clgg,jldwmc,nvl(sum(cksl),0) sl ";
		sql+=" from cl_ckmx,cl_ckd,sq_dwxx  ";
		sql+=" where cl_ckd.ckph=cl_ckmx.ckph and cl_ckd.dwbh=sq_dwxx.dwbh ";
		sql+=" and cl_ckmx.clbm='"+clbm+"' ";
		sql+=" and ckzt='2' ";
		sql+=wheresql;
		sql+=whereck;
		sql+=" group by cl_ckmx.clbm,cl_ckmx.clmc,cl_ckmx.clgg,jldwmc ";
		...
		<td> 
			<div align="center"><a href="ckmxList2.jsp?clbm=<%=clbm%>" target="_back"><%=sumck%>&nbsp;<%=jldwmc%></a></div>
		</td>
		<td> 
			<div align="left">
			<img title="<%=clmc%> : <%=sumrk%>&nbsp;<%=jldwmc%>" width="<%=txkcsl/5%>" height="8" src="/images/ColuGif.gif">
			<BR>
			<img title="<%=clmc%> : <%=sumck%>&nbsp;<%=jldwmc%>" width="<%=txkcsl2/5%>" height="8" src="/images/ColuGif.gif"></div>
		</td>
	</tr>
					<%
			}
