...
|		显示库存数据												|
\***************************************************************/
ls_sql="select cl_rkmx.clbm,cl_rkmx.clmc,cl_rkmx.clgg,jldwmc,nvl(sum(rksl),0) sl,to_char(rksj,'YYYY-MM') tjrksj";
ls_sql+=" from cl_rkmx,cl_rkd,jdm_jldwbm,cl_clbm,sq_dwxx  ";
ls_sql+=" where cl_rkd.rkph=cl_rkmx.rkph and cl_rkmx.clbm=cl_clbm.clbm and cl_clbm.jldwbm=jdm_jldwbm.jldwbm(+) ";
ls_sql+=" and cl_rkd.dwbh=sq_dwxx.dwbh and rkzt='2' ";
ls_sql+=wheresql;
ls_sql+=whererk;
ls_sql+=" group by cl_rkmx.clbm,cl_rkmx.clmc,cl_rkmx.clgg,jldwmc,to_char(rksj,'YYYY-MM') ";
...
while (rs.next())
	{
		...
		tjrksj=cf.fillNull(rs.getString("tjrksj"));
...
//		sql+=" and to_char(shqrsj,'YYYY-MM') like '"+tjrksj+"%' ";
		sql+=" and ckzt='3' ";
		sql+=wheresql;
		sql+=whereck;
