...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT rkph,DECODE(rklx,'1','新货','2','退货'),DECODE(rkzt,'1','<a href=tjrkd.jsp?rkph=\'||rkph||\' target=_back>提交入库单</a>','2','已提交'),dwmc,gysmc,sgdmc,rkr,rksj,rkzsl,rkzje,sgph,lrr,lrsj,cl_rkd.bz  ";
	ls_sql+=" FROM cl_rkd,sq_dwxx,sq_sgd "; 
    ls_sql+=" where cl_rkd.dwbh=sq_dwxx.dwbh(+) and cl_rkd.sgd=sq_sgd.sgd(+)";
    ls_sql+=wheresql;
    ls_sql+=" order by rkph";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Cl_rkdList.jsp","SelectCl_rkd.jsp","","EditCl_rkd.jsp");
...
//设置按钮参数
	...
	String[] buttonLink={"Cl_rkdList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from cl_rkd where "+chooseitem;
		sql[1]="delete from cl_rkmx where "+chooseitem;
