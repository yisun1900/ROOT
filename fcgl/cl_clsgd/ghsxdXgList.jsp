...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by cl_clsgd.lrsj desc";
    pageObj.sql=ls_sql;
//	out.print(ls_sql);
//进行对象初始化
	pageObj.initPage("ghsxdXgList.jsp","SelectXgghsxd.jsp","","");
...
//设置按钮参数
	String[] buttonName={"删除-供货商下单"};//按钮的显示名称
	String[] buttonLink={"ghsxdXgList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]=" update cl_clsgd set sgzt='5',gysxdr=null,gysxdsj=null where "+chooseitem;
