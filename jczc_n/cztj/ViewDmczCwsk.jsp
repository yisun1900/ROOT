...
<%
	ls_sql="SELECT '',crm_khxx.sjs,hth,khxm,fwdz,DECODE(crm_khxx.jzbz,'1','家装','2','工装'),cxhdbm,wdzgce,TO_CHAR(zkl),qye,cwsfke,DECODE(qye,0,'0',ROUND(cwsfke*100/qye,1)||'%'),sfke,zjxje,NVL(zjxje,0)+qye,sjf,DECODE(crm_khxx.sffj,'Y','返卷','N','否'),crm_khxx.fjje,crm_khxx.gsfje,DECODE(crm_khxx.sfyrz,'Y','认证','N','否'),b.dwmc rzsc,a.dwmc qydm,TO_CHAR(crm_khxx.lrsj,'YYYY-MM-DD HH24:MI'),TO_CHAR(qyrq,'YYYY-MM-DD'),DECODE(TO_CHAR(crm_khxx.lrsj,'MM')-TO_CHAR(qyrq,'MM'),0,'是','<font color=\"#FF0000\">跨月</font>'),sgdmc sgd,zjxm";
...
<BR>
<CENTER >
  <B><font size="3">本时间段退单客户（<%=dwmc%>）<font color=red>&nbsp;&nbsp;-</font>
  <BR>(时间范围：<%=sjfw%>--<%=sjfw2%>)</font></b>
</CENTER>
<table border="1" width="200%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
<tr bgcolor="#CCCCCC"  align="center">
...
<%
	...
	ls_sql+=" FROM crm_khxx,sq_dwxx a,sq_dwxx b,sq_sgd,crm_tddj";
    ls_sql+=" where crm_khxx.dwbh=a.dwbh(+) and crm_khxx.rzsc=b.dwbh(+) and crm_khxx.sgd=sq_sgd.sgd(+)";
    ls_sql+=" and (crm_khxx.khbh in (select khbh from cw_khfkjl where skdd='1' or (skdd='2' and dsksjbz='Y')";
    ls_sql+=" and cw_khfkjl.dwbh='"+dwbh+"'  )) ";
    ls_sql+=" and crm_khxx.dwbh='"+dwbh+"' and crm_khxx.cwjzkscsj is not null";
	ls_sql+=" and crm_khxx.zt='3' and crm_tddj.tdxh=crm_khxx.tdxh";
