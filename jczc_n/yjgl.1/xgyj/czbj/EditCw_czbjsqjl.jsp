...
////////////////
	...
	gxhwdzgce=dzbj.getBjje(conn,khbh,"yz");//合同原报价
	gxhsuijinbfb=dzbj.getSuijin(conn,khbh,"yz");//税金百分比
	gxhsuijin=dzbj.getZqSuijinJe(conn,khbh,"yz");//『折前』税金
	gxhguanlif=dzbj.getZqGlfJe(conn,khbh,"yz");//『折前』管理费

	if (zklx.equals("1"))//1：全额折扣；2：单项折扣
	{
		gxhqye=gxhwdzgce*spzkl/10;//签约额打折
		gxhqye=cf.round(gxhqye,2);

		gxhsuijin=gxhsuijin*spzkl/10;//税金打折
		gxhsuijin=cf.round(gxhsuijin,2);

		gxhguanlif=gxhguanlif*spzkl/10;//管理费打折
	...
	else if (zklx.equals("2"))//1：全额折扣；2：单项折扣
	{
		...
		gxhqye=gxhzjf*spzkl/10+gxhzhqtsf;//签约额打折
		gxhqye=cf.round(gxhqye,2);

		gxhsuijin=dzbj.getZhSuijinJe(conn,khbh,"yz");//税金打折
