...
try{
	...
	out.println("<BR>[更新后合同原报价]类型转换发生意外:"+e);
	return;
}
...
try{
	...
	out.println("<BR>[更新后管理费]类型转换发生意外:"+e);
	return;
}
...
try {
	...
	//更新：重做报价授权记录
	ls_sql="update cw_czbjsqjl set wcbz='2',wcsj=?,wcr=?,gxhwdzgce=?,gxhzkl=?,gxhqye=?,gxhsuijin=?,gxhsuijinbfb=?,gxhsjf=?,gxhglf=?,gxhguanlif=?,bz=? ";
	ls_sql+=" where khbh='"+khbh+"' and  sqsj=TO_DATE('"+sqsj+"','yyyy-mm-dd') ";
	ps= conn.prepareStatement(ls_sql);
	ps.setDate(1,wcsj);
	ps.setString(2,wcr);
	...
	ls_sql+=" set wdzgce=?,zkl=?,qye=?,suijin=?,suijinbfb=?,sjf=?,glf=?,guanlif=?";
	ls_sql+=" where khbh='"+khbh+"'";
	ps= conn.prepareStatement(ls_sql);
