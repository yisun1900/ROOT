...
String sjfw=yjjzsj.substring(0,8)+"01";
String sjfw2=yjjzsj;
...
<tr bgcolor="#CCCCCC"  align="center" class="STYLE_TR">
  ...
  <td  width="7%" class="STYLE2">签约额</td>
...
<%
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT cw_qmjzmx.fgsbh,cw_qmjzmx.dwbh,count(*) qys,sum(qye) qye,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(sjf) sjf,sum(fwmj) fwmj";
	ls_sql+=" FROM cw_qmjzmx";
	ls_sql+=" where cw_qmjzmx.zt in ('2')";
	ls_sql+=wheresql;
	...
	while (rs.next())
	{
		...
		//退单
		ls_sql="SELECT sum(cw_qmjzmx.qye) cwsgqye ";
		ls_sql+=" FROM cw_qmjzmx";
		...
		%>
		<tr align="center">
			<td  class="STYLE3"><A HREF="/jczc_n/yjgl/yjtj/ViewDmcz.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getdwbh%>&jzbz=0" target="_blank"><%=row%></A></td>
...
<%
	...
	ls_sql+=" from ( ";
	ls_sql+=" SELECT cw_qmjzmx.fgsbh,count(*) qys,sum(qye) qye,sum(wdzgce) wdzgce,sum(zhzjxje) zhzjxje,sum(sjf) sjf,sum(fwmj) fwmj";
	ls_sql+=" FROM cw_qmjzmx";
	ls_sql+=" where cw_qmjzmx.zt in ('2')";
	ls_sql+=wheresql;
	...
	while (rs.next())
	{
		...
		%>
		<tr align="center">
			<td  class="STYLE3"><A HREF="/jczc_n/yjgl/yjtj/ViewFgscz.jsp?sjfw=<%=sjfw%>&sjfw2=<%=sjfw2%>&dwbh=<%=getfgsbh%>&jzbz=0" target="_blank"><%=row%></A></td>
			<td  class="STYLE3"><%=getfgsmc%></td>
			<td  align="right" class="STYLE4"><%=cf.formatDouble(qye)%></td>
			<td  align="right" class="STYLE4"><%=cf.formatDouble(tdje)%></td>
			<td  align="right" class="STYLE4"><%=cf.formatDouble(yjje)%></td>
