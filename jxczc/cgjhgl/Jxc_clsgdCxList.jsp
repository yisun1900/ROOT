...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jxc_clsgd.sgph, DECODE(jxc_clsgd.sgdzt,'0','未提交','1','等待审核','2','等待出库','5','部分出库','3','已出库','4','已入库','6','审核未通过'),DECODE(jxc_clsgd.sglb,'0','主材','1','辅材'),a.dwmc fgs,jxc_clsgd.sgr,jxc_clsgd.sgrq,jxc_clsgd.sgzsl,jxc_clsgd.sgzje, DECODE(jxc_clsgd.psfs,'1','配送','2','自取','9','其他'),jxc_clsgd.jhshsj,jxc_clsgd.shr,jxc_clsgd.shrq,jxc_clsgd.shyj,jxc_clsgd.lrr,jxc_clsgd.lrsj,b.dwmc ssbm,jxc_clsgd.bz  ";
	ls_sql+=" FROM jxc_clsgd,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where jxc_clsgd.ssfgs=a.dwbh and jxc_clsgd.ssbm=b.dwbh";
    ls_sql+=" and jxc_clsgd.sgdzt in('2','6') and jxc_clsgd.sfzcgd='N'";
    ...
    ls_sql+=wheresql;
    ls_sql+=" order by jxc_clsgd.shrq desc";
    pageObj.sql=ls_sql;
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jxc_clsgdCxList.jsp","","","");
...
//设置按钮参数
	String[] buttonName={"申购单批量转采购计划"};//按钮的显示名称
	String[] buttonLink={"SgdToCgjh.jsp?cgjhph="+cgjhph+"&"};//按钮单击调用的网页，可以增加参数
...
//设置列超级连接
	...
	coluParm.link="/jxczc/sgdgl/ViewJxc_clsgd.jsp";//为列参数：coluParm.link设置超级链接
...
<CENTER >
  <B><font size="3">选择－材料申购单（采购计划批号：<%=cgjhph%>）</font></B>
</CENTER>
<%
	pageObj.out=out;
