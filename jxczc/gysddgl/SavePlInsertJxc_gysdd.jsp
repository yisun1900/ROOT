...
String sfrksgdw=cf.GB2Uni(request.getParameter("sfrksgdw"));//是否直接入库申购公司
...
try {
	...
	for (int i=0;i<sgph.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("错误！已转为采购计划："+sgph[i]);
			return;
		}
		else if (sfzcgd.equals("M"))//N：未转；Y：转为采购计划；M：转为供应商订单
		{
			conn.rollback();
			out.println("错误！已转为供应商订单："+sgph[i]);
			return;
		}

	
		String clbm=null;
		ls_sql="select jxc_clsgmx.clbm ";
		...
		while (rs.next())
		{
			...
			{
				conn.rollback();
				out.print("<font color=red>错误！申购单["+sgph[i]+"]材料编码["+clbm+"]在下单公司不存在！</font><br>");
				return;
			}
		}
		rs.close();
	...
	if (!sgphStr.equals(""))
	{
		sgphStr=sgphStr.substring(1);
	}

	String gysmc=null;
	ls_sql="select distinct jxc_cljgb.gysmc ";
	ls_sql+=" from  jxc_clsgmx,jxc_cljgb ";
	ls_sql+=" where jxc_clsgmx.clbm=jxc_cljgb.clbm and jxc_cljgb.dqbm='"+dqbm+"' ";
	ls_sql+=" and "+cf.arrayToInSQL(sgph,"jxc_clsgmx.sgph");
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		for (int i=0;i<sgph.length ;i++ )
		{
			ls_sql="insert into jxc_gysddglb( gysddph,sgph) ";
			ls_sql+=" values ( ?,?) ";
			ps1= conn.prepareStatement(ls_sql);
			ps1.setString(1,gysddph);
			ps1.setString(2,sgph[i]);
			ps1.executeUpdate();
			ps1.close();
		}
		...
		ls_sql+=" from  jxc_clsgmx,jxc_cljgb";
		ls_sql+=" where jxc_cljgb.gysmc='"+gysmc+"'";
		ls_sql+=" and "+cf.arrayToInSQL(sgph,"jxc_clsgmx.sgph");
