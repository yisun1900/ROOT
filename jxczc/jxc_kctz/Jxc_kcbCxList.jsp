...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	String scph=request.getParameter("scph");
	if (scph!=null)
	{
		scph=cf.GB2Uni(scph);
		if (!(scph.equals("")))	wheresql+=" and  (jxc_kcb.scph='"+scph+"')";
	}
	String scrq=null;
	scrq=request.getParameter("scrq");
	if (scrq!=null)
	{
		scrq=scrq.trim();
		if (!(scrq.equals("")))	wheresql+="  and (jxc_kcb.scrq>=TO_DATE('"+scrq+"','YYYY-MM-DD'))";
	}
	scrq=request.getParameter("scrq2");
	if (scrq!=null)
	{
		scrq=scrq.trim();
		if (!(scrq.equals("")))	wheresql+="  and (jxc_kcb.scrq<=TO_DATE('"+scrq+"','YYYY-MM-DD'))";
	...
	ls_sql="SELECT jxc_kcb.lsh,DECODE(jxc_kcb.ztbz,'Y','在途','N','否'),hjmc,jxc_kcb.scph,jxc_kcb.clbm,jxc_kcb.nbbm,jxc_kcb.clmc,jxc_kcb.xh,jxc_kcb.kcsl,TO_CHAR(jxc_cljgb.zdkcsl),DECODE(SIGN(jxc_cljgb.zcpzsl-jxc_cljgb.zdkcsl),-1,'报警',0,'即将报警',1,'正常' ) kcbj,TO_CHAR(jxc_cljgb.xsj),jxc_kcb.kcsl*jxc_cljgb.xsj,TO_CHAR(jxc_cljgb.lsj),dqmc,ckmc,hwmc,jxc_kcb.scrq,DECODE(jxc_kcb.cllb,'0','代销品主材','1','辅材','2','库存品主材'),jxc_kcb.cldlbm,jxc_kcb.clxlbm,jxc_kcb.scsmc,jxc_kcb.ppmc,jxc_kcb.gysmc,jxc_kcb.cllbbm,jxc_kcb.gg,jxc_kcb.clysbm,jxc_kcb.jldwbm,jxc_kcb.pjts,jxc_kcb.cpjb,jxc_kcb.fgsx,jxc_kcb.cpsm,jxc_kcb.cd,jxc_kcb.cgzq,jxc_kcb.bz";
...
//	out.print(ls_sql);
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_kcbCxList.jsp","","","InsertJxc_hjtzmx.jsp?tzph="+tzph);
...
//设置主键
	...
	pageObj.setEditStr("调货架");


//设置行颜色
	Hashtable colorHash=new Hashtable();

	ColuParm colorParm=null;
	colorParm=new ColuParm();//生成一个列参数对象

	colorParm.link="报警";//为列参数：coluParm.link设置超级链接
	colorParm.bolt="#FF0000";//为列参数：coluParm.link设置超级链接
	colorHash.put("kcbj",colorParm);//列参数对象加入Hash表
	
	pageObj.setColor(colorHash);//列参数对象加入Hash表
