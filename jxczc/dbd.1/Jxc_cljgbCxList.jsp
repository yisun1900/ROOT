...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT clxlbm,jxc_cljgb.clbm,nbbm,clmc,xh,gg,jldwbm,jxc_cljgb.dgzjg,jxc_cljgb.xsj,DECODE(jxc_cljgb.xsfs,'1','代销品','2','现货',3,'期货调拨',4,'可负库存') xsfs,DECODE(jxc_cljgb.sfcy,'Y','常用','N','否') sfcy,bzgg,djzl,bzq,ppmc,DECODE(cglb,'J','集采','D','地方采'),jxc_clbm.bz ";
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_cljgbCxList.jsp?dbph="+dbph,"","","InsertJxc_clbm.jsp?dbph="+dbph+"&dqbm="+dqbm);
...
<CENTER >
  <B><font size="3">调拨单－选择材料（调拨单号：<%=dbph%>）</font></B>
</CENTER>
<%
	pageObj.out=out;
