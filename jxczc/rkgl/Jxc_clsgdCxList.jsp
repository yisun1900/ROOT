...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jxc_gysdd.gysddph,jxc_gysdd.gysmc,jxc_gysdd.xdrq,jxc_clsgd.sgph, DECODE(jxc_clsgd.sgdzt,'0','未提交','1','已提交','2','审核通过','5','部分出库','3','已出库','4','已入库','6','审核未通过'),a.dwmc fgs,jxc_clsgd.sgrq,DECODE(jxc_gysdd.sfrksgdw,'Y','直接入库申购公司','N','否'),jxc_clsgd.sgzsl,jxc_clsgd.sgzje,DECODE(jxc_clsgd.sglb,'0','主材','1','辅材'),jxc_clsgd.sgr, DECODE(jxc_clsgd.psfs,'1','配送','2','自取','9','其他'),jxc_clsgd.jhshsj,jxc_clsgd.shr,jxc_clsgd.shrq, DECODE(jxc_clsgd.sfzcgd,'Y','转为采购计划','N','未转','M','转为供应商订单'),jxc_clsgd.lrr,jxc_clsgd.lrsj,jxc_clsgd.bz  ";
	ls_sql+=" FROM jxc_clsgd,sq_dwxx a,jxc_gysdd  ";
    ls_sql+=" where jxc_clsgd.ssfgs=a.dwbh  ";
    ls_sql+=" and jxc_clsgd.sgph=jxc_gysdd.sgph";
    ls_sql+=" and jxc_gysdd.ddzt='1'";//7：未提交；0：已提交；1：已下单；2：已接受；3：部分发货；5：已完成发货；4：已入库；6：正在办理入库； 9：已结算
    ls_sql+=" and jxc_gysdd.sfrksgdw='Y' ";//是否直接入库申购公司
...
//设置列超级连接
	...
	coluParm=new ColuParm();//生成一个列参数对象
	String[] coluKey1={"gysddph"};//设置列参数：coluParm.key的主键
...
<CENTER >
  <B><font size="3">申购单－直接入库申购公司</font></B>
</CENTER>
<%
	pageObj.out=out;
