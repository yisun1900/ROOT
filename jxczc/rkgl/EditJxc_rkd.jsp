...
<div align="center">修改入库单（入库批号：<%=rkph%>）</div>
<div align="center">
...
<%
	cf.selectToken(out,"C+采购入库&T+退货入库&R+调拨入库&S+申购入库&J+代金券退货入库&D+主材订单退货&F+发货单入库&Z+直接入库&P+盘点入库&Q+其它入库",rklx,false);
%>
    </select>	  </td>
  <tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><input type="text" name="lydh" value="<%=lydh%>" size="58" maxlength="50" >
收货人签字单据号</td>
  </tr>
<tr bgcolor="#FFFFFF">
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><textarea name="bz" cols="72" rows="3" ><%=bz%></textarea></td>
    ...
    <tr align="center"> 
      <td colspan="4"> 
	    <p>
	      <input type="hidden" name="rkph" value="<%=rkph%>"  ><input type="button"  value="快速录入明细" onClick="f_pllr(insertform)" >
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveEditJxc_rkd.jsp";
	FormName.submit();
	return true;
}
