...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jxc_gysdd.gysddph, DECODE(jxc_gysdd.ddzt,'7','未提交','0','等待下单','1','已下单','2','已接受','3','部分发货','5','已完成发货','4','已入库','6','正在办理入库','9','已结算'),jxc_gysdd.sgph,a.dwmc xdfgs,jxc_gysdd.gysmc, DECODE(jxc_gysdd.cglb,'J','集团采购','D','地方采购'), DECODE(jxc_gysdd.cllb,'0','主材','1','辅材'),jxc_gysdd.dhzsl,jxc_gysdd.dhzje,jxc_gysdd.xdr,jxc_gysdd.xdrq, DECODE(jxc_gysdd.psfs,'1','配送','2','自取','3','其他'),jxc_gysdd.yqdhsj,jxc_gysdd.gysjsr,jxc_gysdd.gysjsrq,jxc_gysdd.lrr,jxc_gysdd.lrsj,b.dwmc lrbm,jxc_gysdd.bz  ";
	ls_sql+=" FROM jxc_gysdd,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where jxc_gysdd.ssfgs=a.dwbh and jxc_gysdd.lrbm=b.dwbh ";
    ls_sql+=" and jxc_gysdd.sfrksgdw='N' ";//是否直接入库申购公司
    ls_sql+=" and jxc_gysdd.cllb='0'";//1：辅材；0：库存品主材
    ls_sql+=wheresql;
    ls_sql+=" order by lrsj desc";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_gysddCxList.jsp","","","InsertJxc_rkdGys.jsp?rklx=C");
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="4%">下单日期</td>
	<td  width="3%">配送方式</td>
	<td  width="4%">要求到货时间</td>
	<td  width="3%">供应商接收人</td>
	<td  width="4%">供应商接收日期</td>
	<td  width="3%">录入人</td>
	<td  width="4%">录入时间</td>
	<td  width="6%">录入部门</td>
	<td  width="18%">备注</td>
