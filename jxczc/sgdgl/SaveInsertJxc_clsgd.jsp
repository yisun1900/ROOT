...
try {
	...
	sgph="ZS"+ssfgs+nian+cf.addZero(count+1,4);

	ls_sql="insert into jxc_clsgd ( sgph,psfs,jhshsj,sgr,sgrq,ssfgs,ssbm,bz,sglb,nian  ,lrr,lrsj,sgzsl,sgzje,sfsk,sfzcgd,sgdzt,sfrksgdw,sgdlx ) ";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?  ,?,SYSDATE,0,0,'N','N','0','N','1' ) ";//1：录入；2：主材订单
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,sgph);
	ps.setString(2,psfs);
	ps.setDate(3,jhshsj);
