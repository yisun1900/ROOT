...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	if (sgdlx!=null)
	{
		...
		if (!(sgdlx.equals("")))	wheresql+=" and  (jxc_clsgd.sgdlx='"+sgdlx+"')";
	}
	String ddbh=null;
	...
	{
		ddbh=cf.GB2Uni(ddbh);
		if (!(ddbh.equals("")))	wheresql+=" and  (jxc_clsgd.ddbh='"+ddbh+"')";
	}
	...
	ls_sql="SELECT jxc_clsgd.sgph, DECODE(jxc_clsgd.sgdzt,'0','未提交','1','已提交','2','审核通过','5','部分出库','3','已出库','4','已入库','6','审核未通过'), DECODE(jxc_clsgd.sfzcgd,'Y','转为采购计划','N','未转','M','转为供应商订单'),DECODE(jxc_clsgd.sglb,'0','主材','1','辅材'),a.dwmc fgs,jxc_clsgd.sgr,jxc_clsgd.sgrq,jxc_clsgd.sgzsl,jxc_clsgd.sgzje, DECODE(jxc_clsgd.psfs,'1','配送','2','自取','9','其他'),jxc_clsgd.jhshsj,jxc_clsgd.shr,jxc_clsgd.shrq,jxc_clsgd.shyj, DECODE(jxc_clsgd.sfsk,'Y','已收款','N','未收款'),jxc_clsgd.skqrr,jxc_clsgd.skqrsj,DECODE(jxc_clsgd.sgdlx,'1','录入','2','主材订单') sgdlx,jxc_clsgd.ddbh,jxc_clsgd.ckph,jxc_clsgd.rkph,jxc_clsgd.lrr,jxc_clsgd.lrsj,b.dwmc ssbm,jxc_clsgd.bz  ";
	ls_sql+=" FROM jxc_clsgd,sq_dwxx a,sq_dwxx b  ";
    ls_sql+=" where jxc_clsgd.ssfgs=a.dwbh and jxc_clsgd.ssbm=b.dwbh";
    ls_sql+=" and jxc_clsgd.sglb='0'";//0：主材；1：辅材
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_clsgdCxList.jsp","","/jxcdy/printzcsgd.jsp","");
...
//设置列超级连接
	...
	coluParm.link="ViewJxc_clsgd.jsp";//为列参数：coluParm.link设置超级链接
