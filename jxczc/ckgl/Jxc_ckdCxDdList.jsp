...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT jxc_ckd.ckph,DECODE(jxc_ckd.ckdzt,'0','未提交','1','等待报废审核','2','等待退货审核','3','等待调拨入库','4','已结算','5','退货审核通过','6','报废审核未通过','7','退货审核未通过','8','审核通过','9','提交'), DECODE(jxc_ckd.cklx,'D','订单出库','B','报废出库','T','退货出库','S','申购出库','P','材料单出库','J','辅材代金券出库','X','销售出库','R','调拨出库'),jxc_ckd.lydh ddbh,ckmc,jxc_ckd.khbh,crm_khxx.khxm,crm_khxx.fwdz,crm_khxx.hth,crm_khxx.sjs, jxc_ckd.ckzsl,jxc_ckd.ckzje, DECODE(jxc_ckd.ckfs,'1','先生产先出库','2','先入库先出库','3','手工出库'), DECODE(jxc_ckd.psfs,'0','厂家配送','1','公司送货','2','自取'),jxc_ckd.jhshsj,jxc_ckd.ckr,jxc_ckd.cksj, DECODE(jxc_ckd.sfyfkc,'1','无负库存','2','负库存未入库','3','负库存入库'),jxc_ckd.lrr,jxc_ckd.lrsj,b.dwmc lrbm,a.dwmc fgsbh,jxc_ckd.bz  ";
...
//	out.println(ls_sql);
//进行对象初始化
	pageObj.initPage("Jxc_ckdCxDdList.jsp","","","");
...
<CENTER >
  <B><font size="3">主材订单出库－查询</font></B>
</CENTER>
<%
	pageObj.out=out;
