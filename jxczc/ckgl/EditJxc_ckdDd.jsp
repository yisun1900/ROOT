...
<tr bgcolor="#FFFFFF">
  ...
  <td><select name="cklx" style="FONT-SIZE:12PX;WIDTH:152PX">
    <%
	cf.selectToken(out,"D+主材订单出库&B+报废出库&T+退货出库&S+申购出库&P+材料单出库&J+辅材代金券出库&X+销售出库&R+调拨出库",cklx,false);
%>
  </select></td>
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><textarea name="bz" cols="71" rows="3"><%=bz%></textarea></td>
	...
	<div align="center">
	  ...
	  <input type="button" onClick="f_sgd(editform)"  value="订单转申购单">
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="SaveEditJxc_ckdDd.jsp";
	FormName.submit();
	return true;
}
...
function f_lrmx(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="InsertJxc_ckmxDd.jsp";
	FormName.submit();
	return true;
}
...
function f_xgmx(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="EditAllJxc_ckmxDd.jsp";
	FormName.submit();
	return true;
}
...
function f_dyckd(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="/jxcdy/printzcckd.jsp";
	FormName.submit();
	return true;
}
