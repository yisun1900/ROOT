...
try {
	...
	//0：等待提交；1：等待审核；2：等待出库；3：已出库；5：部分出库；6：已审核
	if (!psdzt.equals("2") && !psdzt.equals("5") && !(psdzt.equals("6") && shjl.equals("Y"))  )
	{
		out.print("错误！配送单["+lydh+"]状态不正确，应为【2：等待出库 或 5：部分出库；6：已审核】！");
		return;
	}
	...
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,SYSDATE,?,?,?,0,0,0,'0','N','N','1',?,? ) ";//1：无负库存；2：负库存未入库；3：负库存入库
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,ckph);
	ps.setString(2,dqbm);
