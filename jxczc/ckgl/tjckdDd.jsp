...
try {
	...
	String ckjxz="";//出库单价选择,1：取价格表（jxc_cljgb.dgzjg）；2：取入库时销售价（jxc_kcb.dgzjg）；3：取主材订单价格(jc_zcddmx.dj)
	String cklx="";//D：订单出库；B：报废出库；T：退货出库； S：申购出库；P：材料单出库；J：辅材代金券出库；X：销售出库；R:调拨出库
	String ckr=null;
	java.sql.Date cksj=null;
	ls_sql=" select ckdzt,dqbm,lydh,ckfs,ckjxz,cklx,khbh,ckbh,ckr,cksj ";
	ls_sql+=" from jxc_ckd ";
	...
	ls_sql=" select clbm,zljb,ckbh,hjbh,hwbh,scph,ckdj,sum(cksl) cksl ";
	ls_sql+=" from jxc_ckmx ";
	ls_sql+=" where ckph='"+ckph+"' ";
	ls_sql+=" group by clbm,zljb,ckbh,hjbh,hwbh,scph,ckdj ";
	ps= conn.prepareStatement(ls_sql);
	rs=ps.executeQuery();
	while (rs.next())
	{
		...
		if(!hwbh.equals(""))
		{
			...
			if (!hjbh.equals(""))
			{
				ls_sql +=" and jxc_kcb.hjbh='"+hjbh+"'";
			}
			if (!hwbh.equals(""))
			{
				ls_sql +=" and jxc_kcb.hwbh='"+hwbh+"'";
			}
			if (!scph.equals(""))
			{
				ls_sql +=" and jxc_kcb.scph='"+scph+"'";
			}
	...
	ls_sql=" update jc_zcdd set ycksl="+ycksl+",yckje="+yckje+",ckclzt='"+ckclzt+"',ckr=?,cksj=?";
	ls_sql+=" where ddbh='"+lydh+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,ckr);
	ps.setDate(2,cksj);
