...
try {
	...
	for (i=0;i<xuhao.length ;i++ )
	{
		...
		ls_sql="insert into cl_jgmx ( dwbh,clbm,xjgsfgx,xjgsxsj,xcj,xcjjsj,xfgsdj,xgdj,sfrk,ckmc  ,xsfycx,xcxkssj,xcxjzsj,xcxcj,xcxcjjsj,xcxfgsdj,xcxgdj  ,tzr,tzsj,cj,cjjsj,fgsdj,gdj,sfycx,cxcj,cxcjjsj,cxfgsdj,cxgdj ) ";
		ls_sql+=" values ( ?,?,'1',?,?,?,?,?,?,?  ,?,?,?,?,?,?,?   ,?,SYSDATE,0,0,0,0,'N',0,0,0,0) ";
		ps= conn.prepareStatement(ls_sql);
		ps.setString(1,dwbh);
		ps.setString(2,clbm[i]);
		ps.setDate(3,xjgsxsj);
		ps.setDouble(4,xcj);
		ps.setDouble(5,xcjjsj);
		ps.setDouble(6,xfgsdj);
		ps.setDouble(7,xgdj);
		ps.setString(8,sfrk[i]); 
		ps.setString(9,cf.GB2Uni(ckmc[i]));

		ps.setString(10,sfycx[i]);
		ps.setDate(11,cxkssj);
		ps.setDate(12,cxjzsj);
		ps.setDouble(13,cxcj);
		ps.setDouble(14,cxcjjsj);
		ps.setDouble(15,cxfgsdj);
		ps.setDouble(16,cxgdj);
