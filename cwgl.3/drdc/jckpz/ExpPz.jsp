...
<%
	...
	if (pzfl.equals("1"))//收工程款（现金）
	{
		...
		if (czlx.equals("1"))//没有导出
		{
			...
			wheresql+=" and cw_khfkjl.fklxbm in(select fklxbm from cw_fklxbm where pzfl='2')";
	...
	else if (pzfl.equals("2"))//收集成款（非现金）
	{
		//工程款,1：工程款；2：集成款 
		if (czlx.equals("1"))//没有导出
		{
	...
	else if (pzfl.equals("3"))//转入、转出集成款
	{
		//集成款,1：工程款；2：集成款 + 转出集成款
		if (czlx.equals("1"))//没有导出
		{
			wheresql+=" and (";
			wheresql+="           ( cw_khfkjl.zffs in('31') ";//转入集成款
			wheresql+="             and (cw_khfkjl.czlx='1' OR (cw_khfkjl.czlx='2' and cw_khfkjl.zrkdcpz='0'))";
			wheresql+="             and  cw_khfkjl.fklxbm in( select fklxbm from cw_fklxbm where pzfl='2') )";
			wheresql+="       OR  ( cw_khfkjl.zffs in('31')";//转出集成款
			wheresql+="             and (cw_khfkjl.czlx='1' OR (cw_khfkjl.czlx='2' and cw_khfkjl.zckdcpz='0'))";
			wheresql+="             and cw_khfkjl.zckx   in( select fklxbm from cw_fklxbm where pzfl='2') )";
			wheresql+="     )";
		}
		else if (czlx.equals("2"))//已导出
		{
...
try {
	...
	while(rs1.next())
	{
		...
		ls_sql=" select count(*)" ;
		ls_sql+=" from sq_yhxx " ;
		ls_sql+=" where dwbh='"+getdwbh+"' and yhmc='"+sjs+"' and zwbm='04' and sfzszg in('Y','N')";
		ps2= conn1.prepareStatement(ls_sql);
		rs2=ps2.executeQuery();
		if(rs2.next())
		{         
		...
		if (sjs.equals(""))//多个
		{
			out.println("<BR>提醒！收款序号<A HREF='/cwgl/qyecx/ViewCw_khfkjl.jsp?fkxh="+fkxh+"' target='_blank'>["+fkxh+"]</A>设计师为空");
			sjs="未知";
		}
		else if (count>1)//多个
		{
			conn1.rollback();
			out.println("<BR>错误！在["+dwmc+"]店，?谥吧杓剖?<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>有多个");
			return;
		}
		else if (count==1)//一个
		{
			...
			{
				conn1.rollback();
				out.println("<BR>错误！在["+dwmc+"]店，离职设计师<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>有多个");
				return;
			}
			else if (count==1)//一个
			{
				...
				out.println("<BR>提醒！在["+dwmc+"]店，设计师<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>已离职");
			}
			else{//无离职设计师
				...
				{
					conn1.rollback();
					out.println("<BR>错误！设计师<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>有多个");
					return;
				}
				if (count==1)//一个
				{
					String lsdwmc=null;
					ls_sql=" select bianhao,dwmc" ;
					...
					{         
						bianhao = rs2.getString("bianhao");			    
						lsdwmc = rs2.getString("dwmc");			    
					}
					rs2.close();
					ps2.close();

					out.println("<BR>提醒！设计师<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>已异动到["+lsdwmc+"]");
				}
				else{
//					conn1.rollback();
					bianhao="未知";
					out.println("<BR>提醒！无此设计师<A HREF='/cwgl/qyecx/ViewKhxx.jsp?fkxh="+fkxh+"' target='_blank'>["+sjs+"]</A>");
//					return;
				}
			}
		}
...
//			return;
		...
		if (drpzfl.equals("2"))//1：工程款；2：集成款
		{
			allsk+=fkje;

			if (getfklxbm.equals("21"))//21：木门款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getgysbh+"' and fklxbm='"+getfklxbm+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
			...
			else if (getfklxbm.equals("22"))//22：橱柜款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getgysbh+"' and fklxbm='"+getfklxbm+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				...
				{         
					...
					out.println("<BR>提醒！付款序号<A HREF='/cwgl/qyecx/ViewCw_khfkjl.jsp?fkxh="+fkxh+"' target='_blank'>["+fkxh+"]</A>家居设计师不存在");
					fobjname4="未知";
				}
				rs1.close();
				ps1.close();
				...
				ls_sql=" select bianhao" ;
				ls_sql+=" from sq_yhxx " ;
				ls_sql+=" where ssfgs='"+fgsbh+"' and yhmc='"+fobjname4+"' and zwbm='12' and sfzszg in('Y','N')";
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
					fobjid4 = rs2.getString("bianhao");			    
				}
				else{
					out.println("<BR>提醒！付款序号<A HREF='/cwgl/qyecx/ViewCw_khfkjl.jsp?fkxh="+fkxh+"' target='_blank'>["+fkxh+"]</A>家居设计师工号不存在");
					fobjid4="未知";
				}
				rs2.close();
				ps2.close();
			...
			else if (getfklxbm.equals("23"))//23：主材款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getgysbh+"' and fklxbm='"+getfklxbm+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				...
				ls_sql=" select clgw" ;
				ls_sql+=" from jc_zcdd " ;
				ls_sql+=" where khbh='"+khbh+"' and ppbm='"+getgysbh+"'";
				ps1= conn1.prepareStatement(ls_sql);
				rs1=ps1.executeQuery();
				if(rs1.next())
				{         
					fobjname4 = rs1.getString("clgw");			    
				}
				else{
					out.println("<BR>提醒！付款序号<A HREF='/cwgl/qyecx/ViewCw_khfkjl.jsp?fkxh="+fkxh+"' target='_blank'>["+fkxh+"]</A>材料顾问不存在");
					fobjname4="未知";
				}
				rs1.close();
				ps1.close();
				...
				ls_sql=" select bianhao" ;
				ls_sql+=" from sq_yhxx " ;
				ls_sql+=" where ssfgs='"+fgsbh+"' and yhmc='"+fobjname4+"' and zwbm='08' and sfzszg in('Y','N')";
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
					...
					out.println("<BR>提醒！付款序号<A HREF='/cwgl/qyecx/ViewCw_khfkjl.jsp?fkxh="+fkxh+"' target='_blank'>["+fkxh+"]</A>材料顾问工号不存在");
					fobjid4="未知";
				}
				rs2.close();
				ps2.close();
			}
			else if (getfklxbm.equals("24"))//24：家具款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getgysbh+"' and fklxbm='"+getfklxbm+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
			...
			else if (getfklxbm.equals("62"))//62：集成预付款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and fklxbm='"+getfklxbm+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
				...
				conn1.rollback();
				out.println("<BR>错误！收款类型编码：["+getfklxbm+"]不属于集成款");
				return;
			}
		...
		//导出[转出款凭证]，收款金额为负值＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋开始
		if (getzffs.equals("31") && dcpzfl.equals("2"))//31：转出,//1：工程款；2：集成款
		{
			allsk-=fkje;

			if (getzckx.equals("21"))//21：木门款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getzcgysbh+"' and fklxbm='"+getzckx+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
			...
			else if (getzckx.equals("22"))//22：橱柜款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getzcgysbh+"' and fklxbm='"+getzckx+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
			...
			else if (getzckx.equals("23"))//23：主材款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getzcgysbh+"' and fklxbm='"+getzckx+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				...
				ls_sql=" select clgw" ;
				ls_sql+=" from jc_zcdd " ;
				ls_sql+=" where khbh='"+khbh+"' and ppbm='"+getzcgysbh+"'";
				ps1= conn1.prepareStatement(ls_sql);
				rs1=ps1.executeQuery();
				if(rs1.next())
				{         
			...
			else if (getzckx.equals("24"))//24：家具款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and gysbh='"+getzcgysbh+"' and fklxbm='"+getzckx+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
			...
			else if (getzckx.equals("62"))//62：集成预付款
			{
				ls_sql=" select kmdm,kmmc" ;
				ls_sql+=" from cw_jckmdm " ;
				ls_sql+=" where fgsbh='"+fgsbh+"' and fklxbm='"+getzckx+"' and jdbz='"+fdc+"'";//1：借（退款）；0：贷（收款）
				ps2= conn1.prepareStatement(ls_sql);
				rs2=ps2.executeQuery();
				if(rs2.next())
				{         
				...
				conn1.rollback();
				out.println("<BR>错误！收款类型编码：["+getzckx+"]不属于集成款");
				return;
			}
