...
Hashtable<String,Double> jeHz=new Hashtable<String,Double>();//存放店面实收款汇总
...
try {
	...
	for (int i=0;i<hth.length ;i++ )
	{
		...
		if (jestr[i]!=null && !jestr[i].trim().equals(""))
		{
			try{
				...
				conn.rollback();
				out.print("序号["+i+"]出错！[金额]非数字：" + jestr[i]);
				return;
			}
		}
		...
		{         
			...
			conn.rollback();
			out.println("<BR>合同号["+hth[i]+"]错误！店面编码["+dwbh+"]不存在");
			return;
		}
		rs2.close();
		ps2.close();
		...
		{
			conn.rollback();
			out.println("<BR>合同号["+hth[i]+"]错误！店面["+dwmc+"]的财务代码不存在");
			return;
		}
		//取店面财务代码、名称＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝结束
		...
		{         
			...
			{         
				bianhao = rs1.getString("bianhao");			    
				out.println("<BR>提醒！合同号["+hth[i]+"]，设计师["+sjs+"]在店面["+dwmc+"]不存在，已转店");
			}
			else{
				conn.rollback();
				out.println("<BR>合同号["+hth[i]+"]错误！设计师["+sjs+"]不存在");
				return;
			}
			rs1.close();
			ps1.close();
		...
		//计算各店面的汇总＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋开始
		double lsHz=0;
		if ((Double)jeHz.get(dwbh)==null)//不存在，为第一个
		{
			lsHz=je;
		}
		else{
			lsHz=(Double)jeHz.get(dwbh).doubleValue()+je;//先取后加
		}
	...
	while (dwbmList.hasMoreElements() )
	{
		...
		dmje=(Double)jeHz.get(dwbh).doubleValue();//获得列类型
