...
double kbjf=0;
ls=request.getParameter("kbjf");
try{
	if (!(ls.equals("")))  kbjf=Double.parseDouble(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量[kbjf]不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[扣保洁费]类型转换发生意外:"+e);
	return;
}
double kywx=0;
ls=request.getParameter("kywx");
try{
	if (!(ls.equals("")))  kywx=Double.parseDouble(ls.trim());
}
catch (java.lang.NullPointerException nulle){
	out.println("<BR>变量[kywx]不存在");
	return;
}
catch (Exception e){
	out.println("<BR>[扣意外险]类型转换发生意外:"+e);
	return;
}
...
try {
	...
	ls_sql="update rs_gzmxb set jbgz=?,gwgz=?,glgz=?,jtbt=?,dhbt=?,cb=?,buzhu=?,dxsf=?,khgz=?,jxfz=?,jxgz=?,khyj=?,bqyjze=?,cqts=?,sjts=?,bjts=?,kgts=?,sjkk=?,bjkk=?,cjkk=?,kgkk=?,qqkk=?,jbf=?,kbx=?,jf=?,kzbj=?,qtkk=?,yfgz=?,ksk=?,sfgz=?,lrr=?,lrsj=?,bz=?,khzq=?,jxgzjs=?,yjkhkk=?,cjts=?,jbts=?,dixin=?,sqyj=?,kbjf=?,kywx=? ";
	...
	ps.setDouble(41,kbjf);
	ps.setDouble(42,kywx);
