...
<%
	...
	if (bjtyMark.compareTo("0")>0)//已停用，从表[bj_jzbjb1]取数据而非表[bj_jzbjb]
	{
		...
		ls_sql+=" and bj_jzbjb1.xmbh  in(select xmbh from bj_bjxmmx where khbh='"+khbh+"')";
		ls_sql+=wheresql;
		...
		ls_sql+=" and bj_jzbjb.xmbh  in(select xmbh from bj_bjxmmx where khbh='"+khbh+"')";
		ls_sql+=wheresql;
...
<%
	...
	if (bjtyMark.compareTo("0")>0)//已停用，从表[bj_jzbjb1]取数据而非表[bj_jzbjb]
	{
		...
		ls_sql+=" and bj_jzbjb1.dqbm='"+dqbm+"' and bj_jzbjb1.bjjbbm='"+bjjb+"' and bj_jzbjb1.bjbbh='"+bjbbh+"'";
		ls_sql+=" and bj_jzbjb1.sfbxx!='9'";//1：必报项；2：必列项；3：可选项；9：作废项
		ls_sql+=" and bj_jzbjb1.xmbh not in(select xmbh from bj_bjxmmx where khbh='"+khbh+"')";
		ls_sql+=wheresql;
		ls_sql+=" order by bj_jzbjb1.dqbm,bj_jzbjb1.xh,bj_jzbjb1.xmbh";
	}
	else{
