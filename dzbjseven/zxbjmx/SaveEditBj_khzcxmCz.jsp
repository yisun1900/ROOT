...
try {
	...
	ls_sql="update tc_csrjkhczjmj set sjsl="+sjsl;
	ls_sql+=" where khbh='"+khbh+"' and tccplbbm='"+tccplbbm+"' and jgwzbm='"+jgwzbm+"'";
	ps= conn.prepareStatement(ls_sql);
	ps.executeUpdate();
	ps.close();
	...
	if (tccplbbm.equals("21"))//21 墙砖
	{
		if (sjsl>cf.round(fjmj*3.5,2))
		{
			conn.rollback();
			out.println("错误！墙砖不能超过面积的3.5倍，实际数量["+sjsl+"]不能超过最大数量["+cf.round(fjmj*3.5,2)+"]");
			return;
		}
	}
	else if (tccplbbm.equals("22"))//22 地砖 
	{
		if (sjsl>cf.round(fjmj*1.1,2))
		{
			conn.rollback();
			out.println("错误！地砖不能超过面积的1.1倍，实际数量["+sjsl+"]不能超过最大数量["+cf.round(fjmj*1.1,2)+"]");
			return;
		}
	}
