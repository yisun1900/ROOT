...
try {
	...
	ls_sql="select khbh,sfxmbm,DECODE(sflx,'1','工程直接费百分比（不可改）','A','工程直接费百分比（可改）','2','固定金额（可改）','3','税金（折后按基数计算）','8','税金（折后按折扣计算）','4','固定金额（不可改）','5','管理费','6','公司购主材百分比','7','自购主材百分比','9','按百分比收设计费','B','精装工程管理费','C','精装主材管理费','D','精装工程费百分比','E','精装主材费百分比') sflx,dj,sfbfb,jldw,sl,zqsfje,dxzkl,sfje,bz ";
            ...
            <tr bgcolor="#FFFFFF"> 
              ...
              <td colspan="3"> 
                <select name="sfxmbm" style="FONT-SIZE:12PX;WIDTH:522PX" onChange="getSfxm(insertform)">
                  <%
	cf.selectItem(out,"select sfxmbm,sfxmmc||DECODE(sfbxx,'1','（必选）','2','') from bdm_sfxmbm where sfjzsf='N' and dqbm='"+dqbm+"' and (sfxsq='1' OR (sfxsq='2' and sfxmbm in(select sfxmbm from sq_sfxmsq where ygbh='"+ygbh+"'))) order by sfxmbm",sfxmbm);
%> 
                </select>              </td>
            </tr>
            <tr bgcolor="#FFFFFF">
