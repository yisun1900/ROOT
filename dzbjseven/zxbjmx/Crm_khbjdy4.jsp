...
/*	
	...
	else if (sfkdybj.equals("Y"))//Y：需授权打??
	{
...
%> 
  ...
  <tr bgcolor="#FFFFFF" align="center"  height="25"> 
	...
	<td colspan="4">折前：<%=cf.formatDouble(allzqqye,"#########.##")%> &nbsp;&nbsp;&nbsp;&nbsp;折后：<%=cf.formatDouble(allqye,"#########.##")%>元</td>
	<td>&nbsp;</td>
  </tr>
  <%  
	double allsfje1=0;
...
<%
	...
	String sjsl=""; 
	String zsxxbz=""; 
	ls_sql="SELECT zsxm,jldw,sjsl,bz";
	...
	while (rs.next())
	{
		...
		zsxxbz=cf.fillNull(rs.getString("bz"));

		khzsxx+=zsxm+"："+sjsl+jldw+"("+zsxxbz+")、";
	}
	rs.close();
	ps.close();
