...
<div align="center">录入客户主材项目－－橱柜（独立收费）</div>
...
//	String ppmc=null;
	...
	while (rs1.next())
	{
		...
		if (rs.next())
		{
			...
			<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			  <td rowspan="<%=count%>">
				<A HREF="SaveInsertBj_khzcxmCgCl.jsp?khbh=<%=khbh%>&tccplbbm=<%=tccplbbm%>&tccpdlbm=<%=tccpdlbm%>&jgwzbm=<%=jgwzbm%>&bzmc=<%=bzmc%>">选择</A>
			  </td>
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
		  <td>
			<A  HREF="SaveInsertBj_khzcxmCgCl.jsp?khbh=<%=khbh%>&tccplbbm=<%=tccplbbm%>&tccpdlbm=<%=tccpdlbm%>&jgwzbm=<%=jgwzbm%>&cpbm=<%=cpbm%>">选择</A>
		  </td>
		  <td>&nbsp;</td>
		  <td><%=nbbm%></td>
		  <td><%=ppmc%></td>
