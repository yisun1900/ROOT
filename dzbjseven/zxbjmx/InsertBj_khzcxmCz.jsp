...
<div align="center">录入客户主材项目－－（<%=jgwzbm%>）磁砖</div>

<table border="1" width="150%" cellspacing="0" cellpadding="1"  bgcolor="#000000" style='FONT-SIZE: 12px'>
<tr bgcolor="#CCCCCC"  align="center">
	<td  width="4%">&nbsp;</td>
	<td  width="2%">序号</td>
	<td  width="3%"><span class="STYLE2">计量单位</span></td>
	<td  width="3%"><span class="STYLE2">数量</span></td>
	<td  width="3%"><span class="STYLE2">下单计量单位</span></td>
	<td  width="3%"><span class="STYLE2">下单数量</span></td>
	<td  width="4%"><span class="STYLE2">下单单价</span></td>
	<td  width="5%"><span class="STYLE2">结构位置</span></td>
	<td  width="7%"><span class="STYLE2">配套说明</span></td>
	<td  width="4%">类型</td>
	<td  width="6%">编组名称</td>
	<td  width="9%">产品名称</td>
	<td  width="6%">品牌</td>
...
<%
	...
	ls_sql="SELECT tc_csrjkhzcqd.xdsl,DECODE(tc_csrjkhzcqd.lx,'1','标配','2','升级','3','独立收费') lx,tc_csrjkhzcqd.bz,tc_csrjkhzcqd.dj,tc_csrjkhzcqd.sxh,bjjbmc,tc_csrjkhzcqd.sl,tc_csrjkhzcqd.jgwzbm,tc_csrjkhzcqd.ptcpsm,tc_csrjkhzcqd.bzmc,tc_csrjkhzcqd.cpbm,tc_csrjkhzcqd.cpmc,tc_csrjkhzcqd.ppmc,tc_csrjkhzcqd.xh,tc_csrjkhzcqd.gg,tc_csrjkhzcqd.jldw,tc_csrjkhzcqd.xdjldw,clxlmc ";
    ...
    ls_sql+=" and tc_csrjkhzcqd.khbh='"+khbh+"' and tc_csrjkhzcqd.tccplbbm='"+tccplbbm+"' and tc_csrjkhzcqd.jgwzbm='"+jgwzbm+"'";
	...
	while (rs.next())
	{
		...
		xdsl=cf.fillNull(rs.getString("xdsl"));
		...
		xdjldw=cf.fillHtml(rs.getString("xdjldw"));
		...
		<tr bgcolor="#FFFFFF" align='center'>
		  <td>
			<A HREF="DeleteBj_khzcxmCz.jsp?khbh=<%=khbh%>&sxh=<%=sxh%>" target="_blank">删除</A>
			<A HREF="EditBj_khzcxmCz.jsp?khbh=<%=khbh%>&sxh=<%=sxh%>" target="_blank">修改</A>
		  </td>
		  <td><%=sxh%></td>
		  <td><%=jldw%></td>
		  <td><%=sl%></td>
...
//	String ppmc=null;
	...
	while (rs1.next())
	{
		...
		ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,xdjldw,ppmc,tc_csrjzck.bjjbbm,bjjbmc,bz,zp";
		...
		if (rs.next())
		{
			...
			if (zcbjjbbm.equals(bjjbbm))
			{
				...
				if (sfdz.equals("2") && dzkssj.compareTo(cf.today())<=0 && dzjzsj.compareTo(cf.today())>=0)//1：无折扣；2：打折
				{
					if (dzsjsfj!=0)
					{
						sjsfjStr=dzsjsfj+"/"+xdjldw+"<BR>原价"+sjsfj;
					}
				}
				else{
					if (sjsfj!=0)
					{
						sjsfjStr=sjsfj+"/"+xdjldw;
					}
				}
			}
			...
			<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			  <td rowspan="<%=count%>">
				<A HREF="SaveInsertBj_khzcxmCz.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&bzmc=<%=bzmc%>&jgwzbm=<%=jgwzbm%>">选择</A>
			  </td>
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
		  <td>
			<A  HREF="SaveInsertBj_khzcxmCz.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&cpbm=<%=cpbm%>&jgwzbm=<%=jgwzbm%>">选择</A>
		  </td>
		  <td>&nbsp;</td>
		  <td><%=nbbm%></td>
		  <td><%=ppmc%></td>
