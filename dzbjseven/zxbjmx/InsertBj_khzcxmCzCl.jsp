...
<div align="center">录入客户主材项目－－（<%=jgwzbm%>）磁砖（独立收费）</div>
...
//	String ppmc=null;
	...
	while (rs1.next())
	{
		...
		ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,xdjldw,ppmc,tc_csrjzck.bjjbbm,bjjbmc,bz,zp,khxsdj";
		...
		if (rs.next())
		{
			...
			<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			  <td rowspan="<%=count%>">
				<A HREF="SaveInsertBj_khzcxmCzCl.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&bzmc=<%=bzmc%>&jgwzbm=<%=jgwzbm%>">选择</A>
			  </td>
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
		  <td>
			<A  HREF="SaveInsertBj_khzcxmCzCl.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&cpbm=<%=cpbm%>&jgwzbm=<%=jgwzbm%>">选择</A>
		  </td>
		  <td>&nbsp;</td>
		  <td><%=nbbm%></td>
		  <td><%=ppmc%></td>
