...
try {
	...
	ls_sql="select bzsl,sjjmsl,sjsl";
...
<div align="center">录入客户主材项目－－洁具</div>
...
<%
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF" align='center'>
		  <td>
			<A HREF="DeleteBj_khzcxmJj.jsp?khbh=<%=khbh%>&sxh=<%=sxh%>" target="_blank">删除</A>
		  </td>
		  <td><%=sxh%></td>
		  <td><%=sl%></td>
		  <td><%=dj%></td>
...
//	String ppmc=null;
	...
	while (rs1.next())
	{
		...
		ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,ppmc,tc_csrjzck.bjjbbm,bjjbmc,bz,zp";
		...
		if (rs.next())
		{
			...
			<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			  <td rowspan="<%=count%>">
				<A HREF="SaveInsertBj_khzcxmJj.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&jgwzbm=<%=jgwzbm%>&bzmc=<%=bzmc%>">选择</A>
			  </td>
	...
	ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,ppmc,tc_csrjzck.bjjbbm,bz,bjjbmc,zp";
	ls_sql+=" FROM tc_csrjzck,bdm_bjjbbm";
	ls_sql+=" where tc_csrjzck.bjjbbm=bdm_bjjbbm.bjjbbm(+) ";
	ls_sql+=" and tccplbbm='"+tccplbbm+"' and  dqbm='"+dqbm+"'";
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
		  <td>
			<A  HREF="SaveInsertBj_khzcxmJj.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&jgwzbm=<%=jgwzbm%>&cpbm=<%=cpbm%>">选择</A>
		  </td>
		  <td>&nbsp;</td>
		  <td><%=nbbm%></td>
		  <td><%=ppmc%></td>
