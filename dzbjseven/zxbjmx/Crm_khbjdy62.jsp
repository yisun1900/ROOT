...
<%
	...
	//	out.println(ls_sql);
		...
		//输出项目明细
		ls_sql="SELECT bj_bjxmmx.xh xh,trim(bj_bjxmmx.xmbh)||DECODE(bjlx,'1','','2','增') xmbh,xmmc,jldwmc,bj_bjxmmx.dj,bj_gclmx.sl,ROUND(bj_gclmx.sl*bj_bjxmmx.zqdj,2) as je,gycl,bj_bjxmmx.flmcgg,bj_bjxmmx.bz,bj_bjxmmx.zcf,bj_bjxmmx.cldj,bj_bjxmmx.shf,bj_bjxmmx.qtf,bj_bjxmmx.ysf,bj_bjxmmx.rgdj,bj_bjxmmx.jxdj";
		...
		while (rs.next())
		{
			...
			%> 
			  <tr bgcolor="#FFFFFF"> 
				...
				<td width="6%" align="center"> <%=qye%> </td>
...
%> 
  ...
  <tr bgcolor="#FFFFFF" align="center"  height="25"> 
	...
	<td colspan="4" align="left">折前：<%=cf.formatDouble(allzqqye,"#########.##")%>元</td>
	<td colspan="5" align="left">折后：<%=cf.formatDouble(allqye,"#########.##")%>元</td>
	<td>&nbsp;</td>
  </tr>
  <%  
	double allsfje1=0;
    ...
    ls_sql+=" where bj_sfxmmx.khbh='"+khbh+"' and  sfxmlx='5'";//5：工程管理费
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		%> 
		  <tr bgcolor="#FFFFFF" align="center"  height="31"> 
			...
			<td colspan="4" align="left">折前：<%=cf.formatDouble(zqsfje1,"#########.##")%>元</td>
			<td colspan="5" align="left">折后：<%=cf.formatDouble(sfje1,"#########.##")%>元</td>
			<td>&nbsp;</td>
		 </tr>
 		<%
	}
    ...
    <tr bgcolor="#CCCCCC" align="center"> 
		...
		<td>单位</td>
		<td>数量</td>
		<td colspan="3" >折前价格</td>
		<td colspan="3" >折后价格</td>
		<td>&nbsp;</td>
		<td>备注</td>
    </tr>
  <%  
	...
	ls_sql+=" FROM bj_sfxmmx  ";
    ls_sql+=" where bj_sfxmmx.khbh='"+khbh+"' and  sfxmlx not in('5','6')";
    ls_sql+=" order by bj_sfxmmx.sfxmbm";
//	out.println(ls_sql);
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	while (rs.next())
	{
		...
		%> 
		  ...
		  <tr bgcolor="#FFFFFF" align="center"  height="31"> 
			...
			<td ><%=jldwmc%></td>
			<td ><%=qtfsl%></td>
			<td colspan="3"><%=cf.formatDouble(zqsfje,"#########.##")%>元</td>	
			<td colspan="3"><%=cf.formatDouble(sfje,"#########.##")%>元</td>	
			<td>&nbsp;</td>
			<td align="left"  ><%=bz%></td>
		  </tr>
	...
	<td colspan="2">小计：</td>
	<td colspan="2">&nbsp;</td>
	<td colspan="3"><%=cf.formatDouble(allzqsfje,"#########.##")%>元</td>	
...
<%  
    ...
    ls_sql="select count(*)";
	ls_sql+=" from  bj_sfxmmx";
	ls_sql+=" where khbh='"+khbh+"'and sfxmlx='6' ";//6：工程税金
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery(ls_sql);
	if (rs.next())
	{
		...
		while (rs.next())
		{
			...
			%> 
			  <tr bgcolor="#FFFFFF" align="center"  height="31"> 
				...
				<td >项</td>
				<td >1</td>
				<td colspan="3" > <%=cf.formatDouble(zqsfje2,"#########.##")%>元</td>
				<td colspan="3" > <%=cf.formatDouble(sfje2,"#########.##")%>元</td>
				<td>&nbsp;</td>
