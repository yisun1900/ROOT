...
try {
	...
	if (!cpbm.equals(""))
	{
		...
		if (rs.next())
		{
			...
			ls_sql+=" select              ?+rownum,?   ,cpbm,nbbm,tccpdlbm,tccplbbm,bzmc,cpmc,xh,gg,ppmc,gysmc,jldw,xdjldw,xdb,sh,sfxclxs,jsj,sgddj,khxsdj,? ,sfyx,yxkssj,yxjzsj,bz,bjjbbm,?     ,0 ,0   ,''    ,'1' ,''";//1：木门；2：橱柜；3：洁具；4：磁砖；5：铝扣板；6：地板
			ls_sql+=" FROM tc_csrjzck";
	...
	<!--
		window.location="InsertBj_khzcxmCgCl.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&jgwzbm=<%=jgwzbm%>";
	//-->
	</SCRIPT>
	<%
}
