...
try {
	...
	if (!cpbm.equals(""))
	{
		...
		if (rs.next())
		{
			...
			ls_sql+=" select                     ?,?   ,cpbm,nbbm,tccpdlbm,tccplbbm,bzmc,cpmc,xh,gg,ppmc,gysmc,jldw,xdjldw,xdb,sh,sfxclxs,jsj,sgddj,? ,? ,sfyx,yxkssj,yxjzsj,bz,bjjbbm,?     ,0 ,0   ,''    ,'1' ,''";//1：木门；2：橱柜；3：洁具；4：铝扣板；5：铝扣板；6：地板
			ls_sql+=" FROM tc_csrjzck";
	...
	<!--
		window.location="InsertBj_khzcxmLkb.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&jgwzbm=<%=jgwzbm%>";
	//-->
	</SCRIPT>
	<%
}
