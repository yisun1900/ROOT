...
<div align="center">录入客户主材－－五金电器（独立收费）</div>
...
<%
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF" align='center'>
		  <td>
			<A HREF="DeleteBj_khzcxmWj.jsp?khbh=<%=khbh%>&sxh=<%=sxh%>" target="_blank">删除</A>
			<A HREF="EditBj_khzcxmWj.jsp?khbh=<%=khbh%>&sxh=<%=sxh%>" target="_blank">修改</A>
		  </td>
		  <td><%=sxh%></td>
		  <td><%=sl%></td>
		  <td><%=dj%></td>
...
//	String ppmc=null;
	...
	while (rs1.next())
	{
		...
		ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,ppmc,tc_csrjzck.bjjbbm,bjjbmc,bz,tc_csrjzck.khxsdj,zp";
		...
		if (rs.next())
		{
			...
			<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
			  <td rowspan="<%=count%>">
				<A HREF="SaveInsertBj_khzcxmWjCl.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&bzmc=<%=bzmc%>">选择</A>
			  </td>
	...
	ls_sql="SELECT cpmc,cpbm,nbbm,xh,gg,jldw,ppmc,tc_csrjzck.bjjbbm,bz,bjjbmc,tc_csrjzck.khxsdj,zp";
	ls_sql+=" FROM tc_csrjzck,bdm_bjjbbm";
	ls_sql+=" where tc_csrjzck.bjjbbm=bdm_bjjbbm.bjjbbm(+) ";
	ls_sql+=" and tccplbbm='"+tccplbbm+"' and  dqbm='"+dqbm+"'";
	...
	while (rs.next())
	{
		...
		<tr bgcolor="#FFFFFF"  align="center" onMouseOver="inTr(this)" onMouseOut="outTr(this)"  onclick="onclickTr(this)">
		  <td>
			<A  HREF="SaveInsertBj_khzcxmWjCl.jsp?khbh=<%=khbh%>&tccpdlbm=<%=tccpdlbm%>&tccplbbm=<%=tccplbbm%>&cpbm=<%=cpbm%>">选择</A>
		  </td>
		  <td>&nbsp;</td>
		  <td><%=nbbm%></td>
		  <td><%=ppmc%></td>
