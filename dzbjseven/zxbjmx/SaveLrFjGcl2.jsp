...
try {
	...
	for (int i=0;i<xmbh.length ;i++ )
	{
		...
		{
			conn.rollback();
			out.println("<BR>错误！自定义项目『"+xmbh[i]+"』已过期");
			return;
		}
		...
		if (count>0)
		{
			ls_sql="update bj_gclmx set sl="+sl+",bz='"+cf.GB2Uni(bz[i])+"'";
			ls_sql+=" where khbh='"+khbh+"' and xmbh='"+xmbh[i]+"' and jgwzbm='"+jgwzbm+"'";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
			...
			ls_sql="insert into bj_gclmx(khbh,dqbm,xmbh,jgwzbm,sl,bz)";
			ls_sql+=" values('"+khbh+"','"+dqbm+"','"+xmbh[i]+"','"+jgwzbm+"',"+sl+",'"+cf.GB2Uni(bz[i])+"') ";
			ps= conn.prepareStatement(ls_sql);
			ps.executeUpdate();
			ps.close();
		}
