...
try {
	...
	if (pageStr==null)//判断是否需要初始化，若第一次进入此页，则需要初始化
	{
		...
		String bj_jzglfxb_glfxclmc=null;
		...
		String inglfxclbm=null;
		inglfxclbm=request.getParameter("glfxclbm");
		if (inglfxclbm!=null)
		{
			if (!(inglfxclbm.equals("")))	wheresql+=" and (bj_jzglfxb.glfxclbm='"+inglfxclbm+"') ";
		}
		...
		bj_jzglfxb_glfxclmc=request.getParameter("bj_jzglfxb_glfxclmc");
		if (bj_jzglfxb_glfxclmc!=null)
		{
			bj_jzglfxb_glfxclmc=cf.GB2Uni(bj_jzglfxb_glfxclmc);
			if (!(bj_jzglfxb_glfxclmc.equals("")))	wheresql+=" and  (bj_jzglfxb.glfxclmc='"+bj_jzglfxb_glfxclmc+"')";
		}
...
//	out.println(ls_sql);
	...
	while (rs.next())
	{
		...
		while (rs1.next())
		{
			...
			ls_sql="SELECT bj_jzglfxb.glbxh,bj_jzglfxb.glfxclbm,DECODE(bj_jzglfxb.wlllbz,'Y','需要','N','否') wlllbz,DECODE(bj_jzglfxb.lx,'1','打印','2','不打印') lx,bj_jzglfxb.glfxclmc,jdm_jldwbm.jldwmc,bj_jzglfxb.xhl,bj_jzglfxb.dj,ROUND(bj_jzglfxb.xhl*bj_jzglfxb.dj,2) je,bj_jzglfxb.bz,bj_jzglfxb.lrr,bj_jzglfxb.lrsj,bj_jzglfxb.jqsfl,bj_jzglfxb.bsfl,bj_jzglfxb.tvocsfl   ";
			ls_sql+=" FROM jdm_jldwbm,bj_jzglfxb  ";
			ls_sql+=" where bj_jzglfxb.jldwbm=jdm_jldwbm.jldwbm(+) ";
			ls_sql+=" and dqbm='"+dqbm+"' and xmbh='"+xmbh+"' and bjjbbm='"+bjjbbm+"' and glflbm='"+glflbm+"'";
			...
			while (rs2.next())
			{
				...
				jldwmc=cf.fillNull(rs2.getString("jldwmc"));
