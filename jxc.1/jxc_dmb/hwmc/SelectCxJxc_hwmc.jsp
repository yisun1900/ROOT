...
<body bgcolor="#FFFFFF">
<form method="post" action="Jxc_hwmcCxList.jsp" name="selectform">
<div align="center">请输入查询条件</div>
<table width="100%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%">所属地区</td> 
  <td width="35%"> 
    <select name="jxc_hwmc_dqbm" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changedqbm(selectform)">
	<option value=""></option>
    <%
...
%>
  ...
  <td width="35%"> 
   <select name="jxc_hwmc_ckbh" style="FONT-SIZE:12PX;WIDTH:152PX" onChange="changeckbh(selectform)">
    <option value=""></option>
    <%
	if (yhjs.equals("A0") || yhjs.equals("A1"))
	{
...
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%">所属货架</td> 
  <td width="35%">
  <select name="jxc_hwmc_hjbh" style="FONT-SIZE:12PX;WIDTH:152PX">
    <option value=""></option>
  </select></td>
  <td align="right" width="15%">货位编号</td> 
  <td width="35%"><input type="text" name="jxc_hwmc_hwbh" size="20" maxlength="8" ></td>
</tr>
<tr bgcolor="#FFFFFF">
  <td align="right" width="15%">货位名称</td> 
  <td colspan="3"><input type="text" name="jxc_hwmc_hwmc" size="60" maxlength="50" ></td>
  </tr>
<tr align="center"> 
  <td colspan="4"> 
...
{
	FormName.jxc_hwmc_ckbh.length=1;

	if (FormName.jxc_hwmc_dqbm.value=="")
	{
		return;
	}
	else
	{
	var sql;
	sql="select ckbh,ckmc from jxc_ckmc where dqbm='"+FormName.jxc_hwmc_dqbm.value+"' order by ckmc";
...
//	alert("Ajax返回值："+ajaxRetStr);
	...
	//需要用javaScript处理返回值
	strToSelect(selectform.jxc_hwmc_ckbh,ajaxRetStr);
}
function changeckbh(FormName) //改变下拉列表的值，触发Ajax
{
	FormName.jxc_hwmc_hjbh.length=1;

	if (FormName.jxc_hwmc_ckbh.value=="")
	{
		return;
	}
	else
	{
	var sql;
	sql="select hjbh,hjmc from jxc_hjmc where ckbh='"+FormName.jxc_hwmc_ckbh.value+"' order by hjmc";
...
//	alert("Ajax返回值："+ajaxRetStr);
	...
	//需要用javaScript处理返回值
	strToSelect(selectform.jxc_hwmc_hjbh,ajaxRetStr);
}
