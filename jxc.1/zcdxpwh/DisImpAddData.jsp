...
<input name="dqbm" type="hidden" value="<%=dqbm%>" >
  <table width="340%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		cxyjjrblstr=outStr[i][26].trim();


		sfbhpjstr=outStr[i][27].trim();
		pjtsstr=outStr[i][28].trim();
		sfyhstr=outStr[i][29].trim();
		cpjb=outStr[i][30].trim();
		fgsx=outStr[i][31].trim();
		cpsm=outStr[i][32].trim();
		cd=outStr[i][33].trim();
		sftjcpstr=outStr[i][34].trim();
		xsfsstr=outStr[i][35].trim();
		zp=outStr[i][36].trim();
		bz=outStr[i][37].trim();
		...
		if (ppmc!=null && !ppmc.equals(""))
		{
			...
			{         
				...
				getcldlbm = cf.fillNull(rs.getString("cldl"));			    
			}
			else{
...
//				return;
		...
		if (sfycx.equals("Y"))
		{
			...
			if (cxyjjrblstr!=null && !cxyjjrblstr.trim().equals("") && !cxyjjrblstr.trim().equals("0"))
			{
				out.print("<BR>序号["+xuhao+"]提醒！因为没有促销，不能有[促销期业绩记入比例]");
				cxyjjrblstr="";
				cxyjjrbl=0;
			}
		}
		...
		//销售方式
		xsfs="1";//1：代销品；2：现货销售；3：期货销售；4：虚拟出入库
