...
try {
	...
	ls_sql=" SELECT fccbj ";
	ls_sql+=" FROM sq_dwxx ";
	ls_sql+=" where dqbm='"+dqbm+"' and fccbj is not null";
	...
	ls_sql="select jxc_cljgb.sfcy,jxc_cljgb.gysmc,jxc_cljgb.xsfs,jxc_cljgb.cbj,jqpjcbj,jxc_cljgb.dfgsjg,jxc_cljgb.dgzjg,jxc_cljgb.zxqdl,jxc_cljgb.sfyh,jxc_cljgb.bz ";
...
<body bgcolor="#FFFFFF">
<form method="post" action="SaveEditCbjJxc_cljgb.jsp" name="editform">
<div align="center">修改成本价</div>
...
<tr bgcolor="#FFFFFF">
  <td align="right">分公司价</td>
  <td><%=dfgsjg%></td>
  <td align="right">对工长价</td>
  <td><%=dgzjg%></td>
</tr>
...
<%
	if (fccbj.equals("2"))//1：入库价；2：加权平均价
	{
		...
		<tr bgcolor="#FFFFFF">
		  ...
		  <td><input type="text" name="jqpjcbj" size="20"   value="<%=jqpjcbj%>" ></td>
		  <td align="right">&nbsp;</td>
		  <td>&nbsp;</td>
		</tr>
		<%
...
<tr bgcolor="#FFFFFF">
  ...
  <td colspan="3"><textarea name="bz" cols="71" rows="2"><%=bz%></textarea></td>
    ...
    <tr> 
      <td colspan="4" >
	<div align="center">
		<input type="hidden" name="fccbj"  value="<%=fccbj%>" >
...
<%
	...
	{
		%>
		if(	javaTrim(FormName.jqpjcbj)=="") {
			alert("请输入[成本价]！");
			FormName.jqpjcbj.focus();
			return false;
		}
		if(!(isFloat(FormName.jqpjcbj, "成本价"))) {
			return false;
		}
		<%
	}
