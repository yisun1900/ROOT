...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
    ...
    ls_sql+=" and crm_khpfd.pfzt='2'";//1：等待审核；2：已审核；3：财务接收；4：赔付完成
    ...
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Crm_khpfdJsList.jsp","","","");
...
//设置按钮参数
	...
	String[] buttonLink={"Crm_khpfdJsList.jsp?"};//按钮单击调用的网页，可以增加参数
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="update crm_khpfd set cwjsr='"+yhmc+"',cwjssj=SYSDATE,pfzt='3' where pfzt='2' and "+chooseitem;
