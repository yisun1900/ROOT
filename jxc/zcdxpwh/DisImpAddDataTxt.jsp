...
<input name="dqbm" type="hidden" value="<%=dqbm%>" >
  <table width="340%" border="0" style="FONT-SIZE:12" bgcolor="#999999" cellpadding="2" cellspacing="2">
	...
	for (int i=1;i<outStr.length ;i++ )
	{
		...
		clxlbm=outStr[i][2].trim();
		cllbbm=outStr[i][3].trim();
		clbm=outStr[i][4].trim();
		clmc=outStr[i][5].trim();
		xh=outStr[i][6].trim();
		gg=outStr[i][7].trim();
		clysbm=outStr[i][8].trim();
		jldwbmstr=outStr[i][9].trim();
		lsjstr=outStr[i][10].trim();
		xsjstr=outStr[i][11].trim();

		cbjstr=outStr[i][12].trim();
		jsblstr=outStr[i][13].trim();
		cxhdblstr=outStr[i][14].trim();
		sfycxstr=outStr[i][15].trim();
		cxkssjstr=outStr[i][16].trim();
		cxjzsjstr=outStr[i][17].trim();
		cxjstr=outStr[i][18].trim();
		cxcbjstr=outStr[i][19].trim();
		lscxhdblstr=outStr[i][20].trim();
		sfbhpjstr=outStr[i][21].trim();

		pjtsstr=outStr[i][22].trim();
		sfyhstr=outStr[i][23].trim();
		cpjb=outStr[i][24].trim();
		fgsx=outStr[i][25].trim();
		cpsm=outStr[i][26].trim();
		cd=outStr[i][27].trim();
		sftjcpstr=outStr[i][28].trim();
		zp=outStr[i][29].trim();
		bz=outStr[i][30].trim();
...
//				return;
		...
		{         
			...
			out.println("<BR>！！！序号["+xuhao+"]错误，[材料小类]不存在");
			return;
		}
		rs.close();
		ps.close();
