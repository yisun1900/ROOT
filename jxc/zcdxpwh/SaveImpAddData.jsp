...
try {
	...
	for (i=0;i<xuhao.length ;i++ )
	{
		...
		String cllb="0";//0：代销品主材；1：辅材；2：库存品主材
		...
		sql+=" ,'"+cf.GB2Uni(xh[i])+"','"+cf.GB2Uni(gg[i])+"','"+cf.GB2Uni(clysbm[i])+"','"+cf.GB2Uni(jldwbm[i])+"','"+cllb+"'";
...
//		out.println(sql);
		...
		sql+=" ,lrr,lrsj,lrbm,ssfgs,dfgsjg,dgzjg,zxqdl,xsfs,zcpzsl,ccpzsl,bfpzsl,dhsl,ycgsl,zdkcsl,zdkcje,jsfs,zdxsj,yjjrbl,cxjsbl,cxyjjrbl,cllb)";
		...
		sql+=" ,'"+yhmc+"',SYSDATE,'"+lrbm+"','"+ssfgs+"',0,0,0,'"+xsfs[i]+"',0,0,0,0,0,0,0,'"+jsfs[i]+"',"+zdxsj+","+yjjrbl+","+cxjsbl+","+cxyjjrbl+",'"+cllb+"'";
		sql+="  )";
//		out.println(sql);
		ps= conn.prepareStatement(sql);
		ps.executeUpdate();
