...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	ls_sql="SELECT dqmc,DECODE(jxc_cljgb.xsfs,'1','代销品','2','现货','3','期货销售','4','虚拟出入库') xsfs,DECODE(jxc_cljgb.sfyh,'Y','有货','N','无货') sfyh,DECODE(jxc_cljgb.sfcy,'Y','常用','N','否') sfcy,jxc_clbm.clbm,clxlbm,nbbm,clmc,xh,gg,jldwbm,TO_CHAR(jxc_cljgb.cbj),TO_CHAR(jxc_cljgb.dfgsjg),TO_CHAR(jxc_cljgb.dgzjg),jxc_cljgb.zcpzsl,jxc_cljgb.dgzjg*jxc_cljgb.zcpzsl zcpje,jxc_cljgb.ccpzsl,jxc_cljgb.bfpzsl,jxc_cljgb.dhsl,jxc_cljgb.ycgsl,jxc_cljgb.gysmc,bzjldwbm,bzgg,djzl,cgzq,bzq,bztxq,cldlbm,ppmc,DECODE(cglb,'J','集团','D','地方'),jxc_clbm.bz ";
    ...
    ls_sql+=" and jxc_cljgb.dgzjg=0";
    ls_sql+=wheresql;
    ls_sql+=" order by cldlbm,clxlbm,clmc,jxc_clbm.clbm";
    pageObj.sql=ls_sql;
//进行对象初始化
	pageObj.initPage("Jxc_cljgbWjgList.jsp?dqbm="+dqbm,"","","EditJxc_cljgb.jsp?dqbm="+dqbm);
...
//设置按钮参数
	String[] buttonName={"批量调整材料价格","调销售方式","调是否有货","调是否常用","删除地区材料"};//按钮的显示名称
	String[] buttonLink={"pltzcljgfc.jsp?dqbm="+dqbm,"pltzxsfs.jsp?dqbm="+dqbm,"pltzsfyh.jsp?dqbm="+dqbm,"pltzsfcy.jsp?dqbm="+dqbm,"DeleteJxc_cljgb.jsp?dqbm="+dqbm};//按钮单击调用的网页，可以增加参数
	int[] buttonNew={1,1,1,1,1};//设置按钮单击是否打开新的窗口，0：不打开；1：打开
