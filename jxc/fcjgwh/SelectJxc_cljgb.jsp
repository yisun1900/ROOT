...
<tr bgcolor="#FFFFFF">
  <td align="right" width="18%">单品重量 从</td> 
  <td width="32%"> 
...
<tr align="center"> 
  <td colspan="4"> 
    <p>
      <input type="button"  value="查询已初始化材料" onClick="f_do(insertform)">
      <input type="button" onClick="f_do2(insertform)"  value="查询无价格材料">
      <input type="button" onClick="f_do1(insertform)"  value="全部未初始化材料">
    ...
    <p>
      ...
      <input type="button" onClick="f_cljgdc(insertform)"  value="导出材料及价格">
      <input type="button" onClick="f_cljgdr(insertform)"  value="导入材料及价格">
<BR>
      <BR> 
      <A HREF="辅材价格模板.xls"><B>下载价格模板</B></A>  
	  &nbsp;&nbsp;&nbsp;&nbsp;
      <A HREF="辅材材料及价格模板.xls"><B>下载材料及价格模板</B></A>    
...
function f_do(FormName)//参数FormName:Form的名称
{
	...
	if(!(isNumber(FormName.djzl, "单品重量"))) {
		return false;
	}
	if(!(isNumber(FormName.djzl2, "单品重量"))) {
		return false;
	}
	...
	FormName.action="Jxc_cljgbList.jsp"
	FormName.submit();
	return true;
}
...
function f_do1(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="Jxc_clbmCshList.jsp"
	FormName.submit();
	return true;
}
...
function f_do2(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="Jxc_cljgbWjgList.jsp"
	FormName.submit();
	return true;
}
...
function f_dc(FormName)//参数FormName:Form的名称
{
	...
	FormName.action="Jxc_cljgbDcList.jsp"
	FormName.submit();
	return true;
}
...
function f_cljgdc(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.dqbm)=="") {
		alert("请选择[地区]！");
		FormName.dqbm.focus();
	...
	FormName.action="Jxc_cljgbDcCljgList.jsp"
	FormName.submit();
	return true;
}

function f_cljgdr(FormName)//参数FormName:Form的名称
{
	if(	javaTrim(FormName.dqbm)=="") {
		alert("请选择[地区]！");
		FormName.dqbm.focus();
	...
	FormName.action="LoadDataCljg.jsp";
	FormName.submit();
	return true;
}
//-->
