...
try {
	...
	ls_sql+=" where ppmc='"+ppmc+"' and dqbm='"+dqbm+"'";
	ps= conn.prepareStatement(ls_sql);
	rs=ps.executeQuery();
	if(rs.next())
	{         
	...
	ls_sql+=" where cllb='0'";//0：代销品主材；1：辅材；2：库存品主材
	ps= conn.prepareStatement(ls_sql);
	rs =ps.executeQuery();
	if (rs.next())
	{
	...
	ls_sql="insert into jxc_clbm (clbm,clmc,cllb,cldlbm,clxlbm,scsmc,ppmc,cllbbm,xh,gg   ,clysbm,jldwbm,sfbhpj,pjts,cpjb,fgsx,cpsm,cd,zp,lrr   ,lrsj,lrbm,bz,nbbm,bzjldwbm,bzgg)";
	ls_sql+=" values ( ?,?,?,?,?,?,?,?,?,?   ,?,?,?,?,?,?,?,?,?,?   ,SYSDATE,?,?,?,?,?) ";
	ps= conn.prepareStatement(ls_sql);
	ps.setString(1,clbm);
	ps.setString(2,clmc);
	ps.setString(3,"0");//0：代销品主材；1：辅材；2：库存品主材
	...
	ps.setDouble(24,0);
	ps.setString(25,"3");//1：库存品主材；2：现货销售；3：期货销售；4：虚拟出入库
	ps.setDouble(26,0);
	ps.setDouble(27,0);
	ps.setDouble(28,0);
	ps.setDouble(29,0);
	...
	ps.setString(34,"0");//0：代销品主材；1：辅材；2：库存品主材
	ps.executeUpdate();
	ps.close();
