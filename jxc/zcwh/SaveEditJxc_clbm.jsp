...
try {
	...
	ls_sql="update jxc_clbm set cldlbm=?,clxlbm=?,ppmc=?,scsmc=?,cllbbm=?,clmc=?,xh=?,gg=?,clysbm=?,jldwbm=?   ,sfbhpj=?,pjts=?,cpjb=?,fgsx=?,cpsm=?,cd=?,lrr=?,lrsj=SYSDATE,bz=?,nbbm=?,bzjldwbm=?,bzgg=?";
	...
	ls_sql="update jxc_cljgb set gysmc=?,lsj=?,xsj=?,cbj=?,jsbl=?,cxhdbl=?,sfycx=?,cxkssj=?,cxjzsj=?,cxj=?  ,cxcbj=?,lscxhdbl=?,sfyh=?,sftjcp=? ,lrr=?,lrsj=SYSDATE,bz=?";
