...
if (pageObj.needInit(pageStr))//判断是否需要初始化，若第一次进入此页，则需要初始化
{
	...
	{
		cglb=cf.GB2Uni(cglb);
		if (!(cglb.equals("")))	wheresql+=" and  (jxc_clbm.cglb='"+cglb+"')";
	}
	...
	String zcpzsl=null;
	zcpzsl=request.getParameter("zcpzsl");
	if (zcpzsl!=null)
	{
		zcpzsl=cf.GB2Uni(zcpzsl);
		if (!(zcpzsl.equals("")))	wheresql+=" and  (jxc_cljgb.zcpzsl>="+zcpzsl+")";
	}
	zcpzsl=request.getParameter("zcpzsl2");
	if (zcpzsl!=null)
	{
		zcpzsl=cf.GB2Uni(zcpzsl);
		if (!(zcpzsl.equals("")))	wheresql+=" and  (jxc_cljgb.zcpzsl<="+zcpzsl+")";
	...
	ls_sql="SELECT jxc_clbm.clbm,jxc_clbm.nbbm,jxc_clbm.clmc,jxc_clbm.xh,jxc_clbm.gg,jxc_clbm.ppmc,DECODE(jxc_cljgb.xsfs,'1','代销品','2','现货','3','期货销售','4','虚拟出入库') xsfs,DECODE(jxc_cljgb.sftjcp,'Y','是','N','否'),DECODE(jxc_cljgb.sfyh,'Y','有货','N','<font color=\"#FF0000\">无货</font>'),DECODE(jxc_cljgb.sfycx,'N','无','Y','<font color=\"#FF0000\">有促销</font>'),jxc_cljgb.cxkssj,jxc_cljgb.cxjzsj,jxc_clbm.cllbbm,jxc_clbm.clysbm,jxc_clbm.jldwbm,jxc_cljgb.lsj,jxc_cljgb.xsj,jxc_cljgb.zdxsj,jxc_cljgb.yjjrbl||'%',jxc_cljgb.cxhdbl||'%',jxc_cljgb.cxj,jxc_cljgb.lscxhdbl||'%',jxc_cljgb.cxyjjrbl||'%',DECODE(jxc_clbm.sfbhpj,'Y','<font color=\"#FF0000\">含</font>','N','不含'),jxc_clbm.pjts,jxc_clbm.cpjb,jxc_clbm.fgsx,jxc_clbm.cpsm,jxc_clbm.cd,jxc_clbm.zp,jxc_clbm.cldlbm,jxc_clbm.clxlbm,jxc_clbm.scsmc,jxc_cljgb.gysmc,jxc_cljgb.zcpzsl,jxc_cljgb.dhsl,jxc_cljgb.ycgsl,jxc_cljgb.zdkcsl,jxc_cljgb.zdkcje,jxc_cljgb.jqpjcbj,jxc_cljgb.yfjqpjcbj,DECODE(jxc_clbm.cglb,'J','集团采购','D','地方采购') cglb,jxc_clbm.lrr,jxc_clbm.lrsj,jxc_clbm.bz";
	ls_sql+=" FROM jxc_clbm,jxc_cljgb";
    ls_sql+=" where jxc_clbm.clbm=jxc_cljgb.clbm";
    ls_sql+=" and jxc_cljgb.xsfs not in('1','4')";//1：代销品；2：现货销售；3：期货销售；4：虚拟出入库
    ls_sql+=" and jxc_clbm.cllb='2'";//0：代销品主材；1：辅材；2：库存品主材";
	if (yhjs.equals("G0"))
	{
		ls_sql+=" and jxc_cljgb.dqbm in(select sq_dwxx.dqbm from jxc_ppgysdzb,sq_dwxx where  jxc_ppgysdzb.ssfgs=sq_dwxx.dwbh and jxc_ppgysdzb.gysbm='"+ygbh+"' )";
	}
	else{
		if (kfgssq.equals("1") || kfgssq.equals("2"))//0：不授权；1：授权单个分公司；2：授权多个分公司；3：授权单个店面；4：授权某一分公司多个店面；5：授权多个分公司多个店面
		{
			ls_sql+=" and jxc_cljgb.dqbm in( select dqbm from sq_dwxx where dwbh in(select ssfgs from sq_sqfgs where ygbh='"+ygbh+"' ))";
		}
		else
		{
			ls_sql+=" and jxc_cljgb.dqbm in( select dqbm from sq_dwxx where dwbh in(select dwbh from sq_sqbm where ygbh='"+ygbh+"' ))";
		}
	}
...
<tr bgcolor="#CCCCCC"  align="center">
	...
	<td  width="2%">加权平均成本价</td>
	<td  width="2%">含运费加权平均成本价</td>
	<td  width="2%">采购类别</td>
	<td  width="2%">录入人</td>
	<td  width="2%">录入时间</td>
	<td  width="59%">备注</td>
