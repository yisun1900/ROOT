...
//		if (!(lrbm.equals("")))	wheresql+=" and  (jxc_clbm.lrbm='"+lrbm+"')";
	...
	ls_sql="SELECT jxc_clbm.clbm,jxc_clbm.nbbm,jxc_clbm.clmc,jxc_clbm.xh,jxc_clbm.gg,jxc_clbm.ppmc,DECODE(jxc_cljgb.xsfs,'1','代销品','2','现货','3','期货销售','4','虚拟出入库') xsfs,DECODE(jxc_cljgb.sftjcp,'Y','是','N','否'),DECODE(jxc_cljgb.sfyh,'Y','有货','N','<font color=\"#FF0000\">无货</font>'),DECODE(jxc_cljgb.sfycx,'N','无','Y','<font color=\"#FF0000\">有促销</font>'),jxc_cljgb.cxkssj,jxc_cljgb.cxjzsj,jxc_clbm.cllbbm,jxc_clbm.clysbm,jxc_clbm.jldwbm,jxc_cljgb.lsj,jxc_cljgb.xsj,jxc_cljgb.zdxsj,jxc_cljgb.yjjrbl||'%',DECODE(jxc_cljgb.jsfs,'1','结算价','2','结算比例'),jxc_cljgb.cbj,jxc_cljgb.jsbl||'%',jxc_cljgb.cxhdbl||'%',jxc_cljgb.cxj,jxc_cljgb.cxcbj,jxc_cljgb.cxjsbl||'%',jxc_cljgb.lscxhdbl||'%',DECODE(jxc_clbm.sfbhpj,'Y','<font color=\"#FF0000\">含</font>','N','不含'),jxc_clbm.pjts,jxc_clbm.cpjb,jxc_clbm.fgsx,jxc_clbm.cpsm,jxc_clbm.cd,jxc_clbm.zp,jxc_clbm.cldlbm,jxc_clbm.clxlbm,jxc_clbm.scsmc,jxc_cljgb.gysmc,jxc_cljgb.zcpzsl,jxc_cljgb.zdkcsl,jxc_cljgb.zdkcje,DECODE(jxc_clbm.cglb,'J','集团采购','D','地方采购') cglb,jxc_clbm.lrr,jxc_clbm.lrsj,jxc_clbm.bz";
...
//设置按钮参数
	String[] buttonName={"促销开始","促销结束","有货","无货","删除价格信息","调销售方式"};//按钮的显示名称
	String[] buttonLink={"StartCx.jsp?dqbm="+dqbm,"SaveEndCx.jsp?dqbm="+dqbm,"SaveIsSfyh.jsp?dqbm="+dqbm,"SaveNotSfyh.jsp?dqbm="+dqbm,"Jxc_clbmList.jsp?dqbm="+dqbm,"pltzxsfs.jsp?dqbm="+dqbm};//按钮单击调用的网页，可以增加参数
	int[] buttonNew={1,1,1,1,0,1};//设置按钮单击是否打开新的窗口，0：不打开；1：打开
...
else//非第一次进入此页，不需要初始化
{
	...
	if (pageObj.needDelete(chooseitem))//判断是否需要执行删除操作
	{
		...
		sql[0]="delete from jxc_cljgb where  dqbm='"+dqbm+"' and "+chooseitem;
