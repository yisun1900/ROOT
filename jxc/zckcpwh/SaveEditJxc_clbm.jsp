...
try {
	...
	ls_sql="update jxc_clbm set cldlbm=?,clxlbm=?,ppmc=?,scsmc=?,cllbbm=?,clmc=?,xh=?,gg=?,clysbm=?,jldwbm=?   ,sfbhpj=?,pjts=?,cpjb=?,fgsx=?,cpsm=?,cd=?,lrr=?,lrsj=SYSDATE,bz=?,nbbm=?   ,bzjldwbm=?,bzgg=?,cglb=?,cgzq=?,djzl=?,bzq=?,bztxq=?,clcd=?,clkd=?,clgd=?";
	...
	ps.setString(20,bzjldwbm);
	ps.setLong(21,bzgg);
	ps.setString(22,cglb);
	ps.setLong(23,cgzq);
	ps.setDouble(24,djzl);
	ps.setLong(25,bzq);
	ps.setLong(26,bztxq);
	ps.setDouble(27,clcd);
	ps.setDouble(28,clkd);
	ps.setDouble(29,clgd);
	...
	ls_sql="update jxc_cljgb set gysmc=?,lsj=?,xsj=?,cbj=?,jsbl=?,cxhdbl=?,sfycx=?,cxkssj=?,cxjzsj=?,cxj=?  ,cxcbj=?,lscxhdbl=?,sfyh=?,sftjcp=? ,lrr=?,lrsj=SYSDATE,bz=?,xsfs=?,jsfs=?,zdxsj=?,yjjrbl=?,cxjsbl=?,cxyjjrbl=?,zdkcsl=?,zdkcje=?";
	...
	ps.setDouble(20,yjjrbl);
	ps.setDouble(21,cxjsbl);
	ps.setDouble(22,cxyjjrbl);
	ps.setDouble(23,zdkcsl);
	ps.setDouble(24,zdkcje);
	ps.executeUpdate();
	ps.close();
