...
try {
	...
	ls_sql="update jxc_ppgysdzb set gysbm=?,gysmc=?,yhdlm=?,yhkl=?,yhmc=?,kdlxtbz=?,lxdlsbcs=?,mmsyzq=?,yxdlsbcs=?,sfsd=?  ,sfyyh=?,yhkssj=?,yhjzsj=?,yhnr=?,tcycf=?,sfxcl=?,sfkgtcsl=?,cllx=?";
	ls_sql+=" where  (ppbm="+whereppbm+") and  (gysbm="+wheregysbm+") and  (ssfgs='"+ssfgs+"')  ";
	ps= conn.prepareStatement(ls_sql);
	ps.setLong(1,gysbm);
	...
	ps.setDouble(15,tcycf);
	ps.setString(16,sfxcl);
	ps.setString(17,sfkgtcsl);
